...
import { WorkspaceHeader, RoleNavigation } from '../shared/Atmosfera';
...
setOwnerNewBookingWorkers([]);
  ...
  return (
    <div className={`${isDark ? 'dark' : ''} atmosfera-shell ${bg} ${text} min-h-screen flex flex-col`} data-owner-build="2026-04-03-5">
      <WorkspaceHeader eyebrow="Executive workspace" title={page==='dashboard'?'╨Ю╨▒╨╖╨╛╤А ╨▒╨╕╨╖╨╜╨╡╤Б╨░':'╨Р╤В╨╝╨╛╤Б╤Д╨╡╤А╨░'} subtitle={`${todayLabel} ┬╖ ╤Г╨┐╤А╨░╨▓╨╗╨╡╨╜╤З╨╡╤Б╨║╨╕╨╣ ╨║╨╛╨╜╤В╤Г╤А`} isDark={isDark} onToggleTheme={toggleTheme} notificationCount={unreadCount} onNotifications={()=>setShowNotifications(true)}/><RoleNavigation<OwnerPage> active={page} onSelect={setPage} items={[{id:'dashboard',label:'╨Ю╨▒╨╖╨╛╤А',icon:Home},{id:'calendar',label:'╨Ю╨┐╨╡╤А╨░╤Ж╨╕╨╕',icon:CalendarDays},{id:'payroll',label:'╨Ъ╨╛╨╝╨░╨╜╨┤╨░',icon:Users},{id:'stock',label:'╨б╨║╨╗╨░╨┤',icon:Package},{id:'reports',label:'╨д╨╕╨╜╨░╨╜╤Б╤Л',icon:BarChart3},{id:'clients',label:'╨Ъ╨╗╨╕╨╡╨╜╤В╤Л',icon:User},{id:'settings',label:'╨Э╨░╤Б╤В╤А╨╛╨╣╨║╨╕',icon:Settings}]}/>
      {/* Header */}
        ...
        <AnimatePresence mode="wait">
          ...
          {page === 'dashboard' && (
            <motion.div key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="px-4 py-4">
              <section className="role-hero role-hero--owner mb-5"><div className="text-xs uppercase tracking-[.18em] opacity-75">Executive overview</div><div className="mt-2 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-semibold">╨С╨╕╨╖╨╜╨╡╤Б ╨▓ ╨╛╨┤╨╜╨╛╨╝ ╨║╨░╨┤╤А╨╡</h2><p className="mt-1 text-sm opacity-80">╨Ф╨╡╨╜╤М╨│╨╕, ╤В╨╡╨╝╨┐ ╨╛╨┐╨╡╤А╨░╤Ж╨╕╨╣ ╨╕ ╤А╨╡╤И╨╡╨╜╨╕╤П, ╨║╨╛╤В╨╛╤А╤Л╨╡ ╨╢╨┤╤Г╤В ╨▓╨╗╨░╨┤╨╡╨╗╤М╤Ж╨░.</p></div><TrendingUp size={42} className="opacity-70"/></div><div className="mt-5 grid grid-cols-3 gap-2 text-center text-sm"><div className="rounded-xl bg-white/10 p-3"><b>{todayRevenue.toLocaleString('ru-RU')} тВ╜</b><span className="block opacity-70">╤Б╨╡╨│╨╛╨┤╨╜╤П</span></div><div className="rounded-xl bg-white/10 p-3"><b>{profit.toLocaleString('ru-RU')} тВ╜</b><span className="block opacity-70">╤А╨╡╨╖╤Г╨╗╤М╤В╨░╤В</span></div><div className="rounded-xl bg-white/10 p-3"><b>{todayBookings.length}</b><span className="block opacity-70">╨╛╨┐╨╡╤А╨░╤Ж╨╕╨╣</span></div></div></section>
              <section className="grid gap-3 md:grid-cols-[1.5fr_.5fr] mb-5"><div className="work-surface p-4"><div className="text-xs uppercase tracking-wider text-muted-foreground">Cashflow band</div><div className="mt-3 grid grid-cols-3 gap-3"><div><span className="text-xs text-muted-foreground">╨Т╤Л╤А╤Г╤З╨║╨░</span><div className="font-semibold">{totalRevenue.toLocaleString('ru-RU')} тВ╜</div></div><div><span className="text-xs text-muted-foreground">╨а╨░╤Б╤Е╨╛╨┤╤Л</span><div className="font-semibold">{totalExpenses.toLocaleString('ru-RU')} тВ╜</div></div><div><span className="text-xs text-muted-foreground">╨а╨╡╨╖╤Г╨╗╤М╤В╨░╤В</span><div className="font-semibold">{profit.toLocaleString('ru-RU')} тВ╜</div></div></div></div><aside className="work-surface work-surface--action p-4"><div className="text-xs uppercase tracking-wider text-muted-foreground">Action rail</div><button onClick={()=>setPage('reports')} className="semantic-primary-button mt-3 w-full">╨д╨╕╨╜╨░╨╜╤Б╤Л <ChevronRight size={17}/></button><button onClick={()=>setPage('payroll')} className="semantic-secondary-button mt-2 w-full">╨б╨╛╨│╨╗╨░╤Б╨╛╨▓╨░╨╜╨╕╤П</button></aside></section>
