...
| W4-001 | часовой пояс сервера vs Europe/Moscow | `_local_day_bounds` берёт TZ сервера; на UTC-хосте границы дней ЗП едут на 3ч у полуночи. Проверить TZ прода / пинить TZ env | open |
| W4-002 | дробный Decimal в int-поле ответа → потенциальный 500 | Не доказан (fuzz держит int-целостность сплита; сюита зелёная). Целевой property-тест по wallet/piggy-ответам | open |
