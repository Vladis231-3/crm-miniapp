...
| SESS-001 | `backend/app/main.py:23967` (`GET /api/auth/sessions` → `return []`), `frontend/.../OwnerApp.tsx:8190`, `WorkerProfileScreen.tsx:604` | Сессии заглушены с обеих сторон; revoke-эндпоинта нет вовсе. Решение: мёртвые карточки удалены из OwnerApp + WorkerProfileScreen (карточка всегда врала «нет сессий», кнопки были недостижимы; контекст API и AdminApp-секция не тронуты). Настоящий sessions-контракт (таблица + revoke + IDOR) — отдельной продуктовой задачей; backend-стаб помечен | fixed (removal) |
| E2E-001 | сквозная цепочка денег (`test_money_e2e_chain.py`) | create 10000 → complete → распил ровно в чек (3000/2400/4600), записанные piggy-транзакции == депозит (M-002 happy-path) | done |
...
| F-018 | Decimal в int/float-поля схем (mypy arg-type 127→~84) | Частично: `_format_money` расширен до `int \| float \| Decimal` (14 ошибок, поведение то же); `found_booking` вместо переиспользования имени. Остальное — runtime-безопасная pydantic-коэрция (≤2dp точно); паттерн зачистки: money_int на границах. Риск дробных Decimal→int (500) не доказан — см. W4-002 | partial |
