    ...
    <div className={`${isDark ? 'dark' : ''} atmosfera-shell ${bg} ${text} min-h-screen flex flex-col`}>
      ...
      <div className={`work-header ${glass} flex items-center justify-between`}>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={openNewBookingModal} className="p-2 rounded-xl text-white" style={{ background: primary }}><Plus size={18} strokeWidth={1.75} /></button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        <AnimatePresence mode="wait">
          ...
          {/* CLIENTS */}
          {page === 'clients' && (
            <AdminClientsPage onAddPastBooking={openNewBookingForClient} />
          )}
                      ...
                      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 300, damping: 30 }}
                        ...
                        <div className="p-4 border-b shrink-0" style={{ borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }}>
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => setShowEditMaterialPicker(false)} className={`p-1.5 rounded-lg ${glass}`}><X size={16} strokeWidth={1.75} /></button>
                          </div>
