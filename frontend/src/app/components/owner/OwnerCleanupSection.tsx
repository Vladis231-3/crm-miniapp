...
const ENTITY_FALLBACK: Record<string, { title: string; description: string }> = {
  bookings: { title: 'Записи', description: 'Записи клиентов: брони, визиты, статусы и привязки мастеров. Уберутся из календаря, истории и расчётов. Вместе с записями уйдут начисления копилки по ним.' },
  ...
  piggy: { title: 'Копилка', description: 'Очистится всё связанное за период: начисления и траты копилки, записи клиентов (Выручка/ЗП обнулятся) и зеркальные расходы. Не трогает ручные Доходы/Расходы и Зарплату — для полного сброса выберите их тоже.' },
