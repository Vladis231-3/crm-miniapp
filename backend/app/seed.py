...
def seed_database(db: Session, *, include_demo_staff: bool = True, is_production: bool = False) -> None:
    if is_production:
    ...
    if not db.scalar(select(Service.id).limit(1)):
        services = [
            ...
            Service(id="s2", name="Полировка стекла", category="Детейлинг", price=3500, duration=60, description="Устранение царапин и замутнений, нанесение гидрофобного покрытия.", resource_group="detailing", active=True),
            ...
            Service(id="s5", name="Мойка + полировка", category="Детейлинг", price=4200, duration=90, description="Комплекс: ручная мойка, полировка кузова и стекол.", resource_group="detailing", active=True),
            Service(id="s6", name="Химчистка салона", category="Детейлинг", price=5500, duration=120, description="Глубокая чистка всех поверхностей салона.", resource_group="detailing", active=False),
            Service(id="s7", name="Обучение ремонту стекла", category="Детейлинг", price=10000, duration=480, description="Обучение ремонту автомобильных стекол.", resource_group="detailing", active=True),
        ]
        db.add_all(services)
    ...
    settings = [
        ...
        AppSetting(key="owner_company", value={"name": "ATMOSFERA", "legalName": "", "inn": "", "address": "", "phone": "+7 (495) 000-00-00", "email": "info@atmosfera.ru", "operatingMode": "open"}),
        AppSetting(key="owner_notification_settings", value={"telegramBot": True, "emailReports": True, "smsReminders": False, "lowStock": True, "dailyReport": True, "weeklyReport": False, "bookingReminders": True, "bookingReminderHours": 24, "bookingReminderDays": 1}),
