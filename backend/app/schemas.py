...
class PiggyBankTransactionPayload(BaseModel):
    ...
    weeklyWithdrawn: float | None = None
...
DATA_CLEANUP_ENTITY_DESCRIPTIONS: dict[str, str] = {
    "bookings": "Записи клиентов: брони, визиты, статусы и привязки мастеров. Уберутся из календаря, истории и расчётов. Вместе с записями уйдут начисления копилки по ним.",
    ...
    "piggy": "Копилка: очистится всё связанное за период — начисления и траты копилки, записи клиентов (Выручка/ЗП обнулятся) и зеркальные расходы. Не трогает: ручные Доходы/Расходы и Зарплату — для полного сброса таба выберите их тоже.",
