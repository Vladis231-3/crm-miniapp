...
DATA_CLEANUP_ENTITY_DESCRIPTIONS: dict[str, str] = {
    ...
    "piggy": "Копилка: накопления процента с записей и траты из копилки. Вместе с копилкой уйдут связанные записи и зеркальные расходы — иначе Выручка/ЗП останутся, а баланс уйдёт в минус. Для полного сброса таба добавьте Доходы+Расходы за тот же период.",
