...
> Автосгенерировано 2026-09-24 05:48 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
### backend/app/main.py (27021 строк)
...
```
  ...
  `POST /api/owner/data-cleanup/preview` -> `preview_data_cleanup` (декоратор: стр. 9106)
  `POST /api/owner/data-cleanup/execute` -> `execute_data_cleanup` (декоратор: стр. 9116)
  `GET /api/owner/data-cleanup/batches` -> `list_data_cleanup_batches` (декоратор: стр. 9191)
  `GET /api/owner/trash` -> `list_trash` (декоратор: стр. 9241)
  `GET /api/owner/audit-log` -> `list_audit_log` (декоратор: стр. 9280)
  `POST /api/owner/trash/restore` -> `restore_trash` (декоратор: стр. 9321)
  `POST /api/owner/trash/purge` -> `purge_trash` (декоратор: стр. 9387)
  `GET /api/owner/exports/{kind}` -> `download_owner_export` (декоратор: стр. 10362)
  `POST /api/owner/exports/{kind}/telegram` -> `send_owner_export_to_telegram` (декоратор: стр. 10394)
  `POST /api/owner/reports/{period}/{segment}/telegram` -> `send_owner_summary_report_to_telegram` (декоратор: стр. 10426)
  `PATCH /api/clients/me` -> `update_client_me` (декоратор: стр. 11624)
  `DELETE /api/clients/{client_id}` -> `delete_client` (декоратор: стр. 11704)
  `PATCH /api/clients/{client_id}/card` -> `update_client_card` (декоратор: стр. 11729)
  `POST /api/clients` -> `create_client` (декоратор: стр. 11829)
  `GET /api/health` -> `health` (декоратор: стр. 11885)
  `GET /api/debug/db` -> `debug_db` (декоратор: стр. 11910)
  `GET /api/debug/mojibake-scan` -> `debug_mojibake_scan` (декоратор: стр. 11993)
  `POST /api/debug/mojibake-repair` -> `debug_mojibake_repair` (декоратор: стр. 12003)
  `GET /api/content` -> `get_public_content` (декоратор: стр. 12199)
  `PUT /api/content` -> `save_content` (декоратор: стр. 12213)
  `POST /api/upload` -> `upload_file` (декоратор: стр. 12277)
  `GET /api/uploads/{filename}` -> `serve_upload` (декоратор: стр. 12329)
  `POST /api/contact` -> `submit_contact` (декоратор: стр. 12350)
  `POST settings.telegram_webhook_path` -> `handle_telegram_webhook` (декоратор: стр. 12412)
  `POST /api/telegram/webhook/sync` -> `resync_telegram_webhook` (декоратор: стр. 12462)
  `GET /api/stock-categories` -> `list_stock_categories` (декоратор: стр. 12544)
  `POST /api/stock-categories` -> `create_stock_category` (декоратор: стр. 12557)
  `PATCH /api/stock-categories/{category_id}` -> `update_stock_category` (декоратор: стр. 12582)
  `DELETE /api/stock-categories/{category_id}` -> `delete_stock_category` (декоратор: стр. 12615)
  `GET /api/bookings/availability` -> `get_booking_availability` (декоратор: стр. 12649)
  `POST /api/bookings` -> `create_booking` (декоратор: стр. 12720)
  `PATCH /api/bookings/{booking_id}` -> `update_booking` (декоратор: стр. 14042)
  `DELETE /api/bookings/{booking_id}` -> `delete_booking` (декоратор: стр. 14749)
  `POST /api/bookings/{booking_id}/services` -> `add_booking_service` (декоратор: стр. 14855)
  `POST /api/bookings/{booking_id}/additional-services` -> `add_booking_additional_service` (декоратор: стр. 14926)
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 15063)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 15135)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 15228)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 15310)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 15388)
  `GET /api/notifications` -> `list_my_notifications` (декоратор: стр. 15454)
  `POST /api/broadcasts/workers` -> `broadcast_to_workers` (декоратор: стр. 15476)
  `POST /api/notifications/{notification_id}/take-to-work` -> `take_notification_to_work` (декоратор: стр. 15536)
  `POST /api/notifications/{notification_id}/complete` -> `complete_notification_task` (декоратор: стр. 15568)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 15600)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 15636)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 15684)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 15733)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 15764)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 15800)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 15842)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 15964)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 16010)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 16092)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 16248)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 16287)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 16413)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 16492)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 16558)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 16606)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 16702)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 16795)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 18006)
  `POST /api/owner/piggy-bank/repay` -> `piggy_bank_repay` (декоратор: стр. 18451)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 18771)
  `DELETE /api/owner/piggy-bank/transactions/{tx_id}` -> `delete_piggy_bank_transaction` (декоратор: стр. 18954)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 19322)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 19366)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 19401)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 19428)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 19454)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 19472)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 19484)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 19502)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 19515)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 19594)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 19679)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 19739)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 19966)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 20062)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 20142)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 20210)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 20389)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 20522)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 20672)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 20814)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 20960)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 21014)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 21098)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 21156)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 21204)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 21278)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 21302)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 21362)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 21404)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 21428)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 21452)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 21487)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 21558)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 21656)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 21675)
  `POST /api/owner/integrations/google/invites` -> `create_google_calendar_invite` (декоратор: стр. 21712)
  `DELETE /api/owner/integrations/google/connections/{connection_id}` -> `delete_google_calendar_connection` (декоратор: стр. 21747)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 21773)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 21806)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 21818)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 21842)
  `GET /api/cron/reminders` -> `run_reminders_cron` (декоратор: стр. 21867)
  `POST /api/owner/inactive-clients/remind-admin` -> `remind_admin_about_inactive_clients` (декоратор: стр. 21899)
  `POST /api/owner/reminders/dispatch` -> `dispatch_owner_booking_reminders` (декоратор: стр. 21957)
  `GET /api/cron/reports` -> `run_reports_cron` (декоратор: стр. 21976)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 22022)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 22058)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 22161)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 22263)
  `GET /api/owner/outsource/payroll` -> `get_owner_outsource_payroll` (декоратор: стр. 22332)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 22403)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 22684)
  `DELETE /api/payroll/entries/{entry_id}` -> `delete_payroll_entry` (декоратор: стр. 22845)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 22935)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 23166)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 23254)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 23462)
  `GET /api/owner/money-flow` -> `get_owner_money_flow` (декоратор: стр. 23787)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 24335)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 24349)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 24740)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 25191)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 25601)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 25862)
  `PATCH /api/owner/owners/{owner_id}/master-role` -> `set_owner_master_role` (декоратор: стр. 26128)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 26155)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 26432)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 26570)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 26630)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 26826)
  `GET /api/auth/role-preview` -> `get_role_preview` (декоратор: стр. 26840)
  `POST /api/auth/role-preview` -> `set_role_preview` (декоратор: стр. 26855)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 26907)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 26919)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 26943)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 26951)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 26964)
```
...
- `AsciiJSONResponse._created_at_in_scopedef _created_at_in_scope(created_at: datetime | None, fallback_dmy: str | None = None) -> bool: # Для payroll пробуем entry_date, иначе created_at. if fallback_dmy: try: d = parse_` (стр. 8945)
- `_data_cleanup_labeldef _data_cleanup_label(entity: str, obj: Any) -> tuple[str, str]: try: if entity == "bookings": return (f"{getattr(obj, 'date', '')} {getattr(obj, 'time', '')} · {getattr(obj, 'se` (стр. 8988)
- `_data_cleanup_preview_payloaddef _data_cleanup_preview_payload(db: Session, payload: DataCleanupRequest) -> DataCleanupPreviewPayload: period = _data_cleanup_resolve_period(payload) mode, dmy_from, dmy_to, _pa` (стр. 9015)
- `_trash_get_entitydef _trash_get_entity(obj_type: str, entity_id: str, db: Session) -> Any | None: mapping = { "bookings": Booking, "clients": Client, "incomes": Income, "expenses": Expense, "piggy"` (стр. 9063)
- `_trash_purge_expireddef _trash_purge_expired(db: Session) -> int: now = _now() expired = db.scalars( select(TrashItem).where( TrashItem.purged_at.is_(None), TrashItem.restored_at.is_(None), TrashItem.` (стр. 9080)
- `_parse_datedef _parse_date(s: str) -> date | None: if "." in s: parts = s.split(".") try: return date(int(parts[2]), int(parts[1]), int(parts[0])) except (ValueError, IndexError):` (стр. 9462)
- `_owner_export_filedef _owner_export_file( db: Session, actor_id: str, kind: str, segment: str = "all", date_from: str | None = None, date_to: str | None = None,` (стр. 9488)
- `AsciiJSONResponse._in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_to an` (стр. 9640)
- `_piggy_bank_export_filedef _piggy_bank_export_file( db: Session, actor_id: str, date_from: str | None = None, date_to: str | None = None, resource_group: str | None = None,` (стр. 9720)
- `_download_responsedef _download_response(export_file: GeneratedExport) -> Response: return Response( content=export_file.content, media_type=export_file.media_type, headers={ "Content-Disposition": ` (стр. 9790)
- `class _PartialBroadcastError(Exception):` (стр. 9810)
- `_PartialBroadcastError.__init__def __init__(self, payload: TelegramBroadcastPayload) -> None: super().__init__("partial broadcast failure") self.payload = payload` (стр. 9816)
- `_partial_broadcast_error_handlerasync def _partial_broadcast_error_handler( request: Request, exc: _PartialBroadcastError` (стр. 9825)
- `_send_export_to_telegramdef _send_export_to_telegram( db: Session, actor_id: str, export_file: GeneratedExport` (стр. 9864)
- `_owner_summary_reportdef _owner_summary_report( db: Session, actor_id: str, period: str, segment: str` (стр. 9989)
- `_owner_summary_export_filedef _owner_summary_export_file( db: Session, actor_id: str, period: str, segment: str` (стр. 10109)
- `_send_owner_summary_reportdef _send_owner_summary_report( db: Session, actor_id: str, report: OwnerSummaryReport, export_file: GeneratedExport,` (стр. 10229)
- `_booking_car_labeldef _booking_car_label(car: str | None, plate: str | None) -> str: car_value = (car or "").strip() or "Авто не указано" plate_value = (plate or "").strip() return f"{car_value}, {p` (стр. 10442)
- `_admin_booking_notification_titledef _admin_booking_notification_title( client_name: str, car: str | None, plate: str | None` (стр. 10454)
- `_booking_datetime_labeldef _booking_datetime_label(date: str | None, time: str | None) -> str: if not (date or "").strip():` (стр. 10466)
- `_admin_booking_notification_textdef _admin_booking_notification_text( client_name: str, car: str | None, plate: str | None, date: str | None, time: str | None,` (стр. 10482)
- `_additional_services_blockdef _additional_services_block(booking: Booking) -> str: """Форматирует блок доп. услуг для вставки в Telegram/внутренние сообщения.""" services = getattr(booking, "additional_serv` (стр. 10502)
- `_booking_all_worker_idsdef _booking_all_worker_ids(booking: Booking) -> set[str]: """Все мастера записи: основная услуга + доп. услуги.""" ids: set[str] = {link.worker_id for link in (booking.worker_link` (стр. 10525)
- `_notify_admins_about_bookingdef _notify_admins_about_booking(db: Session, booking: Booking) -> None: admins = db.scalars( select(StaffUser).where(StaffUser.role == "admin", StaffUser.active.is_(True)) ).all()` (стр. 10534)
- `_notify_owners_about_bookingdef _notify_owners_about_booking(db: Session, booking: Booking) -> None: owners = _all_owner_telegram_recipients(db) add_block = _additional_services_block(booking) text = ( "Новая` (стр. 10568)
- `_service_category_keydef _service_category_key(value: str | None) -> str: return (value or "").strip().lower()` (стр. 10598)
- `_resource_group_keydef _resource_group_key(value: str | None) -> str: return (value or "").strip().lower() or DEFAULT_RESOURCE_GROUP` (стр. 10606)
- `_normalized_textdef _normalized_text(value: str | None) -> str: return (value or "").strip()` (стр. 10614)
- `_default_service_resource_groupdef _default_service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_for_service_category(service.category)` (стр. 10622)
- `_default_box_resource_groupdef _default_box_resource_group(box: Box | None) -> str: if box is None: return DEFAULT_RESOURCE_GROUP name_key = (box.name or "").strip().lower() description_key = (box.descriptio` (стр. 10634)
- `_service_resource_groupdef _service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_key( service.resource_group or _default_servic` (стр. 10654)
- `_compatible_box_namesdef _compatible_box_names(db: Session, resource_group: str | None) -> list[str]: target_group = _resource_group_key(resource_group) return [ box.name for box in db.scalars( select(` (стр. 10670)
- `_is_box_rental_servicedef _is_box_rental_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "аренда бокса" )` (стр. 10696)
- `_is_detailing_servicedef _is_detailing_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "детейлинг" )` (стр. 10710)
- `_resource_group_for_service_categorydef _resource_group_for_service_category(category: str | None) -> str: category_key = _service_category_key(category) if category_key == "детейлинг": return DETAILING_RESOURCE_GROU` (стр. 10722)
- `_box_by_namedef _box_by_name(db: Session, box_name: str) -> Box | None: return db.scalar(select(Box).where(Box.name == box_name))` (стр. 10736)
- `_normalize_service_and_box_resourcesdef _normalize_service_and_box_resources(db: Session) -> None: changed = False # Группа ресурсов услуг больше не привязывается к категории принудительно. boxes = db.scalars(select(` (стр. 10744)
- `_box_hourly_pricedef _box_hourly_price(db: Session, box_name: str, fallback_price: int) -> int: box = _box_by_name(db, box_name) if box is not None and box.price_per_hour > 0: return box.price_per_` (стр. 10952)
- `_payment_type_labeldef _payment_type_label(payment_type: str) -> str: return { "cash": "Наличные", "transfer": "Перевод", "invoice": "По счёту", "credit": "В долг (депозит)", }.get(payment_type, paym` (стр. 10966)
- ...ещё 92
...
- `backend/app/main.py` (2026-09-22 18:23)
...
- `backend/app/config.py` (2026-09-22 08:57)
