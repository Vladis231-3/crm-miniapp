...
> Автосгенерировано 2026-09-14 07:02 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
- Файлов кода: **533**
- Строк кода: **213 012**
- По расширениям: `.js`: 3, `.mjs`: 5, `.py`: 181, `.ts`: 37, `.tsx`: 307
...
│   │   ├── mypy_full.txt
│   │   ├── npm_audit.json
│   │   ├── owner_audit_2026-09-08.md
│   │   ├── owner_fixes_2026-09-08.md
│   │   ├── pip_audit.txt
│   │   ├── route_matrix.md
│   │   ├── ruff_full.txt
│   │   ├── ruff_statistics.txt
...
│   │   ├── test_piggy_idempotency_delete.py
...
### backend/app/exports.py (3810 строк)

Классы и функции (58):

- `class ExportMetric: label: str value: str @dataclass(frozen=True)` (стр. 130)
- `class OwnerExportData: owner_name: str company_name: str generated_at: datetime period_from: str period_to: str …` (стр. 142)
- `class GeneratedExport: file_name: str media_type: str content: bytes telegram_caption: str ReportPeriod = Literal["daily` (стр. 184)
- `class OwnerSummaryReport: title: str message: str @dataclass(frozen=True)` (стр. 208)
- `class OwnerSummaryContext: company_name: str generated_at: datetime period: ReportPeriod segment: ReportSegment period_l` (стр. 220)
- `class OwnerSummaryExportData: owner_name: str company_name: str title: str generated_at: datetime period_label: str …` (стр. 244)
- `build_owner_summary_reportdef build_owner_summary_report( *, company_name: str, bookings: list[Booking], services: list[Service], expenses: list[Expense] | None = None, incomes: list[Income] | None = None, ` (стр. 284)
- `OwnerSummaryExportData._parse_ddmmyyyydef _parse_ddmmyyyy(value: str) -> datetime | None: try: return datetime.strptime(value.strip(), "%d.%m.%Y") except ValueError: return None` (стр. 356)
- `OwnerSummaryExportData._in_perioddef _in_period(date_str: str) -> bool: dt = _parse_ddmmyyyy(date_str) if dt is None: return False # Сравниваем без timezone (period_start/end могут быть aware) ps = period_start.re` (стр. 368)
- `build_owner_summary_exportdef build_owner_summary_export( *, owner: StaffUser, company_name: str, bookings: list[Booking], services: list[Service], penalties: list[Penalty] | None = None, piggy_transactions` (стр. 557)
- `_build_owner_summary_contextdef _build_owner_summary_context( *, company_name: str, bookings: list[Booking], services: list[Service], period: ReportPeriod, segment: ReportSegment, now: datetime | None = None,` (стр. 649)
- `_summary_headerdef _summary_header(context: OwnerSummaryContext) -> str: return f"{context.company_name}\n{context.title}\nПериод: {context.period_label}"` (стр. 733)
- `_build_owner_summary_export_datadef _build_owner_summary_export_data( *, owner_name: str, context: OwnerSummaryContext, penalties: list[Penalty] | None = None, db: Session | None = None,` (стр. 741)
- `_summary_period_boundsdef _summary_period_bounds(period: ReportPeriod, current: datetime) -> tuple[datetime, datetime, str]: end_at = current.replace(hour=0, minute=0, second=0, microsecond=0) + timedel` (стр. 1606)
- `_summary_period_labeldef _summary_period_label(period_start: datetime, period_end: datetime) -> str: last_day = period_end - timedelta(days=1) if period_start.date() == last_day.date():` (стр. 1624)
- `_booking_matches_segmentdef _booking_matches_segment(booking: Booking, service: Service | None, segment: ReportSegment) -> bool: if service is not None and service.category: category = service.category.st` (стр. 1638)
- `build_owner_exportdef build_owner_export( *, kind: ExportKind, owner: StaffUser, company_name: str, bookings: list[Booking], expenses: list[Expense], penalties: list[Penalty], workers: list[StaffUse` (стр. 1664)
- `build_piggy_bank_exportdef build_piggy_bank_export( *, company_name: str, piggy_transactions: list[PiggyBankTransaction], date_from: str | None = None, date_to: str | None = None, resource_group: str | N` (стр. 1758)
- `OwnerSummaryExportData._to_dtdef _to_dt(value: str | None) -> datetime | None: if not value: return None parsed = _parse_date_for_sort(value) return None if parsed == datetime.max else parsed` (стр. 1788)
- `OwnerSummaryExportData._in_rangedef _in_range(tx: PiggyBankTransaction) -> bool: d = _to_dt(tx.date) if d is None: return False if from_dt and d < from_dt: return False return not (to_dt and d > to_dt.replace(hou` (стр. 1797)
- `OwnerSummaryExportData._sort_keydef _sort_key(tx: PiggyBankTransaction) -> tuple[datetime, datetime]: min_aware = datetime.min.replace(tzinfo=generated.tzinfo) tx_created = ( _as_local_datetime(tx.created_at, gen` (стр. 1805)
- `OwnerSummaryExportData._period_sumdef _period_sum(predicate: Any) -> float: return sum(float(t.amount) for t in period_txs if predicate(t))` (стр. 1824)
- `OwnerSummaryExportData.moneydef money(value: float) -> str: return f"{value:,.0f} ₽".replace(",", " ")` (стр. 1859)
- `OwnerSummaryExportData.signeddef signed(value: float) -> str: return f"{value:+,.0f} ₽".replace(",", " ")` (стр. 1862)
- `OwnerSummaryExportData._month_bucketsdef _month_buckets(start: date, end: date) -> list[tuple[str, date, date]]: buckets: list[tuple[str, date, date]] = [] year, month = start.year, start.month while (year, month) <= ` (стр. 1888)
- `OwnerSummaryExportData._week_bucketsdef _week_buckets(start: date, end: date) -> list[tuple[str, date, date]]: buckets: list[tuple[str, date, date]] = [] week_start = start - timedelta(days=(start.weekday() - 5) % 7)` (стр. 1900)
- `OwnerSummaryExportData._balance_atdef _balance_at(end_day: date) -> float: # Нарастающий итог по всей истории (не только период отчёта) return sum(float(t.amount) for d, t in dated_txs if d <= end_day)` (стр. 1911)
- `OwnerSummaryExportData._bucket_metricsdef _bucket_metrics(txs: list[PiggyBankTransaction]) -> dict[str, float]: metrics: dict[str, float] = { "count": 0, "deposits": 0.0, "repayments": 0.0, "deposit_returns": 0.0, "adj` (стр. 1915)
- `OwnerSummaryExportData._comparison_rowsdef _comparison_rows( buckets: list[tuple[str, date, date]],` (стр. 1963)
- `OwnerSummaryExportData._comparison_headersdef _comparison_headers( period_label_ru: str, start_label: str, end_label: str, delta_label: str,` (стр. 2119)
- `OwnerSummaryExportData._append_comparison_sheetdef _append_comparison_sheet(title: str, headers: list[str], rows: list[list[Any]]) -> None: _append_sheet(workbook, title, headers, rows, currency_cols=comparison_currency_cols) i` (стр. 2150)
- `_build_export_datadef _build_export_data( *, owner: StaffUser, company_name: str, bookings: list[Booking], expenses: list[Expense], penalties: list[Penalty], workers: list[StaffUser], stock_items: l` (стр. 2209)
- `OwnerSummaryExportData._is_fixed_bookingdef _is_fixed_booking(booking: Booking) -> bool: # привязка строго по названию — "подготовка к полировке" всегда фиксированная if is_fixed_master_service(booking.service):` (стр. 2295)
- `OwnerSummaryExportData._piggy_sort_keydef _piggy_sort_key(tx: PiggyBankTransaction) -> tuple[datetime, datetime]: min_aware = datetime.min.replace(tzinfo=generated_at.tzinfo) tx_created = _as_local_datetime(tx.created_` (стр. 2773)
- `_render_excel_reportdef _render_excel_report(data: OwnerExportData) -> bytes: workbook = Workbook() summary = workbook.active summary.title = "Сводка" summary.merge_cells("A1:D1") summary["A1"] = data` (стр. 2892)
- `_render_owner_summary_excel_reportdef _render_owner_summary_excel_report(data: OwnerSummaryExportData) -> bytes: workbook = Workbook() summary = workbook.active summary.title = "Сводка" summary.merge_cells("A1:D1")` (стр. 2968)
- `_neutralize_xlsx_valuedef _neutralize_xlsx_value(value: Any) -> Any: if not isinstance(value, str):` (стр. 3165)
- `_append_sheetdef _append_sheet(workbook: Workbook, title: str, headers: list[str], rows: list[list[Any]], *, currency_cols: set[int] | None = None) -> None: sheet = workbook.create_sheet(title)` (стр. 3173)
- `_render_pdf_reportdef _render_pdf_report(data: OwnerExportData) -> bytes: buffer = io.BytesIO() font_name = _pdf_font_name() styles = getSampleStyleSheet() title_style = ParagraphStyle("OwnerTitle",` (стр. 3203)
- `_pdf_sectiondef _pdf_section(story: list[Any], section_style: ParagraphStyle, font_name: str, title: str, headers: list[str], rows: list[list[Any]]) -> None: story.append(Paragraph(title, sect` (стр. 3293)
- `_pdf_tabledef _pdf_table(rows: list[list[Any]], font_name: str, header_color: str = "#0E1624") -> LongTable: normalized = [[Paragraph(_escape(str(cell)), _pdf_cell_style(font_name)) for cell` (стр. 3309)
- `_format_rowsdef _format_rows(rows: list[list[Any]], *, currency_cols: set[int]) -> list[list[Any]]: formatted: list[list[Any]] = [] for row in rows: next_row = [] for index, value in enumerate` (стр. 3349)
- `_style_headingdef _style_heading(sheet, *cells: str) -> None: if cells: sheet[cells[0]].font = Font(size=16, bold=True, color="0B1226") for cell_name in cells[1:]: sheet[cell_name].font = Font(s` (стр. 3379)
- `_style_tabledef _style_table(sheet, header_row: int, start_row: int, end_row: int, end_col: int) -> None: header_fill = PatternFill(fill_type="solid", fgColor="0A84FF") header_font = Font(bold` (стр. 3393)
- `_apply_currencydef _apply_currency(cell) -> None: cell.number_format = '#,##0 "руб."' cell.alignment = Alignment(horizontal="right", vertical="center")` (стр. 3435)
- `_autosizedef _autosize(sheet) -> None: for column in sheet.columns: letter = get_column_letter(column[0].column) max_length = 0 for cell in column: max_length = max(max_length, len("" if ce` (стр. 3445)
- `_pdf_font_namedef _pdf_font_name() -> str: candidates = [ str(Path(__file__).resolve().parent / "assets" / "fonts" / "NotoSans-Regular.ttf"), os.getenv("OWNER_EXPORT_FONT_PATH", ""), "C:/Windows` (стр. 3465)
- `_pdf_cell_styledef _pdf_cell_style(font_name: str) -> ParagraphStyle: return ParagraphStyle("OwnerExportCell", fontName=font_name, fontSize=7.5, leading=9, textColor=colors.HexColor("#111827"))` (стр. 3515)
- `_booking_datetimedef _booking_datetime(booking: Booking) -> datetime | None: raw = f"{booking.date} {booking.time}".strip() for fmt in ("%d.%m.%Y %H:%M", "%Y-%m-%d %H:%M"):` (стр. 3523)
- `_booking_sort_keydef _booking_sort_key(booking: Booking) -> tuple[datetime, datetime]: local_now = datetime.now().astimezone() booking_dt = _booking_datetime(booking) primary = _as_local_datetime(b` (стр. 3543)
- `_as_local_datetimedef _as_local_datetime(value: datetime, reference: datetime) -> datetime: target_tz = reference.tzinfo if value.tzinfo is None: return value.replace(tzinfo=target_tz) return value.` (стр. 3559)
- `_parse_date_for_sortdef _parse_date_for_sort(value: str) -> datetime: for fmt in ("%d.%m.%Y", "%Y-%m-%d"):` (стр. 3573)
- `_format_datetimedef _format_datetime(value: datetime | None) -> str: if value is None: return "" return value.astimezone().strftime("%d.%m.%Y %H:%M") if value.tzinfo is not None else value.strftim` (стр. 3591)
- `_format_moneydef _format_money(value: int | float | Decimal) -> str: # f-string с .0f одинаково работает для int/float/Decimal (F-018: суммы # из Numeric-колонок приходят Decimal — форматирован` (стр. 3603)
- `_escapedef _escape(value: str) -> str: return escape(value).replace("\n", "<br/>")` (стр. 3612)
- `build_deposit_exportdef build_deposit_export( db: Any, client: Client, overview: Any,` (стр. 3625)
- `OwnerSummaryExportData.moneydef money(value: float) -> str: return f"{float(value):,.0f} ₽".replace(",", " ")` (стр. 3640)
- `build_deposit_export_alldef build_deposit_export_all( db: Any, *, date_from: str | None = None, date_to: str | None = None,` (стр. 3724)
...
- `_disable_integration_on_revokeddef _disable_integration_on_revoked(db: Any, connection_id: str | None = None) -> None: """Отключить Google Calendar при отозванном токене. Если задан connection_id — удаляем тольк` (стр. 455)
- `_client_configdef _client_config(settings: Settings) -> dict[str, Any]: """Базовый client_config для построения OAuth-запросов.""" redirect_uri = settings.google_calendar_redirect_uri return { "` (стр. 482)
- `build_auth_urldef build_auth_url( settings: Settings, state: str, db: Any = None, *, fallback_redirect_uri: str = ""` (стр. 492)
- `exchange_codedef exchange_code( settings: Settings, code: str, db: Any = None, *, fallback_redirect_uri: str = ""` (стр. 514)
- `class _GoogleApiError(Exception):` (стр. 548)
- `_GoogleApiError.__init__def __init__( self, status: int, message: str = "", *, reason: str | None = None, details: str | None = None,` (стр. 556)
- `_google_error_from_responsedef _google_error_from_response(resp: Any) -> tuple[str | None, str | None]: """Извлечь (reason, details) из тела ошибки Google API, если возможно. Calendar API: {"error": {"reason` (стр. 570)
- `_refresh_access_tokendef _refresh_access_token( settings: Settings, tokens: dict[str, Any], db: Any = None` (стр. 594)
- `_calendar_requestdef _calendar_request( db: Any, settings: Settings, method: str, path: str, *, params: dict[str, Any] | None = None, body: dict[str, Any] | None = None, conn: dict[str, Any] | None` (стр. 619)
- `_source_labeldef _source_label(source: Any) -> str: """Подпись источника записи для Google-события.""" return SOURCE_LABELS.get(source or "", "CRM")` (стр. 685)
- `_booking_event_bodydef _booking_event_body(booking: Any, settings: Settings) -> dict[str, Any]: """Сформировать тело Google-события из записи Booking.""" from zoneinfo import ZoneInfo # type: ignore ` (стр. 690)
- `sync_booking_to_calendardef sync_booking_to_calendar( db: Any, settings: Settings, booking: Any, *, action: str = "upsert"` (стр. 740)
- `_connection_event_idsdef _connection_event_ids(booking: Any, connections: list[dict[str, Any]]) -> dict[str, str]: """Карта {connection_id: event_id} для записи. Совместимость: у записей, созданных до ` (стр. 785)
- `_sync_booking_to_calendar_impldef _sync_booking_to_calendar_impl( db: Any, settings: Settings, booking: Any, *, action: str` (стр. 805)
- `_load_sync_tokendef _load_sync_token(db: Any) -> str | None: """syncToken первого подключения (совместимость) или None.""" connections = _read_connections(db) if not connections: return None retur` (стр. 941)
- `_save_sync_tokendef _save_sync_token(db: Any, sync_token: str | None) -> None: """Сохранить syncToken первого подключения (совместимость).""" connections = _read_connections(db) if not connections` (стр. 949)
- `_set_connection_sync_tokendef _set_connection_sync_token(conn: dict[str, Any], sync_token: str | None) -> None: """Отметить syncToken подключения (сохранится через _persist_dirty).""" conn["sync_token"] = s` (стр. 959)
- `last_sync_atdef last_sync_at(db: Any) -> str | None: """ISO-метка последней успешной обратной синхронизации или None.""" AppSetting = _appsetting_model() row = db.get(AppSetting, GOOGLE_CALEND` (стр. 965)
- `_save_last_syncdef _save_last_sync(db: Any) -> None: AppSetting = _appsetting_model() row = db.get(AppSetting, GOOGLE_CALENDAR_LAST_SYNC_KEY) now = datetime.now(UTC).isoformat() if row is None: r` (стр. 974)
- `_parse_google_datetimedef _parse_google_datetime(raw: str) -> datetime | None: """RFC3339 (dateTime или date) -> aware datetime, или None."""` (стр. 986)
- `_event_start_enddef _event_start_end( event: dict[str, Any], settings: Settings` (стр. 994)
- `_parse_event_descriptiondef _parse_event_description(description: Any) -> dict[str, str]: """Извлечь поля «Ключ: значение» из описания Google-события.""" fields: dict[str, str] = {} if not description: re` (стр. 1019)
- `_extract_plate_from_textdef _extract_plate_from_text(text: str) -> str: """Найти госномер в свободном тексте (или пустую строку). Сначала российские (авто + мото), затем иностранные. Иностранные распознаё` (стр. 1163)
- `_extract_phone_from_textdef _extract_phone_from_text(text: str) -> str: """Найти российский мобильный телефон в свободном тексте (или пустую строку). Перебор всех подстрок 10-11 цифр: соседние цифры (напр` (стр. 1187)
- `_extract_name_from_textdef _extract_name_from_text(text: str) -> str: """Найти имя клиента по словарю русских имён (или пустую строку).""" for token in re.findall(r"[А-ЯЁа-яё]+", text):` (стр. 1211)
- `_is_plausible_namedef _is_plausible_name(word: str) -> bool: """Подходит ли слово на роль имени клиента (кириллица, не служебное).""" if not re.fullmatch(r"[А-ЯЁа-яё]+", word):` (стр. 1220)
- `_extract_name_by_phone_neighborhooddef _extract_name_by_phone_neighborhood(text: str, phone: str) -> str: """Определить имя клиента по соседству с телефоном (или пустую строку). Когда у события есть свободный текст,` (стр. 1231)
- `_title_case_wordsdef _title_case_words(value: str) -> str: """Привести каждое слово к виду «С заглавной», сохранив аббревиатуры. «тойота камри» -> «Тойота Камри», «BMW x5» -> «BMW X5», «BMW» остане` (стр. 1256)
- `_extract_vehicle_from_textdef _extract_vehicle_from_text(text: str) -> str: """Найти марку и модель автомобиля в свободном тексте (или пустую строку).""" from .schemas import normalize_vehicle_name lowered ` (стр. 1270)
- `_normalize_for_matchdef _normalize_for_match(value: str) -> str: """Нижний регистр без «ё» и лишних пробелов — для сопоставления названий.""" return re.sub(r"\s+", " ", value.lower().replace("ё", "е")` (стр. 1308)
- `_match_service_in_textdef _match_service_in_text(service_names: list[str], text: str) -> str: """Найти услугу из каталога в тексте; вернуть пусто, если нет. Сопоставляем по префиксу названия услуги (от ` (стр. 1313)
- `_parse_event_text_loosedef _parse_event_text_loose(text: Any, service_names: list[str]) -> dict[str, str]: """Определить поля (госномер, телефон, имя, авто, услуга) из свободного текста. Данные могут идт` (стр. 1344)
- `_active_service_namesdef _active_service_names(db: Any) -> list[str]: """Названия активных услуг из каталога — для распознавания в тексте события.""" from .models import Service return [row.name for ro` (стр. 1416)
- `_booking_by_google_eventdef _booking_by_google_event(db: Any, event_id: str) -> Any | None: """Запись по идентификатору события Google из любого подключённого календаря.""" from .models import Booking boo` (стр. 1423)
- `_event_updated_utcdef _event_updated_utc(event: dict[str, Any]) -> datetime | None: """Метка «когда событие последний раз правилось» (event.updated) в UTC.""" raw = event.get("updated") if not raw: ` (стр. 1445)
- `_event_is_staledef _event_is_stale(event: dict[str, Any], booking: Any) -> bool: """Правилось ли событие ПОЗЖЕ последней записи записи в Google. True — событие не менялось после того, как запись ` (стр. 1456)
- `_update_booking_from_eventdef _update_booking_from_event( db: Any, booking: Any, event: dict[str, Any], settings: Settings` (стр. 1475)
- `_find_duplicate_bookingdef _find_duplicate_booking( db: Any, client_id: str, date: str, time: str` (стр. 1587)
- `_create_booking_from_eventdef _create_booking_from_event( db: Any, event: dict[str, Any], settings: Settings` (стр. 1612)
- `_apply_calendar_eventdef _apply_calendar_event( db: Any, settings: Settings, event: dict[str, Any], result: dict[str, Any]` (стр. 1734)
- `pull_calendar_changesdef pull_calendar_changes(db: Any, settings: Settings) -> dict[str, Any]: """Обратная синхронизация «Google Calendar -> CRM» по ВСЕМ подключениям. Инкрементальная через syncToken (` (стр. 1772)
- `_empty_pull_resultdef _empty_pull_result() -> dict[str, Any]: return { "ok": True, "skipped": False, "created": 0, "updated": 0, "cancelled": 0, "duplicates": 0, "error": None, }` (стр. 1805)
...
### backend/app/main.py (25757 строк)

Роуты (149):

```
  `POST /api/auth/client` -> `register_or_login_client` (декоратор: стр. 6291)
  `POST /api/auth/staff/login` -> `staff_login` (декоратор: стр. 6423)
  `POST /api/auth/telegram` -> `authenticate_via_telegram` (декоратор: стр. 6473)
  `POST /api/auth/staff/link` -> `link_staff_account` (декоратор: стр. 6495)
  `POST /api/auth/telegram-owner` -> `authenticate_primary_owner_via_telegram` (декоратор: стр. 6545)
  `POST /api/auth/switch-role` -> `switch_role` (декоратор: стр. 6582)
  `POST /api/owner/database-reset/start` -> `start_owner_database_reset` (декоратор: стр. 8547)
  `POST /api/owner/database-reset/approve` -> `approve_owner_database_reset` (декоратор: стр. 8603)
  `POST /api/owner/database-reset/execute` -> `execute_owner_database_reset` (декоратор: стр. 8657)
  `POST /api/owner/data-cleanup/preview` -> `preview_data_cleanup` (декоратор: стр. 8946)
  `POST /api/owner/data-cleanup/execute` -> `execute_data_cleanup` (декоратор: стр. 8956)
  `GET /api/owner/data-cleanup/batches` -> `list_data_cleanup_batches` (декоратор: стр. 9027)
  `GET /api/owner/trash` -> `list_trash` (декоратор: стр. 9077)
  `POST /api/owner/trash/restore` -> `restore_trash` (декоратор: стр. 9116)
  `POST /api/owner/trash/purge` -> `purge_trash` (декоратор: стр. 9174)
  `GET /api/owner/exports/{kind}` -> `download_owner_export` (декоратор: стр. 10138)
  `POST /api/owner/exports/{kind}/telegram` -> `send_owner_export_to_telegram` (декоратор: стр. 10170)
  `POST /api/owner/reports/{period}/{segment}/telegram` -> `send_owner_summary_report_to_telegram` (декоратор: стр. 10202)
  `PATCH /api/clients/me` -> `update_client_me` (декоратор: стр. 11400)
  `DELETE /api/clients/{client_id}` -> `delete_client` (декоратор: стр. 11480)
  `PATCH /api/clients/{client_id}/card` -> `update_client_card` (декоратор: стр. 11505)
  `POST /api/clients` -> `create_client` (декоратор: стр. 11605)
  `GET /api/health` -> `health` (декоратор: стр. 11661)
  `GET /api/debug/db` -> `debug_db` (декоратор: стр. 11686)
  `GET /api/debug/mojibake-scan` -> `debug_mojibake_scan` (декоратор: стр. 11767)
  `POST /api/debug/mojibake-repair` -> `debug_mojibake_repair` (декоратор: стр. 11777)
  `GET /api/content` -> `get_public_content` (декоратор: стр. 11967)
  `PUT /api/content` -> `save_content` (декоратор: стр. 11981)
  `POST /api/upload` -> `upload_file` (декоратор: стр. 12045)
  `GET /api/uploads/{filename}` -> `serve_upload` (декоратор: стр. 12097)
  `POST /api/contact` -> `submit_contact` (декоратор: стр. 12118)
  `POST settings.telegram_webhook_path` -> `handle_telegram_webhook` (декоратор: стр. 12180)
  `POST /api/telegram/webhook/sync` -> `resync_telegram_webhook` (декоратор: стр. 12230)
  `GET /api/stock-categories` -> `list_stock_categories` (декоратор: стр. 12312)
  `POST /api/stock-categories` -> `create_stock_category` (декоратор: стр. 12325)
  `PATCH /api/stock-categories/{category_id}` -> `update_stock_category` (декоратор: стр. 12350)
  `DELETE /api/stock-categories/{category_id}` -> `delete_stock_category` (декоратор: стр. 12383)
  `GET /api/bookings/availability` -> `get_booking_availability` (декоратор: стр. 12417)
  `POST /api/bookings` -> `create_booking` (декоратор: стр. 12488)
  `PATCH /api/bookings/{booking_id}` -> `update_booking` (декоратор: стр. 13767)
  `DELETE /api/bookings/{booking_id}` -> `delete_booking` (декоратор: стр. 14451)
  `POST /api/bookings/{booking_id}/services` -> `add_booking_service` (декоратор: стр. 14551)
  `POST /api/bookings/{booking_id}/additional-services` -> `add_booking_additional_service` (декоратор: стр. 14617)
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 14754)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 14826)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 14919)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 14999)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 15077)
  `POST /api/broadcasts/workers` -> `broadcast_to_workers` (декоратор: стр. 15147)
  `POST /api/notifications/{notification_id}/take-to-work` -> `take_notification_to_work` (декоратор: стр. 15184)
  `POST /api/notifications/{notification_id}/complete` -> `complete_notification_task` (декоратор: стр. 15216)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 15248)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 15284)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 15332)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 15381)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 15412)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 15448)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 15490)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 15612)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 15658)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 15740)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 15896)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 15935)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 16061)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 16107)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 16173)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 16221)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 16285)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 16378)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 17178)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 17623)
  `DELETE /api/owner/piggy-bank/transactions/{tx_id}` -> `delete_piggy_bank_transaction` (декоратор: стр. 17806)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 18173)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 18217)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 18252)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 18279)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 18305)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 18323)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 18335)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 18353)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 18366)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 18445)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 18530)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 18590)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 18815)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 18911)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 18991)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 19059)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 19238)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 19371)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 19521)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 19663)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 19809)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 19863)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 19939)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 19997)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 20045)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 20119)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 20143)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 20203)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 20245)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 20269)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 20293)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 20328)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 20399)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 20497)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 20516)
  `POST /api/owner/integrations/google/invites` -> `create_google_calendar_invite` (декоратор: стр. 20553)
  `DELETE /api/owner/integrations/google/connections/{connection_id}` -> `delete_google_calendar_connection` (декоратор: стр. 20588)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 20614)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 20647)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 20659)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 20683)
  `GET /api/cron/reminders` -> `run_reminders_cron` (декоратор: стр. 20708)
  `POST /api/owner/inactive-clients/remind-admin` -> `remind_admin_about_inactive_clients` (декоратор: стр. 20740)
  `POST /api/owner/reminders/dispatch` -> `dispatch_owner_booking_reminders` (декоратор: стр. 20798)
  `GET /api/cron/reports` -> `run_reports_cron` (декоратор: стр. 20817)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 20863)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 20899)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 21002)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 21102)
  `GET /api/owner/outsource/payroll` -> `get_owner_outsource_payroll` (декоратор: стр. 21171)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 21241)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 21522)
  `DELETE /api/payroll/entries/{entry_id}` -> `delete_payroll_entry` (декоратор: стр. 21683)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 21773)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 21998)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 22086)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 22260)
  `GET /api/owner/money-flow` -> `get_owner_money_flow` (декоратор: стр. 22577)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 23120)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 23134)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 23523)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 23972)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 24380)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 24641)
  `PATCH /api/owner/owners/{owner_id}/master-role` -> `set_owner_master_role` (декоратор: стр. 24891)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 24918)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 25170)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 25308)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 25368)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 25562)
  `GET /api/auth/role-preview` -> `get_role_preview` (декоратор: стр. 25576)
  `POST /api/auth/role-preview` -> `set_role_preview` (декоратор: стр. 25591)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 25643)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 25655)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 25679)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 25687)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 25700)
```

Классы и функции (286):

- `_resolve_frontend_distdef _resolve_frontend_dist() -> Path: """Каталог собранного React-фронтенда. В обычном режиме — <project>/frontend/dist (родитель каталога app/). В frozen-режиме (PyInstaller bundl` (стр. 613)
- `class AsciiJSONResponse(JSONResponse):` (стр. 829)
- `AsciiJSONResponse.renderdef render(self, content) -> bytes: # Пробуем отдать чистый ASCII \u0412... чтобы WebView, игнорирующий charset, всё равно показал В # Если WebView всё равно декодирует как windows` (стр. 830)
- `_validation_error_handlerasync def _validation_error_handler( request: Request, exc: RequestValidationError` (стр. 877)
- `_check_rate_limitdef _check_rate_limit(ip: str) -> None: global _last_rate_limit_cleanup now = time_module.time() window_start = now - _LOGIN_RATE_LIMIT_WINDOW # Periodic cleanup of stale entries t` (стр. 911)
- `add_security_headersasync def add_security_headers(request: Request, call_next): response = await call_next(request) for key, value in SECURITY_HEADERS.items():` (стр. 1004)
- `serve_single_page_appasync def serve_single_page_app(request: Request, call_next): path = request.url.path index_file = frontend_dist / "index.html" if request.method not in {"GET", "HEAD"}: return awa` (стр. 1030)
- `on_startupdef on_startup() -> None: global bot_thread Base.metadata.create_all(bind=engine) _apply_runtime_migrations() db = next(get_db()) try: seed_database(db, include_demo_staff=settings` (стр. 1078)
- `start_google_sync_threaddef start_google_sync_thread() -> None: """Запускает фоновый цикл синхронизации «Google Calendar -> CRM» (daemon-поток). Поток нужен только при настроенной интеграции: учётные данн` (стр. 1163)
- `_nowdef _now() -> datetime: return datetime.now(timezone.utc)` (стр. 1193)
- `_local_day_boundsdef _local_day_bounds(date_str: str) -> tuple[datetime, datetime]: """Границы локального дня (DD.MM.YYYY) в UTC: (00:00, 23:59:59) местного времени. Периоды ЗП считаются по локальн` (стр. 1198)
- `_as_utcdef _as_utc(value: datetime) -> datetime: if value.tzinfo is None: return value.replace(tzinfo=timezone.utc) return value.astimezone(timezone.utc)` (стр. 1213)
- `_format_moscow_dtdef _format_moscow_dt(dt: datetime | None) -> str: if dt is None: return "" msk = dt.astimezone(timezone(timedelta(hours=3))) return msk.strftime("%H:%M %d.%m.%Y")` (стр. 1222)
- `_request_ipdef _request_ip(request: Request) -> str: # For rate limiting, prefer direct client IP to prevent X-Forwarded-For spoofing if request.client is not None and request.client.host: re` (стр. 1233)
- `_safe_textdef _safe_text(value: Any) -> str: return value if isinstance(value, str) else ""` (стр. 1253)
- `_client_by_phonedef _client_by_phone(db: Session, phone: str) -> Client | None: if not phone.strip():` (стр. 1261)
- `_owner_querydef _owner_query(): return ( select(StaffUser) .where(StaffUser.role == "owner") .order_by(StaffUser.created_at.asc(), StaffUser.id.asc()) )` (стр. 1299)
- `_primary_ownerdef _primary_owner(db: Session) -> StaffUser | None: return db.scalar( select(StaffUser) .where(StaffUser.role == "owner", StaffUser.is_primary_owner.is_(True)) .order_by(StaffUser` (стр. 1315)
- `_owner_master_conditiondef _owner_master_condition() -> Any: """Владелец, которому дополнительно выдана роль мастера (extra_roles != []). Такие владельцы попадают в списки мастеров: назначение на записи,` (стр. 1331)
- `_is_owner_masterdef _is_owner_master(worker: StaffUser) -> bool: return worker.role == "owner" and bool(worker.extra_roles)` (стр. 1346)
- `_ensure_permanent_telegram_ownersdef _ensure_permanent_telegram_owners(db: Session) -> None: """Upsert explicitly configured owners without reassigning existing rows.""" for staff_id, login, chat_id, owner_name in` (стр. 1353)
- `_ensure_owner_accountsdef _ensure_owner_accounts(db: Session) -> None: owners = db.scalars(_owner_query()).all() primary_owner = next((owner for owner in owners if owner.is_primary_owner), None) if prim` (стр. 1459)
- `_device_labeldef _device_label(user_agent: str) -> str: if "Telegram-Android" in user_agent: return "Telegram Android" if "Telegram-iOS" in user_agent: return "Telegram iPhone" if "iPhone" in u` (стр. 1587)
- `_apply_runtime_migrationsdef _apply_runtime_migrations() -> None: from sqlalchemy import text def boolean_default_sql(value: bool) -> str:` (стр. 1623)
- `AsciiJSONResponse.boolean_default_sqldef boolean_default_sql(value: bool) -> str: if engine.dialect.name == "postgresql": return "TRUE" if value else "FALSE" return "1" if value else "0"` (стр. 1627)
- `AsciiJSONResponse.ensure_postgres_varchar_lengthdef ensure_postgres_varchar_length( table_name: str, column_name: str, minimum_length: int` (стр. 1637)
- `AsciiJSONResponse.ensure_postgres_text_columndef ensure_postgres_text_column(table_name: str, column_name: str) -> None: if engine.dialect.name != "postgresql": return column = next( ( item for item in inspect(engine).get_col` (стр. 1683)
- `_apply_default_shift_paydef _apply_default_shift_pay(db: Session) -> None: """Один раз выставляет оклад за смену DEFAULT_SHIFT_PAY сотрудникам (кроме владельцев), у которых ставка не задана (0). Выполняет` (стр. 2875)
- `_cyr_countdef _cyr_count(value: str) -> int: return sum(1 for ch in value if "\u0400" <= ch <= "\u04FF")` (стр. 2905)
- `_strict_utf8_variantsdef _strict_utf8_variants(value: str) -> list[str]: """Детерминированные кандидаты ремонта: строка является UTF-8 байтами, ошибочно декодированными как cp1251/cp1252/latin-1. Никак` (стр. 2915)
- `_repair_text_strict_stepdef _repair_text_strict_step(value: str) -> str | None: """Один шаг строгого ремонта: кандидат обязан содержать кириллицу или ₽.""" for fixed in _strict_utf8_variants(value):` (стр. 2969)
- `_repair_text_valuedef _repair_text_value(value: str) -> str: """Строгий ремонт mojibake (UTF-8 байты, декодированные как cp1251/cp1252/latin-1). Корректный текст не изменяется: кириллица/латиница/em` (стр. 2985)
- `_repair_nested_textdef _repair_nested_text(value): if isinstance(value, str):` (стр. 3019)
- `_repair_model_text_fieldsdef _repair_model_text_fields(db: Session, model, fields: tuple[str, ...]) -> bool: changed = False for item in db.scalars(select(model)).all():` (стр. 3039)
- `_sanitize_notification_messagedef _sanitize_notification_message(message: str) -> str: fixed = _repair_text_value(message).strip() for source, target in { "вЂў": "•", "в€¢": "•", "вВў": "•", "â€¢": "•", "вЂ”": ` (стр. 3067)
- `_repair_text_datadef _repair_text_data(db: Session) -> None: changed = False for model, fields in _TEXT_REPAIR_TARGETS: changed |= _repair_model_text_fields(db, model, fields) for notification in d` (стр. 3116)
- `get_db_with_text_repairdef get_db_with_text_repair() -> Any: """get_db с гарантированным одноразовым ремонтом mojibake. Vercel serverless не исполняет FastAPI startup-события, поэтому ремонт выполняется ` (стр. 3167)
- `_settingdef _setting(db: Session, key: str, default: dict) -> dict: row = db.get(AppSetting, key) if row: return row.value row = AppSetting(key=key, value=default) db.add(row) db.flush() r` (стр. 3203)
- `_merge_setting_dictdef _merge_setting_dict(value: Any, default: dict[str, Any]) -> dict[str, Any]: if not isinstance(value, dict):` (стр. 3223)
- `_normalize_client_vehiclesdef _normalize_client_vehicles( vehicles: list[ClientVehiclePayload] | list[dict[str, Any]] | None, *, fallback_car: str = "", fallback_plate: str = "",` (стр. 3247)
- `_client_vehicles_mapdef _client_vehicles_map(db: Session) -> dict[str, Any]: return _setting(db, "client_vehicles", {})` (стр. 3359)
- `_client_vehicles_payloaddef _client_vehicles_payload(db: Session, client: Client) -> list[ClientVehiclePayload]: raw = _client_vehicles_map(db).get(client.id, []) return _normalize_client_vehicles( raw, f` (стр. 3367)
- `_save_client_vehiclesdef _save_client_vehicles( db: Session, client_id: str, vehicles: list[ClientVehiclePayload]` (стр. 3381)
- `_client_phone_verifications_mapdef _client_phone_verifications_map(db: Session) -> dict[str, Any]: value = _setting(db, CLIENT_PHONE_VERIFICATIONS_KEY, {}) return value if isinstance(value, dict) else {}` (стр. 3401)
- `_client_verified_phone_digitsdef _client_verified_phone_digits(db: Session, telegram_id: str | None) -> str | None: if not telegram_id: return None entry = _client_phone_verifications_map(db).get(str(telegram_` (стр. 3411)
- `_client_phone_is_verifieddef _client_phone_is_verified(db: Session, telegram_id: str | None, phone: str) -> bool: if not phone.strip():` (стр. 3431)
- `_require_client_phone_verificationdef _require_client_phone_verification( db: Session, telegram_id: str | None, phone: str` (стр. 3457)
- `_client_payloaddef _client_payload(client: Client | None) -> ClientProfilePayload | None: if client is None: return None with Session(engine) as vehicles_db: vehicles = _client_vehicles_payload(v` (стр. 3479)
- `_client_summary_payloaddef _client_summary_payload( client: Client, db: Session | None = None` (стр. 3519)
- `_booking_status_labeldef _booking_status_label(status_value: str) -> str: return { "new": "Новая заявка", "confirmed": "Подтверждена", "scheduled": "Запланирована", "in_progress": "В работе", "complete` (стр. 3577)
- `_booking_status_short_labeldef _booking_status_short_label(status_value: str) -> str: return { "new": "Новая", "confirmed": "Подтв.", "scheduled": "Запл.", "in_progress": "В работе", "completed": "Завершена"` (стр. 3603)
- `_format_local_datetimedef _format_local_datetime(value: datetime) -> str: return _as_utc(value).astimezone().strftime("%d.%m.%Y %H:%M")` (стр. 3629)
- `_parse_booking_datetimedef _parse_booking_datetime(date_value: str, time_value: str) -> datetime | None: raw = f"{date_value.strip()} {time_value.strip()}" for fmt in ("%d.%m.%Y %H:%M", "%Y-%m-%d %H:%M")` (стр. 3637)
- `_py_weekday_to_schedule_indexdef _py_weekday_to_schedule_index(py_weekday: int) -> int: # Конвенция day_index (сид и фронт getScheduleDayIndex=(getDay()+1)%7): Сб=0, Вс=1, Пн=2..Пт=6. # Python weekday(): Пн=0.` (стр. 3657)
- `_parse_time_to_minutesdef _parse_time_to_minutes(time_value: str) -> int | None: raw = time_value.strip() if len(raw) != 5 or raw[2] != ":": return None try: hours = int(raw[:2]) minutes = int(raw[3:]) ` (стр. 3668)
- `_today_labeldef _today_label() -> str: return datetime.now().strftime("%d.%m.%Y")` (стр. 3696)
- `_build_schedule_slotsdef _build_schedule_slots( open_minutes: int, close_minutes: int, step_minutes: int = 30` (стр. 3704)
- `_booking_requires_scheduled_slotdef _booking_requires_scheduled_slot(status_value: str) -> bool: return status_value in BOOKING_ACTIVE_STATUSES` (стр. 3728)
- `_booking_slot_fields_changeddef _booking_slot_fields_changed(booking: Booking, updates: dict) -> bool: if "date" in updates and (updates.get("date") or "").strip() != (booking.date or "").strip():` (стр. 3736)
- `_booking_time_rangedef _booking_time_range( date_value: str, time_value: str, duration: int` (стр. 3756)
- `_time_ranges_overlapdef _time_ranges_overlap( start_at: datetime, end_at: datetime, other_start_at: datetime, other_end_at: datetime,` (стр. 3774)
- `_ensure_booking_datetime_not_in_pastdef _ensure_booking_datetime_not_in_past(date_value: str, time_value: str, role: str) -> None: if role in {"admin", "owner"}: return scheduled_at = _parse_booking_datetime(date_val` (стр. 3792)
- `_ensure_booking_within_scheduledef _ensure_booking_within_schedule( db: Session, date_value: str, time_value: str, duration: int` (стр. 3826)
- `AsciiJSONResponse._minutes_labeldef _minutes_label(value: int) -> str: value = max(0, value) day_shift, minutes_of_day = divmod(value, 24 * 60) label = f"{minutes_of_day // 60:02d}:{minutes_of_day % 60:02d}" retu` (стр. 3888)
- `_candidate_day_stringsdef _candidate_day_strings(date_value: str) -> list[str]: """Дата слота + соседи (ловим пересечения через полночь).""" try: base = _dmy_to_date(date_value) except (TypeError, Value` (стр. 3934)
- `_overlapping_bookingsdef _overlapping_bookings( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int,` (стр. 3943)
- `_box_is_availabledef _box_is_available( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str,` (стр. 3981)
- `_pick_available_boxdef _pick_available_box( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, resource_group: str | None = None, preferred_box: str | None = Non` (стр. 4019)
- `_booking_slot_availabilitydef _booking_slot_availability( db: Session, *, date_value: str, duration: int, service_id: str | None = None, resource_group: str | None = None,` (стр. 4081)
- `_ensure_booking_has_no_conflictsdef _ensure_booking_has_no_conflicts( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str, worker_ids: set[str], …` (стр. 4233)
- `_load_penaltiesdef _load_penalties( db: Session, *, worker_ids: set[str] | None = None` (стр. 4306)
- `_complaints_by_workerdef _complaints_by_worker(penalties: list[Penalty]) -> dict[str, list[Penalty]]: grouped: dict[str, list[Penalty]] = {} for penalty in penalties: grouped.setdefault(penalty.worker_` (стр. 4332)
- `_normalize_worker_rulesdef _normalize_worker_rules(db: Session) -> None: changed = False workers = db.scalars(select(StaffUser).where(StaffUser.role == "worker")).all() for worker in workers: capped_perc` (стр. 4346)
- `_worker_payloaddef _worker_payload(worker: StaffUser) -> WorkerPayload: return WorkerPayload( id=worker.id, role=worker.role, # type: ignore[arg-type] name=worker.name, experience=worker.experien` (стр. 4400)
- `_payroll_entry_payloaddef _payroll_entry_payload(entry: PayrollEntry, actor_name: str) -> PayrollEntryPayload: return PayrollEntryPayload( id=entry.id, workerId=entry.worker_id, kind=entry.kind, # type:` (стр. 4442)
- `_worker_payroll_summariesdef _worker_payroll_summaries( db: Session, workers: list[StaffUser], complaints_by_worker: dict[str, list[Penalty]],` (стр. 4470)
- `_worker_payroll_summaries_from_datadef _worker_payroll_summaries_from_data( db: Session, workers: list[StaffUser], completed_bookings: list[Booking], entries: list[PayrollEntry], complaints_by_worker: dict[str, list` (стр. 4518)
- `_worker_payload_with_payrolldef _worker_payload_with_payroll( worker: StaffUser, payroll_summaries: dict[str, WorkerPayrollSummaryPayload] | None = None,` (стр. 4702)
- `_booking_payloaddef _booking_payload( booking: Booking, complaints_by_worker: dict[str, list[Penalty]] | None = None` (стр. 4726)
- `_notification_payloaddef _notification_payload(notification: Notification) -> NotificationPayload: return NotificationPayload( id=notification.id, recipientRole=notification.recipient_role, # type: ign` (стр. 4896)
- `_stock_payloaddef _stock_payload(item: StockItem) -> StockItemPayload: return StockItemPayload( id=item.id, name=item.name, qty=item.qty, unit=item.unit, unitPrice=item.unit_price, category=item` (стр. 4918)
- `_expense_payloaddef _expense_payload(expense: Expense) -> ExpensePayload: return ExpensePayload( id=expense.id, title=expense.title, amount=money_int(expense.amount), category=expense.category, da` (стр. 4934)
- `_penalty_payloaddef _penalty_payload(penalty: Penalty) -> PenaltyPayload: worker_name = penalty.worker.name if penalty.worker else "" return PenaltyPayload( id=penalty.id, workerId=penalty.worker_` (стр. 4958)
- `_service_payloaddef _service_payload(service: Service) -> ServicePayload: return ServicePayload( id=service.id, name=service.name, category=service.category, price=service.price, duration=service.` (стр. 4988)
- `_box_payloaddef _box_payload(box: Box) -> BoxPayload: return BoxPayload( id=box.id, name=box.name, resourceGroup=(box.resource_group or DEFAULT_RESOURCE_GROUP).strip() or DEFAULT_RESOURCE_GROU` (стр. 5032)
- `_visible_boxesdef _visible_boxes(db: Session) -> list[Box]: boxes = db.scalars(select(Box).order_by(Box.name.asc())).all() wash_order_map = {name: index for index, name in enumerate(WASH_BOX_NAM` (стр. 5056)
- `AsciiJSONResponse.box_orderdef box_order(box: Box) -> tuple[int, int, str, str]: resource_group = _resource_group_key( box.resource_group or _default_box_resource_group(box) ) if resource_group == DETAILING_` (стр. 5066)
- `_schedule_payloaddef _schedule_payload(entry: ScheduleEntry) -> SchedulePayload: return SchedulePayload( dayIndex=entry.day_index, day=entry.day_label, open=entry.open_time, close=entry.close_time,` (стр. 5102)
- `_settings_payloaddef _settings_payload(db: Session) -> SettingsBundlePayload: admin_profile_default = { "name": "Администратор", "email": "", "phone": "", "telegramChatId": "", } admin_notification` (стр. 5122)
- `_empty_settings_payloaddef _empty_settings_payload() -> SettingsBundlePayload: return SettingsBundlePayload( adminProfile=AdminProfilePayload( name="", email="", phone="", telegramChatId="" ), adminNotif` (стр. 5346)
- `_scoped_settings_payloaddef _scoped_settings_payload( db: Session, role: str, actor_id: str` (стр. 5430)
- `_session_payloaddef _session_payload(session_data: dict) -> SessionPayload: return SessionPayload( role=session_data["role"], actorId=session_data["actorId"], sessionId=session_data.get("sessionId` (стр. 5516)
- `_mark_overdue_bookings_for_admin_reviewdef _mark_overdue_bookings_for_admin_review(db: Session) -> None: now_local = datetime.now().replace(second=0, microsecond=0) changed = False for booking in db.scalars( select(Book` (стр. 5542)
- `_build_bootstrapdef _build_bootstrap( db: Session, session_data: dict, role_preview: RolePreviewState | None = None,` (стр. 5586)
- `_resolve_user_from_init_datadef _resolve_user_from_init_data(authorization: str, db: Session) -> dict | None: try: validated = validate_telegram_init_data( authorization, settings.telegram_bot_token, max_age_` (стр. 5914)
- `_role_preview_setting_keydef _role_preview_setting_key(telegram_id: str) -> str: return f"{ROLE_PREVIEW_SETTING_PREFIX}{telegram_id}"` (стр. 6045)
- `_read_role_previewdef _read_role_preview(db: Session, telegram_id: str) -> dict: if not telegram_id or telegram_id not in CREATOR_TELEGRAM_IDS: return {} row = db.get(AppSetting, _role_preview_setti` (стр. 6049)
- `_write_role_previewdef _write_role_preview(db: Session, telegram_id: str, state: dict | None) -> None: if not telegram_id or telegram_id not in CREATOR_TELEGRAM_IDS: return key = _role_preview_settin` (стр. 6065)
- `_telegram_id_from_init_datadef _telegram_id_from_init_data(authorization: str) -> str: """Возвращает проверенный Telegram id или '' — без выброса исключений. Используется там, где отсутствие валидного initDa` (стр. 6082)
- `_preview_actor_querydef _preview_actor_query(db: Session, role: str) -> list[RolePreviewActor]: """Список аккаунтов, от имени которых можно посмотреть роль.""" if role == "client": rows = db.scalars( ` (стр. 6099)
- `_preview_session_datadef _preview_session_data(db: Session, role: str, actor_id: str) -> dict | None: """Собирает session_data для предпросмотра указанного аккаунта.""" if role == "client": client = db` (стр. 6134)
- `_apply_role_previewdef _apply_role_preview(db: Session, authorization: str, session_data: dict) -> dict: """Подменяет роль/аккаунт, если создатель включил предпросмотр.""" if not CREATOR_TELEGRAM_IDS` (стр. 6159)
- `_role_preview_statedef _role_preview_state(db: Session, authorization: str, session_data: dict) -> RolePreviewState: """Полное состояние переключателя для текущего пользователя.""" telegram_id = _tel` (стр. 6183)
- `_require_sessiondef _require_session( authorization: str | None = Header(default=None), db: Session = Depends(get_db),` (стр. 6221)
- `_extract_telegram_id_from_init_datadef _extract_telegram_id_from_init_data(authorization: str) -> str: if not authorization: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing initData") t` (стр. 6252)
- `_ensure_staff_roledef _ensure_staff_role(session_data: dict, allowed: set[str]) -> None: if session_data["role"] not in allowed: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Fo` (стр. 6615)
- `_validated_booking_workersdef _validated_booking_workers( db: Session, workers: list[BookingWorkerPayload]` (стр. 6625)
- `_ensure_worker_percent_capdef _ensure_worker_percent_cap(workers: list) -> None: """Суммарный процент бригады не может превышать 100 (M-001). Фронт такие бригады блокирует, API — нет: сплит переплачивал мас` (стр. 6721)
- `_ensure_subtract_fits_netdef _ensure_subtract_fits_net(db: Session, booking: Booking, new_subtract_total: int) -> None: """Вычет не может превышать базу (M-002). Subtract-carve-out паркуется в копилку как ` (стр. 6743)
- `_booking_payload_for_responsedef _booking_payload_for_response(db: Session, booking: Booking) -> BookingPayload: worker_ids = {link.worker_id for link in booking.worker_links} penalties = _load_penalties(db, w` (стр. 6776)
- `_sync_booking_workersdef _sync_booking_workers( db: Session, booking: Booking, workers: list[BookingWorkerPayload]` (стр. 6788)
- `_sync_booking_materialsdef _sync_booking_materials( db: Session, booking: Booking, materials: list[BookingMaterialPayload]` (стр. 6818)
- `_send_telegram_safedef _send_telegram_safe(chat_id: str | None, text: str) -> None: if not chat_id: logger.warning("Пропущена отправка Telegram-уведомления: у получателя нет chat_id") return try: sen` (стр. 6837)
- `_telegram_display_namedef _telegram_display_name(telegram_user: dict, fallback: str) -> str: first_name = str(telegram_user.get("first_name") or "").strip() last_name = str(telegram_user.get("last_name"` (стр. 6857)
- `_owner_two_factor_recipientdef _owner_two_factor_recipient(db: Session) -> StaffUser: owner = _primary_owner(db) if owner is None: raise HTTPException( status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail` (стр. 6873)
- `_all_active_ownersdef _all_active_owners(db: Session) -> list[StaffUser]: """Возвращает всех активных владельцев, отсортированных по created_at asc.""" return list( db.scalars( select(StaffUser) .wh` (стр. 6903)
- `_all_owner_telegram_recipientsdef _all_owner_telegram_recipients(db: Session) -> list[StaffUser]: """Возвращает всех владельцев с непустым telegram_chat_id, отсортированных по created_at asc.""" return list( db` (стр. 6925)
- `_booking_reminder_target_datedef _booking_reminder_target_date(days_ahead: int = 1) -> str: return (datetime.now() + timedelta(days=days_ahead)).strftime("%d.%m.%Y")` (стр. 6953)
- `_get_booking_reminder_hoursdef _get_booking_reminder_hours(owner_settings: dict[str, Any]) -> int: """Вытащить из настроек владельца за сколько часов слать напоминание (1..168).""" raw = owner_settings.get("` (стр. 6958)
- `_worker_notification_settings_mapdef _worker_notification_settings_map(db: Session) -> dict[str, dict[str, Any]]: return _setting(db, "worker_notification_settings", {})` (стр. 6980)
- `_booking_reminder_statedef _booking_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, BOOKING_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6988)
- `_return_reminder_statedef _return_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, RETURN_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6996)
- `_shift_checklists_statedef _shift_checklists_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, SHIFT_CHECKLISTS_KEY, []) return value if isinstance(value, list) else []` (стр. 7004)
- `_admin_shift_inspections_statedef _admin_shift_inspections_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, ADMIN_SHIFT_INSPECTIONS_KEY, []) return value if isinstance(value, list) else []` (стр. 7014)
- `_compute_shift_attendancedef _compute_shift_attendance( inspections: list[dict], worker_id: str, date_from: date, date_to: date,` (стр. 7024)
- `_period_to_date_rangedef _period_to_date_range(period: str) -> tuple[date, date]: """ Преобразует строковый период в диапазон дат (date_from, date_to). - ``week`` → последние 7 дней - ``month`` → после` (стр. 7152)
- `_admin_shift_owner_bot_statedef _admin_shift_owner_bot_state(db: Session) -> dict[str, Any]: value = _setting(db, ADMIN_SHIFT_OWNER_BOT_STATE_KEY, {"pendingIssueByChat": {}}) return value if isinstance(value,` (стр. 7198)
- `_cleanup_booking_reminder_deliveriesdef _cleanup_booking_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=14) cleaned: dict[str, str] = {} for key, value in deliv` (стр. 7208)
- `_cleanup_return_reminder_deliveriesdef _cleanup_return_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=30) cleaned: dict[str, str] = {} for key, value in delive` (стр. 7228)
- `_booking_client_reminder_messagedef _booking_client_reminder_message(booking: Booking) -> str: add_block = _additional_services_block(booking) return ( "Напоминание о записи\n" f"Услуга: {booking.service}{add_blo` (стр. 7248)
...
- `_dispatch_booking_remindersdef _dispatch_booking_reminders( db: Session, *, target_date: str | None = None, force: bool = False,` (стр. 7292)
- `_dispatch_return_visit_remindersdef _dispatch_return_visit_reminders(db: Session) -> int: reminder_state = _return_reminder_state(db) deliveries = reminder_state.get("deliveries") if not isinstance(deliveries, di` (стр. 7603)
- `_shift_checklist_payloaddef _shift_checklist_payload(entry: dict[str, Any]) -> ShiftChecklistPayload: return ShiftChecklistPayload( id=str(entry.get("id") or ""), workerId=str(entry.get("workerId") or "")` (стр. 7713)
- `_chemistry_stock_itemsdef _chemistry_stock_items(db: Session) -> list[StockItem]: return db.scalars( select(StockItem) .where(StockItem.category == "Химия") .order_by(StockItem.name.asc()) ).all()` (стр. 7767)
- `_latest_shift_checklist_entrydef _latest_shift_checklist_entry( entries: list[dict[str, Any]], worker_id: str, phase: str` (стр. 7783)
- `_clean_data_url_prefixdef _clean_data_url_prefix(data_url: str) -> str: return data_url.split(",", 1)[1] if "," in data_url else data_url` (стр. 7805)
- `_decode_data_url_imagedef _decode_data_url_image(data_url: str) -> tuple[str, bytes]: raw = data_url.strip() if not raw.startswith("data:image/"):` (стр. 7813)
- `_admin_shift_inspection_suppliesdef _admin_shift_inspection_supplies(db: Session) -> list[dict[str, Any]]: items = db.scalars( select(StockItem) .where(StockItem.category.in_(("Химия", "Расходники"))) .order_by(S` (стр. 7879)
- `_admin_shift_inspection_payloaddef _admin_shift_inspection_payload( entry: dict[str, Any],` (стр. 7937)
- `_admin_shift_captiondef _admin_shift_caption(entry: dict[str, Any]) -> str: checked_supplies = [ item.get("name") for item in entry.get("supplies", []) if isinstance(item, dict) and item.get("checked"` (стр. 8026)
- `_admin_shift_owner_inline_keyboarddef _admin_shift_owner_inline_keyboard(inspection_id: str) -> dict[str, Any]: return { "inline_keyboard": [ [ { "text": "Подтвердить", "callback_data": f"shiftapprove:{inspection_i` (стр. 8078)
- `_notify_owner_about_admin_shiftdef _notify_owner_about_admin_shift(db: Session, entry: dict[str, Any]) -> None: caption = _admin_shift_caption(entry) mime_type, photo_bytes = _decode_data_url_image( str(entry.ge` (стр. 8106)
- `_apply_admin_shift_reviewdef _apply_admin_shift_review( db: Session, inspection_id: str, *, action: str, issue_note: str, owner_actor_id: str,` (стр. 8176)
- `_serialize_state_datetimedef _serialize_state_datetime(value: datetime | None) -> str | None: if value is None: return None return _as_utc(value).isoformat()` (стр. 8296)
- `_parse_state_datetimedef _parse_state_datetime(value: Any) -> datetime | None: if not value: return None if not isinstance(value, str):` (стр. 8308)
- `_owner_database_reset_statedef _owner_database_reset_state(db: Session) -> dict[str, Any] | None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is None or not isinstance(row.value, dict):` (стр. 8330)
- `_save_owner_database_reset_statedef _save_owner_database_reset_state( db: Session, value: dict[str, Any]` (стр. 8344)
- `_clear_owner_database_reset_statedef _clear_owner_database_reset_state(db: Session) -> None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is not None: db.delete(row) db.flush()` (стр. 8356)
- `_normalize_database_reset_phrasedef _normalize_database_reset_phrase(value: str) -> str: normalized = " ".join(value.replace("\n", " ").split()).strip().upper() return normalized.replace("Ё", "Е")` (стр. 8370)
- `_owner_database_reset_previewdef _owner_database_reset_preview( db: Session,` (стр. 8380)
- `_owner_database_reset_warningsdef _owner_database_reset_warnings( preview: OwnerDatabaseResetPreviewPayload,` (стр. 8434)
- `_perform_owner_database_resetdef _perform_owner_database_reset(db: Session) -> None: db.execute(sa_delete(TelegramLinkCode)) db.execute(sa_delete(Notification)) db.execute(sa_delete(BookingWorker)) db.execute(` (стр. 8474)
- `_data_cleanup_iso_exprdef _data_cleanup_iso_expr(col): return ( func.substr(col, 7, 4) .op("||")(func.substr(col, 4, 2)) .op("||")(func.substr(col, 1, 2)) )` (стр. 8696)
- `_data_cleanup_resolve_perioddef _data_cleanup_resolve_period(payload: DataCleanupRequest): mode = (payload.mode or "range").strip() or "range" if mode not in ("range", "older_than"):` (стр. 8704)
- `_data_cleanup_collectdef _data_cleanup_collect(db: Session, entities: list[str], period) -> dict[str, list[Any]]: mode, dmy_from, dmy_to, parsed_from, parsed_to, bound_a, bound_b = period result: dict[` (стр. 8742)
- `AsciiJSONResponse._range_filter_dmydef _range_filter_dmy(col): conds = [] if parsed_from or parsed_to: lower_iso = (parsed_from or date.min).strftime("%Y%m%d") upper_iso = (parsed_to or date.max).strftime("%Y%m%d") ` (стр. 8746)
- `AsciiJSONResponse._older_filter_dmydef _older_filter_dmy(col, cutoff: date): iso = cutoff.strftime("%Y%m%d") return [_data_cleanup_iso_expr(col) <= iso]` (стр. 8755)
- `AsciiJSONResponse._created_at_in_scopedef _created_at_in_scope(created_at: datetime | None, fallback_dmy: str | None = None) -> bool: # Для payroll пробуем entry_date, иначе created_at. if fallback_dmy: try: d = parse_` (стр. 8799)
- `_data_cleanup_labeldef _data_cleanup_label(entity: str, obj: Any) -> tuple[str, str]: try: if entity == "bookings": return (f"{getattr(obj, 'date', '')} {getattr(obj, 'time', '')} · {getattr(obj, 'se` (стр. 8842)
- `_data_cleanup_preview_payloaddef _data_cleanup_preview_payload(db: Session, payload: DataCleanupRequest) -> DataCleanupPreviewPayload: period = _data_cleanup_resolve_period(payload) mode, dmy_from, dmy_to, par` (стр. 8869)
- `_trash_get_entitydef _trash_get_entity(obj_type: str, entity_id: str, db: Session) -> Any | None: mapping = { "bookings": Booking, "clients": Client, "incomes": Income, "expenses": Expense, "piggy"` (стр. 8903)
- `_trash_purge_expireddef _trash_purge_expired(db: Session) -> int: now = _now() expired = db.scalars( select(TrashItem).where( TrashItem.purged_at.is_(None), TrashItem.restored_at.is_(None), TrashItem.` (стр. 8920)
- `_parse_datedef _parse_date(s: str) -> date | None: if "." in s: parts = s.split(".") try: return date(int(parts[2]), int(parts[1]), int(parts[0])) except (ValueError, IndexError):` (стр. 9244)
- `_owner_export_filedef _owner_export_file( db: Session, actor_id: str, kind: str, segment: str = "all", date_from: str | None = None, date_to: str | None = None,` (стр. 9270)
- `AsciiJSONResponse._in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_to an` (стр. 9420)
- `_piggy_bank_export_filedef _piggy_bank_export_file( db: Session, actor_id: str, date_from: str | None = None, date_to: str | None = None, resource_group: str | None = None,` (стр. 9500)
- `_download_responsedef _download_response(export_file: GeneratedExport) -> Response: return Response( content=export_file.content, media_type=export_file.media_type, headers={ "Content-Disposition": ` (стр. 9570)
- `class _PartialBroadcastError(Exception):` (стр. 9590)
- `_PartialBroadcastError.__init__def __init__(self, payload: TelegramBroadcastPayload) -> None: super().__init__("partial broadcast failure") self.payload = payload` (стр. 9596)
- `_partial_broadcast_error_handlerasync def _partial_broadcast_error_handler( request: Request, exc: _PartialBroadcastError` (стр. 9605)
- `_send_export_to_telegramdef _send_export_to_telegram( db: Session, actor_id: str, export_file: GeneratedExport` (стр. 9644)
- `_owner_summary_reportdef _owner_summary_report( db: Session, actor_id: str, period: str, segment: str` (стр. 9769)
- `_owner_summary_export_filedef _owner_summary_export_file( db: Session, actor_id: str, period: str, segment: str` (стр. 9887)
- `_send_owner_summary_reportdef _send_owner_summary_report( db: Session, actor_id: str, report: OwnerSummaryReport, export_file: GeneratedExport,` (стр. 10005)
- `_booking_car_labeldef _booking_car_label(car: str | None, plate: str | None) -> str: car_value = (car or "").strip() or "Авто не указано" plate_value = (plate or "").strip() return f"{car_value}, {p` (стр. 10218)
- `_admin_booking_notification_titledef _admin_booking_notification_title( client_name: str, car: str | None, plate: str | None` (стр. 10230)
- `_booking_datetime_labeldef _booking_datetime_label(date: str | None, time: str | None) -> str: if not (date or "").strip():` (стр. 10242)
- `_admin_booking_notification_textdef _admin_booking_notification_text( client_name: str, car: str | None, plate: str | None, date: str | None, time: str | None,` (стр. 10258)
- `_additional_services_blockdef _additional_services_block(booking: Booking) -> str: """Форматирует блок доп. услуг для вставки в Telegram/внутренние сообщения.""" services = getattr(booking, "additional_serv` (стр. 10278)
- `_booking_all_worker_idsdef _booking_all_worker_ids(booking: Booking) -> set[str]: """Все мастера записи: основная услуга + доп. услуги.""" ids: set[str] = {link.worker_id for link in (booking.worker_link` (стр. 10301)
- `_notify_admins_about_bookingdef _notify_admins_about_booking(db: Session, booking: Booking) -> None: admins = db.scalars( select(StaffUser).where(StaffUser.role == "admin", StaffUser.active.is_(True)) ).all()` (стр. 10310)
- `_notify_owners_about_bookingdef _notify_owners_about_booking(db: Session, booking: Booking) -> None: owners = _all_owner_telegram_recipients(db) add_block = _additional_services_block(booking) text = ( "Новая` (стр. 10344)
- `_service_category_keydef _service_category_key(value: str | None) -> str: return (value or "").strip().lower()` (стр. 10374)
- `_resource_group_keydef _resource_group_key(value: str | None) -> str: return (value or "").strip().lower() or DEFAULT_RESOURCE_GROUP` (стр. 10382)
- `_normalized_textdef _normalized_text(value: str | None) -> str: return (value or "").strip()` (стр. 10390)
- `_default_service_resource_groupdef _default_service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_for_service_category(service.category)` (стр. 10398)
- `_default_box_resource_groupdef _default_box_resource_group(box: Box | None) -> str: if box is None: return DEFAULT_RESOURCE_GROUP name_key = (box.name or "").strip().lower() description_key = (box.descriptio` (стр. 10410)
- `_service_resource_groupdef _service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_key( service.resource_group or _default_servic` (стр. 10430)
- `_compatible_box_namesdef _compatible_box_names(db: Session, resource_group: str | None) -> list[str]: target_group = _resource_group_key(resource_group) return [ box.name for box in db.scalars( select(` (стр. 10446)
- `_is_box_rental_servicedef _is_box_rental_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "аренда бокса" )` (стр. 10472)
- `_is_detailing_servicedef _is_detailing_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "детейлинг" )` (стр. 10486)
- `_resource_group_for_service_categorydef _resource_group_for_service_category(category: str | None) -> str: category_key = _service_category_key(category) if category_key == "детейлинг": return DETAILING_RESOURCE_GROU` (стр. 10498)
- `_box_by_namedef _box_by_name(db: Session, box_name: str) -> Box | None: return db.scalar(select(Box).where(Box.name == box_name))` (стр. 10512)
- `_normalize_service_and_box_resourcesdef _normalize_service_and_box_resources(db: Session) -> None: changed = False # Группа ресурсов услуг больше не привязывается к категории принудительно. boxes = db.scalars(select(` (стр. 10520)
- `_box_hourly_pricedef _box_hourly_price(db: Session, box_name: str, fallback_price: int) -> int: box = _box_by_name(db, box_name) if box is not None and box.price_per_hour > 0: return box.price_per_` (стр. 10728)
- `_payment_type_labeldef _payment_type_label(payment_type: str) -> str: return { "cash": "Наличные", "transfer": "Перевод", "invoice": "По счёту", "credit": "В долг (депозит)", }.get(payment_type, paym` (стр. 10742)
- `_booking_payment_labeldef _booking_payment_label(booking: Booking) -> str: if not booking.payment_settled: return "Не оплачено" return _payment_type_label(booking.payment_type)` (стр. 10760)
- `_notify_ownersdef _notify_owners(db: Session, text: str) -> None: db.add( Notification( id=f"n-{uuid4()}", recipient_role="owner", recipient_id=None, message=text, read=False, created_at=_now(),` (стр. 10772)
- `_booking_receipt_textdef _booking_receipt_text(booking: Booking, *, worker_name: str | None = None) -> str: worker_line = f"\nМастер: {worker_name}" if worker_name else "" add_block = _additional_servi` (стр. 10808)
- ...ещё 86
...
- `class StaffUser(Base):` (стр. 77)
- `class Service(Base):` (стр. 127)
- `class Box(Base):` (стр. 153)
- `class ScheduleEntry(Base):` (стр. 164)
- `class Booking(Base):` (стр. 175)
- `class BookingWorker(Base):` (стр. 248)
- `class BookingAdditionalService(Base):` (стр. 268)
- `class BookingMaterial(Base):` (стр. 293)
- `class AdditionalServiceWorker(Base):` (стр. 316)
- `class Notification(Base):` (стр. 334)
- `class StockCategory(Base):` (стр. 353)
- `class StockItem(Base):` (стр. 375)
- `class Expense(Base):` (стр. 395)
- `class StockWriteOff(Base):` (стр. 418)
- `class Penalty(Base):` (стр. 445)
- `class PayrollEntry(Base):` (стр. 472)
- `class TelegramLinkCode(Base):` (стр. 506)
- `class AppSetting(Base):` (стр. 521)
- `class UploadedFile(Base):` (стр. 528)
- `class DataConsent(Base):` (стр. 538)
...
### backend/app/schemas.py (2594 строк)

Классы и функции (245):
...
- `_coerce_money_intdef _coerce_money_int(value: Any) -> int: """Coerce Decimal/float/str monetary values to int rubles (ROUND_HALF_UP). Handles Decimal('15881.54') from DB Numeric(18,2) that would ot` (стр. 156)
- `class ClientVehiclePayload(BaseModel):` (стр. 176)
- `ClientVehiclePayload.validate_vehicledef validate_vehicle(self) -> ClientVehiclePayload: if self.car.strip():` (стр. 183)
- `class ClientProfilePayload(BaseModel):` (стр. 191)
- `class ClientProfileInput(BaseModel):` (стр. 202)
- `ClientProfileInput.validate_namedef validate_name(cls, value: str) -> str: return normalize_person_name(value)` (стр. 213)
- `ClientProfileInput.validate_phonedef validate_phone(cls, value: str) -> str: if not value.strip():` (стр. 218)
- `ClientProfileInput.validate_vehicledef validate_vehicle(self) -> ClientProfileInput: if self.plate.strip():` (стр. 224)
- `class ClientSummaryPayload(BaseModel):` (стр. 230)
- `class ClientCreateRequest(BaseModel):` (стр. 255)
- `ClientCreateRequest.validate_namedef validate_name(cls, value: str) -> str: return normalize_person_name(value)` (стр. 266)
- `ClientCreateRequest.validate_phonedef validate_phone(cls, value: str) -> str: if not value.strip():` (стр. 271)
- `ClientCreateRequest.validate_vehicledef validate_vehicle(self) -> ClientCreateRequest: if self.car.strip():` (стр. 277)
- `class WorkerPayload(BaseModel):` (стр. 285)
...
- `class SalaryBookingItem(BaseModel):` (стр. 362)
- `class SalaryPayoutItem(BaseModel):` (стр. 386)
- `class SalaryDetailResponse(BaseModel):` (стр. 394)
- `class PaySalaryRequest(BaseModel):` (стр. 412)
- `class PaySalaryResponse(BaseModel):` (стр. 424)
- `class BookingWorkerPayload(BaseModel):` (стр. 431)
- `class BookingServiceItem(BaseModel):` (стр. 439)
- `class AdditionalServiceWorkerPayload(BaseModel):` (стр. 446)
- `class AdditionalServicePayload(BaseModel):` (стр. 454)
- `class AddAdditionalServiceRequest(BaseModel):` (стр. 468)
- `class UpdateAdditionalServiceRequest(BaseModel):` (стр. 479)
- `class BookingPayload(BaseModel):` (стр. 489)
- `class WorkerCalendarBookingPayload(BaseModel):` (стр. 521)
- `class BookingAvailabilitySlotPayload(BaseModel):` (стр. 540)
- `class BookingAvailabilityPayload(BaseModel):` (стр. 547)
- `class NotificationPayload(BaseModel):` (стр. 553)
- `class StockCategoryPayload(BaseModel):` (стр. 562)
- `class StockItemPayload(BaseModel):` (стр. 568)
- `class BookingMaterialPayload(BaseModel):` (стр. 578)
- `class ShiftChecklistItemPayload(BaseModel):` (стр. 587)
- `class ShiftChecklistPayload(BaseModel):` (стр. 596)
- `class ShiftChecklistSubmitItem(BaseModel):` (стр. 606)
- `class ShiftChecklistSubmitRequest(BaseModel):` (стр. 611)
- `class AdminShiftInspectionSupplyPayload(BaseModel):` (стр. 617)
- `class AdminShiftInspectionMasterPayload(BaseModel):` (стр. 626)
- `class AdminShiftInspectionPayload(BaseModel):` (стр. 632)
- `class AdminShiftInspectionSubmitSupply(BaseModel):` (стр. 649)
- `class AdminShiftInspectionSubmitMaster(BaseModel):` (стр. 654)
- `class AdminShiftInspectionSubmitRequest(BaseModel):` (стр. 659)
- `class AdminShiftInspectionReviewRequest(BaseModel):` (стр. 667)
- `class OwnerShiftOpeningRequest(BaseModel):` (стр. 672)
- `class ExpensePayload(BaseModel):` (стр. 677)
- `ExpensePayload._validate_amountdef _validate_amount(cls, value: Any) -> int: return _coerce_money_int(value)` (стр. 688)
- `class PenaltyPayload(BaseModel):` (стр. 692)
- `class TelegramLinkCodePayload(BaseModel):` (стр. 704)
- `class ServicePayload(BaseModel):` (стр. 710)
- `class DetailingRequestCreateRequest(BaseModel):` (стр. 734)
- `DetailingRequestCreateRequest.validate_cardef validate_car(cls, value: str | None) -> str | None: if value is None: return None return normalize_vehicle_name(value)` (стр. 743)
- `DetailingRequestCreateRequest.validate_plate_fielddef validate_plate_field(self) -> DetailingRequestCreateRequest: if self.plate is not None: if not self.plate.strip():` (стр. 749)
- `class BoxPayload(BaseModel):` (стр. 758)
- `class SchedulePayload(BaseModel):` (стр. 767)
- `class AdminNotificationSettings(BaseModel):` (стр. 775)
- `class AdminProfilePayload(BaseModel):` (стр. 783)
- `class WorkerNotificationSettings(BaseModel):` (стр. 790)
- `class WorkerProfilePayload(BaseModel):` (стр. 798)
- `class OperatingMode(str, Enum):` (стр. 809)
- `class OwnerCompanyPayload(BaseModel):` (стр. 814)
- `class OwnerNotificationSettings(BaseModel):` (стр. 824)
- `OwnerNotificationSettings._coerce_hoursdef _coerce_hours(self) -> OwnerNotificationSettings: if "bookingReminderHours" not in self.model_fields_set and "bookingReminderDays" in self.model_fields_set and self.bookingRemi` (стр. 836)
- `class OwnerIntegrationsPayload(BaseModel):` (стр. 846)
- `class GoogleCredentialsPayload(BaseModel):` (стр. 853)
- `class GoogleInvitePayload(BaseModel):` (стр. 861)
- `class OwnerSecurityPayload(BaseModel):` (стр. 867)
- `class AuthSessionPayload(BaseModel):` (стр. 871)
- `class EmployeeSettingPayload(BaseModel):` (стр. 880)
- `class WorkerCreateRequest(BaseModel):` (стр. 891)
- `class PayrollEntryCreateRequest(BaseModel):` (стр. 903)
- `PayrollEntryCreateRequest.validate_whole_rublesdef validate_whole_rubles(cls, value: float) -> float: # W4-002: учёт в целых рублях (money_int/ROUND_HALF_UP везде ниже по # пайплайну) — дробь молча округлилась бы при чтении (10` (стр. 921)
- `PayrollEntryCreateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 930)
- `class PayrollEntryUpdateRequest(BaseModel):` (стр. 934)
- `PayrollEntryUpdateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 940)
- `class SettingsBundlePayload(BaseModel):` (стр. 944)
- `class SessionPayload(BaseModel):` (стр. 954)
- `class BootstrapPayload(BaseModel):` (стр. 967)
- `class ClientRegisterRequest(BaseModel):` (стр. 986)
- `ClientRegisterRequest.lift_profiledef lift_profile(cls, raw: Any) -> Any: if isinstance(raw, dict) and isinstance(raw.get("profile"), dict):` (стр. 998)
- `ClientRegisterRequest.validate_namedef validate_name(cls, value: str) -> str: if not value.strip():` (стр. 1007)
- `ClientRegisterRequest.validate_phonedef validate_phone(cls, value: str) -> str: if not value.strip():` (стр. 1014)
- `ClientRegisterRequest.validate_vehicledef validate_vehicle(self) -> ClientRegisterRequest: if self.plate.strip():` (стр. 1020)
- `class ConsentRecordPayload(BaseModel):` (стр. 1026)
- `class ConsentCheckResponse(BaseModel):` (стр. 1031)
- `class StaffLinkRequest(BaseModel):` (стр. 1035)
- `class SwitchRoleRequest(BaseModel):` (стр. 1040)
- `class RolePreviewActor(BaseModel):` (стр. 1044)
- `class RolePreviewOption(BaseModel):` (стр. 1053)
- `class RolePreviewState(BaseModel):` (стр. 1060)
- `class RolePreviewRequest(BaseModel):` (стр. 1077)
- `_normalize_booking_datedef _normalize_booking_date(value: str) -> str: """Нормализует дату записи к канону ДД.ММ.ГГГГ. Принимает и ДД.ММ.ГГГГ, и ГГГГ-ММ-ДД (форматы, которые могут слать клиенты). Остальн` (стр. 1084)
- `_normalize_booking_timedef _normalize_booking_time(value: str) -> str: """Normalize booking time to HH:MM (24h) canonical form. Empty allowed.""" v = value.strip() if not v: return "" m = re.fullmatch(r"` (стр. 1105)
- `class BookingCreateRequest(BaseModel):` (стр. 1119)
- `BookingCreateRequest.validate_datedef validate_date(cls, value: str) -> str: return _normalize_booking_date(value)` (стр. 1145)
- `BookingCreateRequest.validate_timedef validate_time(cls, value: str) -> str: return _normalize_booking_time(value)` (стр. 1150)
- `BookingCreateRequest.validate_client_namedef validate_client_name(cls, value: str) -> str: if not value.strip():` (стр. 1155)
- `BookingCreateRequest.validate_client_phonedef validate_client_phone(cls, value: str) -> str: if not value.strip():` (стр. 1162)
- `BookingCreateRequest.validate_vehicledef validate_vehicle(self) -> BookingCreateRequest: if self.car is not None and self.car.strip():` (стр. 1168)
- `class AddBookingServiceRequest(BaseModel):` (стр. 1176)
- `class BookingUpdateRequest(BaseModel):` (стр. 1183)
- `BookingUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None return _normalize_booking_date(value)` (стр. 1209)
- `BookingUpdateRequest.validate_timedef validate_time(cls, value: str | None) -> str | None: if value is None: return None return _normalize_booking_time(value)` (стр. 1216)
- `BookingUpdateRequest.validate_client_namedef validate_client_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1223)
- `BookingUpdateRequest.validate_client_phonedef validate_client_phone(cls, value: str | None) -> str | None: if value is None: return None return normalize_phone(value)` (стр. 1230)
- `BookingUpdateRequest.validate_vehicledef validate_vehicle(self) -> BookingUpdateRequest: if self.car is not None and not self.car.strip():` (стр. 1236)
- `class ClientCardUpdateRequest(BaseModel):` (стр. 1249)
- `ClientCardUpdateRequest.validate_namedef validate_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1267)
- `ClientCardUpdateRequest.validate_phonedef validate_phone(cls, value: str | None) -> str | None: if value is None or not value.strip():` (стр. 1274)
- `ClientCardUpdateRequest.validate_vehicledef validate_vehicle(self) -> ClientCardUpdateRequest: if self.car is not None and not self.car.strip():` (стр. 1280)
- `class NotificationCreateRequest(BaseModel):` (стр. 1293)
- `class ReadAllNotificationsRequest(BaseModel):` (стр. 1300)
- `class WorkerBroadcastRequest(BaseModel):` (стр. 1304)
- `class WorkerBroadcastResponse(BaseModel):` (стр. 1308)
- `class StockItemCreateRequest(BaseModel):` (стр. 1314)
- `class StockItemUpdateRequest(BaseModel):` (стр. 1323)
- `class StockCategoryCreateRequest(BaseModel):` (стр. 1332)
- `class StockCategoryUpdateRequest(BaseModel):` (стр. 1337)
- `class StockWriteOffRequest(BaseModel):` (стр. 1342)
- `class StockWriteOffPayload(BaseModel):` (стр. 1346)
- `class IncomeCreateRequest(BaseModel):` (стр. 1364)
- `IncomeCreateRequest.validate_sourcedef validate_source(cls, value: str) -> str: stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или состоять только из пробелов") return stripp` (стр. 1373)
- `IncomeCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1381)
- `class IncomePayload(BaseModel):` (стр. 1387)
- `IncomePayload._validate_amountdef _validate_amount(cls, value: Any) -> int: return _coerce_money_int(value)` (стр. 1399)
- `class ExpenseCreateRequest(BaseModel):` (стр. 1403)
- `ExpenseCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1413)
- `class PenaltyCreateRequest(BaseModel):` (стр. 1419)
- `class OwnerReminderDispatchRequest(BaseModel):` (стр. 1425)
- `class OwnerReminderDispatchPayload(BaseModel):` (стр. 1430)
- `class StaffLoginRequest(BaseModel):` (стр. 1438)
- `class ChangePasswordRequest(BaseModel):` (стр. 1443)
- `class OwnerDatabaseResetPreviewPayload(BaseModel):` (стр. 1448)
- `class OwnerDatabaseResetStartRequest(BaseModel):` (стр. 1463)
- `class OwnerDatabaseResetApproveRequest(BaseModel):` (стр. 1467)
- `class OwnerDatabaseResetExecuteRequest(BaseModel):` (стр. 1473)
- `class OwnerDatabaseResetStartPayload(BaseModel):` (стр. 1477)
- `class OwnerDatabaseResetApprovePayload(BaseModel):` (стр. 1486)
- `class OwnerDatabaseResetExecutePayload(BaseModel):` (стр. 1494)
- `class ContentAboutPayload(BaseModel):` (стр. 1499)
- `class ContentServicePayload(BaseModel):` (стр. 1505)
- `class ContentWorksPayload(BaseModel):` (стр. 1516)
- `class ContentStatsPayload(BaseModel):` (стр. 1522)
- `class ContentTitlePayload(BaseModel):` (стр. 1527)
- `ContentTitlePayload.to_full_titledef to_full_title(self) -> str: return f"{self.before}{self.highlight}{self.after}"` (стр. 1532)
- `class ContentHeroPayload(BaseModel):` (стр. 1536)
- `class ContentPayload(BaseModel):` (стр. 1552)
- `class ContactPayload(BaseModel):` (стр. 1559)
- `class ResetPasswordRequest(BaseModel):` (стр. 1566)
- `class GenericMessage(BaseModel):` (стр. 1570)
- `class TelegramDeliveryResult(BaseModel):` (стр. 1574)
- `class TelegramBroadcastPayload(BaseModel):` (стр. 1580)
- `class OwnerExportDeliveryPayload(BaseModel):` (стр. 1586)
- `class ShiftAttendancePayload(BaseModel):` (стр. 1593)
- `class ExpenseUpdateRequest(BaseModel):` (стр. 1604)
- `ExpenseUpdateRequest.validate_titledef validate_title(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("title не может быть пустым или с` (стр. 1614)
- `ExpenseUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1624)
- `ExpenseUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> ExpenseUpdateRequest: if all(v is None for v in [self.title, self.amount, self.category, self.date, self.note]):` (стр. 1632)
- `class IncomeUpdateRequest(BaseModel):` (стр. 1638)
- `IncomeUpdateRequest.validate_sourcedef validate_source(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или` (стр. 1647)
- `IncomeUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1657)
- `IncomeUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> IncomeUpdateRequest: # Use model_fields_set to detect explicitly provided fields (including null). # This allows {"note": null} to pass as a` (стр. 1665)
- `class PiggyBankTransactionPayload(BaseModel):` (стр. 1673)
- `class PiggyBankWithdrawRequest(BaseModel):` (стр. 1714)
- `PiggyBankWithdrawRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1743)
- `PiggyBankWithdrawRequest.validate_spent_by_namedef validate_spent_by_name(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() return stripped or None` (стр. 1750)
- `class PiggyBankAdjustRequest(BaseModel):` (стр. 1757)
- `PiggyBankAdjustRequest.validate_datedef validate_date(cls, value: str) -> str: stripped = value.strip() if stripped and not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", stripped):` (стр. 1767)
- `class PiggyBankWashBreakdown(BaseModel):` (стр. 1774)
- `class PiggyBankDetailingBreakdown(BaseModel):` (стр. 1787)
- `class PiggyBankSpenderDebt(BaseModel):` (стр. 1798)
- `class PiggyBankResponse(BaseModel):` (стр. 1805)
- `class WeeklyArchivePayload(BaseModel):` (стр. 1827)
- `class WalletResponse(BaseModel):` (стр. 1841)
- `class OwnerProfitShareItem(BaseModel):` (стр. 1858)
- `class OwnerProfitShareSummary(BaseModel):` (стр. 1875)
- `class OwnerMasterRoleRequest(BaseModel):` (стр. 1886)
- `class OwnerSalaryDetailResponse(BaseModel):` (стр. 1890)
- `class PayOwnerSalaryRequest(BaseModel):` (стр. 1897)
- `class PayOwnerSalaryResponse(BaseModel):` (стр. 1906)
- `class OverrideEarnedRequest(BaseModel):` (стр. 1913)
- `class BookingHistoryItem(BaseModel):` (стр. 1917)
- `class BookingTotalsWorkerItem(BaseModel):` (стр. 1934)
- `class BookingTotalsOwnerItem(BaseModel):` (стр. 1953)
- `class BookingTotalsPiggyItem(BaseModel):` (стр. 1960)
- `class BookingHistoryTotals(BaseModel):` (стр. 1966)
- `class BookingMoneySplitWorkerItem(BaseModel):` (стр. 1972)
- `class BookingMoneySplitOwnerItem(BaseModel):` (стр. 1983)
- `class BookingPiggyTxItem(BaseModel):` (стр. 1991)
- `class BookingAdditionalServiceItem(BaseModel):` (стр. 2000)
- `class BookingAsvcPiggyItem(BaseModel):` (стр. 2009)
- ...ещё 45
...
### backend/app/telegram_linking.py (93 строк)

Классы и функции (6):

- `_nowdef _now() -> datetime: return datetime.now(UTC)` (стр. 13)
- `_as_utcdef _as_utc(value: datetime) -> datetime: if value.tzinfo is None: return value.replace(tzinfo=UTC) return value.astimezone(UTC)` (стр. 17)
...
- `class BotRuntime: token: str webapp_url: str api_base: str training_webapp_url: str | None = None ADMIN_SHIFT_INSPECTION` (стр. 33)
- `_build_runtimedef _build_runtime() -> BotRuntime: settings = get_settings() if not settings.telegram_bot_token: raise RuntimeError("TELEGRAM_BOT_TOKEN is not configured") if not settings.webapp_` (стр. 45)
- `telegram_webhook_secretdef telegram_webhook_secret() -> str: settings = get_settings() if not settings.telegram_bot_token: raise RuntimeError("TELEGRAM_BOT_TOKEN is not configured") raw_secret = f"{setti` (стр. 59)
- `telegram_webhook_urldef telegram_webhook_url() -> str: settings = get_settings() if not settings.webapp_url: raise RuntimeError("WEBAPP_URL is not configured") return f"{settings.webapp_url.rstrip('/'` (стр. 67)
- `_parse_retry_afterdef _parse_retry_after(details: str) -> int | None: try: parsed = json.loads(details) except json.JSONDecodeError: return None parameters = parsed.get("parameters") if not isinstan` (стр. 74)
- `_telegram_calldef _telegram_call( runtime: BotRuntime, method: str, payload: dict[str, Any] | None = None, *, max_attempts: int = 3,` (стр. 88)
- `_telegram_multipart_calldef _telegram_multipart_call( runtime: BotRuntime, method: str, fields: dict[str, Any], files: dict[str, tuple[str, str, bytes]],` (стр. 120)
- `_welcome_reply_markupdef _welcome_reply_markup(webapp_url: str) -> dict[str, Any]: return { "inline_keyboard": [ [ {"text": "✨ О нас", "web_app": {"url": f"{webapp_url}/about"}}, {"text": "📸 Наши работ` (стр. 163)
- `_help_reply_markupdef _help_reply_markup(runtime: BotRuntime) -> dict[str, Any]: training_url = runtime.training_webapp_url or runtime.webapp_url help_url = f"{training_url}?help=1" if "?" not in tr` (стр. 186)
- `_send_help_messagedef _send_help_message(runtime: BotRuntime, chat_id: int) -> None: _send_text_message( runtime, chat_id, HELP_TEXT, reply_markup=_help_reply_markup(runtime), parse_mode="HTML", )` (стр. 198)
- `_configure_bot_metadatadef _configure_bot_metadata(runtime: BotRuntime) -> str | None: me = _telegram_call(runtime, "getMe") _telegram_call( runtime, "setMyCommands", { "commands": [ {"command": "start",` (стр. 208)
- `disable_telegram_webhookdef disable_telegram_webhook(*, drop_pending_updates: bool = False) -> str | None: runtime = _build_runtime() username = _configure_bot_metadata(runtime) _telegram_call(runtime, "d` (стр. 236)
- `sync_telegram_webhookdef sync_telegram_webhook(*, drop_pending_updates: bool = False) -> str | None: runtime = _build_runtime() username = _configure_bot_metadata(runtime) target_url = telegram_webhook` (стр. 243)
- `_send_text_messagedef _send_text_message( runtime: BotRuntime, chat_id: int, text: str, *, reply_markup: dict[str, Any] | None = None, parse_mode: str | None = None,` (стр. 272)
- `_send_start_messagedef _send_start_message(runtime: BotRuntime, chat_id: int) -> None: markup = _welcome_reply_markup(runtime.webapp_url) try: req = request.Request(WELCOME_PHOTO_URL) with request.ur` (стр. 303)
- `_send_about_messagedef _send_about_message(runtime: BotRuntime, chat_id: int) -> None: with session_scope() as db: row = db.get(AppSetting, "content") if row and isinstance(row.value, dict):` (стр. 339)
- `_send_works_messagedef _send_works_message(runtime: BotRuntime, chat_id: int) -> None: with session_scope() as db: row = db.get(AppSetting, "content") works = (row.value or {}).get("works", []) if ro` (стр. 352)
- `send_telegram_messagedef send_telegram_message( chat_id: str | int, text: str, *, parse_mode: str | None = None` (стр. 374)
- `send_telegram_documentdef send_telegram_document( chat_id: str | int, *, file_name: str, content: bytes, caption: str | None = None, mime_type: str = "application/octet-stream",` (стр. 381)
- `send_telegram_photodef send_telegram_photo( chat_id: str | int, *, file_name: str, content: bytes, caption: str | None = None, parse_mode: str | None = None, mime_type: str = "image/jpeg", reply_mark` (стр. 401)
- `_setting_dictdef _setting_dict(db, key: str, default: dict[str, Any]) -> dict[str, Any]: row = db.get(AppSetting, key) if row is None or not isinstance(row.value, dict):` (стр. 427)
- `_setting_listdef _setting_list(db, key: str) -> list[dict[str, Any]]: row = db.get(AppSetting, key) if row is None or not isinstance(row.value, list):` (стр. 434)
- `_upsert_settingdef _upsert_setting(db, key: str, value: Any) -> None: row = db.get(AppSetting, key) if row is None: row = AppSetting(key=key, value=value) db.add(row) else: row.value = value` (стр. 441)
- `_serialize_nowdef _serialize_now() -> str: return datetime.now(UTC).isoformat()` (стр. 450)
- `_owner_by_chat_iddef _owner_by_chat_id(db, chat_id: int) -> StaffUser | None: return db.query(StaffUser).filter(StaffUser.role == "owner", StaffUser.telegram_chat_id == str(chat_id)).first()` (стр. 454)
- `_apply_shift_review_from_botdef _apply_shift_review_from_bot(chat_id: int, inspection_id: str, action: str, issue_note: str = "") -> str: with session_scope() as db: owner = _owner_by_chat_id(db, chat_id) if ` (стр. 458)
- `_remember_pending_issuedef _remember_pending_issue(chat_id: int, inspection_id: str) -> None: with session_scope() as db: state = _setting_dict(db, ADMIN_SHIFT_OWNER_BOT_STATE_KEY, {"pendingIssueByChat":` (стр. 498)
- `_pop_pending_issuedef _pop_pending_issue(chat_id: int) -> str | None: with session_scope() as db: state = _setting_dict(db, ADMIN_SHIFT_OWNER_BOT_STATE_KEY, {"pendingIssueByChat": {}}) pending = sta` (стр. 509)
- `_extract_contact_phonedef _extract_contact_phone(update: dict[str, Any], chat_id: int) -> str | None: message = update.get("message") or {} contact = message.get("contact") or {} phone_number = contact.` (стр. 521)
- `_store_client_phone_verificationdef _store_client_phone_verification(chat_id: int, phone_digits: str) -> None: with session_scope() as db: current = _setting_dict(db, CLIENT_PHONE_VERIFICATIONS_KEY, {}) current[s` (стр. 539)
- `_extract_chat_iddef _extract_chat_id(update: dict[str, Any]) -> int | None: callback = update.get("callback_query") or {} callback_message = callback.get("message") or {} callback_chat = callback_` (стр. 549)
- `_extract_textdef _extract_text(update: dict[str, Any]) -> str: message = update.get("message") or {} text = message.get("text") return text.strip() if isinstance(text, str) else ""` (стр. 562)
- `_extract_callbackdef _extract_callback(update: dict[str, Any]) -> tuple[str, str] | None: callback = update.get("callback_query") or {} callback_id = callback.get("id") data = callback.get("data") ` (стр. 568)
- `_answer_callback_querydef _answer_callback_query(runtime: BotRuntime, callback_id: str, text: str) -> None: _telegram_call(runtime, "answerCallbackQuery", {"callback_query_id": callback_id, "text": text` (стр. 577)
- `_handle_link_commanddef _handle_link_command(chat_id: int, text: str) -> str: parts = text.split(maxsplit=1) code = parts[1].strip() if len(parts) == 2 else "" if not code.isdigit():` (стр. 581)
...
### backend/tests/test_booking_schedule_validation.py (102 строк)

Классы и функции (9):

- `class BookingScheduleValidationTests(unittest.TestCase):` (стр. 20)
- `BookingScheduleValidationTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_schedule_{uuid4().hex}` (стр. 21)
- `BookingScheduleValidationTests.tearDowndef tearDown(self) -> None: self.session.close() self.engine.dispose() if self.db_path.exists():` (стр. 45)
- `BookingScheduleValidationTests._add_daydef _add_day(self, day_index: int, *, open_time: str, close_time: str, active: bool = True) -> None: self.session.add( self.schedule_entry_cls( day_index=day_index, day_label="Тест` (стр. 51)
- `BookingScheduleValidationTests.test_slot_inside_working_hours_passesdef test_slot_inside_working_hours_passes(self) -> None: self._add_day(0, open_time="09:00", close_time="19:00") # 12.09.2026 — суббота (day_index=0): 12:00 + 60 мин = 13:00 < 19:0` (стр. 63)
- `BookingScheduleValidationTests.test_end_after_close_mentions_computed_enddef test_end_after_close_mentions_computed_end(self) -> None: self._add_day(0, open_time="09:00", close_time="19:00") with self.assertRaises(HTTPException) as ctx: self.ensure_with` (стр. 68)
- `BookingScheduleValidationTests.test_start_before_open_mentions_early_startdef test_start_before_open_mentions_early_start(self) -> None: self._add_day(0, open_time="09:00", close_time="19:00") # Запись без времени (00:00): начало раньше открытия with sel` (стр. 79)
- `BookingScheduleValidationTests.test_missing_day_row_reports_unavailable_daydef test_missing_day_row_reports_unavailable_day(self) -> None: with self.assertRaises(HTTPException) as ctx: self.ensure_within_schedule(self.session, "13.09.2026", "12:00", 60) s` (стр. 87)
- `BookingScheduleValidationTests.test_end_crossing_midnight_labels_next_daydef test_end_crossing_midnight_labels_next_day(self) -> None: self._add_day(0, open_time="09:00", close_time="23:00") # 22:00 + 180 мин = 25:00 -> «01:00 (+1 дн.)» with self.assert` (стр. 93)
...
### backend/tests/test_config.py (148 строк)
...
- `test_strong_environments_neutralize_insecure_client_authdef test_strong_environments_neutralize_insecure_client_auth(monkeypatch, environment: str) -> None: """H-01: ALLOW_INSECURE_CLIENT_AUTH=true обязан гаснуть в prod/staging. Иначе л` (стр. 136)

### backend/tests/test_contact_abuse.py (116 строк)

Классы и функции (13):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 20)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 26)
- `class ContactAbuseTests(unittest.TestCase):` (стр. 30)
- `ContactAbuseTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 31)
- `ContactAbuseTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 54)
- `ContactAbuseTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 60)
- `ContactAbuseTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 69)
- `ContactAbuseTests.post_contactdef post_contact(self, **overrides): payload = { "name": "Иван", "phone": "+7 (999) 123-45-67", "service": "Мойка", "message": "Позвоните", } payload.update(overrides) return self.` (стр. 78)
- `ContactAbuseTests.test_legit_contact_still_accepteddef test_legit_contact_still_accepted(self) -> None: response = self.post_contact() self.assertEqual(response.status_code, 200, response.text) self.assertIn("отправлена", response.` (стр. 88)
- `ContactAbuseTests.test_oversized_message_rejected_with_422def test_oversized_message_rejected_with_422(self) -> None: response = self.post_contact(message="x" * 2001) self.assertEqual(response.status_code, 422, response.text)` (стр. 93)
- `ContactAbuseTests.test_oversized_name_rejected_with_422def test_oversized_name_rejected_with_422(self) -> None: response = self.post_contact(name="n" * 101) self.assertEqual(response.status_code, 422, response.text)` (стр. 97)
- `ContactAbuseTests.test_oversized_service_rejected_with_422def test_oversized_service_rejected_with_422(self) -> None: response = self.post_contact(service="s" * 121) self.assertEqual(response.status_code, 422, response.text)` (стр. 101)
- `ContactAbuseTests.test_contact_is_rate_limiteddef test_contact_is_rate_limited(self) -> None: from app.main import _LOGIN_MAX_ATTEMPTS for _ in range(_LOGIN_MAX_ATTEMPTS):` (стр. 105)
...
- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 22)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 34)
- `class DoubleBookingTests(unittest.TestCase):` (стр. 38)
- `DoubleBookingTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 41)
- `DoubleBookingTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 62)
- `DoubleBookingTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 68)
- `DoubleBookingTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 77)
- `DoubleBookingTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select with SessionLocal() as db: staff = d` (стр. 86)
- `DoubleBookingTests.next_active_datedef next_active_date() -> str: candidate = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) for offset in range(1, 8):` (стр. 99)
- `DoubleBookingTests.booking_payloaddef booking_payload(self, phone: str, worker_id: str, worker_name: str) -> dict: return { "clientId": "", "clientName": "Double Slot", "clientPhone": phone, "service": "Мойка базов` (стр. 107)
- `DoubleBookingTests.test_sequential_double_booking_same_box_rejecteddef test_sequential_double_booking_same_box_rejected(self) -> None: first = self.client.post( "/api/bookings", headers={"Authorization": self.admin_token}, json=self.booking_payloa` (стр. 126)
- `DoubleBookingTests.test_concurrent_double_booking_same_box_single_winnerdef test_concurrent_double_booking_same_box_single_winner(self) -> None: barrier = threading.Barrier(2) statuses: list[int] = [] lock = threading.Lock() def worker(phone: str, work` (стр. 144)
- `DoubleBookingTests.workerdef worker(phone: str, worker_id: str, worker_name: str) -> None: barrier.wait(timeout=30) response = self.client.post( "/api/bookings", headers={"Authorization": self.admin_token}` (стр. 149)
...
### backend/tests/test_fractional_money.py (134 строк)

Классы и функции (11):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 20)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 32)
- `class FractionalMoneyTests(unittest.TestCase):` (стр. 36)
- `FractionalMoneyTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 40)
- `FractionalMoneyTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 62)
- `FractionalMoneyTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 68)
- `FractionalMoneyTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 77)
- `FractionalMoneyTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select mapping = {"admin": self.ADMIN_TG_ID` (стр. 86)
- `FractionalMoneyTests.test_fractional_bonus_rejected_with_422def test_fractional_bonus_rejected_with_422(self) -> None: created = self.client.post( "/api/payroll/entries", headers={"Authorization": self.owner_token}, json={"workerId": "w1", ` (стр. 99)
- `FractionalMoneyTests.test_whole_bonus_flows_through_salary_detaildef test_whole_bonus_flows_through_salary_detail(self) -> None: created = self.client.post( "/api/payroll/entries", headers={"Authorization": self.owner_token}, json={"workerId": "` (стр. 107)
- `FractionalMoneyTests.test_fractional_payout_rejected_with_422def test_fractional_payout_rejected_with_422(self) -> None: created = self.client.post( "/api/payroll/entries", headers={"Authorization": self.owner_token}, json={"workerId": "w1",` (стр. 124)
...
### backend/tests/test_owner_masters.py (326 строк)
...
- `OwnerMasterTests.test_owner_master_pay_salarydef test_owner_master_pay_salary(self) -> None: """Выплата ЗП как мастеру владельцу-мастеру проходит, обычному владельцу — 404.""" owner_token = build_init_data(self.OWNER_TG_2) pa` (стр. 236)
- `OwnerMasterTests.test_owner_master_role_toggledef test_owner_master_role_toggle(self) -> None: """Тумблер «Работает как мастер»: вкл/выкл меняет выплаты и списки.""" owner_token = build_init_data(self.OWNER_TG_1) # Включаем фл` (стр. 254)
- `OwnerMasterTests.test_owner_master_role_authdef test_owner_master_role_auth(self) -> None: """Тумблер доступен только владельцам; мастеру по id владельца — 404.""" # Админ не может переключать forbidden = self.client.patch( ` (стр. 301)
- `OwnerMasterTests._next_active_datedef _next_active_date(self) -> str: candidate = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) for offset in range(1, 8):` (стр. 320)
...
### backend/tests/test_phone_reclaim.py (144 строк)
...
- `class PhoneReclaimTests(unittest.TestCase):` (стр. 35)
- `PhoneReclaimTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 36)
- `PhoneReclaimTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 55)
- `PhoneReclaimTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 61)
- `PhoneReclaimTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 70)
- `PhoneReclaimTests.registerdef register(self, name: str, phone: str, telegram_id: str): return self.client.post( "/api/auth/client", json={ "profile": {"name": name, "phone": phone, "registered": True}, "ini` (стр. 79)
- `PhoneReclaimTests.verify_phonedef verify_phone(self, telegram_id: str, phone: str) -> None: from app.schemas import normalize_phone_digits from bot import _store_client_phone_verification _store_client_phone_ve` (стр. 88)
- `PhoneReclaimTests.client_telegramdef client_telegram(self, actor_id: str) -> str | None: from app.database import SessionLocal from app.models import Client with SessionLocal() as db: client = db.get(Client, actor` (стр. 94)
- `PhoneReclaimTests.count_clientsdef count_clients(self) -> int: from sqlalchemy import select from app.database import SessionLocal from app.models import Client with SessionLocal() as db: return len(db.scalars(s` (стр. 102)
- `PhoneReclaimTests.test_verified_victim_reclaims_squatted_phonedef test_verified_victim_reclaims_squatted_phone(self) -> None: squatter = self.register("Squatter", SQUAT_PHONE, "9001") self.assertEqual(squatter.status_code, 200, squatter.text)` (стр. 111)
- `PhoneReclaimTests.test_verified_holder_keeps_phonedef test_verified_holder_keeps_phone(self) -> None: self.verify_phone("9001", SQUAT_PHONE) first = self.register("Alice", SQUAT_PHONE, "9001") self.assertEqual(first.status_code, 2` (стр. 124)
- `PhoneReclaimTests.test_unverified_requester_still_rejecteddef test_unverified_requester_still_rejected(self) -> None: first = self.register("Squatter", SQUAT_PHONE, "9001") self.assertEqual(first.status_code, 200, first.text) second = sel` (стр. 134)
...
### backend/tests/test_piggy_idempotency_delete.py (194 строк)

Классы и функции (13):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 19)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 30)
- `class PiggyIdempotencyDeleteTests(unittest.TestCase):` (стр. 34)
- `PiggyIdempotencyDeleteTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 38)
- `PiggyIdempotencyDeleteTests.tearDowndef tearDown(self) -> None: if hasattr(self, "client_manager"):` (стр. 64)
- `PiggyIdempotencyDeleteTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select with SessionLocal() as db: owner = d` (стр. 77)
- `PiggyIdempotencyDeleteTests._auth_headersdef _auth_headers(token: str) -> dict[str, str]: return {"Authorization": token}` (стр. 92)
- `PiggyIdempotencyDeleteTests._piggy_bankdef _piggy_bank(self) -> dict: r = self.client.get("/api/owner/piggy-bank", headers=self._auth_headers(self.owner_token)) self.assertEqual(r.status_code, 200, r.text) return r.json` (стр. 95)
- `PiggyIdempotencyDeleteTests.test_adjust_idempotent_same_keydef test_adjust_idempotent_same_key(self) -> None: before = self._piggy_bank() key = f"adj-{uuid4().hex}" p1 = self.client.post( "/api/owner/piggy-bank/adjust", headers=self._auth_` (стр. 100)
- `PiggyIdempotencyDeleteTests.test_withdraw_idempotent_same_keydef test_withdraw_idempotent_same_key(self) -> None: before = self._piggy_bank() key = f"wd-{uuid4().hex}" body = { "resourceGroup": "detailing", "materialName": "Test material", "` (стр. 119)
- `PiggyIdempotencyDeleteTests.test_delete_adjust_restores_balancedef test_delete_adjust_restores_balance(self) -> None: before = self._piggy_bank() r = self.client.post( "/api/owner/piggy-bank/adjust", headers=self._auth_headers(self.owner_token` (стр. 138)
- `PiggyIdempotencyDeleteTests.test_delete_withdraw_removes_expense_mirrordef test_delete_withdraw_removes_expense_mirror(self) -> None: before = self._piggy_bank() r = self.client.post( "/api/owner/piggy-bank/withdraw", headers=self._auth_headers(self.o` (стр. 158)
- `PiggyIdempotencyDeleteTests.test_delete_system_deposit_forbiddendef test_delete_system_deposit_forbidden(self) -> None: data = self._piggy_bank() system_tx = next((t for t in data["transactions"] if t["transactionType"] == "deposit_24percent"),` (стр. 181)

### backend/tests/test_piggy_startup_migration.py (122 строк)

Классы и функции (5):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 24)
- `class PiggyStartupMigrationTests(unittest.TestCase):` (стр. 35)
- `PiggyStartupMigrationTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 36)
- `PiggyStartupMigrationTests.tearDowndef tearDown(self) -> None: try: from app.database import engine except ModuleNotFoundError: pass else: engine.dispose() reset_app_modules() if self.db_path.exists():` (стр. 54)
- `PiggyStartupMigrationTests.test_backfill_links_one_to_one_and_rerun_is_noopdef test_backfill_links_one_to_one_and_rerun_is_noop(self) -> None: from decimal import Decimal from app.database import Base, SessionLocal, engine from app.models import Expense, ` (стр. 65)
...
- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 16)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 28)
- `class CoverageGapsTests(unittest.TestCase):` (стр. 32)
- `CoverageGapsTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 37)
- `CoverageGapsTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 60)
- `CoverageGapsTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 66)
- `CoverageGapsTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 75)
- `CoverageGapsTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select mapping = { "admin": self.ADMIN_TG_I` (стр. 84)
- `CoverageGapsTests.test_save_boxes_roundtrip_and_group_scoped_deletedef test_save_boxes_roundtrip_and_group_scoped_delete(self) -> None: first = self.client.put( "/api/settings/boxes", headers={"Authorization": self.admin_token}, json=[ {"id": "b1"` (стр. 101)
- `CoverageGapsTests.test_save_boxes_forbidden_for_workerdef test_save_boxes_forbidden_for_worker(self) -> None: response = self.client.put( "/api/settings/boxes", headers={"Authorization": self.worker_token}, json=[], ) self.assertEqual` (стр. 130)
- `CoverageGapsTests.test_save_schedule_roundtripdef test_save_schedule_roundtrip(self) -> None: response = self.client.put( "/api/settings/schedule", headers={"Authorization": self.owner_token}, json=[ {"dayIndex": 0, "day": "Пн` (стр. 138)
- `CoverageGapsTests.test_submit_contact_accepts_and_escapesdef test_submit_contact_accepts_and_escapes(self) -> None: response = self.client.post( "/api/contact", json={ "name": "<script>alert(1)</script>", "phone": "+7 (999) 123-45-67", "` (стр. 152)
...
### backend/tests/test_worker_broadcast.py (204 строк)

Классы и функции (11):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 15)
- `class WorkerBroadcastTests(unittest.TestCase):` (стр. 27)
- `WorkerBroadcastTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 28)
- `WorkerBroadcastTests.tearDowndef tearDown(self) -> None: self.shutdown_app() reset_app_modules() if self.db_path.exists():` (стр. 47)
- `WorkerBroadcastTests.shutdown_appdef shutdown_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 53)
- `WorkerBroadcastTests.restart_appdef restart_app(self) -> None: if hasattr(self, "client_manager"):` (стр. 62)
- `WorkerBroadcastTests.login_staffdef login_staff(self, login: str, password: str) -> str: response = self.client.post( "/api/auth/staff/login", json={"login": login, "password": password}, ) self.assertEqual(respo` (стр. 71)
- `WorkerBroadcastTests.auth_headersdef auth_headers(token: str) -> dict[str, str]: return {"Authorization": token}` (стр. 80)
- `WorkerBroadcastTests.disable_owner_two_factordef disable_owner_two_factor(self) -> None: from app.database import SessionLocal from app.models import AppSetting with SessionLocal() as db: setting = db.get(AppSetting, "owner_s` (стр. 83)
- `WorkerBroadcastTests.test_owner_broadcast_reaches_all_workers_and_claim_flowdef test_owner_broadcast_reaches_all_workers_and_claim_flow(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") from app.database import` (стр. 94)
- `WorkerBroadcastTests.test_broadcast_forbidden_for_worker_and_emptydef test_broadcast_forbidden_for_worker_and_empty(self) -> None: from app.database import SessionLocal from app.models import StaffUser self.disable_owner_two_factor() owner_token ` (стр. 179)

### backend/tests/test_worker_calendar.py (276 строк)

Классы и функции (17):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 26)
- `build_init_datadef build_init_data(telegram_id: str) -> str: """Build Telegram init data that passes insecure validation (no HMAC).""" return urllib.parse.urlencode({"user": json.dumps({"id": int` (стр. 38)
- `class WorkerCalendarTests(unittest.TestCase):` (стр. 43)
- `WorkerCalendarTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 49)
- `WorkerCalendarTests.tearDowndef tearDown(self) -> None: if hasattr(self, "client_manager"):` (стр. 76)
- `WorkerCalendarTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select with SessionLocal() as db: ivan = db` (стр. 93)
- `WorkerCalendarTests._auth_headersdef _auth_headers(token: str) -> dict[str, str]: return {"Authorization": token}` (стр. 108)
- `WorkerCalendarTests._next_active_datedef _next_active_date() -> str: candidate = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0) for offset in range(1, 8):` (стр. 112)
- `WorkerCalendarTests._create_clientdef _create_client(self) -> tuple[str, str]: from app.database import SessionLocal from app.models import Client client_id = f"c-{uuid4().hex[:12]}" phone = f"+7 (999) 000-{str(uui` (стр. 120)
- `WorkerCalendarTests._create_bookingdef _create_booking(self, *, worker_id: str = "w2", status: str = "new", time: str = "10:00") -> str: client_id, client_phone = self._create_client() response = self.client.post( "` (стр. 139)
- `WorkerCalendarTests.test_worker_sees_own_bookingsdef test_worker_sees_own_bookings(self) -> None: # ivan (w1) видит свои записи (основная услуга) booking_id = self._create_booking(worker_id="w1") response = self.client.get( "/api` (стр. 170)
- `WorkerCalendarTests.test_worker_does_not_see_bookings_of_other_workersdef test_worker_does_not_see_bookings_of_other_workers(self) -> None: # чужие записи (только w2, без участия w1) скрыты booking_id = self._create_booking(worker_id="w2") response =` (стр. 184)
- `WorkerCalendarTests.test_worker_sees_booking_with_only_additional_servicedef test_worker_sees_booking_with_only_additional_service(self) -> None: # запись, где мастер участвует ТОЛЬКО в доп. услуге, видна в календаре booking_id = self._create_booking(wo` (стр. 196)
- `WorkerCalendarTests.test_worker_calendar_omits_sensitive_fieldsdef test_worker_calendar_omits_sensitive_fields(self) -> None: self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/calendar", headers=self._auth_headers(s` (стр. 231)
- `WorkerCalendarTests.test_worker_calendar_excludes_cancelled_bookingsdef test_worker_calendar_excludes_cancelled_bookings(self) -> None: active_id = self._create_booking(worker_id="w1") cancel_response = self.client.patch( f"/api/bookings/{active_id` (стр. 244)
- `WorkerCalendarTests.test_worker_calendar_forbidden_for_ownerdef test_worker_calendar_forbidden_for_owner(self) -> None: response = self.client.get( "/api/worker/calendar", headers=self._auth_headers(self.owner_token), ) self.assertEqual(res` (стр. 263)
- `WorkerCalendarTests.test_worker_calendar_forbidden_without_authdef test_worker_calendar_forbidden_without_auth(self) -> None: response = self.client.get("/api/worker/calendar") self.assertEqual(response.status_code, 401, response.text)` (стр. 270)

### backend/tests/test_worker_car_search.py (378 строк)

Классы и функции (24):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 27)
- `build_init_datadef build_init_data(telegram_id: str) -> str: """Build Telegram init data that passes insecure validation (no HMAC).""" return urllib.parse.urlencode({"user": json.dumps({"id": int` (стр. 38)
- `class WorkerCarSearchTests(unittest.TestCase):` (стр. 43)
- `WorkerCarSearchTests.setUpdef setUp(self) -> None: data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path = data_dir / f"test_suite_{uuid4().hex}.sq` (стр. 49)
- `WorkerCarSearchTests.tearDowndef tearDown(self) -> None: if hasattr(self, "client_manager"):` (стр. 76)
- `WorkerCarSearchTests._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from app.database import SessionLocal from app.models import StaffUser from sqlalchemy import select with SessionLocal() as db: ivan = db` (стр. 93)
- `WorkerCarSearchTests._auth_headersdef _auth_headers(token: str) -> dict[str, str]: return {"Authorization": token}` (стр. 108)
- `WorkerCarSearchTests._next_active_datedef _next_active_date() -> str: candidate = datetime.now(timezone.utc).replace(hour=0, minute=0, second=0, microsecond=0) for offset in range(1, 8):` (стр. 112)
- `WorkerCarSearchTests._today_labeldef _today_label() -> str: return datetime.now().astimezone().strftime("%d.%m.%Y")` (стр. 121)
- `WorkerCarSearchTests._create_clientdef _create_client(self) -> tuple[str, str]: from app.database import SessionLocal from app.models import Client client_id = f"c-{uuid4().hex[:12]}" phone = f"+7 (999) 000-{str(uui` (стр. 124)
- `WorkerCarSearchTests._create_bookingdef _create_booking(self, *, worker_id: str = "w1", status: str = "new", time: str = "10:00") -> str: client_id, client_phone = self._create_client() response = self.client.post( "` (стр. 143)
- `WorkerCarSearchTests._create_booking_directdef _create_booking_direct( self, *, date: str, plate: str = "M001AA", car: str = "BMW", client_name: str = "Тест Клиент", status: str = "new", worker_id: str = "w1",` (стр. 170)
- `WorkerCarSearchTests.test_worker_searches_by_plate_case_insensitivedef test_worker_searches_by_plate_case_insensitive(self) -> None: booking_id = self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", headers=s` (стр. 231)
- `WorkerCarSearchTests.test_worker_searches_by_plate_with_spaces_and_dashesdef test_worker_searches_by_plate_with_spaces_and_dashes(self) -> None: booking_id = self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", hea` (стр. 243)
- `WorkerCarSearchTests.test_worker_searches_by_car_modeldef test_worker_searches_by_car_model(self) -> None: booking_id = self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", headers=self._auth_hea` (стр. 255)
- `WorkerCarSearchTests.test_worker_searches_by_client_namedef test_worker_searches_by_client_name(self) -> None: booking_id = self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", headers=self._auth_h` (стр. 267)
- `WorkerCarSearchTests.test_worker_search_no_match_returns_empty_listdef test_worker_search_no_match_returns_empty_list(self) -> None: self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", headers=self._auth_hea` (стр. 279)
- `WorkerCarSearchTests.test_worker_search_empty_query_returns_today_onlydef test_worker_search_empty_query_returns_today_only(self) -> None: today_id = self._create_booking_direct(date=self._today_label()) tomorrow = (datetime.now().astimezone() + time` (стр. 290)
- `WorkerCarSearchTests.test_worker_search_respects_date_paramdef test_worker_search_respects_date_param(self) -> None: tomorrow = (datetime.now().astimezone() + timedelta(days=1)).strftime("%d.%m.%Y") future_id = self._create_booking_direct(` (стр. 304)
- `WorkerCarSearchTests.test_worker_search_excludes_cancelleddef test_worker_search_excludes_cancelled(self) -> None: active_id = self._create_booking(worker_id="w1") cancel_response = self.client.patch( f"/api/bookings/{active_id}", headers` (стр. 317)
- `WorkerCarSearchTests.test_worker_search_reports_workers_on_bookingdef test_worker_search_reports_workers_on_booking(self) -> None: self._create_booking(worker_id="w1") response = self.client.get( "/api/worker/cars/search", headers=self._auth_head` (стр. 337)
- `WorkerCarSearchTests.test_worker_search_excludes_other_workers_bookingsdef test_worker_search_excludes_other_workers_bookings(self) -> None: own_id = self._create_booking(worker_id="w1") alien_id = self._create_booking(worker_id="w2", time="11:00") re` (стр. 350)
- `WorkerCarSearchTests.test_worker_search_forbidden_for_ownerdef test_worker_search_forbidden_for_owner(self) -> None: response = self.client.get( "/api/worker/cars/search", headers=self._auth_headers(self.owner_token), params={"q": "M001AA"` (стр. 364)
- `WorkerCarSearchTests.test_worker_search_forbidden_without_authdef test_worker_search_forbidden_without_auth(self) -> None: response = self.client.get("/api/worker/cars/search", params={"q": "M001AA"}) self.assertEqual(response.status_code, 40` (стр. 372)
...
### backend/tests/test_xlsx_neutralization.py (70 строк)

Классы и функции (4):

- `class XlsxNeutralizationTests(unittest.TestCase):` (стр. 19)
...
### frontend/src/app/components/admin/AdminApp.tsx (3697 строк)
...
- `STOCK_UNITS` (стр. 167) — локальный
- `isDetailingService` (стр. 168) — локальный
- `serviceResourceGroup` (стр. 172) — локальный
- `hasManualScheduling` (стр. 176) — локальный
- `bookingBoxesForService` (стр. 180) — локальный
- `bookingLocationLabel` (стр. 188) — локальный
- `parseBookingMinutes` (стр. 192) — локальный
- `match` (стр. 193) — локальный
- `hours` (стр. 195) — локальный
- `minutes` (стр. 196) — локальный
- `bookingBlocksBox` (стр. 201) — локальный
- `nextStart` (стр. 204) — локальный
- `existingStart` (стр. 205) — локальный
- `nextEnd` (стр. 207) — локальный
- `existingEnd` (стр. 208) — локальный
- `pickDefaultBookingBox` (стр. 212) — локальный
- `resourceGroup` (стр. 221) — локальный
- `preferred` (стр. 222) — локальный
- `fallback` (стр. 223) — локальный
- `candidates` (стр. 224) — локальный
- `paymentLabel` (стр. 229) — локальный
- `normalizePhoneSearchValue` (стр. 238) — локальный
- `bookingStatusRequiresScheduledSlot` (стр. 242) — локальный
- `numberInputValue` (стр. 246) — локальный
- `numberFromInput` (стр. 250) — локальный
- `toISODate` (стр. 254) — локальный
- `parsed` (стр. 255) — локальный
- `y` (стр. 257) — локальный
- `m` (стр. 258) — локальный
- `d` (стр. 259) — локальный
- `TIME_SLOTS` (стр. 263) — локальный
- `h` (стр. 264) — локальный
- `m` (стр. 265) — локальный
- `formatSlotMinutesLabel` (стр. 269) — локальный
- `safe` (стр. 270) — локальный
- `dayShift` (стр. 271) — локальный
- `minutesOfDay` (стр. 272) — локальный
- `label` (стр. 273) — локальный
- `dataUrlApproxBytes` (стр. 277) — локальный
- `padding` (стр. 279) — локальный
- `loadImage` (стр. 283) — локальный
- `image` (стр. 285) — локальный
- `compressShiftPhoto` (стр. 292) — локальный
- `objectUrl` (стр. 293) — локальный
- `image` (стр. 295) — локальный
- `scale` (стр. 296) — локальный
- `width` (стр. 297) — локальный
- `height` (стр. 298) — локальный
- `canvas` (стр. 299) — локальный
- `context` (стр. 302) — локальный
- `AdminApp` (стр. 320)
- `parentCategories` (стр. 392) — локальный
- `selectableBookingDates` (стр. 473) — локальный
- `masterWorkers` (стр. 479) — локальный
- `selectedClient` (стр. 480) — локальный
- `normalizedClientSearchQuery` (стр. 481) — локальный
- `filteredClients` (стр. 484) — локальный
- `plates` (стр. 489) — локальный
- `selectedClientBookings` (стр. 497) — локальный
- `leftDate` (стр. 501) — локальный
- `rightDate` (стр. 502) — локальный
- `selectedClientFilteredBookings` (стр. 507) — локальный
- `svc` (стр. 509) — локальный
- `selectedClientVehicles` (стр. 513) — локальный
- `newBookingClientVehicles` (стр. 517) — локальный
- `client` (стр. 519) — локальный
- `selectedClientSpent` (стр. 525) — локальный
- `selectedClientCompletedCount` (стр. 528) — локальный
- `selectedClientUpcoming` (стр. 529) — локальный
- `selectedClientLastVisit` (стр. 530) — локальный
- `shiftSupplies` (стр. 531) — локальный
- `uploadedShiftPhotos` (стр. 536) — локальный
- `selectedService` (стр. 549) — локальный
- `defaultBoxForService` (стр. 567) — локальный
- `settingsBoxes` (стр. 579) — локальный
- `bookingFormBoxes` (стр. 580) — локальный
- `editBookingBoxes` (стр. 581) — локальный
- `newBookingLocationLabel` (стр. 584) — локальный
- `editBookingLocationLabel` (стр. 585) — локальный
- `modalMaxHeight` (стр. 637) — локальный
- `vv` (стр. 641) — локальный
- `handler` (стр. 643) — локальный
- `el` (стр. 644) — локальный
- `staffRoleTitle` (стр. 657) — локальный
- `staffNotificationsRole` (стр. 658) — локальный
- `adminNotifications` (стр. 659) — локальный
- `unreadCount` (стр. 664) — локальный
- `todayBookings` (стр. 665) — локальный
- `completedAll` (стр. 666) — локальный
- `totalRevenue` (стр. 667) — локальный
- `glass` (стр. 669) — локальный
- `bg` (стр. 670) — локальный
- `text` (стр. 671) — локальный
- `sub` (стр. 672) — локальный
- `primary` (стр. 673) — локальный
- `accent` (стр. 674) — локальный
- `surface` (стр. 675) — локальный
- `inputCls` (стр. 676) — локальный
- `selectCls` (стр. 677) — локальный
- `timeToMinutes` (стр. 678) — локальный
- `match` (стр. 679) — локальный
- `hours` (стр. 681) — локальный
- `minutes` (стр. 682) — локальный
- `byService` (стр. 688) — локальный
- `byStatus` (стр. 694) — локальный
- `byPayment` (стр. 705) — локальный
- `workerStats` (стр. 712) — локальный
- `bw` (стр. 716) — локальный
- `avgCheck` (стр. 723) — локальный
- `conversionRate` (стр. 724) — локальный
- `scheduleSummary` (стр. 725) — локальный
- `revenueData` (стр. 726) — локальный
- `formatted` (стр. 727) — локальный
- `hourData` (стр. 733) — локальный
- `handleStatusChange` (стр. 737) — локальный
- `target` (стр. 738) — локальный
- `statusNeedsSlot` (стр. 739) — локальный
- `handleDeleteClient` (стр. 751) — локальный
- `confirmed` (стр. 752) — локальный
- `handleCreateClient` (стр. 757) — локальный
- `nameError` (стр. 759) — локальный
- `phoneError` (стр. 763) — локальный
- `carError` (стр. 767) — локальный
- `plateError` (стр. 771) — локальный
- `created` (стр. 779) — локальный
- `handleSaveClientCard` (стр. 801) — локальный
- `draft` (стр. 802) — локальный
- `handleShiftPhotoChange` (стр. 816) — локальный
- `file` (стр. 817) — локальный
- `dataUrl` (стр. 821) — локальный
- `handleSubmitShiftInspection` (стр. 833) — локальный
- `primaryPhoto` (стр. 837) — локальный
- `uploadedCategoriesLabel` (стр. 844) — локальный
- `composedNote` (стр. 845) — локальный
- `saved` (стр. 849) — локальный
- `validateClientName` (стр. 866) — локальный
- `validateClientPhone` (стр. 870) — локальный
- `validateBookingDate` (стр. 874) — локальный
- `parsedDate` (стр. 876) — локальный
- `scheduleDay` (стр. 881) — локальный
- `normalizedTime` (стр. 886) — локальный
- `slotStart` (стр. 887) — локальный
- `openMinutes` (стр. 896) — локальный
- `closeMinutes` (стр. 897) — локальный
- `slotEnd` (стр. 898) — локальный
...
- `validateBookingDateTimeFormat` (стр. 943) — локальный
- `parsedDate` (стр. 945) — локальный
- `validateNewBookingForm` (стр. 958) — локальный
- `selectedService` (стр. 960) — локальный
- `nameError` (стр. 962) — локальный
- `phoneError` (стр. 966) — локальный
- `carError` (стр. 970) — локальный
- `plateError` (стр. 974) — локальный
- `hasDate` (стр. 977) — локальный
- `hasTime` (стр. 978) — локальный
- `requiresScheduledSlot` (стр. 979) — локальный
- `validation` (стр. 996) — локальный
- `resetNewBookingDraft` (стр. 1006) — локальный
- `openNewBookingModal` (стр. 1036) — локальный
- `openAdditionalServiceModal` (стр. 1041) — локальный
- `openNewBookingForClient` (стр. 1050) — локальный
- `historyDate` (стр. 1052) — локальный
- `clientVehicles` (стр. 1054) — локальный
- `mainVehicle` (стр. 1056) — локальный
- `hasPriorVisits` (стр. 1057) — локальный
- `closeNewBookingModal` (стр. 1075) — локальный
- `handleAddService` (стр. 1080) — локальный
- `svc` (стр. 1085) — локальный
- `workersList` (стр. 1086) — локальный
- `worker` (стр. 1087) — локальный
- `updatedBooking` (стр. 1090) — локальный
- `handleRemoveService` (стр. 1110) — локальный
- `handleCreateServiceFromQuery` (стр. 1114) — локальный
- `name` (стр. 1115) — локальный
- `createDraftId` (стр. 1116) — локальный
- `newId` (стр. 1117) — локальный
- `handleOpenEditAsvc` (стр. 1145) — локальный
- `handleSaveEditAsvc` (стр. 1153) — локальный
- `workersList` (стр. 1158) — локальный
...
### frontend/src/app/components/admin/screens/AdminCalendarDayScreen.tsx (394 строк)
...
- `mainServiceBasePrice` (стр. 379) — локальный
- `additionalAddTotal` (стр. 380) — локальный
- `legacyTotal` (стр. 384) — локальный
- `formatWorkerPay` (стр. 388) — локальный
- `percent` (стр. 391) — локальный
- `earned` (стр. 392) — локальный
...
### frontend/src/app/components/admin/screens/AdminPayrollPage.tsx (412 строк)

- `AdminPayrollPage` (стр. 24)
- `loadPayrollData` (стр. 61) — локальный
- `params` (стр. 66) — локальный
- `handleSaveSettings` (стр. 84) — локальный
...
- `liveWorker` (стр. 191) — локальный
- `payrollSummary` (стр. 192) — локальный
- `r` (стр. 235) — локальный
- `n` (стр. 240) — локальный
- `resolvedKind` (стр. 382) — локальный
...
### frontend/src/app/components/atmosfera/EditAmountPencil.tsx (43 строк)

- `EditAmountPencil` (стр. 22)

### frontend/src/app/components/atmosfera/FormRow.tsx (35 строк)

- `FormRow` (стр. 17)

### frontend/src/app/components/atmosfera/index.ts (30 строк)
...
### frontend/src/app/components/atmosfera/StatTile.tsx (43 строк)
...
- `m` (стр. 27) — локальный
- `d` (стр. 28) — локальный
- `TIME_SLOTS` (стр. 32) — локальный
- `h` (стр. 33) — локальный
- `m` (стр. 34) — локальный
- `MONTH_LABELS` (стр. 38) — локальный
- `PLAN_OPTIONS` (стр. 40) — локальный
- `TXN_TYPE_OPTIONS` (стр. 47) — локальный
- `monthKeyToLabel` (стр. 55) — локальный
- `match` (стр. 56) — локальный
- `monthIndex` (стр. 58) — локальный
- `currentMonthKey` (стр. 62) — локальный
- `now` (стр. 63) — локальный
- `txnMonthOf` (стр. 67) — локальный
- `parts` (стр. 68) — локальный
- `formatMoney` (стр. 72) — локальный
- `planLabel` (стр. 76) — локальный
- `fieldLabel` (стр. 80) — локальный
- `BalanceSparkline` (стр. 84) — локальный
- `points` (стр. 85) — локальный
- `sorted` (стр. 87) — локальный
- `values` (стр. 88) — локальный
- `min` (стр. 89) — локальный
- `max` (стр. 90) — локальный
- `range` (стр. 91) — локальный
- `w` (стр. 92) — локальный
- `h` (стр. 93) — локальный
- `coords` (стр. 100) — локальный
- `Sheet` (стр. 124) — локальный
- `DepositPanel` (стр. 154)
- `glass` (стр. 172) — локальный
- `sub` (стр. 173) — локальный
- `primary` (стр. 174) — локальный
- `accent` (стр. 175) — локальный
...
### frontend/src/app/components/owner/OwnerApp.tsx (12687 строк)

- `stockCategoryIdsWithDescendants` (стр. 45) — локальный
- `map` (стр. 46) — локальный
- `queue` (стр. 49) — локальный
- `visited` (стр. 50) — локальный
- `EXPENSE_CATEGORIES` (стр. 308) — локальный
- `STOCK_UNITS` (стр. 319) — локальный
- `SERVICE_TYPE_OPTIONS` (стр. 320) — локальный
- `ownerBookingStatusRequiresScheduledSlot` (стр. 331) — локальный
- `employeeRoleLabel` (стр. 334) — локальный
- `ownerServiceResourceGroup` (стр. 340) — локальный
- `ownerBookingBoxes` (стр. 344) — локальный
- `ownerLocationLabel` (стр. 352) — локальный
- `parseOwnerBookingMinutes` (стр. 356) — локальный
- `match` (стр. 357) — локальный
- `hours` (стр. 359) — локальный
- `minutes` (стр. 360) — локальный
- `formatSlotMinutesLabel` (стр. 365) — локальный
- `safe` (стр. 366) — локальный
- `dayShift` (стр. 367) — локальный
- `minutesOfDay` (стр. 368) — локальный
- `label` (стр. 369) — локальный
- `OWNER_CALENDAR_WEEKDAYS` (стр. 373) — локальный
- `OWNER_CALENDAR_MONTHS` (стр. 374) — локальный
- `OWNER_CALENDAR_DEFAULT_OPEN` (стр. 378) — локальный
- `OWNER_CALENDAR_DEFAULT_CLOSE` (стр. 379) — локальный
- `ownerScheduleTimeToMinutes` (стр. 381) — локальный
- `ownerMonthTitle` (стр. 385) — локальный
- `ownerBuildMonthCells` (стр. 389) — локальный
- `year` (стр. 390) — локальный
- `month` (стр. 391) — локальный
- `first` (стр. 392) — локальный
- `offset` (стр. 393) — локальный
- `daysInMonth` (стр. 394) — локальный
- `date` (стр. 400) — локальный
- `ownerCalendarDayHours` (стр. 409) — локальный
- `parsedDate` (стр. 410) — локальный
- `daySchedule` (стр. 414) — локальный
- `open` (стр. 418) — локальный
- `close` (стр. 419) — локальный
- `OWNER_CALENDAR_LOAD_COLORS` (стр. 423) — локальный
- `ownerCalendarLoadTone` (стр. 429) — локальный
- `ratio` (стр. 431) — локальный
- `ownerGroupBookingsByHour` (стр. 441) — локальный
- `timed` (стр. 446) — локальный
- `hourLabel` (стр. 449) — локальный
- `slotEnd` (стр. 450) — локальный
- `slotBookings` (стр. 451) — локальный
- `start` (стр. 453) — локальный
- `ownerOpenBookingDetail` (стр. 465) — локальный
- `ownerBookingBlocksBox` (стр. 474) — локальный
- `nextStart` (стр. 477) — локальный
- `existingStart` (стр. 478) — локальный
- `nextEnd` (стр. 480) — локальный
- `existingEnd` (стр. 481) — локальный
- `ownerPickDefaultBookingBox` (стр. 485) — локальный
- `resourceGroup` (стр. 494) — локальный
- `preferred` (стр. 495) — локальный
- `fallback` (стр. 496) — локальный
- `candidates` (стр. 497) — локальный
- `serviceResourceGroupForCategory` (стр. 502) — локальный
- `numberInputValue` (стр. 506) — локальный
- `ORDER_STEPS` (стр. 520) — локальный
- `serviceMoneySummary` (стр. 527) — локальный
- `piggyTargetLabel` (стр. 528) — локальный
- `master` (стр. 532) — локальный
- `piggy` (стр. 537) — локальный
- `owners` (стр. 546) — локальный
- `previewServiceSplit` (стр. 554) — локальный
- `materials` (стр. 559) — локальный
- `net` (стр. 560) — локальный
- `order` (стр. 561) — локальный
- `pipeline` (стр. 562) — локальный
- `piggyType` (стр. 563) — локальный
- `computeMaster` (стр. 570) — локальный
- `computePiggy` (стр. 579) — локальный
- `m` (стр. 587) — локальный
- `p` (стр. 589) — локальный
- `afterMasterPiggy` (стр. 593) — локальный
- `m` (стр. 612) — локальный
- `p` (стр. 616) — локальный
- `isLast` (стр. 620) — локальный
- `claimed` (стр. 621) — локальный
- `numberFromInput` (стр. 647) — локальный
- `parseDecimalInput` (стр. 652) — локальный
- `normalized` (стр. 653) — локальный
- `parsed` (стр. 654) — локальный
- `isValidAmountInput` (стр. 658) — локальный
- `n` (стр. 659) — локальный
- `toISODate` (стр. 663) — локальный
- `parsed` (стр. 664) — локальный
- `y` (стр. 666) — локальный
- `m` (стр. 667) — локальный
- `d` (стр. 668) — локальный
- `TIME_SLOTS` (стр. 672) — локальный
- `h` (стр. 673) — локальный
- `m` (стр. 674) — локальный
- `OwnerApp` (стр. 681)
- `isAccountant` (стр. 749) — локальный
- `modalMaxHeight` (стр. 750) — локальный
- `financeRoleTitle` (стр. 751) — локальный
- `financeNotificationRole` (стр. 752) — локальный
- `__nowRpt` (стр. 825) — локальный
- `__dowRpt` (стр. 826) — локальный
- `__monRpt` (стр. 827) — локальный
- `__sunRpt` (стр. 828) — локальный
- `newPayRequestId` (стр. 843) — локальный
- `piggyWithdrawRequestIdRef` (стр. 852) — локальный
- `piggyAdjustRequestIdRef` (стр. 854) — локальный
- `entryRequestIdRef` (стр. 909) — локальный
- `today` (стр. 1063) — локальный
- `clearOwnerResetFlow` (стр. 1183) — локальный
- `nextBoxes` (стр. 1206) — локальный
- `params` (стр. 1244) — локальный
- `params` (стр. 1259) — локальный
- `handlePayOwnerSalary` (стр. 1270) — локальный
- `amount` (стр. 1271) — локальный
- `updated` (стр. 1290) — локальный
- `handleToggleOwnerMasterRole` (стр. 1298) — локальный
- `params` (стр. 1303) — локальный
- `updated` (стр. 1308) — локальный
- `loadPiggyBank` (стр. 1319) — локальный
- `params` (стр. 1323) — локальный
- `qs` (стр. 1326) — локальный
- `data` (стр. 1328) — локальный
- `loadWallet` (стр. 1336) — локальный
- `params` (стр. 1340) — локальный
- `qs` (стр. 1343) — локальный
- `data` (стр. 1345) — локальный
- `handlePiggyWithdraw` (стр. 1351) — локальный
- `f` (стр. 1352) — локальный
- `amount` (стр. 1355) — локальный
- `buyerLabel` (стр. 1387) — локальный
- `segmentLabel` (стр. 1390) — локальный
- `toastMsg` (стр. 1391) — локальный
- `openPiggyWithdraw` (стр. 1406) — локальный
- `handlePiggyBankExport` (стр. 1413) — локальный
- `openPiggyAdjust` (стр. 1424) — локальный
- `current` (стр. 1425) — локальный
- `currentPrecise` (стр. 1429) — локальный
- `handlePiggyAdjust` (стр. 1437) — локальный
- `newBalance` (стр. 1438) — локальный
- `delta` (стр. 1441) — локальный
- `handlePiggyDeleteTx` (стр. 1475) — локальный
- `syncCountdown` (стр. 1534) — локальный
- `diffMs` (стр. 1535) — локальный
- `intervalId` (стр. 1540) — локальный
- `handleOpenShiftForMasters` (стр. 1570) — локальный
- `saved` (стр. 1579) — локальный
- `ownerNotifications` (стр. 1595) — локальный
- `unreadCount` (стр. 1596) — локальный
- `completedBookings` (стр. 1597) — локальный
- `todayBookings` (стр. 1598) — локальный
- `activeMasters` (стр. 1600) — локальный
- `masterCameOutTodayAt` (стр. 1605) — локальный
- `times` (стр. 1606) — локальный
- `mastersCameOutToday` (стр. 1615) — локальный
- `vv` (стр. 1619) — локальный
- `handler` (стр. 1621) — локальный
- `el` (стр. 1622) — локальный
- `bookingFormBoxes` (стр. 1629) — локальный
- `bookingFormLocationLabel` (стр. 1630) — локальный
- `editBookingLocationLabel` (стр. 1631) — локальный
- `todayRevenue` (стр. 1632) — локальный
- `now` (стр. 1635) — локальный
- `dayOfWeek` (стр. 1636) — локальный
- `diffToSaturday` (стр. 1637) — локальный
- `weekSaturday` (стр. 1638) — локальный
- `weekFriday` (стр. 1641) — локальный
- `isDateInWeek` (стр. 1644) — локальный
- `d` (стр. 1645) — локальный
- `weeklyCompletedBookings` (стр. 1648) — локальный
- `weeklyBookings` (стр. 1649) — локальный
- `weeklyExpenses` (стр. 1650) — локальный
- `weeklyIncomes` (стр. 1651) — локальный
- `totalRevenue` (стр. 1652) — локальный
- `totalExpenses` (стр. 1653) — локальный
- `totalIncomes` (стр. 1654) — локальный
- `profit` (стр. 1655) — локальный
- `averageCheck` (стр. 1656) — локальный
- `activeBookings` (стр. 1657) — локальный
- `pipelineCounts` (стр. 1658) — локальный
- `statusListItems` (стр. 1665) — локальный
- `totalStockValue` (стр. 1670) — локальный
- `resourceGroupLabel` (стр. 1672) — локальный
- `payrollRows` (стр. 1677) — локальный
- `workerPenalties` (стр. 1678) — локальный
- `complaintState` (стр. 1679) — локальный
- `payrollTotal` (стр. 1687) — локальный
- `formatComplaintDate` (стр. 1688) — локальный
- `resetPreviewRows` (стр. 1689) — локальный
- `resetExecuteLocked` (стр. 1703) — локальный
- `glass` (стр. 1705) — локальный
- `bg` (стр. 1706) — локальный
- `text` (стр. 1707) — локальный
- `sub` (стр. 1708) — локальный
- `muted` (стр. 1712) — локальный
- `segTrack` (стр. 1713) — локальный
- `primary` (стр. 1714) — локальный
- `accent` (стр. 1715) — локальный
- `surface` (стр. 1716) — локальный

### frontend/src/app/components/owner/screens/OwnerClientsScreen.tsx (843 строк)
...
- `isMain` (стр. 618) — локальный
- `client` (стр. 634) — локальный
- `current` (стр. 636) — локальный
- `selected` (стр. 646) — локальный
- `client` (стр. 680) — локальный
- `current` (стр. 682) — локальный
- `client` (стр. 726) — локальный
- `current` (стр. 728) — локальный
- `ownerPaymentLabel` (стр. 838) — локальный

### frontend/src/app/components/owner/screens/OwnerPiggyBankScreen.tsx (673 строк)

- `OwnerPiggyBankScreen` (стр. 62)
- `ownerStatusBadge` (стр. 118) — локальный
- `tabBalance` (стр. 170) — локальный
- `tabLabel` (стр. 173) — локальный
- `rem` (стр. 220) — локальный
- `otherWd` (стр. 379) — локальный
- `debtTxs` (стр. 448) — локальный
- `total` (стр. 452) — локальный
- `isOther` (стр. 478) — локальный
- `filteredTxs` (стр. 520) — локальный
- `legacyById` (стр. 528) — локальный
- `isDeposit` (стр. 536) — локальный
- `txLabel` (стр. 537) — локальный
- `booking` (стр. 543) — локальный
- `handleClick` (стр. 544) — локальный
- `Wrapper` (стр. 551) — локальный
- `txRunningBalance` (стр. 554) — локальный
- `canDelete` (стр. 557) — локальный
- `deleteLabel` (стр. 558) — локальный

### frontend/src/app/components/owner/screens/OwnerStockPage.tsx (364 строк)

- `STOCK_UNITS` (стр. 9) — локальный
- `buildCategoryTree` (стр. 19) — локальный
- `map` (стр. 20) — локальный
- `node` (стр. 24) — локальный
- `sortRec` (стр. 28) — локальный
- `flattenCategories` (стр. 35) — локальный
- `tree` (стр. 36) — локальный
- `dfs` (стр. 38) — локальный
- `getDescendantIds` (стр. 44) — локальный
- `map` (стр. 45) — локальный
- `result` (стр. 47) — локальный
- `queue` (стр. 48) — локальный
- `visited` (стр. 49) — локальный
- `cur` (стр. 51) — локальный
- `children` (стр. 54) — локальный
- `collectSubtreeIds` (стр. 59) — локальный
- `ids` (стр. 60) — локальный
- `OwnerStockPage` (стр. 71)
- `isAccountant` (стр. 84) — локальный
- `categoryTree` (стр. 92) — локальный
- `flattenedForSelect` (стр. 93) — локальный
- `adminShiftPhotoUrlsRef` (стр. 104) — локальный
- `latestShiftChecklists` (стр. 106) — локальный
- `latestAdminShiftInspections` (стр. 107) — локальный
- `latestAdminShiftInspectionKey` (стр. 108) — локальный
- `totalStockValue` (стр. 110) — локальный
- `activeIds` (стр. 115) — локальный
- `currentPhotoUrls` (стр. 121) — локальный
- `missing` (стр. 122) — локальный
- `handleAddStock` (стр. 133) — локальный
- `qty` (стр. 135) — локальный
- `rawPrice` (стр. 136) — локальный
- `unitPrice` (стр. 137) — локальный
- `selectedCat` (стр. 138) — локальный
- `handleWriteOff` (стр. 145) — локальный
- `item` (стр. 147) — локальный
- `glass` (стр. 154) — локальный
- `sub` (стр. 155) — локальный
- `inputCls` (стр. 156) — локальный
- `selectCls` (стр. 157) — локальный
- `renderStockTree` (стр. 159) — локальный
- `allIds` (стр. 161) — локальный
- `subtreeItems` (стр. 162) — локальный
- `childContent` (стр. 164) — локальный
- `hasVisibleChild` (стр. 165) — локальный
- `cid` (стр. 166) — локальный
- `directItems` (стр. 180) — локальный
- `cat` (стр. 197) — локальный
- `renderCategoryManagerTree` (стр. 226) — локальный
- `descendantCount` (стр. 228) — локальный
- `withoutCat` (стр. 277) — локальный
- `InlineRename` (стр. 355) — локальный

### frontend/src/app/components/owner/screens/OwnerWalletScreen.tsx (262 строк)

- `OwnerWalletScreen` (стр. 36)
...
### frontend/src/app/components/worker/screens/WorkerEarningsScreen.tsx (451 строк)

- `DANGER` (стр. 20) — локальный
- `SUCCESS` (стр. 21) — локальный
- `WARNING` (стр. 22) — локальный
- `groupBookingsByDate` (стр. 24) — локальный
- `WorkerEarningsScreen` (стр. 46)
- `params` (стр. 69) — локальный
- `myPenalties` (стр. 85) — локальный
- `complaintState` (стр. 86) — локальный
- `glass` (стр. 90) — локальный
- `sub` (стр. 91) — локальный
- `shiftPay` (стр. 182) — локальный
- `bonuses` (стр. 183) — локальный
- `advances` (стр. 184) — локальный
- `isLegacyFine` (стр. 185) — локальный
- `deductions` (стр. 186) — локальный
- `fines` (стр. 187) — локальный
- `adjustments` (стр. 188) — локальный
- `totalAccrued` (стр. 189) — локальный
- `totalDeducted` (стр. 190) — локальный
- `resolvedKind` (стр. 340) — локальный
- `payouts` (стр. 393) — локальный
- `shiftPay` (стр. 421) — локальный
- `bonuses` (стр. 422) — локальный
- `advances` (стр. 423) — локальный
- `isLegacyFine` (стр. 424) — локальный
- `deductions` (стр. 425) — локальный
- `fines` (стр. 426) — локальный
- `adjustments` (стр. 427) — локальный
...
### frontend/src/app/components/worker/screens/WorkerScheduleScreen.tsx (170 строк)

- `WorkerScheduleScreen` (стр. 28)
- `loadCalendar` (стр. 40) — локальный
- `isMyTask` (стр. 59) — локальный
- `dayTasks` (стр. 86) — локальный
- `completed` (стр. 96) — локальный
...
### frontend/src/app/components/worker/shared/CarSearch.tsx (182 строк)

- `CarSearch` (стр. 22)
- `isMainParticipant` (стр. 28) — локальный
- `myExtras` (стр. 30) — локальный
- `isMine` (стр. 32) — локальный
- `participationLabel` (стр. 33) — локальный
- `isMain` (стр. 34) — локальный
- `hasExtras` (стр. 35) — локальный
- `myResults` (стр. 43) — локальный
- `timer` (стр. 48) — локальный
- `params` (стр. 50) — локальный
- `extras` (стр. 130) — локальный
- `isMain` (стр. 131) — локальный
...
### frontend/src/app/components/worker/WorkerApp.tsx (760 строк)
...
- `WorkerApp` (стр. 95)
- `workerId` (стр. 114) — локальный
- `myNotifications` (стр. 143) — локальный
- `unreadCount` (стр. 144) — локальный
- `isBroadcast` (стр. 147) — локальный
- `isMyTask` (стр. 149) — локальный
- `allTasks` (стр. 153) — локальный
- `todayTasks` (стр. 156) — локальный
- `formatTimer` (стр. 164) — локальный
- `glass` (стр. 166) — локальный
- `bg` (стр. 167) — локальный
- `text` (стр. 168) — локальный
- `sub` (стр. 169) — локальный
- `primary` (стр. 170) — локальный
- `accent` (стр. 171) — локальный
- `surface` (стр. 172) — локальный
- `handleStartTask` (стр. 174) — локальный
- `openFinishModal` (стр. 182) — локальный
- `handleFinish` (стр. 191) — локальный
- `nextNote` (стр. 197) — локальный
- `headerTitle` (стр. 237) — локальный
- `isMyService` (стр. 317) — локальный
- `isOutsource` (стр. 318) — локальный
- `myBaseLink` (стр. 366) — локальный
- `myAdditionalServices` (стр. 367) — локальный
- `baseEarned` (стр. 369) — локальный
- `additionalEarned` (стр. 370) — локальный
- `total` (стр. 371) — локальный
- `earned` (стр. 382) — локальный
- `created` (стр. 434) — локальный
- `started` (стр. 435) — локальный
- `completed` (стр. 436) — локальный
- `isActive` (стр. 514) — локальный

### frontend/src/app/components/worker/WorkerCalendar.tsx (552 строк)

- `WORKER_CALENDAR_WEEKDAYS` (стр. 32) — локальный
- `WORKER_CALENDAR_MONTHS` (стр. 33) — локальный
- `WORKER_CALENDAR_DEFAULT_OPEN` (стр. 37) — локальный
- `WORKER_CALENDAR_DEFAULT_CLOSE` (стр. 38) — локальный
- `WORKER_CALENDAR_LOAD_COLORS` (стр. 40) — локальный
- `workerParseBookingMinutes` (стр. 46) — локальный
- `match` (стр. 47) — локальный
- `hours` (стр. 49) — локальный
- `minutes` (стр. 50) — локальный
- `workerScheduleTimeToMinutes` (стр. 55) — локальный
- `workerMonthTitle` (стр. 59) — локальный
- `workerBuildMonthCells` (стр. 63) — локальный
- `year` (стр. 64) — локальный
- `month` (стр. 65) — локальный
- `first` (стр. 66) — локальный
- `offset` (стр. 68) — локальный
- `daysInMonth` (стр. 69) — локальный
- `date` (стр. 75) — локальный
- `workerCalendarDayHours` (стр. 84) — локальный
- `parsedDate` (стр. 85) — локальный
- `daySchedule` (стр. 89) — локальный
- `open` (стр. 93) — локальный
- `close` (стр. 94) — локальный
- `workerCalendarLoadTone` (стр. 98) — локальный
- `ratio` (стр. 100) — локальный
- `workerGroupBookingsByHour` (стр. 110) — локальный
- `timed` (стр. 115) — локальный
- `hourLabel` (стр. 118) — локальный
- `slotEnd` (стр. 119) — локальный
- `slotBookings` (стр. 120) — локальный
- `start` (стр. 122) — локальный
- `workerCalendarStatusLabel` (стр. 134) — локальный
- `workerCalendarStatusBadge` (стр. 155) — локальный
- `WorkerCalendar` (стр. 189)
- `now` (стр. 204) — локальный
- `isMineBooking` (стр. 209) — локальный
- `myExtras` (стр. 213) — локальный
- `isMainParticipant` (стр. 216) — локальный
- `relevantBookings` (стр. 221) — локальный
- `bookingsByDate` (стр. 224) — локальный
- `dateLabel` (стр. 225) — локальный
- `monthCells` (стр. 233) — локальный
- `monthLabel` (стр. 234) — локальный
- `monthLoads` (стр. 235) — локальный
- `monthMaxLoad` (стр. 238) — локальный
- `dayBookings` (стр. 240) — локальный
- `dayHours` (стр. 241) — локальный
- `hourSlots` (стр. 242) — локальный
- `untimedBookings` (стр. 243) — локальный
- `dayTitle` (стр. 244) — локальный
- `carTitle` (стр. 250) — локальный
- `car` (стр. 251) — локальный
- `plate` (стр. 252) — локальный
- `statusLine` (стр. 261) — локальный
- `extras` (стр. 262) — локальный
- `today` (стр. 312) — локальный
- `dayItems` (стр. 332) — локальный
- `loadTone` (стр. 333) — локальный
- `loadWidth` (стр. 334) — локальный
- `isToday` (стр. 337) — локальный
- `today` (стр. 412) — локальный
- `extras` (стр. 463) — локальный
- `isMain` (стр. 464) — локальный
- `extras` (стр. 515) — локальный
- `isMain` (стр. 516) — локальный

### frontend/src/app/constants/referralSources.ts (8 строк)

- `REFERRAL_SOURCES` (стр. 1)

### frontend/src/app/context/AppContext.tsx (2241 строк)

- `EMPTY_CONTENT` (стр. 987)
- `timeToMinutes` (стр. 1009) — локальный
- `minutesToTime` (стр. 1016) — локальный
- `hours` (стр. 1017) — локальный
- `minutes` (стр. 1018) — локальный
- `buildTimeSlots` (стр. 1022) — локальный
- `timeRangesOverlap` (стр. 1030) — локальный
- `AppContext` (стр. 1034) — локальный
- `normalizeWorker` (стр. 1036) — локальный
- `normalizeBootstrap` (стр. 1050) — локальный
- `AppProvider` (стр. 1074)
- `THEME_OVERRIDE_KEY` (стр. 1083) — локальный
- `readThemeOverride` (стр. 1084) — локальный
- `raw` (стр. 1086) — локальный
- `writeThemeOverride` (стр. 1092) — локальный
- `override` (стр. 1101) — локальный
- `upcomingDates` (стр. 1122) — локальный
- `todayLabel` (стр. 1123) — локальный
- `tomorrowLabel` (стр. 1124) — локальный
- `applyBootstrap` (стр. 1126) — локальный
- `normalized` (стр. 1127) — локальный
- `refreshBootstrap` (стр. 1156) — локальный
- `bootstrap` (стр. 1157) — локальный
- `startRolePreview` (стр. 1161) — локальный
- `bootstrap` (стр. 1165) — локальный
- `stopRolePreview` (стр. 1178) — локальный
- `bootstrap` (стр. 1182) — локальный
- `handleError` (стр. 1195) — локальный
- `message` (стр. 1196) — локальный
- `restoreSession` (стр. 1201) — локальный
- `bootstrap` (стр. 1203) — локальный
- `refreshActiveSessions` (стр. 1212) — локальный
- `applyTelegramTheme` (стр. 1216) — локальный
- `root` (стр. 1220) — локальный
- `theme` (стр. 1221) — локальный
- `cssVar` (стр. 1224) — локальный
- `tg` (стр. 1231) — локальный
- `logout` (стр. 1248) — локальный
- `loginClient` (стр. 1273) — локальный
- `bootstrap` (стр. 1277) — локальный
- `linkStaff` (стр. 1291) — локальный
- `bootstrap` (стр. 1295) — локальный
- `switchRole` (стр. 1309) — локальный
- `bootstrap` (стр. 1313) — локальный
- `updateClientProfile` (стр. 1327) — локальный
- `payload` (стр. 1328) — локальный
- `saved` (стр. 1329) — локальный
- `remindAdminAboutInactiveClients` (стр. 1333) — локальный
- `response` (стр. 1334) — локальный
- `addClient` (стр. 1338) — локальный
- `created` (стр. 1339) — локальный
- `normalized` (стр. 1340) — локальный
- `updateClientCard` (стр. 1345) — локальный
- `saved` (стр. 1346) — локальный
- `normalized` (стр. 1347) — локальный
- `deleteClient` (стр. 1351) — локальный
- `listDepositClients` (стр. 1356) — локальный
- `items` (стр. 1357) — локальный
- `getDepositOverview` (стр. 1361) — локальный
- `overview` (стр. 1362) — локальный
- `updateDepositSubscription` (стр. 1370) — локальный
- `overview` (стр. 1371) — локальный
- `depositTopUp` (стр. 1379) — локальный
- `txn` (стр. 1380) — локальный
- `depositAdjust` (стр. 1384) — локальный
- `overview` (стр. 1385) — локальный
- `depositRecordWash` (стр. 1393) — локальный
- `overview` (стр. 1394) — локальный
- `depositSettleMonth` (стр. 1402) — локальный
- `overview` (стр. 1403) — локальный
- `resolveOwnerExportDelivery` (стр. 1417) — локальный
- `failedCount` (стр. 1419) — локальный
- `downloadDepositExport` (стр. 1434) — локальный
- `downloadDepositExportAll` (стр. 1438) — локальный
- `sendDepositExport` (стр. 1442) — локальный
- `sendDepositExportAll` (стр. 1446) — локальный
- `addBooking` (стр. 1450) — локальный
- `created` (стр. 1451) — локальный
- `existingClient` (стр. 1471) — локальный
- `nextClient` (стр. 1472) — локальный
- `updateBooking` (стр. 1496) — локальный
- `updated` (стр. 1497) — локальный
- `deleteBooking` (стр. 1524) — локальный
- `addBookingService` (стр. 1529) — локальный
- `updated` (стр. 1530) — локальный
- `addBookingAdditionalService` (стр. 1551) — локальный
- `updated` (стр. 1552) — локальный
- `updateBookingAdditionalService` (стр. 1573) — локальный
- `updated` (стр. 1574) — локальный
- `removeBookingAdditionalService` (стр. 1595) — локальный
- `updated` (стр. 1596) — локальный
- `addNotification` (стр. 1617) — локальный
- `created` (стр. 1618) — локальный
- `markNotificationRead` (стр. 1638) — локальный
- `markAllNotificationsRead` (стр. 1643) — локальный
- `sendWorkerBroadcast` (стр. 1657) — локальный
- `result` (стр. 1658) — локальный
- `takeNotificationToWork` (стр. 1663) — локальный
- `updated` (стр. 1664) — локальный
- `completeNotificationTask` (стр. 1668) — локальный
- `updated` (стр. 1669) — локальный
- `addStockItem` (стр. 1673) — локальный
- `created` (стр. 1674) — локальный
- `updateStockItem` (стр. 1678) — локальный
- `updated` (стр. 1679) — локальный
- `writeOffStock` (стр. 1683) — локальный
- `updated` (стр. 1684) — локальный
- `getWriteOffHistory` (стр. 1688) — локальный
- `deleteStockItem` (стр. 1692) — локальный
- `addStockCategory` (стр. 1697) — локальный
- `created` (стр. 1698) — локальный
- `updateStockCategory` (стр. 1702) — локальный
- `updated` (стр. 1703) — локальный
- `deleteStockCategory` (стр. 1707) — локальный
- `addExpense` (стр. 1716) — локальный
- `created` (стр. 1717) — локальный
- `addIncome` (стр. 1721) — локальный
- `created` (стр. 1722) — локальный
- `updateExpense` (стр. 1726) — локальный
- `updated` (стр. 1727) — локальный
- `updateIncome` (стр. 1731) — локальный
- `updated` (стр. 1732) — локальный
- `addPenalty` (стр. 1736) — локальный
- `revokePenalty` (стр. 1741) — локальный
- `revokeAllPenalties` (стр. 1746) — локальный
- `createTelegramLinkCode` (стр. 1751) — локальный
- `created` (стр. 1752) — локальный
- `downloadOwnerExport` (стр. 1756) — локальный
- `fallback` (стр. 1757) — локальный
- `qs` (стр. 1760) — локальный
- `qstr` (стр. 1765) — локальный
- `sendOwnerExportToTelegram` (стр. 1771) — локальный
- `fallback` (стр. 1772) — локальный
- `qs` (стр. 1775) — локальный
- `qstr` (стр. 1780) — локальный
- `sendOwnerSummaryReport` (стр. 1786) — локальный
- `response` (стр. 1787) — локальный
- `failedCount` (стр. 1791) — локальный
- `dispatchOwnerReminders` (стр. 1797) — локальный
- `saveServices` (стр. 1807) — локальный
- `saveBoxes` (стр. 1812) — локальный
- `saveSchedule` (стр. 1816) — локальный
- `saveAdminProfile` (стр. 1820) — локальный
- `saved` (стр. 1821) — локальный
- `saveAdminNotificationSettings` (стр. 1825) — локальный
- `saved` (стр. 1826) — локальный
- `saveWorkerProfile` (стр. 1830) — локальный
- `saved` (стр. 1831) — локальный
- `normalized` (стр. 1832) — локальный
- `saveWorkerNotificationSettings` (стр. 1839) — локальный
- `saved` (стр. 1840) — локальный
- `saveOwnerCompany` (стр. 1847) — локальный
- `saved` (стр. 1848) — локальный
- `saveOwnerNotificationSettings` (стр. 1852) — локальный
- `saved` (стр. 1853) — локальный
- `saveOwnerIntegrations` (стр. 1857) — локальный
- `saved` (стр. 1858) — локальный
- `saveOwnerSecurity` (стр. 1862) — локальный
- `saved` (стр. 1863) — локальный
- `saveWorkerSettings` (стр. 1867) — локальный
- `saved` (стр. 1868) — локальный
- `saveAdminWorkerPayroll` (стр. 1872) — локальный
- `saved` (стр. 1873) — локальный
- `normalized` (стр. 1874) — локальный
- `nextWorker` (стр. 1876) — локальный
- `setOwnerMasterRole` (стр. 1881) — локальный
- `saved` (стр. 1882) — локальный
- `normalized` (стр. 1883) — локальный
- `exists` (стр. 1885) — локальный
- `saveContent` (стр. 1892) — локальный
- `saved` (стр. 1893) — локальный
- `createPayrollEntry` (стр. 1897) — локальный
- `checkConsent` (стр. 1902) — локальный
- `response` (стр. 1903) — локальный
- `submitConsent` (стр. 1907) — локальный
- `listShiftChecklists` (стр. 1911) — локальный
- `entries` (стр. 1912) — локальный
- `submitShiftChecklist` (стр. 1916) — локальный
- `entry` (стр. 1917) — локальный
- `listAdminShiftInspections` (стр. 1924) — локальный
- `entries` (стр. 1925) — локальный
- `submitAdminShiftInspection` (стр. 1933) — локальный
- `entry` (стр. 1940) — локальный
- `openShiftForMasters` (стр. 1951) — локальный
- `entry` (стр. 1952) — локальный
- `hireWorker` (стр. 1963) — локальный
- `created` (стр. 1964) — локальный
- `normalized` (стр. 1965) — локальный
- `fireWorker` (стр. 1975) — локальный
- `resetWorkerPassword` (стр. 1980) — локальный
- `changePassword` (стр. 1987) — локальный
- `requestOwnerDatabaseReset` (стр. 1994) — локальный
- `response` (стр. 1995) — локальный
- `approveOwnerDatabaseReset` (стр. 2012) — локальный
- `response` (стр. 2013) — локальный
- `executeOwnerDatabaseReset` (стр. 2029) — локальный
- `response` (стр. 2030) — локальный
- `getTimeSlotsForDate` (стр. 2038) — локальный
- `parsedDate` (стр. 2039) — локальный
- `day` (стр. 2041) — локальный
...
- `encodeMapCache` (стр. 9) — локальный
- `getEncodeMap` (стр. 10) — локальный
- `cached` (стр. 11) — локальный
- `map` (стр. 13) — локальный
- `bytes` (стр. 15) — локальный
- `chars` (стр. 17) — локальный
- `ch` (стр. 19) — локальный
- `ch` (стр. 24) — локальный
- `encodeWith` (стр. 31) — локальный
- `map` (стр. 32) — локальный
- `out` (стр. 33) — локальный
- `b` (стр. 35) — локальный
...
- `backend/app/main.py` (2026-09-14 10:02)
- `backend/app/schemas.py` (2026-09-14 09:58)
- `backend/app/models.py` (2026-09-14 09:58)
- `backend/tests/test_worker_broadcast.py` (2026-09-14 09:53)
- `frontend/src/app/components/owner/OwnerApp.tsx` (2026-09-14 09:53)
- `frontend/src/app/components/owner/screens/OwnerStockPage.tsx` (2026-09-14 09:52)
- `frontend/src/app/components/worker/WorkerApp.tsx` (2026-09-14 09:52)
- `frontend/src/app/context/AppContext.tsx` (2026-09-14 09:51)
- `backend/tests/test_piggy_startup_migration.py` (2026-09-14 09:47)
- `frontend/src/app/components/owner/screens/OwnerClientsScreen.tsx` (2026-09-14 09:46)
- `frontend/src/app/components/owner/DepositPanel.tsx` (2026-09-14 09:46)
- `frontend/src/app/components/owner/screens/OwnerWalletScreen.tsx` (2026-09-14 09:46)
- `frontend/src/app/components/owner/screens/OwnerPiggyBankScreen.tsx` (2026-09-14 09:45)
- `frontend/src/app/components/admin/AdminApp.tsx` (2026-09-14 09:45)
- `frontend/src/app/components/atmosfera/index.ts` (2026-09-14 09:45)
