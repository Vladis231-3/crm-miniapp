...
> Автосгенерировано 2026-08-19 07:39 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
### backend/app/exports.py (3153 строк)
...
- `_render_excel_reportdef _render_excel_report(data: OwnerExportData) -> bytes: workbook = Workbook() summary = workbook.active summary.title = "Сводка" summary.merge_cells("A1:D1") summary["A1"] = data` (стр. 2277)
- `_render_owner_summary_excel_reportdef _render_owner_summary_excel_report(data: OwnerSummaryExportData) -> bytes: workbook = Workbook() summary = workbook.active summary.title = "Сводка" summary.merge_cells("A1:D1")` (стр. 2345)
- `_append_sheetdef _append_sheet(workbook: Workbook, title: str, headers: list[str], rows: list[list[Any]], *, currency_cols: set[int] | None = None) -> None: sheet = workbook.create_sheet(title)` (стр. 2521)
- `_render_pdf_reportdef _render_pdf_report(data: OwnerExportData) -> bytes: buffer = io.BytesIO() font_name = _pdf_font_name() styles = getSampleStyleSheet() title_style = ParagraphStyle("OwnerTitle",` (стр. 2551)
- `_pdf_sectiondef _pdf_section(story: list[Any], section_style: ParagraphStyle, font_name: str, title: str, headers: list[str], rows: list[list[Any]]) -> None: story.append(Paragraph(title, sect` (стр. 2637)
- `_pdf_tabledef _pdf_table(rows: list[list[Any]], font_name: str, header_color: str = "#0E1624") -> LongTable: normalized = [[Paragraph(_escape(str(cell)), _pdf_cell_style(font_name)) for cell` (стр. 2653)
- `_format_rowsdef _format_rows(rows: list[list[Any]], *, currency_cols: set[int]) -> list[list[Any]]: formatted: list[list[Any]] = [] for row in rows: next_row = [] for index, value in enumerate` (стр. 2693)
- `_style_headingdef _style_heading(sheet, *cells: str) -> None: if cells: sheet[cells[0]].font = Font(size=16, bold=True, color="0B1226") for cell_name in cells[1:]: sheet[cell_name].font = Font(s` (стр. 2723)
- `_style_tabledef _style_table(sheet, header_row: int, start_row: int, end_row: int, end_col: int) -> None: header_fill = PatternFill(fill_type="solid", fgColor="0A84FF") header_font = Font(bold` (стр. 2737)
- `_apply_currencydef _apply_currency(cell) -> None: cell.number_format = '#,##0 "руб."' cell.alignment = Alignment(horizontal="right", vertical="center")` (стр. 2779)
- `_autosizedef _autosize(sheet) -> None: for column in sheet.columns: letter = get_column_letter(column[0].column) max_length = 0 for cell in column: max_length = max(max_length, len("" if ce` (стр. 2789)
- `_pdf_font_namedef _pdf_font_name() -> str: candidates = [ str(Path(__file__).resolve().parent / "assets" / "fonts" / "NotoSans-Regular.ttf"), os.getenv("OWNER_EXPORT_FONT_PATH", ""), "C:/Windows` (стр. 2809)
- `_pdf_cell_styledef _pdf_cell_style(font_name: str) -> ParagraphStyle: return ParagraphStyle("OwnerExportCell", fontName=font_name, fontSize=7.5, leading=9, textColor=colors.HexColor("#111827"))` (стр. 2859)
- `_booking_datetimedef _booking_datetime(booking: Booking) -> datetime | None: raw = f"{booking.date} {booking.time}".strip() for fmt in ("%d.%m.%Y %H:%M", "%Y-%m-%d %H:%M"):` (стр. 2867)
- `_booking_sort_keydef _booking_sort_key(booking: Booking) -> tuple[datetime, datetime]: local_now = datetime.now().astimezone() booking_dt = _booking_datetime(booking) primary = _as_local_datetime(b` (стр. 2887)
- `_as_local_datetimedef _as_local_datetime(value: datetime, reference: datetime) -> datetime: target_tz = reference.tzinfo if value.tzinfo is None: return value.replace(tzinfo=target_tz) return value.` (стр. 2903)
- `_parse_date_for_sortdef _parse_date_for_sort(value: str) -> datetime: for fmt in ("%d.%m.%Y", "%Y-%m-%d"):` (стр. 2917)
- `_format_datetimedef _format_datetime(value: datetime | None) -> str: if value is None: return "" return value.astimezone().strftime("%d.%m.%Y %H:%M") if value.tzinfo is not None else value.strftim` (стр. 2935)
- `_format_moneydef _format_money(value: int) -> str: return f"{value:,.0f}".replace(",", " ") + " руб."` (стр. 2947)
- `_escapedef _escape(value: str) -> str: return escape(value).replace("\n", "<br/>")` (стр. 2955)
- `build_deposit_exportdef build_deposit_export( db: Any, client: Client, overview: Any,` (стр. 2968)
- `OwnerSummaryExportData.moneydef money(value: float) -> str: return f"{float(value):,.0f} ₽".replace(",", " ")` (стр. 2983)
- `build_deposit_export_alldef build_deposit_export_all( db: Any, *, date_from: str | None = None, date_to: str | None = None,` (стр. 3067)
...
### backend/app/main.py (21113 строк)

Роуты (120):

```
  `POST /api/auth/client` -> `register_or_login_client` (декоратор: стр. 5545)
  `POST /api/auth/telegram` -> `authenticate_via_telegram` (декоратор: стр. 5634)
  `POST /api/auth/staff/link` -> `link_staff_account` (декоратор: стр. 5656)
  `POST /api/auth/telegram-owner` -> `authenticate_primary_owner_via_telegram` (декоратор: стр. 5690)
  `POST /api/auth/switch-role` -> `switch_role` (декоратор: стр. 5727)
  `GET /api/owner/exports/{kind}` -> `download_owner_export` (декоратор: стр. 8339)
  `POST /api/owner/exports/{kind}/telegram` -> `send_owner_export_to_telegram` (декоратор: стр. 8360)
  `POST /api/owner/reports/{period}/{segment}/telegram` -> `send_owner_summary_report_to_telegram` (декоратор: стр. 8381)
  `PATCH /api/clients/me` -> `update_client_me` (декоратор: стр. 9429)
  `PATCH /api/clients/{client_id}/card` -> `update_client_card` (декоратор: стр. 9485)
  `POST /api/clients` -> `create_client` (декоратор: стр. 9585)
  `GET /api/health` -> `health` (декоратор: стр. 9642)
  `GET /api/content` -> `get_public_content` (декоратор: стр. 9799)
  `PUT /api/content` -> `save_content` (декоратор: стр. 9813)
  `POST /api/upload` -> `upload_file` (декоратор: стр. 9877)
  `GET /api/uploads/{filename}` -> `serve_upload` (декоратор: стр. 9929)
  `POST /api/contact` -> `submit_contact` (декоратор: стр. 9950)
  `POST settings.telegram_webhook_path` -> `handle_telegram_webhook` (декоратор: стр. 10000)
  `POST /api/telegram/webhook/sync` -> `resync_telegram_webhook` (декоратор: стр. 10050)
  `GET /api/stock-categories` -> `list_stock_categories` (декоратор: стр. 10091)
  `POST /api/stock-categories` -> `create_stock_category` (декоратор: стр. 10104)
  `PATCH /api/stock-categories/{category_id}` -> `update_stock_category` (декоратор: стр. 10122)
  `DELETE /api/stock-categories/{category_id}` -> `delete_stock_category` (декоратор: стр. 10141)
  `GET /api/shift-checklists` -> `get_booking_availability` (декоратор: стр. 10162)
  `POST /api/bookings` -> `create_booking` (декоратор: стр. 10239)
  `PATCH /api/bookings/{booking_id}` -> `update_booking` (декоратор: стр. 11454)
  `DELETE /api/bookings/{booking_id}` -> `delete_booking` (декоратор: стр. 12114)
  `POST /api/bookings/{booking_id}/services` -> `add_booking_service` (декоратор: стр. 12214)
  `POST /api/bookings/{booking_id}/additional-services` -> `add_booking_additional_service` (декоратор: стр. 12280)
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 12398)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 12470)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 12554)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 12634)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 12712)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 12778)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 12814)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 12862)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 12911)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 12942)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 12978)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 13020)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 13142)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 13188)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 13270)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 13426)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 13465)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 13591)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 13637)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 13695)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 13743)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 13807)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 13887)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 14388)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 14530)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 14976)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 15020)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 15055)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 15082)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 15108)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 15126)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 15138)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 15156)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 15169)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 15248)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 15333)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 15393)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 15605)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 15701)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 15781)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 15849)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 15984)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 16064)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 16214)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 16356)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 16502)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 16556)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 16632)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 16690)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 16738)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 16812)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 16836)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 16896)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 16938)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 16962)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 16986)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 17021)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 17076)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 17144)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 17162)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 17195)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 17228)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 17240)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 17264)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 17289)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 17325)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 17428)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 17528)
  `GET /api/owner/outsource/payroll` -> `get_owner_outsource_payroll` (декоратор: стр. 17597)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 17667)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 17897)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 17993)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 18218)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 18297)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 18470)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 18772)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 18786)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 19124)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 19541)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 19943)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 20134)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 20386)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 20604)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 20742)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 20802)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 20996)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 21004)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 21016)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 21040)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 21048)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 21056)
```

Классы и функции (228):

- `_resolve_frontend_distdef _resolve_frontend_dist() -> Path: """Каталог собранного React-фронтенда. В обычном режиме — <project>/frontend/dist (родитель каталога app/). В frozen-режиме (PyInstaller bundl` (стр. 545)
- `_check_rate_limitdef _check_rate_limit(ip: str) -> None: global _last_rate_limit_cleanup now = time_module.time() window_start = now - _LOGIN_RATE_LIMIT_WINDOW # Periodic cleanup of stale entries t` (стр. 751)
- `add_security_headersasync def add_security_headers(request: Request, call_next): response = await call_next(request) for key, value in SECURITY_HEADERS.items():` (стр. 844)
- `serve_single_page_appasync def serve_single_page_app(request: Request, call_next): path = request.url.path index_file = frontend_dist / "index.html" if request.method not in {"GET", "HEAD"}: return awa` (стр. 857)
- `on_startupdef on_startup() -> None: global bot_thread Base.metadata.create_all(bind=engine) _apply_runtime_migrations() db = next(get_db()) try: seed_database(db, include_demo_staff=settings` (стр. 905)
- `start_google_sync_threaddef start_google_sync_thread() -> None: """Запустить фоновый поток обратной синхронизации Google Calendar (идемпотентно).""" global google_sync_thread if google_sync_thread is None` (стр. 990)
- `_nowdef _now() -> datetime: return datetime.now(timezone.utc)` (стр. 1003)
- `_local_day_boundsdef _local_day_bounds(date_str: str) -> tuple[datetime, datetime]: """Границы локального дня (DD.MM.YYYY) в UTC: (00:00, 23:59:59) местного времени. Периоды ЗП считаются по локальн` (стр. 1008)
- `_as_utcdef _as_utc(value: datetime) -> datetime: if value.tzinfo is None: return value.replace(tzinfo=timezone.utc) return value.astimezone(timezone.utc)` (стр. 1023)
- `_format_moscow_dtdef _format_moscow_dt(dt: datetime | None) -> str: if dt is None: return "" msk = dt.astimezone(timezone(timedelta(hours=3))) return msk.strftime("%H:%M %d.%m.%Y")` (стр. 1032)
- `_request_ipdef _request_ip(request: Request) -> str: # For rate limiting, prefer direct client IP to prevent X-Forwarded-For spoofing if request.client is not None and request.client.host: re` (стр. 1043)
- `_safe_textdef _safe_text(value: Any) -> str: return value if isinstance(value, str) else ""` (стр. 1063)
- `_client_by_phonedef _client_by_phone(db: Session, phone: str) -> Client | None: if not phone.strip():` (стр. 1071)
- `_owner_querydef _owner_query(): return ( select(StaffUser) .where(StaffUser.role == "owner") .order_by(StaffUser.created_at.asc(), StaffUser.id.asc()) )` (стр. 1109)
- `_primary_ownerdef _primary_owner(db: Session) -> StaffUser | None: return db.scalar( select(StaffUser) .where(StaffUser.role == "owner", StaffUser.is_primary_owner.is_(True)) .order_by(StaffUser` (стр. 1125)
- `_owner_master_conditiondef _owner_master_condition() -> Any: """Владелец, которому дополнительно выдана роль мастера (extra_roles != []). Такие владельцы попадают в списки мастеров: назначение на записи,` (стр. 1141)
- `_is_owner_masterdef _is_owner_master(worker: StaffUser) -> bool: return worker.role == "owner" and bool(worker.extra_roles)` (стр. 1156)
- `_ensure_permanent_telegram_ownersdef _ensure_permanent_telegram_owners(db: Session) -> None: """Upsert explicitly configured owners without reassigning existing rows.""" for staff_id, login, chat_id, owner_name in` (стр. 1163)
- `_ensure_owner_accountsdef _ensure_owner_accounts(db: Session) -> None: owners = db.scalars(_owner_query()).all() primary_owner = next((owner for owner in owners if owner.is_primary_owner), None) if prim` (стр. 1269)
- `_device_labeldef _device_label(user_agent: str) -> str: if "Telegram-Android" in user_agent: return "Telegram Android" if "Telegram-iOS" in user_agent: return "Telegram iPhone" if "iPhone" in u` (стр. 1397)
- `_apply_runtime_migrationsdef _apply_runtime_migrations() -> None: from sqlalchemy import text def boolean_default_sql(value: bool) -> str:` (стр. 1433)
- `boolean_default_sqldef boolean_default_sql(value: bool) -> str: if engine.dialect.name == "postgresql": return "TRUE" if value else "FALSE" return "1" if value else "0"` (стр. 1437)
- `ensure_postgres_varchar_lengthdef ensure_postgres_varchar_length( table_name: str, column_name: str, minimum_length: int` (стр. 1447)
- `ensure_postgres_text_columndef ensure_postgres_text_column(table_name: str, column_name: str) -> None: if engine.dialect.name != "postgresql": return column = next( ( item for item in inspect(engine).get_col` (стр. 1493)
- `_apply_default_shift_paydef _apply_default_shift_pay(db: Session) -> None: """Один раз выставляет оклад за смену DEFAULT_SHIFT_PAY сотрудникам (кроме владельцев), у которых ставка не задана (0). Выполняет` (стр. 2562)
- `_repair_text_valuedef _repair_text_value(value: str) -> str: if not value or not any(ord(char) > 127 for char in value):` (стр. 2592)
- `_repair_nested_textdef _repair_nested_text(value): if isinstance(value, str):` (стр. 2612)
- `_repair_model_text_fieldsdef _repair_model_text_fields(db: Session, model, fields: tuple[str, ...]) -> bool: changed = False for item in db.scalars(select(model)).all():` (стр. 2632)
- `_sanitize_notification_messagedef _sanitize_notification_message(message: str) -> str: fixed = _repair_text_value(message).strip() for source, target in { "вЂў": "•", "в€¢": "•", "вВў": "•", "â€¢": "•", "вЂ”": ` (стр. 2660)
- `_repair_text_datadef _repair_text_data(db: Session) -> None: changed = False changed |= _repair_model_text_fields( db, StaffUser, ("name", "city", "experience", "specialty", "about"), ) changed |= ` (стр. 2694)
- `_settingdef _setting(db: Session, key: str, default: dict) -> dict: row = db.get(AppSetting, key) if row: return row.value row = AppSetting(key=key, value=default) db.add(row) db.flush() r` (стр. 2840)
- `_merge_setting_dictdef _merge_setting_dict(value: Any, default: dict[str, Any]) -> dict[str, Any]: if not isinstance(value, dict):` (стр. 2860)
- `_normalize_client_vehiclesdef _normalize_client_vehicles( vehicles: list[ClientVehiclePayload] | list[dict[str, Any]] | None, *, fallback_car: str = "", fallback_plate: str = "",` (стр. 2884)
- `_client_vehicles_mapdef _client_vehicles_map(db: Session) -> dict[str, Any]: return _setting(db, "client_vehicles", {})` (стр. 2978)
- `_client_vehicles_payloaddef _client_vehicles_payload(db: Session, client: Client) -> list[ClientVehiclePayload]: raw = _client_vehicles_map(db).get(client.id, []) return _normalize_client_vehicles( raw, f` (стр. 2986)
- `_save_client_vehiclesdef _save_client_vehicles( db: Session, client_id: str, vehicles: list[ClientVehiclePayload]` (стр. 3000)
- `_client_phone_verifications_mapdef _client_phone_verifications_map(db: Session) -> dict[str, Any]: value = _setting(db, CLIENT_PHONE_VERIFICATIONS_KEY, {}) return value if isinstance(value, dict) else {}` (стр. 3020)
- `_client_verified_phone_digitsdef _client_verified_phone_digits(db: Session, telegram_id: str | None) -> str | None: if not telegram_id: return None entry = _client_phone_verifications_map(db).get(str(telegram_` (стр. 3030)
- `_client_phone_is_verifieddef _client_phone_is_verified(db: Session, telegram_id: str | None, phone: str) -> bool: if not phone.strip():` (стр. 3050)
- `_require_client_phone_verificationdef _require_client_phone_verification( db: Session, telegram_id: str | None, phone: str` (стр. 3076)
- `_client_payloaddef _client_payload(client: Client | None) -> ClientProfilePayload | None: if client is None: return None with Session(engine) as vehicles_db: vehicles = _client_vehicles_payload(v` (стр. 3098)
- `_client_summary_payloaddef _client_summary_payload( client: Client, db: Session | None = None` (стр. 3138)
- `_booking_status_labeldef _booking_status_label(status_value: str) -> str: return { "new": "Новая заявка", "confirmed": "Подтверждена", "scheduled": "Запланирована", "in_progress": "В работе", "complete` (стр. 3196)
- `_booking_status_short_labeldef _booking_status_short_label(status_value: str) -> str: return { "new": "Новая", "confirmed": "Подтв.", "scheduled": "Запл.", "in_progress": "В работе", "completed": "Завершена"` (стр. 3222)
- `_format_local_datetimedef _format_local_datetime(value: datetime) -> str: return _as_utc(value).astimezone().strftime("%d.%m.%Y %H:%M")` (стр. 3248)
- `_parse_booking_datetimedef _parse_booking_datetime(date_value: str, time_value: str) -> datetime | None: raw = f"{date_value.strip()} {time_value.strip()}" for fmt in ("%d.%m.%Y %H:%M", "%Y-%m-%d %H:%M")` (стр. 3256)
- `_py_weekday_to_schedule_indexdef _py_weekday_to_schedule_index(py_weekday: int) -> int: return (py_weekday + 2) % 7` (стр. 3276)
- `_parse_time_to_minutesdef _parse_time_to_minutes(time_value: str) -> int | None: raw = time_value.strip() if len(raw) != 5 or raw[2] != ":": return None try: hours = int(raw[:2]) minutes = int(raw[3:]) ` (стр. 3284)
- `_today_labeldef _today_label() -> str: return datetime.now().strftime("%d.%m.%Y")` (стр. 3312)
- `_build_schedule_slotsdef _build_schedule_slots( open_minutes: int, close_minutes: int, step_minutes: int = 30` (стр. 3320)
- `_booking_requires_scheduled_slotdef _booking_requires_scheduled_slot(status_value: str) -> bool: return status_value in BOOKING_ACTIVE_STATUSES` (стр. 3344)
- `_booking_slot_fields_changeddef _booking_slot_fields_changed(booking: Booking, updates: dict) -> bool: if "date" in updates and (updates.get("date") or "").strip() != (booking.date or "").strip():` (стр. 3352)
- `_booking_time_rangedef _booking_time_range( date_value: str, time_value: str, duration: int` (стр. 3372)
- `_time_ranges_overlapdef _time_ranges_overlap( start_at: datetime, end_at: datetime, other_start_at: datetime, other_end_at: datetime,` (стр. 3390)
- `_ensure_booking_datetime_not_in_pastdef _ensure_booking_datetime_not_in_past(date_value: str, time_value: str, role: str) -> None: if role in {"admin", "owner"}: return scheduled_at = _parse_booking_datetime(date_val` (стр. 3408)
- `_ensure_booking_within_scheduledef _ensure_booking_within_schedule( db: Session, date_value: str, time_value: str, duration: int` (стр. 3442)
- `_box_is_availabledef _box_is_available( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str,` (стр. 3516)
- `_pick_available_boxdef _pick_available_box( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, resource_group: str | None = None, preferred_box: str | None = Non` (стр. 3540)
- `_booking_slot_availabilitydef _booking_slot_availability( db: Session, *, date_value: str, duration: int, service_id: str | None = None, resource_group: str | None = None,` (стр. 3602)
- `_ensure_booking_has_no_conflictsdef _ensure_booking_has_no_conflicts( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str, worker_ids: set[str], …` (стр. 3754)
- `_load_penaltiesdef _load_penalties( db: Session, *, worker_ids: set[str] | None = None` (стр. 3798)
- `_complaints_by_workerdef _complaints_by_worker(penalties: list[Penalty]) -> dict[str, list[Penalty]]: grouped: dict[str, list[Penalty]] = {} for penalty in penalties: grouped.setdefault(penalty.worker_` (стр. 3824)
- `_normalize_worker_rulesdef _normalize_worker_rules(db: Session) -> None: changed = False workers = db.scalars(select(StaffUser).where(StaffUser.role == "worker")).all() for worker in workers: capped_perc` (стр. 3838)
- `_worker_payloaddef _worker_payload(worker: StaffUser) -> WorkerPayload: return WorkerPayload( id=worker.id, role=worker.role, # type: ignore[arg-type] name=worker.name, experience=worker.experien` (стр. 3892)
- `_payroll_entry_payloaddef _payroll_entry_payload(entry: PayrollEntry, actor_name: str) -> PayrollEntryPayload: return PayrollEntryPayload( id=entry.id, workerId=entry.worker_id, kind=entry.kind, # type:` (стр. 3932)
- `_worker_payroll_summariesdef _worker_payroll_summaries( db: Session, workers: list[StaffUser], complaints_by_worker: dict[str, list[Penalty]],` (стр. 3960)
- `_worker_payroll_summaries_from_datadef _worker_payroll_summaries_from_data( db: Session, workers: list[StaffUser], completed_bookings: list[Booking], entries: list[PayrollEntry], complaints_by_worker: dict[str, list` (стр. 4008)
- `_worker_payload_with_payrolldef _worker_payload_with_payroll( worker: StaffUser, payroll_summaries: dict[str, WorkerPayrollSummaryPayload] | None = None,` (стр. 4182)
- `_booking_payloaddef _booking_payload( booking: Booking, complaints_by_worker: dict[str, list[Penalty]] | None = None` (стр. 4206)
- `_notification_payloaddef _notification_payload(notification: Notification) -> NotificationPayload: return NotificationPayload( id=notification.id, recipientRole=notification.recipient_role, # type: ign` (стр. 4374)
- `_stock_payloaddef _stock_payload(item: StockItem) -> StockItemPayload: return StockItemPayload( id=item.id, name=item.name, qty=item.qty, unit=item.unit, unitPrice=item.unit_price, category=item` (стр. 4396)
- `_expense_payloaddef _expense_payload(expense: Expense) -> ExpensePayload: return ExpensePayload( id=expense.id, title=expense.title, amount=expense.amount, category=expense.category, date=expense.` (стр. 4412)
- `_penalty_payloaddef _penalty_payload(penalty: Penalty) -> PenaltyPayload: worker_name = penalty.worker.name if penalty.worker else "" return PenaltyPayload( id=penalty.id, workerId=penalty.worker_` (стр. 4436)
- `_service_payloaddef _service_payload(service: Service) -> ServicePayload: return ServicePayload( id=service.id, name=service.name, category=service.category, price=service.price, duration=service.` (стр. 4466)
- `_box_payloaddef _box_payload(box: Box) -> BoxPayload: return BoxPayload( id=box.id, name=box.name, resourceGroup=(box.resource_group or DEFAULT_RESOURCE_GROUP).strip() or DEFAULT_RESOURCE_GROU` (стр. 4510)
- `_visible_boxesdef _visible_boxes(db: Session) -> list[Box]: boxes = db.scalars(select(Box).order_by(Box.name.asc())).all() wash_order_map = {name: index for index, name in enumerate(WASH_BOX_NAM` (стр. 4534)
- `box_orderdef box_order(box: Box) -> tuple[int, int, str, str]: resource_group = _resource_group_key( box.resource_group or _default_box_resource_group(box) ) if resource_group == DETAILING_` (стр. 4544)
- `_schedule_payloaddef _schedule_payload(entry: ScheduleEntry) -> SchedulePayload: return SchedulePayload( dayIndex=entry.day_index, day=entry.day_label, open=entry.open_time, close=entry.close_time,` (стр. 4580)
- `_settings_payloaddef _settings_payload(db: Session) -> SettingsBundlePayload: admin_profile_default = { "name": "Администратор", "email": "", "phone": "", "telegramChatId": "", } admin_notification` (стр. 4600)
- `_empty_settings_payloaddef _empty_settings_payload() -> SettingsBundlePayload: return SettingsBundlePayload( adminProfile=AdminProfilePayload( name="", email="", phone="", telegramChatId="" ), adminNotif` (стр. 4820)
- `_scoped_settings_payloaddef _scoped_settings_payload( db: Session, role: str, actor_id: str` (стр. 4900)
- `_session_payloaddef _session_payload(session_data: dict) -> SessionPayload: return SessionPayload( role=session_data["role"], actorId=session_data["actorId"], sessionId=session_data.get("sessionId` (стр. 4986)
- `_mark_overdue_bookings_for_admin_reviewdef _mark_overdue_bookings_for_admin_review(db: Session) -> None: now_local = datetime.now().replace(second=0, microsecond=0) changed = False for booking in db.scalars( select(Book` (стр. 5006)
- `_build_bootstrapdef _build_bootstrap(db: Session, session_data: dict) -> BootstrapPayload: role = session_data["role"] actor_id = session_data["actorId"] _mark_overdue_bookings_for_admin_review(db` (стр. 5050)
- `_resolve_user_from_init_datadef _resolve_user_from_init_data(authorization: str, db: Session) -> dict | None: try: validated = validate_telegram_init_data( authorization, settings.telegram_bot_token, max_age_` (стр. 5372)
- `_require_sessiondef _require_session( authorization: str | None = Header(default=None), db: Session = Depends(get_db),` (стр. 5479)
- `_extract_telegram_id_from_init_datadef _extract_telegram_id_from_init_data(authorization: str) -> str: if not authorization: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing initData") t` (стр. 5510)
- `_ensure_staff_roledef _ensure_staff_role(session_data: dict, allowed: set[str]) -> None: if session_data["role"] not in allowed: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Fo` (стр. 5760)
- `_validated_booking_workersdef _validated_booking_workers( db: Session, workers: list[BookingWorkerPayload]` (стр. 5770)
- `_booking_payload_for_responsedef _booking_payload_for_response(db: Session, booking: Booking) -> BookingPayload: worker_ids = {link.worker_id for link in booking.worker_links} penalties = _load_penalties(db, w` (стр. 5866)
- `_sync_booking_workersdef _sync_booking_workers( db: Session, booking: Booking, workers: list[BookingWorkerPayload]` (стр. 5878)
- `_sync_booking_materialsdef _sync_booking_materials( db: Session, booking: Booking, materials: list[BookingMaterialPayload]` (стр. 5908)
- `_send_telegram_safedef _send_telegram_safe(chat_id: str | None, text: str) -> None: if not chat_id: logger.warning("Пропущена отправка Telegram-уведомления: у получателя нет chat_id") return try: sen` (стр. 5927)
- `_telegram_display_namedef _telegram_display_name(telegram_user: dict, fallback: str) -> str: first_name = str(telegram_user.get("first_name") or "").strip() last_name = str(telegram_user.get("last_name"` (стр. 5947)
- `_owner_two_factor_recipientdef _owner_two_factor_recipient(db: Session) -> StaffUser: owner = _primary_owner(db) if owner is None: raise HTTPException( status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail` (стр. 5963)
- `_all_active_ownersdef _all_active_owners(db: Session) -> list[StaffUser]: """Возвращает всех активных владельцев, отсортированных по created_at asc.""" return list( db.scalars( select(StaffUser) .wh` (стр. 5993)
- `_all_owner_telegram_recipientsdef _all_owner_telegram_recipients(db: Session) -> list[StaffUser]: """Возвращает всех владельцев с непустым telegram_chat_id, отсортированных по created_at asc.""" return list( db` (стр. 6015)
- `_booking_reminder_target_datedef _booking_reminder_target_date(days_ahead: int = 1) -> str: return (datetime.now() + timedelta(days=days_ahead)).strftime("%d.%m.%Y")` (стр. 6043)
- `_worker_notification_settings_mapdef _worker_notification_settings_map(db: Session) -> dict[str, dict[str, Any]]: return _setting(db, "worker_notification_settings", {})` (стр. 6051)
- `_booking_reminder_statedef _booking_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, BOOKING_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6059)
- `_return_reminder_statedef _return_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, RETURN_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6067)
- `_shift_checklists_statedef _shift_checklists_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, SHIFT_CHECKLISTS_KEY, []) return value if isinstance(value, list) else []` (стр. 6075)
- `_admin_shift_inspections_statedef _admin_shift_inspections_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, ADMIN_SHIFT_INSPECTIONS_KEY, []) return value if isinstance(value, list) else []` (стр. 6085)
- `_compute_shift_attendancedef _compute_shift_attendance( inspections: list[dict], worker_id: str, date_from: date, date_to: date,` (стр. 6095)
- `_period_to_date_rangedef _period_to_date_range(period: str) -> tuple[date, date]: """ Преобразует строковый период в диапазон дат (date_from, date_to). - ``week`` → последние 7 дней - ``month`` → после` (стр. 6223)
- `_admin_shift_owner_bot_statedef _admin_shift_owner_bot_state(db: Session) -> dict[str, Any]: value = _setting(db, ADMIN_SHIFT_OWNER_BOT_STATE_KEY, {"pendingIssueByChat": {}}) return value if isinstance(value,` (стр. 6269)
- `_cleanup_booking_reminder_deliveriesdef _cleanup_booking_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=14) cleaned: dict[str, str] = {} for key, value in deliv` (стр. 6279)
- `_cleanup_return_reminder_deliveriesdef _cleanup_return_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=30) cleaned: dict[str, str] = {} for key, value in delive` (стр. 6299)
...
- `_booking_worker_reminder_messagedef _booking_worker_reminder_message(booking: Booking, worker_name: str) -> str: return ( f"Напоминание мастеру {worker_name}\n" f"Клиент: {booking.client_name}\n" f"Услуга: {booki` (стр. 6339)
- `_dispatch_booking_remindersdef _dispatch_booking_reminders( db: Session, *, target_date: str | None = None, force: bool = False,` (стр. 6359)
- `_dispatch_return_visit_remindersdef _dispatch_return_visit_reminders(db: Session) -> int: reminder_state = _return_reminder_state(db) deliveries = reminder_state.get("deliveries") if not isinstance(deliveries, di` (стр. 6621)
- `_shift_checklist_payloaddef _shift_checklist_payload(entry: dict[str, Any]) -> ShiftChecklistPayload: return ShiftChecklistPayload( id=str(entry.get("id") or ""), workerId=str(entry.get("workerId") or "")` (стр. 6731)
- `_chemistry_stock_itemsdef _chemistry_stock_items(db: Session) -> list[StockItem]: return db.scalars( select(StockItem) .where(StockItem.category == "Химия") .order_by(StockItem.name.asc()) ).all()` (стр. 6785)
- `_latest_shift_checklist_entrydef _latest_shift_checklist_entry( entries: list[dict[str, Any]], worker_id: str, phase: str` (стр. 6801)
- `_clean_data_url_prefixdef _clean_data_url_prefix(data_url: str) -> str: return data_url.split(",", 1)[1] if "," in data_url else data_url` (стр. 6823)
- `_decode_data_url_imagedef _decode_data_url_image(data_url: str) -> tuple[str, bytes]: raw = data_url.strip() if not raw.startswith("data:image/"):` (стр. 6831)
- `_admin_shift_inspection_suppliesdef _admin_shift_inspection_supplies(db: Session) -> list[dict[str, Any]]: items = db.scalars( select(StockItem) .where(StockItem.category.in_(("Химия", "Расходники"))) .order_by(S` (стр. 6897)
- `_admin_shift_inspection_payloaddef _admin_shift_inspection_payload( entry: dict[str, Any],` (стр. 6955)
- `_admin_shift_captiondef _admin_shift_caption(entry: dict[str, Any]) -> str: checked_supplies = [ item.get("name") for item in entry.get("supplies", []) if isinstance(item, dict) and item.get("checked"` (стр. 7044)
- `_admin_shift_owner_inline_keyboarddef _admin_shift_owner_inline_keyboard(inspection_id: str) -> dict[str, Any]: return { "inline_keyboard": [ [ { "text": "Подтвердить", "callback_data": f"shiftapprove:{inspection_i` (стр. 7096)
- `_notify_owner_about_admin_shiftdef _notify_owner_about_admin_shift(db: Session, entry: dict[str, Any]) -> None: caption = _admin_shift_caption(entry) mime_type, photo_bytes = _decode_data_url_image( str(entry.ge` (стр. 7124)
- `_apply_admin_shift_reviewdef _apply_admin_shift_review( db: Session, inspection_id: str, *, action: str, issue_note: str, owner_actor_id: str,` (стр. 7194)
- `_serialize_state_datetimedef _serialize_state_datetime(value: datetime | None) -> str | None: if value is None: return None return _as_utc(value).isoformat()` (стр. 7314)
- `_parse_state_datetimedef _parse_state_datetime(value: Any) -> datetime | None: if not value: return None if not isinstance(value, str):` (стр. 7326)
- `_owner_database_reset_statedef _owner_database_reset_state(db: Session) -> dict[str, Any] | None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is None or not isinstance(row.value, dict):` (стр. 7348)
- `_save_owner_database_reset_statedef _save_owner_database_reset_state( db: Session, value: dict[str, Any]` (стр. 7362)
- `_clear_owner_database_reset_statedef _clear_owner_database_reset_state(db: Session) -> None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is not None: db.delete(row) db.flush()` (стр. 7374)
- `_normalize_database_reset_phrasedef _normalize_database_reset_phrase(value: str) -> str: normalized = " ".join(value.replace("\n", " ").split()).strip().upper() return normalized.replace("Ё", "Е")` (стр. 7388)
- `_owner_database_reset_previewdef _owner_database_reset_preview( db: Session,` (стр. 7398)
- `_owner_database_reset_warningsdef _owner_database_reset_warnings( preview: OwnerDatabaseResetPreviewPayload,` (стр. 7452)
- `_perform_owner_database_resetdef _perform_owner_database_reset(db: Session) -> None: db.execute(sa_delete(TelegramLinkCode)) db.execute(sa_delete(Notification)) db.execute(sa_delete(BookingWorker)) db.execute(` (стр. 7492)
- `_parse_datedef _parse_date(s: str) -> date | None: if "." in s: parts = s.split(".") try: return date(int(parts[2]), int(parts[1]), int(parts[0])) except (ValueError, IndexError):` (стр. 7562)
- `_owner_export_filedef _owner_export_file( db: Session, actor_id: str, kind: str, segment: str = "all", date_from: str | None = None, date_to: str | None = None,` (стр. 7588)
- `_in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_to an` (стр. 7732)
- `_download_responsedef _download_response(export_file: GeneratedExport) -> Response: return Response( content=export_file.content, media_type=export_file.media_type, headers={ "Content-Disposition": ` (стр. 7812)
- `class _PartialBroadcastError(Exception):` (стр. 7832)
- `_PartialBroadcastError.__init__def __init__(self, payload: TelegramBroadcastPayload) -> None: super().__init__("partial broadcast failure") self.payload = payload` (стр. 7838)
- `_send_export_to_telegramdef _send_export_to_telegram( db: Session, actor_id: str, export_file: GeneratedExport` (стр. 7848)
- `_owner_summary_reportdef _owner_summary_report( db: Session, actor_id: str, period: str, segment: str` (стр. 7968)
- `_owner_summary_export_filedef _owner_summary_export_file( db: Session, actor_id: str, period: str, segment: str` (стр. 8088)
- `_send_owner_summary_reportdef _send_owner_summary_report( db: Session, actor_id: str, report: OwnerSummaryReport, export_file: GeneratedExport,` (стр. 8206)
- `_booking_car_labeldef _booking_car_label(car: str | None, plate: str | None) -> str: car_value = (car or "").strip() or "Авто не указано" plate_value = (plate or "").strip() return f"{car_value}, {p` (стр. 8397)
- `_admin_booking_notification_titledef _admin_booking_notification_title( client_name: str, car: str | None, plate: str | None` (стр. 8409)
- `_booking_datetime_labeldef _booking_datetime_label(date: str | None, time: str | None) -> str: if not (date or "").strip():` (стр. 8421)
- `_admin_booking_notification_textdef _admin_booking_notification_text( client_name: str, car: str | None, plate: str | None, date: str | None, time: str | None,` (стр. 8437)
- `_notify_admins_about_bookingdef _notify_admins_about_booking(db: Session, booking: Booking) -> None: admins = db.scalars( select(StaffUser).where(StaffUser.role == "admin", StaffUser.active.is_(True)) ).all()` (стр. 8457)
...
- `_service_category_keydef _service_category_key(value: str | None) -> str: return (value or "").strip().lower()` (стр. 8517)
- `_resource_group_keydef _resource_group_key(value: str | None) -> str: return (value or "").strip().lower() or DEFAULT_RESOURCE_GROUP` (стр. 8525)
- `_normalized_textdef _normalized_text(value: str | None) -> str: return (value or "").strip()` (стр. 8533)
- `_default_service_resource_groupdef _default_service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_for_service_category(service.category)` (стр. 8541)
- `_default_box_resource_groupdef _default_box_resource_group(box: Box | None) -> str: if box is None: return DEFAULT_RESOURCE_GROUP name_key = (box.name or "").strip().lower() description_key = (box.descriptio` (стр. 8553)
- `_service_resource_groupdef _service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_key( service.resource_group or _default_servic` (стр. 8573)
- `_compatible_box_namesdef _compatible_box_names(db: Session, resource_group: str | None) -> list[str]: target_group = _resource_group_key(resource_group) return [ box.name for box in db.scalars( select(` (стр. 8589)
- `_is_box_rental_servicedef _is_box_rental_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "аренда бокса" )` (стр. 8615)
- `_is_detailing_servicedef _is_detailing_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "детейлинг" )` (стр. 8629)
- `_resource_group_for_service_categorydef _resource_group_for_service_category(category: str | None) -> str: category_key = _service_category_key(category) if category_key == "детейлинг": return DETAILING_RESOURCE_GROU` (стр. 8641)
- `_box_by_namedef _box_by_name(db: Session, box_name: str) -> Box | None: return db.scalar(select(Box).where(Box.name == box_name))` (стр. 8655)
- `_normalize_service_and_box_resourcesdef _normalize_service_and_box_resources(db: Session) -> None: changed = False # Группа ресурсов услуг больше не привязывается к категории принудительно. boxes = db.scalars(select(` (стр. 8663)
- `_box_hourly_pricedef _box_hourly_price(db: Session, box_name: str, fallback_price: int) -> int: box = _box_by_name(db, box_name) if box is not None and box.price_per_hour > 0: return box.price_per_` (стр. 8871)
- `_payment_type_labeldef _payment_type_label(payment_type: str) -> str: return { "cash": "Наличные", "transfer": "Перевод", "invoice": "По счёту", "credit": "В долг (депозит)", }.get(payment_type, paym` (стр. 8885)
- `_booking_payment_labeldef _booking_payment_label(booking: Booking) -> str: if not booking.payment_settled: return "Не оплачено" return _payment_type_label(booking.payment_type)` (стр. 8903)
- `_notify_ownersdef _notify_owners(db: Session, text: str) -> None: db.add( Notification( id=f"n-{uuid4()}", recipient_role="owner", recipient_id=None, message=text, read=False, created_at=_now(),` (стр. 8915)
...
- `_notify_booking_completion_receiptdef _notify_booking_completion_receipt( db: Session, booking: Booking, *, worker_name: str | None = None` (стр. 8981)
- `_notify_owner_about_worker_booking_eventdef _notify_owner_about_worker_booking_event( db: Session, booking: Booking, *, worker_name: str, event_label: str` (стр. 9053)
- `_notify_workers_about_assignmentdef _notify_workers_about_assignment( db: Session, booking: Booking, worker_ids: set[str]` (стр. 9094)
- `_notify_workers_about_notedef _notify_workers_about_note( db: Session, booking: Booking, worker_ids: set[str]` (стр. 9180)
- `_notify_workers_about_rescheduledef _notify_workers_about_reschedule( db: Session, booking: Booking, worker_ids: set[str], previous_date: str, previous_time: str, previous_box: str,` (стр. 9250)
- `_payroll_entry_labeldef _payroll_entry_label(kind: str) -> str: return { "bonus": "премия", "advance": "аванс", "deduction": "удержание", "payout": "выплата", "adjustment": "корректировка", }.get(kind` (стр. 9338)
- `_notify_worker_about_payroll_entrydef _notify_worker_about_payroll_entry( db: Session, worker: StaffUser, *, actor_role: str, actor_id: str, kind: str, amount: int, note: str, …` (стр. 9358)
- `_default_contentdef _default_content() -> ContentPayload: return ContentPayload( hero=ContentHeroPayload(), about=ContentAboutPayload( text=( "<b>\u2728 \u041e \u0441\u0442\u0443\u0434\u0438\u0438` (стр. 9658)
- `_normalize_legacy_contentdef _normalize_legacy_content(value: dict) -> dict: """Мигрирует старый формат контента (hero.title строкой + hero.titleHighlight) в новый (hero.title => {before, highlight, after}` (стр. 9738)
- `_get_or_create_contentdef _get_or_create_content(db: Session) -> ContentPayload: row = db.get(AppSetting, "content") if row is None or not isinstance(row.value, dict):` (стр. 9759)
- `_detected_image_formatdef _detected_image_format(header: bytes) -> str | None: if header.startswith(b"\xff\xd8\xff"):` (стр. 9857)
- `_upload_headersdef _upload_headers(filename: str) -> dict[str, str]: return { "Cache-Control": "public, max-age=31536000, immutable", "Content-Disposition": f'inline; filename="{filename}"', "X-C` (стр. 9869)
- `_google_sync_bookingdef _google_sync_booking(db, booking, *, action="upsert") -> None: """Best-effort синхронизация записи с Google Calendar. No-op, если интеграция не настроена или токены не привязан` (стр. 10208)
- `_google_sync_loopdef _google_sync_loop() -> None: """Фоновый цикл обратной синхронизации «Google Calendar -> CRM». Запускается daemon-потоком при старте приложения. No-op, если интеграция не настро` (стр. 10220)
- `_write_off_booking_materialsdef _write_off_booking_materials(db: Session, booking: Booking) -> None: if booking.materials_written_off: print(f"[WRITE_OFF] skip booking {booking.id[:8]} — already written off")` (стр. 10742)
- `_booking_materials_costdef _booking_materials_cost(db: Session, booking: Booking) -> int: """Фактическая стоимость материалов по записи; fallback — материалы услуги со склада.""" override = (booking.mone` (стр. 10829)
- `_booking_materials_cost_actualdef _booking_materials_cost_actual(db: Session, booking: Booking) -> int: """Фактическая стоимость материалов по записи; fallback — материалы услуги со склада.""" materials_cost = ` (стр. 10837)
- `_asvc_paid_amountdef _asvc_paid_amount(asvc: BookingAdditionalService) -> int: """Сколько уходит с доп. услуги: аутсорсеру или мастерам (фикс/процент).""" if asvc.is_outsource: return int(asvc.outs` (стр. 10856)
- `_booking_money_splitdef _booking_money_split( db: Session, booking: Booking, complaints_by_worker: dict[str, list] | None = None,` (стр. 10869)
- `_PartialBroadcastError._compute_masterdef _compute_master(base: int) -> tuple[dict[str, int], int, int]: """Доля мастеров: явные суммы (override/fixed) + сервисный режим/проценты профиля от base. Возвращает (master_by_` (стр. 10909)
- `_PartialBroadcastError._compute_piggydef _compute_piggy(base: int) -> int: if piggy_pay_type == "fixed": return piggy_pay_value if piggy_pay_type == "percent": return round(base * piggy_pay_value / 100) if piggy_pay_t` (стр. 11043)
- `_PartialBroadcastError._allocate_ownersdef _allocate_owners(claimed: int, limit: int) -> tuple[int, dict[str, int]]: owner_by_owner: dict[str, int] = {} if claimed <= 0 or not owner_split_enabled: return 0, owner_by_own` (стр. 11054)
- `_process_piggy_bank_for_bookingdef _process_piggy_bank_for_booking(db: Session, booking: Booking) -> None: """Auto-deposit 24% into piggy bank for detailing bookings and repay material withdrawals for any servic` (стр. 11153)
- `_process_owner_profit_sharedef _process_owner_profit_share(db: Session, booking: Booking) -> None: """Расчёт доли владельцев: цена → материалы → мастера → копилка → остаток владельцам (50/50).""" if booking.` (стр. 11382)
- `_PartialBroadcastError._parse_date_strdef _parse_date_str(s: str) -> date | None: try: if "." in s: parts = s.split(".") return date(int(parts[2]), int(parts[1]), int(parts[0])) return date.fromisoformat(s) except (Val` (стр. 13909)
- `_PartialBroadcastError._in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date_str(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_t` (стр. 13933)
- `_deposit_balancedef _deposit_balance(db: Session, client_id: str) -> Decimal: return sum( ( t.amount for t in db.scalars( select(DepositTransaction).where(DepositTransaction.client_id == client_id` (стр. 14648)
- `_deposit_add_transactiondef _deposit_add_transaction( db: Session, client_id: str, txn_type: str, amount: float, description: str, *, date: str, booking_id: str | None = None, created_by_id: str | None = ` (стр. 14660)
- `_deposit_txn_payloaddef _deposit_txn_payload(db: Session, txn: DepositTransaction) -> DepositTransactionPayload: car, plate = "", "" if txn.booking_id: booking = db.get(Booking, txn.booking_id) if boo` (стр. 14689)
- `_deposit_month_labeldef _deposit_month_label() -> str: return datetime.now().strftime("%m.%Y")` (стр. 14712)
- `_deposit_month_ofdef _deposit_month_of(date_str: str) -> str: parts = (date_str or "").split(".") if len(parts) == 3: return f"{parts[1]}.{parts[2]}" return ""` (стр. 14714)
- `_deposit_month_wash_total_fordef _deposit_month_wash_total_for(db: Session, client_id: str, month: str) -> float: total = 0.0 rows = db.scalars( select(Booking).where( Booking.client_id == client_id, Booking.p` (стр. 14721)
- `_deposit_plan_keydef _deposit_plan_key(value: str | None) -> str: plan = (value or "").strip() or "fee" return plan if plan in {"fee", "washes", "per_wash", "unlimited"} else "fee"` (стр. 14737)
- `_deposit_prev_monthdef _deposit_prev_month(month: str) -> str: try: m, y = month.split(".") yi = int(y) - (1 if int(m) == 1 else 0) mi = 12 if int(m) == 1 else int(m) - 1 return f"{mi:02d}.{yi}" exce` (стр. 14742)
- `_deposit_month_wash_count_fordef _deposit_month_wash_count_for(db: Session, client_id: str, month: str) -> int: rows = db.scalars( select(Booking).where( Booking.client_id == client_id, Booking.payment_type ==` (стр. 14752)
- ...ещё 28

### backend/app/models.py (600 строк)
...
- `class TelegramLinkCode(Base):` (стр. 445)
- `class AppSetting(Base):` (стр. 460)
- `class UploadedFile(Base):` (стр. 467)
- `class DataConsent(Base):` (стр. 477)
- `class Income(Base):` (стр. 485)
- `class WeeklyArchive(Base):` (стр. 504)
- `class PiggyBankTransaction(Base):` (стр. 522)
- `class DepositTransaction(Base):` (стр. 545)
- `class DepositMonth(Base):` (стр. 564)
- `class OwnerProfitShare(Base):` (стр. 582)

### backend/app/schemas.py (2107 строк)

Классы и функции (205):
...
- `class SalaryPayoutItem(BaseModel):` (стр. 357)
- `class SalaryDetailResponse(BaseModel):` (стр. 365)
- `class PaySalaryRequest(BaseModel):` (стр. 383)
- `class PaySalaryResponse(BaseModel):` (стр. 392)
- `class BookingWorkerPayload(BaseModel):` (стр. 399)
- `class BookingServiceItem(BaseModel):` (стр. 407)
- `class AdditionalServiceWorkerPayload(BaseModel):` (стр. 414)
- `class AdditionalServicePayload(BaseModel):` (стр. 422)
- `class AddAdditionalServiceRequest(BaseModel):` (стр. 436)
- `class UpdateAdditionalServiceRequest(BaseModel):` (стр. 447)
- `class BookingPayload(BaseModel):` (стр. 457)
- `class WorkerCalendarBookingPayload(BaseModel):` (стр. 487)
- `class BookingAvailabilitySlotPayload(BaseModel):` (стр. 503)
- `class BookingAvailabilityPayload(BaseModel):` (стр. 510)
- `class NotificationPayload(BaseModel):` (стр. 516)
- `class StockCategoryPayload(BaseModel):` (стр. 525)
- `class StockItemPayload(BaseModel):` (стр. 531)
- `class BookingMaterialPayload(BaseModel):` (стр. 541)
- `class ShiftChecklistItemPayload(BaseModel):` (стр. 550)
- `class ShiftChecklistPayload(BaseModel):` (стр. 559)
- `class ShiftChecklistSubmitItem(BaseModel):` (стр. 569)
- `class ShiftChecklistSubmitRequest(BaseModel):` (стр. 574)
- `class AdminShiftInspectionSupplyPayload(BaseModel):` (стр. 580)
- `class AdminShiftInspectionMasterPayload(BaseModel):` (стр. 589)
- `class AdminShiftInspectionPayload(BaseModel):` (стр. 595)
- `class AdminShiftInspectionSubmitSupply(BaseModel):` (стр. 612)
- `class AdminShiftInspectionSubmitMaster(BaseModel):` (стр. 617)
- `class AdminShiftInspectionSubmitRequest(BaseModel):` (стр. 622)
- `class AdminShiftInspectionReviewRequest(BaseModel):` (стр. 630)
- `class OwnerShiftOpeningRequest(BaseModel):` (стр. 635)
- `class ExpensePayload(BaseModel):` (стр. 640)
- `class PenaltyPayload(BaseModel):` (стр. 650)
- `class TelegramLinkCodePayload(BaseModel):` (стр. 662)
- `class ServicePayload(BaseModel):` (стр. 668)
- `class DetailingRequestCreateRequest(BaseModel):` (стр. 692)
- `DetailingRequestCreateRequest.validate_cardef validate_car(cls, value: str | None) -> str | None: if value is None: return None return normalize_vehicle_name(value)` (стр. 701)
- `DetailingRequestCreateRequest.validate_plate_fielddef validate_plate_field(self) -> "DetailingRequestCreateRequest": if self.plate is not None: if not self.plate.strip():` (стр. 707)
- `class BoxPayload(BaseModel):` (стр. 716)
- `class SchedulePayload(BaseModel):` (стр. 725)
- `class AdminNotificationSettings(BaseModel):` (стр. 733)
- `class AdminProfilePayload(BaseModel):` (стр. 741)
- `class WorkerNotificationSettings(BaseModel):` (стр. 748)
- `class WorkerProfilePayload(BaseModel):` (стр. 756)
- `class OperatingMode(str, Enum):` (стр. 767)
- `class OwnerCompanyPayload(BaseModel):` (стр. 772)
- `class OwnerNotificationSettings(BaseModel):` (стр. 782)
- `class OwnerIntegrationsPayload(BaseModel):` (стр. 792)
- `class GoogleCredentialsPayload(BaseModel):` (стр. 799)
- `class OwnerSecurityPayload(BaseModel):` (стр. 807)
- `class AuthSessionPayload(BaseModel):` (стр. 811)
- `class EmployeeSettingPayload(BaseModel):` (стр. 820)
- `class WorkerCreateRequest(BaseModel):` (стр. 831)
- `class PayrollEntryCreateRequest(BaseModel):` (стр. 843)
- `PayrollEntryCreateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 856)
- `class PayrollEntryUpdateRequest(BaseModel):` (стр. 860)
- `PayrollEntryUpdateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 866)
- `class SettingsBundlePayload(BaseModel):` (стр. 870)
- `class SessionPayload(BaseModel):` (стр. 880)
- `class BootstrapPayload(BaseModel):` (стр. 888)
- `class ClientRegisterRequest(BaseModel):` (стр. 906)
- `ClientRegisterRequest.lift_profiledef lift_profile(cls, raw: Any) -> Any: if isinstance(raw, dict) and isinstance(raw.get("profile"), dict):` (стр. 918)
- `ClientRegisterRequest.validate_namedef validate_name(cls, value: str) -> str: if not value.strip():` (стр. 927)
- `ClientRegisterRequest.validate_phonedef validate_phone(cls, value: str) -> str: if not value.strip():` (стр. 934)
- `ClientRegisterRequest.validate_vehicledef validate_vehicle(self) -> "ClientRegisterRequest": if self.plate.strip():` (стр. 940)
- `class ConsentRecordPayload(BaseModel):` (стр. 946)
- `class ConsentCheckResponse(BaseModel):` (стр. 951)
- `class StaffLinkRequest(BaseModel):` (стр. 955)
- `class SwitchRoleRequest(BaseModel):` (стр. 960)
- `class BookingCreateRequest(BaseModel):` (стр. 964)
- `BookingCreateRequest.validate_client_namedef validate_client_name(cls, value: str) -> str: if not value.strip():` (стр. 989)
- `BookingCreateRequest.validate_client_phonedef validate_client_phone(cls, value: str) -> str: if not value.strip():` (стр. 996)
- `BookingCreateRequest.validate_vehicledef validate_vehicle(self) -> "BookingCreateRequest": if self.car is not None and self.car.strip():` (стр. 1002)
- `class AddBookingServiceRequest(BaseModel):` (стр. 1010)
- `class BookingUpdateRequest(BaseModel):` (стр. 1017)
- `BookingUpdateRequest.validate_client_namedef validate_client_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1041)
- `BookingUpdateRequest.validate_client_phonedef validate_client_phone(cls, value: str | None) -> str | None: if value is None: return None return normalize_phone(value)` (стр. 1048)
- `BookingUpdateRequest.validate_vehicledef validate_vehicle(self) -> "BookingUpdateRequest": if self.car is not None and not self.car.strip():` (стр. 1054)
- `class ClientCardUpdateRequest(BaseModel):` (стр. 1067)
- `ClientCardUpdateRequest.validate_namedef validate_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1085)
- `ClientCardUpdateRequest.validate_phonedef validate_phone(cls, value: str | None) -> str | None: if value is None or not value.strip():` (стр. 1092)
- `ClientCardUpdateRequest.validate_vehicledef validate_vehicle(self) -> "ClientCardUpdateRequest": if self.car is not None and not self.car.strip():` (стр. 1098)
- `class NotificationCreateRequest(BaseModel):` (стр. 1111)
- `class ReadAllNotificationsRequest(BaseModel):` (стр. 1118)
- `class StockItemCreateRequest(BaseModel):` (стр. 1122)
- `class StockItemUpdateRequest(BaseModel):` (стр. 1131)
- `class StockCategoryCreateRequest(BaseModel):` (стр. 1140)
- `class StockCategoryUpdateRequest(BaseModel):` (стр. 1145)
- `class StockWriteOffRequest(BaseModel):` (стр. 1150)
- `class StockWriteOffPayload(BaseModel):` (стр. 1154)
- `class IncomeCreateRequest(BaseModel):` (стр. 1172)
- `IncomeCreateRequest.validate_sourcedef validate_source(cls, value: str) -> str: stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или состоять только из пробелов") return stripp` (стр. 1181)
- `IncomeCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1189)
- `class IncomePayload(BaseModel):` (стр. 1195)
- `class ExpenseCreateRequest(BaseModel):` (стр. 1206)
- `ExpenseCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1216)
- `class PenaltyCreateRequest(BaseModel):` (стр. 1222)
- `class OwnerReminderDispatchRequest(BaseModel):` (стр. 1228)
- `class OwnerReminderDispatchPayload(BaseModel):` (стр. 1233)
- `class ChangePasswordRequest(BaseModel):` (стр. 1241)
- `class OwnerDatabaseResetPreviewPayload(BaseModel):` (стр. 1246)
- `class OwnerDatabaseResetStartRequest(BaseModel):` (стр. 1261)
- `class OwnerDatabaseResetApproveRequest(BaseModel):` (стр. 1265)
- `class OwnerDatabaseResetExecuteRequest(BaseModel):` (стр. 1271)
- `class OwnerDatabaseResetStartPayload(BaseModel):` (стр. 1275)
- `class OwnerDatabaseResetApprovePayload(BaseModel):` (стр. 1284)
- `class OwnerDatabaseResetExecutePayload(BaseModel):` (стр. 1292)
- `class ContentAboutPayload(BaseModel):` (стр. 1297)
- `class ContentServicePayload(BaseModel):` (стр. 1303)
- `class ContentWorksPayload(BaseModel):` (стр. 1314)
- `class ContentStatsPayload(BaseModel):` (стр. 1320)
- `class ContentTitlePayload(BaseModel):` (стр. 1325)
- `ContentTitlePayload.to_full_titledef to_full_title(self) -> str: return f"{self.before}{self.highlight}{self.after}"` (стр. 1330)
- `class ContentHeroPayload(BaseModel):` (стр. 1334)
- `class ContentPayload(BaseModel):` (стр. 1350)
- `class ContactPayload(BaseModel):` (стр. 1357)
- `class ResetPasswordRequest(BaseModel):` (стр. 1364)
- `class GenericMessage(BaseModel):` (стр. 1368)
- `class TelegramDeliveryResult(BaseModel):` (стр. 1372)
- `class TelegramBroadcastPayload(BaseModel):` (стр. 1378)
- `class OwnerExportDeliveryPayload(BaseModel):` (стр. 1384)
- `class ShiftAttendancePayload(BaseModel):` (стр. 1391)
- `class ExpenseUpdateRequest(BaseModel):` (стр. 1402)
- `ExpenseUpdateRequest.validate_titledef validate_title(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("title не может быть пустым или с` (стр. 1412)
- `ExpenseUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1422)
- `ExpenseUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> "ExpenseUpdateRequest": if all(v is None for v in [self.title, self.amount, self.category, self.date, self.note]):` (стр. 1430)
- `class IncomeUpdateRequest(BaseModel):` (стр. 1436)
- `IncomeUpdateRequest.validate_sourcedef validate_source(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или` (стр. 1445)
- `IncomeUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1455)
- `IncomeUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> "IncomeUpdateRequest": # Use model_fields_set to detect explicitly provided fields (including null). # This allows {"note": null} to pass as` (стр. 1463)
- `class PiggyBankTransactionPayload(BaseModel):` (стр. 1471)
- `class PiggyBankWithdrawRequest(BaseModel):` (стр. 1493)
- `PiggyBankWithdrawRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1502)
- `class PiggyBankAdjustRequest(BaseModel):` (стр. 1508)
- `PiggyBankAdjustRequest.validate_datedef validate_date(cls, value: str) -> str: stripped = value.strip() if stripped and not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", stripped):` (стр. 1516)
- `class PiggyBankWashBreakdown(BaseModel):` (стр. 1523)
- `class PiggyBankDetailingBreakdown(BaseModel):` (стр. 1536)
- `class PiggyBankResponse(BaseModel):` (стр. 1547)
- `class WeeklyArchivePayload(BaseModel):` (стр. 1566)
- `class WalletResponse(BaseModel):` (стр. 1580)
- `class OwnerProfitShareItem(BaseModel):` (стр. 1597)
- `class OwnerProfitShareSummary(BaseModel):` (стр. 1614)
- `class OwnerSalaryDetailResponse(BaseModel):` (стр. 1623)
- `class PayOwnerSalaryRequest(BaseModel):` (стр. 1630)
- `class PayOwnerSalaryResponse(BaseModel):` (стр. 1636)
- `class OverrideEarnedRequest(BaseModel):` (стр. 1643)
- `class BookingHistoryItem(BaseModel):` (стр. 1647)
- `class BookingTotalsWorkerItem(BaseModel):` (стр. 1664)
- `class BookingTotalsOwnerItem(BaseModel):` (стр. 1682)
- `class BookingTotalsPiggyItem(BaseModel):` (стр. 1689)
- `class BookingHistoryTotals(BaseModel):` (стр. 1695)
- `class BookingMoneySplitWorkerItem(BaseModel):` (стр. 1701)
- `class BookingMoneySplitOwnerItem(BaseModel):` (стр. 1712)
- `class BookingPiggyTxItem(BaseModel):` (стр. 1720)
- `class BookingAdditionalServiceItem(BaseModel):` (стр. 1729)
- `class BookingAsvcPiggyItem(BaseModel):` (стр. 1738)
- `class BookingAsvcWorkerItem(BaseModel):` (стр. 1744)
- `class BookingMoneySplitDetail(BaseModel):` (стр. 1755)
- `class BookingWorkerEarnedUpdate(BaseModel):` (стр. 1803)
- `class BookingMoneySplitOwnerUpdate(BaseModel):` (стр. 1808)
- `class BookingMoneySplitUpdateRequest(BaseModel):` (стр. 1813)
- `class ArchiveBookingWorkerItem(BaseModel):` (стр. 1823)
- `class ArchiveAdditionalServiceItem(BaseModel):` (стр. 1833)
- `class ArchiveBookingItem(BaseModel):` (стр. 1839)
- `class ArchivePayrollItem(BaseModel):` (стр. 1866)
- `class ArchiveOwnerItem(BaseModel):` (стр. 1884)
- `class ArchiveSummary(BaseModel):` (стр. 1892)
- `class ArchiveResponse(BaseModel):` (стр. 1908)
- `class DepositSubscriptionUpdateRequest(BaseModel):` (стр. 1925)
- `DepositSubscriptionUpdateRequest.validate_plandef validate_plan(cls, value: str) -> str: value = value.strip() if value and value not in DEPOSIT_PLANS: raise ValueError("Неизвестный тип абонемента") return value` (стр. 1939)
- `class DepositTopUpRequest(BaseModel):` (стр. 1946)
- `DepositTopUpRequest.validate_datedef validate_date(cls, value: str) -> str: value = value.strip() if value and not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1954)
- `class DepositAdjustRequest(BaseModel):` (стр. 1961)
- `DepositAdjustRequest.validate_datedef validate_date(cls, value: str) -> str: value = value.strip() if value and not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1969)
- `class DepositWashRequest(BaseModel):` (стр. 1976)
- `DepositWashRequest.validate_vehicledef validate_vehicle(self) -> "DepositWashRequest": if not self.car.strip() and not self.plate.strip():` (стр. 1992)
- `class DepositSettleRequest(BaseModel):` (стр. 2002)
- `DepositSettleRequest.validate_monthdef validate_month(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{4}", value.strip()):` (стр. 2008)
- `class DepositTransactionPayload(BaseModel):` (стр. 2014)
- ...ещё 5
...
### backend/tests/test_booking_logic.py (4360 строк)
...
- `BookingLogicTests.test_admin_cannot_create_scheduled_booking_without_date_and_timedef test_admin_cannot_create_scheduled_booking_without_date_and_time(self) -> None: """Статус scheduled требует валидный слот при создании — иначе запись нельзя впоследствии переве` (стр. 1315)
- `BookingLogicTests.test_scheduled_booking_without_slot_cannot_be_moved_to_active_statusdef test_scheduled_booking_without_slot_cannot_be_moved_to_active_status(self) -> None: """Слотless запись (устаревшие данные) не может быть переведена в активный статус: PATCH отв` (стр. 1344)
- `BookingLogicTests.test_owner_can_create_client_and_past_booking_visible_on_first_client_logindef test_owner_can_create_client_and_past_booking_visible_on_first_client_login(self) -> None: owner_token = self.login_staff("owner", "owner") client_response = self.client.post( ` (стр. 1390)
- `BookingLogicTests.test_service_resource_group_preserved_on_savedef test_service_resource_group_preserved_on_save(self) -> None: owner_token = self.login_staff("owner", "owner") bootstrap = self.client.get("/api/auth/session", headers=self.auth` (стр. 1443)
- `BookingLogicTests.test_fired_worker_loses_access_and_future_assignmentsdef test_fired_worker_loses_access_and_future_assignments(self) -> None: admin_token = self.login_staff("admin", "admin") self.disable_owner_two_factor() owner_token = self.login_s` (стр. 1460)
- `BookingLogicTests.test_same_telegram_client_reuses_existing_accountdef test_same_telegram_client_reuses_existing_account(self) -> None: self.verify_client_phone("1001", "+7 (999) 111-22-33") first = self.client.post( "/api/auth/client", json=self.` (стр. 1520)
- `BookingLogicTests.test_generic_telegram_auth_logs_in_linked_clientdef test_generic_telegram_auth_logs_in_linked_client(self) -> None: self.verify_client_phone("1001", "+7 (999) 111-22-33") first = self.client.post( "/api/auth/client", json=self.c` (стр. 1550)
- `BookingLogicTests.test_generic_telegram_auth_tolerates_legacy_client_profile_datadef test_generic_telegram_auth_tolerates_legacy_client_profile_data(self) -> None: from app.database import SessionLocal from app.models import Client self.verify_client_phone("100` (стр. 1567)
- `BookingLogicTests.test_generic_telegram_auth_prefers_linked_staff_windowdef test_generic_telegram_auth_prefers_linked_staff_window(self) -> None: self.set_staff_telegram("ivan", "7001") self.verify_client_phone("7001", "+7 (999) 111-22-33") client = se` (стр. 1597)
- `BookingLogicTests.test_generic_telegram_auth_does_not_claim_primary_ownerdef test_generic_telegram_auth_does_not_claim_primary_owner(self) -> None: self.set_primary_owner_telegram("") response = self.client.post("/api/auth/telegram", json={"initData": s` (стр. 1614)
- `BookingLogicTests.test_primary_owner_telegram_route_rejects_unlinked_ownerdef test_primary_owner_telegram_route_rejects_unlinked_owner(self) -> None: self.set_primary_owner_telegram("") response = self.client.post( "/api/auth/telegram-owner", json={"init` (стр. 1621)
- `BookingLogicTests.test_nullable_text_values_are_treated_as_empty_stringsdef test_nullable_text_values_are_treated_as_empty_strings(self) -> None: from app.main import _safe_text self.assertEqual(_safe_text(None), "") self.assertEqual(_safe_text(" 9001 ` (стр. 1633)
- `BookingLogicTests.test_primary_owner_can_log_in_via_dedicated_telegram_routedef test_primary_owner_can_log_in_via_dedicated_telegram_route(self) -> None: self.set_primary_owner_telegram("9001") response = self.client.post( "/api/auth/telegram-owner", json=` (стр. 1639)
- `BookingLogicTests.test_generic_telegram_auth_rejects_expired_init_datadef test_generic_telegram_auth_rejects_expired_init_data(self) -> None: self.set_staff_telegram("ivan", "7002") response = self.client.post( "/api/auth/telegram", json={"initData":` (стр. 1656)
- `BookingLogicTests.test_generic_telegram_auth_rejects_duplicate_staff_bindingsdef test_generic_telegram_auth_rejects_duplicate_staff_bindings(self) -> None: self.set_staff_telegram("ivan", "7007") self.set_staff_telegram("oleg", "7007") response = self.clien` (стр. 1666)
- `BookingLogicTests.test_client_registration_rejects_same_phone_for_different_telegram_idsdef test_client_registration_rejects_same_phone_for_different_telegram_ids(self) -> None: self.verify_client_phone("1001", "+7 (999) 111-22-33") first = self.client.post( "/api/aut` (стр. 1674)
- `BookingLogicTests.test_client_profile_cannot_take_phone_of_another_clientdef test_client_profile_cannot_take_phone_of_another_client(self) -> None: first_token, first_id = self.login_client(name="Alice", phone="+7 (999) 111-22-33") second_token, _ = sel` (стр. 1689)
- `BookingLogicTests.test_client_booking_creates_notification_for_same_client_iddef test_client_booking_creates_notification_for_same_client_id(self) -> None: token, actor_id = self.login_client(name="Alice", phone="+7 (999) 111-22-33") response = self.client.` (стр. 1708)
- `BookingLogicTests.test_client_cannot_mark_other_clients_notification_as_readdef test_client_cannot_mark_other_clients_notification_as_read(self) -> None: admin_token = self.login_staff("admin", "admin") first_token, first_id = self.login_client(name="Alice` (стр. 1739)
- `BookingLogicTests.test_client_login_rejects_foreign_telegram_id_for_existing_phonedef test_client_login_rejects_foreign_telegram_id_for_existing_phone(self) -> None: self.verify_client_phone("1001", "+7 (999) 111-22-33") first = self.client.post( "/api/auth/clie` (стр. 1768)
- `BookingLogicTests.test_client_read_all_marks_only_own_notificationsdef test_client_read_all_marks_only_own_notifications(self) -> None: admin_token = self.login_staff("admin", "admin") first_token, first_id = self.login_client(name="Alice", phone=` (стр. 1789)
- `BookingLogicTests.test_client_read_all_rejects_foreign_role_payloaddef test_client_read_all_rejects_foreign_role_payload(self) -> None: token, _ = self.login_client(name="Alice", phone="+7 (999) 111-22-33") response = self.client.post( "/api/notif` (стр. 1831)
- `BookingLogicTests.test_deleting_client_removes_client_sessions_and_notificationsdef test_deleting_client_removes_client_sessions_and_notifications(self) -> None: admin_token = self.login_staff("admin", "admin") client_token, client_id = self.login_client(name=` (стр. 1840)
- `BookingLogicTests.test_client_cancel_booking_creates_client_and_admin_notificationsdef test_client_cancel_booking_creates_client_and_admin_notifications(self) -> None: token, actor_id = self.login_client(name="Alice", phone="+7 (999) 111-22-33") create_response =` (стр. 1868)
- `BookingLogicTests.test_deleted_client_can_register_again_with_same_phone_and_telegramdef test_deleted_client_can_register_again_with_same_phone_and_telegram(self) -> None: admin_token = self.login_staff("admin", "admin") self.verify_client_phone("1001", "+7 (999) 1` (стр. 1909)
- `BookingLogicTests.test_secure_client_auth_requires_valid_init_datadef test_secure_client_auth_requires_valid_init_data(self) -> None: self.shutdown_app() os.environ["ALLOW_INSECURE_CLIENT_AUTH"] = "false" self.restart_app() missing = self.client.` (стр. 1933)
- `BookingLogicTests.test_admin_reschedule_creates_client_notificationdef test_admin_reschedule_creates_client_notification(self) -> None: admin_token = self.login_staff("admin", "admin") client_token, actor_id = self.login_client(name="Alice", phone` (стр. 1964)
- `BookingLogicTests.test_admin_completion_creates_client_notificationdef test_admin_completion_creates_client_notification(self) -> None: admin_token = self.login_staff("admin", "admin") client_token, actor_id = self.login_client(name="Alice", phone` (стр. 2002)
- `BookingLogicTests.test_admin_booking_reuses_existing_client_by_normalized_phonedef test_admin_booking_reuses_existing_client_by_normalized_phone(self) -> None: admin_token = self.login_staff("admin", "admin") _, client_id = self.login_client(name="Alice", pho` (стр. 2046)
- `BookingLogicTests.test_admin_cannot_create_booking_with_conflicting_client_and_phonedef test_admin_cannot_create_booking_with_conflicting_client_and_phone(self) -> None: admin_token = self.login_staff("admin", "admin") _, first_client_id = self.login_client(name="` (стр. 2080)
- `BookingLogicTests.test_admin_can_save_profile_and_notification_settingsdef test_admin_can_save_profile_and_notification_settings(self) -> None: admin_token = self.login_staff("admin", "admin") profile_response = self.client.put( "/api/settings/admin/p` (стр. 2108)
- `BookingLogicTests.test_owner_can_create_admin_like_worker_and_update_telegram_idsdef test_owner_can_create_admin_like_worker_and_update_telegram_ids(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") create_admin = s` (стр. 2144)
- `BookingLogicTests.test_owner_can_create_and_login_accountantdef test_owner_can_create_and_login_accountant(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") create_accountant = self.client.post(` (стр. 2226)
- `BookingLogicTests.test_owner_can_rehire_employee_with_same_telegram_after_dismissaldef test_owner_can_rehire_employee_with_same_telegram_after_dismissal(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") create_worker ` (стр. 2265)
- `BookingLogicTests.test_admin_can_manage_master_payroll_and_private_client_ratingdef test_admin_can_manage_master_payroll_and_private_client_rating(self) -> None: admin_token = self.login_staff("admin", "admin") _, client_id = self.login_client(name="Alice", ph` (стр. 2316)
- `BookingLogicTests.test_admin_payroll_propagates_period_and_matches_salary_base_helperdef test_admin_payroll_propagates_period_and_matches_salary_base_helper(self) -> None: from app.database import SessionLocal from app.finance import money_int, salary_base_for_peri` (стр. 2358)
- `BookingLogicTests.test_owner_and_admin_can_see_detailed_worker_payroll_summarydef test_owner_and_admin_can_see_detailed_worker_payroll_summary(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") admin_token = self.` (стр. 2418)
- `BookingLogicTests.test_payroll_entry_notifies_worker_and_updates_summarydef test_payroll_entry_notifies_worker_and_updates_summary(self) -> None: from app.database import SessionLocal from app.models import Notification, StaffUser self.disable_owner_tw` (стр. 2511)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str | int, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 2526)
- `BookingLogicTests.test_admin_cannot_issue_advance_before_worker_earns_1000def test_admin_cannot_issue_advance_before_worker_earns_1000(self) -> None: admin_token = self.login_staff("admin", "admin") response = self.client.post( "/api/payroll/entries", he` (стр. 2552)
- `BookingLogicTests._tg_headersdef _tg_headers(self, login: str, telegram_id: str) -> dict[str, str]: """Авторизация текущего флоу (Telegram init data): привязываем telegram_chat_id к сотруднику и возвращаем заг` (стр. 2568)
- `BookingLogicTests.test_payroll_entries_with_period_are_attributed_to_selected_perioddef test_payroll_entries_with_period_are_attributed_to_selected_period(self) -> None: """Премия/штраф/списание с выбранным периодом учитываются за этот период (08.08–14.08), а не п` (стр. 2587)
- `BookingLogicTests.test_payroll_entry_without_period_keeps_created_at_behaviordef test_payroll_entry_without_period_keeps_created_at_behavior(self) -> None: """Операция без периода (legacy) учитывается по дате создания.""" owner_headers = self._tg_headers("o` (стр. 2652)
- `BookingLogicTests.test_owner_pay_salary_attributes_payout_to_selected_perioddef test_owner_pay_salary_attributes_payout_to_selected_period(self) -> None: """Выплата мастеру тоже привязывается к выбранному периоду.""" owner_headers = self._tg_headers("owner` (стр. 2685)
- `BookingLogicTests.test_owner_pdf_export_returns_pdf_filedef test_owner_pdf_export_returns_pdf_file(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") response = self.client.get("/api/owner/ex` (стр. 2711)
- `BookingLogicTests.test_owner_can_create_booking_with_assigned_master_without_platedef test_owner_can_create_booking_with_assigned_master_without_plate(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") response = self` (стр. 2720)
- `BookingLogicTests.test_admin_reschedule_notifies_assigned_workerdef test_admin_reschedule_notifies_assigned_worker(self) -> None: from app.database import SessionLocal from app.models import Notification, StaffUser admin_token = self.login_staf` (стр. 2749)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str | int, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 2788)
- `BookingLogicTests.test_worker_start_and_completion_notify_owner_and_send_receiptdef test_worker_start_and_completion_notify_owner_and_send_receipt(self) -> None: from app.database import SessionLocal from app.models import Client, Notification self.disable_own` (стр. 2816)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str | int, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 2861)
- `BookingLogicTests.test_client_can_store_multiple_vehiclesdef test_client_can_store_multiple_vehicles(self) -> None: token, client_id = self.login_client(name="Alice", phone="+7 (999) 111-22-33") response = self.client.patch( "/api/client` (стр. 2907)
- `BookingLogicTests.test_owner_can_notify_admin_about_inactive_clientsdef test_owner_can_notify_admin_about_inactive_clients(self) -> None: from app.database import SessionLocal from app.models import Booking, Notification self.disable_owner_two_fact` (стр. 2944)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str | int, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 2979)
- `BookingLogicTests.test_owner_dispatches_return_visit_reminders_to_clientsdef test_owner_dispatches_return_visit_reminders_to_clients(self) -> None: from app.database import SessionLocal from app.models import Booking, Notification self.disable_owner_two` (стр. 2999)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str | int, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 3043)
- `BookingLogicTests.test_worker_can_submit_shift_checklists_and_owner_can_review_themdef test_worker_can_submit_shift_checklists_and_owner_can_review_them(self) -> None: from app.database import SessionLocal from app.models import Notification self.disable_owner_tw` (стр. 3073)
- `BookingLogicTests.test_admin_shift_inspection_sends_owner_photo_and_can_be_approveddef test_admin_shift_inspection_sends_owner_photo_and_can_be_approved(self) -> None: from app.database import SessionLocal from app.models import Notification self.disable_owner_tw` (стр. 3142)
- `BookingLogicTests.fake_send_photodef fake_send_photo(chat_id: str | int, **kwargs) -> None: sent_photos.append({"chat_id": chat_id, **kwargs})` (стр. 3162)
- `BookingLogicTests.test_admin_shift_inspection_list_uses_photo_endpointdef test_admin_shift_inspection_list_uses_photo_endpoint(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") admin_token = self.login_st` (стр. 3198)
- `BookingLogicTests.test_bot_can_reject_admin_shift_with_issue_notedef test_bot_can_reject_admin_shift_with_issue_note(self) -> None: from bot import BotRuntime, process_telegram_update from app.database import SessionLocal from app.models import ` (стр. 3241)
- `BookingLogicTests.fake_telegram_calldef fake_telegram_call(_runtime, method: str, payload: dict[str, object] | None = None, **_kwargs): telegram_calls.append((method, payload or {})) return {}` (стр. 3276)
- `BookingLogicTests.test_admin_mark_read_all_affects_only_admin_notificationsdef test_admin_mark_read_all_affects_only_admin_notifications(self) -> None: admin_token = self.login_staff("admin", "admin") owner_token = self.login_staff("owner", "owner") if Fa` (стр. 3308)
- `BookingLogicTests.test_admin_cannot_access_owner_only_endpointsdef test_admin_cannot_access_owner_only_endpoints(self) -> None: admin_token = self.login_staff("admin", "admin") create_worker = self.client.post( "/api/workers", headers=self.aut` (стр. 3351)
- `BookingLogicTests.test_worker_can_update_only_own_assigned_booking_status_and_notesdef test_worker_can_update_only_own_assigned_booking_status_and_notes(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "maste` (стр. 3397)
- `BookingLogicTests.test_worker_completion_creates_admin_notification_with_amount_client_and_servicedef test_worker_completion_creates_admin_notification_with_amount_client_and_service(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff` (стр. 3443)
- `BookingLogicTests.test_worker_cannot_change_time_or_workers_even_on_own_bookingdef test_worker_cannot_change_time_or_workers_even_on_own_booking(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "master") ` (стр. 3483)
- `BookingLogicTests.test_worker_must_specify_payment_state_when_completing_bookingdef test_worker_must_specify_payment_state_when_completing_booking(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "master")` (стр. 3525)
- `BookingLogicTests.test_worker_can_save_only_own_profiledef test_worker_can_save_only_own_profile(self) -> None: worker_token = self.login_staff("ivan", "master") worker = self.get_staff(login="ivan") other_worker = self.get_staff(login` (стр. 3578)
- `BookingLogicTests.test_worker_can_save_only_own_notification_settingsdef test_worker_can_save_only_own_notification_settings(self) -> None: worker_token = self.login_staff("ivan", "master") worker = self.get_staff(login="ivan") other_worker = self.g` (стр. 3616)
- `BookingLogicTests.test_worker_mark_read_all_affects_only_own_notificationsdef test_worker_mark_read_all_affects_only_own_notifications(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") worker_token = self.log` (стр. 3649)
- `BookingLogicTests.test_worker_cannot_create_penaltiesdef test_worker_cannot_create_penalties(self) -> None: worker_token = self.login_staff("ivan", "master") other_worker = self.get_staff(login="oleg") response = self.client.post( "/` (стр. 3682)
- `BookingLogicTests.test_worker_cannot_create_notifications_for_other_rolesdef test_worker_cannot_create_notifications_for_other_roles(self) -> None: worker_token = self.login_staff("ivan", "master") _, client_id = self.login_client(name="Alice", phone="+` (стр. 3692)
- `BookingLogicTests.test_worker_can_create_notification_for_assigned_clientdef test_worker_can_create_notification_for_assigned_client(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "master") worker` (стр. 3707)
- `BookingLogicTests.test_worker_can_generate_telegram_link_codedef test_worker_can_generate_telegram_link_code(self) -> None: worker_token = self.login_staff("ivan", "master") response = self.client.post( "/api/telegram/link-code", headers=sel` (стр. 3748)
- `BookingLogicTests.test_telegram_webhook_rejects_invalid_secretdef test_telegram_webhook_rejects_invalid_secret(self) -> None: os.environ["WEBAPP_URL"] = "https://crm.example" os.environ["TELEGRAM_DELIVERY_MODE"] = "webhook" self.restart_app()` (стр. 3759)
- `BookingLogicTests.test_telegram_webhook_processes_update_with_valid_secretdef test_telegram_webhook_processes_update_with_valid_secret(self) -> None: os.environ["WEBAPP_URL"] = "https://crm.example" os.environ["TELEGRAM_DELIVERY_MODE"] = "webhook" self.r` (стр. 3771)
- `BookingLogicTests.test_client_bootstrap_contains_only_own_bookings_and_no_worker_directorydef test_client_bootstrap_contains_only_own_bookings_and_no_worker_directory(self) -> None: admin_token = self.login_staff("admin", "admin") first_token, first_id = self.login_clie` (стр. 3786)
- `BookingLogicTests.test_worker_bootstrap_contains_only_assigned_bookingsdef test_worker_bootstrap_contains_only_assigned_bookings(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "master") first_wo` (стр. 3846)
- `BookingLogicTests.test_admin_can_update_booking_alias_fields_and_service_canonical_datadef test_admin_can_update_booking_alias_fields_and_service_canonical_data(self) -> None: admin_token = self.login_staff("admin", "admin") create_response = self.client.post( "/api/` (стр. 3906)
- `BookingLogicTests.test_owner_stock_write_off_rejects_negative_qtydef test_owner_stock_write_off_rejects_negative_qty(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") create_response = self.client.po` (стр. 3957)
- `BookingLogicTests.test_admin_can_read_targeted_admin_notificationsdef test_admin_can_read_targeted_admin_notifications(self) -> None: self.disable_owner_two_factor() owner_token = self.login_staff("owner", "owner") admin_token = self.login_staff(` (стр. 3986)
- `BookingLogicTests.test_deleting_client_removes_related_bookings_and_sessionsdef test_deleting_client_removes_related_bookings_and_sessions(self) -> None: admin_token = self.login_staff("admin", "admin") client_token, client_id = self.login_client(name="Ali` (стр. 4039)
- `BookingLogicTests.test_worker_cannot_message_client_from_only_completed_bookingdef test_worker_cannot_message_client_from_only_completed_booking(self) -> None: admin_token = self.login_staff("admin", "admin") worker_token = self.login_staff("ivan", "master") ` (стр. 4076)
- `BookingLogicTests.test_owner_database_reset_execute_requires_delay_after_approvaldef test_owner_database_reset_execute_requires_delay_after_approval(self) -> None: self.disable_owner_two_factor() self.set_primary_owner_telegram() owner_token = self.login_staff(` (стр. 4123)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 4129)
- `BookingLogicTests.test_owner_database_reset_clears_operational_data_and_preserves_ownersdef test_owner_database_reset_clears_operational_data_and_preserves_owners(self) -> None: from app.database import SessionLocal from app.models import ( AppSetting, Booking, Box, C` (стр. 4163)
- `BookingLogicTests.fake_send_messagedef fake_send_message(chat_id: str, text: str) -> None: sent_messages.append((chat_id, text))` (стр. 4234)
- `BookingLogicTests.test_normalize_service_and_box_resources_handles_legacy_null_box_fieldsdef test_normalize_service_and_box_resources_handles_legacy_null_box_fields(self) -> None: from app.main import DETAILING_BOX_NAME, WASH_BOX_NAMES, _normalize_service_and_box_resou` (стр. 4297)
- `class FakeScalarResult: def __init__(self, items: list[object]) -> None: self._items = items def all(self) -> list[objec` (стр. 4301)
- `FakeScalarResult.__init__def __init__(self, items: list[object]) -> None: self._items = items` (стр. 4302)
- `FakeScalarResult.alldef all(self) -> list[object]: return self._items` (стр. 4305)
- `class FakeSession: def __init__(self, services: list[Service], boxes: list[Box]) -> None: self.services = services self.` (стр. 4308)
- `FakeSession.__init__def __init__(self, services: list[Service], boxes: list[Box]) -> None: self.services = services self.boxes = boxes self.flushed = False` (стр. 4309)
- `FakeSession.scalarsdef scalars(self, statement): entity = statement.column_descriptions[0]["entity"] if entity is Service: return FakeScalarResult(self.services) if entity is Box: return FakeScalarRe` (стр. 4314)
- `FakeSession.adddef add(self, _item: object) -> None: return None` (стр. 4322)
- `FakeSession.flushdef flush(self) -> None: self.flushed = True` (стр. 4325)
...
### backend/tests/test_owner_salary_asvc_only.py (192 строк)
...
- `OwnerSalaryAsvcOnlyTest._create_clientdef _create_client(self) -> tuple[str, str]: from app.database import SessionLocal from app.models import Client client_id = f"c-{uuid4().hex[:12]}" phone = f"+7 (999) 000-{str(uui` (стр. 107)
- `OwnerSalaryAsvcOnlyTest.test_owner_salary_detail_includes_asvc_only_workerdef test_owner_salary_detail_includes_asvc_only_worker(self) -> None: """Owner should see earnings for worker assigned ONLY to additional service.""" client_id, client_phone = self` (стр. 126)
...
### frontend/src/app/components/admin/AdminApp.tsx (4627 строк)
...
- `target` (стр. 689) — локальный
- `statusNeedsSlot` (стр. 690) — локальный
- `handleDeleteClient` (стр. 702) — локальный
- `confirmed` (стр. 703) — локальный
- `handleCreateClient` (стр. 708) — локальный
- `nameError` (стр. 710) — локальный
- `phoneError` (стр. 714) — локальный
- `carError` (стр. 718) — локальный
- `plateError` (стр. 722) — локальный
- `created` (стр. 730) — локальный
- `handleSaveClientCard` (стр. 752) — локальный
- `draft` (стр. 753) — локальный
- `handleShiftPhotoChange` (стр. 767) — локальный
- `file` (стр. 768) — локальный
- `dataUrl` (стр. 772) — локальный
- `handleSubmitShiftInspection` (стр. 784) — локальный
- `primaryPhoto` (стр. 788) — локальный
- `uploadedCategoriesLabel` (стр. 795) — локальный
- `composedNote` (стр. 796) — локальный
- `saved` (стр. 800) — локальный
- `validateClientName` (стр. 817) — локальный
- `validateClientPhone` (стр. 821) — локальный
- `validateBookingDate` (стр. 825) — локальный
- `parsedDate` (стр. 827) — локальный
- `scheduleDay` (стр. 832) — локальный
- `normalizedTime` (стр. 837) — локальный
- `slotStart` (стр. 838) — локальный
- `openMinutes` (стр. 847) — локальный
- `closeMinutes` (стр. 848) — локальный
- `slotEnd` (стр. 849) — локальный
- `validateBookingDateForEdit` (стр. 859) — локальный
- `parsedDate` (стр. 861) — локальный
- `scheduleDay` (стр. 866) — локальный
- `normalizedTime` (стр. 871) — локальный
- `slotStart` (стр. 872) — локальный
- `openMinutes` (стр. 878) — локальный
- `closeMinutes` (стр. 879) — локальный
- `slotEnd` (стр. 880) — локальный
- `validateBookingDateTimeFormat` (стр. 890) — локальный
- `parsedDate` (стр. 892) — локальный
- `validateNewBookingForm` (стр. 905) — локальный
- `selectedService` (стр. 907) — локальный
- `nameError` (стр. 909) — локальный
- `phoneError` (стр. 913) — локальный
- `carError` (стр. 917) — локальный
- `plateError` (стр. 921) — локальный
- `hasDate` (стр. 924) — локальный
- `hasTime` (стр. 925) — локальный
- `requiresScheduledSlot` (стр. 926) — локальный
- `validation` (стр. 943) — локальный
- `resetNewBookingDraft` (стр. 953) — локальный
- `openNewBookingModal` (стр. 981) — локальный
- `openAdditionalServiceModal` (стр. 986) — локальный
- `openNewBookingForClient` (стр. 995) — локальный
- `historyDate` (стр. 997) — локальный
- `clientVehicles` (стр. 999) — локальный
- `mainVehicle` (стр. 1001) — локальный
- `closeNewBookingModal` (стр. 1017) — локальный
- `handleAddService` (стр. 1022) — локальный
- `svc` (стр. 1027) — локальный
- `workersList` (стр. 1028) — локальный
- `worker` (стр. 1029) — локальный
- `updatedBooking` (стр. 1032) — локальный
- `handleRemoveService` (стр. 1052) — локальный
- `handleOpenEditAsvc` (стр. 1056) — локальный
- `handleSaveEditAsvc` (стр. 1064) — локальный
- `workersList` (стр. 1069) — локальный
- `worker` (стр. 1070) — локальный
- `updatedBooking` (стр. 1073) — локальный
- `closeAddServiceModal` (стр. 1090) — локальный
- `openEditModal` (стр. 1096) — локальный
- `handleSaveEditedBooking` (стр. 1118) — локальный
- `editServiceId` (стр. 1120) — локальный
- `detailingBooking` (стр. 1121) — локальный
- `requiresScheduledSlot` (стр. 1122) — локальный
- `statusNeedsSlot` (стр. 1124) — локальный
- `slotChanged` (стр. 1125) — локальный
- `validationErrors` (стр. 1127) — локальный
- `handleDeleteBooking` (стр. 1181) — локальный
- `name` (стр. 1183) — локальный
- `handleAssignWorkers` (стр. 1190) — локальный
- `updatedWorkers` (стр. 1192) — локальный
- `w` (стр. 1193) — локальный
...
### frontend/src/app/components/owner/OwnerApp.tsx (12320 строк)

- `EXPENSE_CATEGORIES` (стр. 232) — локальный
- `STOCK_UNITS` (стр. 233) — локальный
- `SERVICE_TYPE_OPTIONS` (стр. 234) — локальный
- `ownerBookingStatusRequiresScheduledSlot` (стр. 245) — локальный
- `employeeRoleLabel` (стр. 248) — локальный
- `ownerServiceResourceGroup` (стр. 254) — локальный
- `ownerDefaultBoxForService` (стр. 258) — локальный
- `rg` (стр. 259) — локальный
- `match` (стр. 260) — локальный
- `ownerBookingBoxes` (стр. 264) — локальный
- `ownerLocationLabel` (стр. 272) — локальный
- `parseOwnerBookingMinutes` (стр. 276) — локальный
- `match` (стр. 277) — локальный
- `hours` (стр. 279) — локальный
- `minutes` (стр. 280) — локальный
- `OWNER_CALENDAR_WEEKDAYS` (стр. 285) — локальный
- `OWNER_CALENDAR_MONTHS` (стр. 286) — локальный
- `OWNER_CALENDAR_DEFAULT_OPEN` (стр. 290) — локальный
- `OWNER_CALENDAR_DEFAULT_CLOSE` (стр. 291) — локальный
- `ownerScheduleTimeToMinutes` (стр. 293) — локальный
- `ownerMonthTitle` (стр. 297) — локальный
- `ownerBuildMonthCells` (стр. 301) — локальный
- `year` (стр. 302) — локальный
- `month` (стр. 303) — локальный
- `first` (стр. 304) — локальный
- `offset` (стр. 305) — локальный
- `daysInMonth` (стр. 306) — локальный
- `date` (стр. 312) — локальный
- `ownerCalendarDayHours` (стр. 321) — локальный
- `parsedDate` (стр. 322) — локальный
- `daySchedule` (стр. 326) — локальный
- `open` (стр. 330) — локальный
- `close` (стр. 331) — локальный
- `OWNER_CALENDAR_LOAD_COLORS` (стр. 335) — локальный
- `ownerCalendarLoadTone` (стр. 341) — локальный
- `ratio` (стр. 343) — локальный
- `ownerGroupBookingsByHour` (стр. 353) — локальный
- `timed` (стр. 358) — локальный
- `hourLabel` (стр. 361) — локальный
- `slotEnd` (стр. 362) — локальный
- `slotBookings` (стр. 363) — локальный
- `start` (стр. 365) — локальный
- `ownerOpenBookingDetail` (стр. 377) — локальный
- `ownerBookingBlocksBox` (стр. 386) — локальный
- `nextStart` (стр. 389) — локальный
- `existingStart` (стр. 390) — локальный
- `nextEnd` (стр. 392) — локальный
- `existingEnd` (стр. 393) — локальный
- `ownerPickDefaultBookingBox` (стр. 397) — локальный
- `resourceGroup` (стр. 406) — локальный
- `preferred` (стр. 407) — локальный
- `fallback` (стр. 408) — локальный
- `candidates` (стр. 409) — локальный
- `serviceResourceGroupForCategory` (стр. 414) — локальный
- `numberInputValue` (стр. 418) — локальный
- `ORDER_STEPS` (стр. 432) — локальный
- `serviceMoneySummary` (стр. 439) — локальный
- `piggyTargetLabel` (стр. 440) — локальный
- `master` (стр. 444) — локальный
- `piggy` (стр. 449) — локальный
- `owners` (стр. 456) — локальный
- `previewServiceSplit` (стр. 464) — локальный
- `materials` (стр. 469) — локальный
- `net` (стр. 470) — локальный
- `order` (стр. 471) — локальный
- `pipeline` (стр. 472) — локальный
- `piggyType` (стр. 473) — локальный
- `computeMaster` (стр. 480) — локальный
- `computePiggy` (стр. 489) — локальный
- `m` (стр. 496) — локальный
- `p` (стр. 498) — локальный
- `afterMasterPiggy` (стр. 500) — локальный
- `m` (стр. 519) — локальный
- `p` (стр. 523) — локальный
- `isLast` (стр. 527) — локальный
- `claimed` (стр. 528) — локальный
- `ownerPaymentLabel` (стр. 547) — локальный
- `normalizeOwnerPhoneSearchValue` (стр. 554) — локальный
- `numberFromInput` (стр. 560) — локальный
- `toISODate` (стр. 564) — локальный
- `parsed` (стр. 565) — локальный
- `y` (стр. 567) — локальный
- `m` (стр. 568) — локальный
- `d` (стр. 569) — локальный
- `TIME_SLOTS` (стр. 573) — локальный
- `h` (стр. 574) — локальный
- `m` (стр. 575) — локальный
- `OwnerApp` (стр. 582)
- `isAccountant` (стр. 660) — локальный
- `modalMaxHeight` (стр. 661) — локальный
- `financeRoleTitle` (стр. 662) — локальный
- `financeNotificationRole` (стр. 663) — локальный
- `__nowRpt` (стр. 736) — локальный
- `__dowRpt` (стр. 737) — локальный
- `__monRpt` (стр. 738) — локальный
- `__sunRpt` (стр. 739) — локальный
- `parentCategories` (стр. 760) — локальный
- `today` (стр. 925) — локальный
- `adminShiftPhotoUrlsRef` (стр. 952) — локальный
- `clearOwnerResetFlow` (стр. 1044) — локальный
- `nextBoxes` (стр. 1067) — локальный
- `params` (стр. 1105) — локальный
- `params` (стр. 1120) — локальный
- `handlePayOwnerSalary` (стр. 1131) — локальный
- `amount` (стр. 1132) — локальный
- `res` (стр. 1136) — локальный
- `updated` (стр. 1145) — локальный
- `loadPiggyBank` (стр. 1153) — локальный
- `params` (стр. 1157) — локальный
- `qs` (стр. 1160) — локальный
- `data` (стр. 1162) — локальный
- `loadWallet` (стр. 1170) — локальный
- `params` (стр. 1174) — локальный
- `qs` (стр. 1177) — локальный
- `data` (стр. 1179) — локальный
- `handlePiggyWithdraw` (стр. 1185) — локальный
- `f` (стр. 1186) — локальный
- `openPiggyAdjust` (стр. 1208) — локальный
- `current` (стр. 1209) — локальный
- `handlePiggyAdjust` (стр. 1218) — локальный
- `newBalance` (стр. 1219) — локальный
- `delta` (стр. 1221) — локальный
- `syncCountdown` (стр. 1294) — локальный
- `diffMs` (стр. 1295) — локальный
- `intervalId` (стр. 1300) — локальный
- `handleOpenShiftForMasters` (стр. 1330) — локальный
- `saved` (стр. 1339) — локальный
- `ownerNotifications` (стр. 1355) — локальный
- `unreadCount` (стр. 1356) — локальный
- `completedBookings` (стр. 1357) — локальный
- `todayBookings` (стр. 1358) — локальный
- `activeMasters` (стр. 1360) — локальный
- `masterCameOutTodayAt` (стр. 1365) — локальный
- `times` (стр. 1366) — локальный
- `mastersCameOutToday` (стр. 1375) — локальный
- `latestShiftChecklists` (стр. 1376) — локальный
- `latestAdminShiftInspections` (стр. 1377) — локальный
- `latestAdminShiftInspectionKey` (стр. 1378) — локальный
- `activeIds` (стр. 1394) — локальный
- `currentPhotoUrls` (стр. 1407) — локальный
- `missing` (стр. 1408) — локальный
- `next` (стр. 1420) — локальный
- `vv` (стр. 1442) — локальный
- `handler` (стр. 1444) — локальный
- `el` (стр. 1445) — локальный
- `bookingFormBoxes` (стр. 1457) — локальный
- `bookingFormLocationLabel` (стр. 1458) — локальный
- `editBookingLocationLabel` (стр. 1459) — локальный
- `todayRevenue` (стр. 1460) — локальный
- `now` (стр. 1463) — локальный
- `dayOfWeek` (стр. 1464) — локальный
- `diffToSaturday` (стр. 1465) — локальный
- `weekSaturday` (стр. 1466) — локальный
- `weekFriday` (стр. 1469) — локальный
- `isDateInWeek` (стр. 1472) — локальный
- `d` (стр. 1473) — локальный
- `weeklyCompletedBookings` (стр. 1476) — локальный
- `weeklyBookings` (стр. 1477) — локальный
- `weeklyExpenses` (стр. 1478) — локальный
- `weeklyIncomes` (стр. 1479) — локальный
- `totalRevenue` (стр. 1480) — локальный
- `totalExpenses` (стр. 1481) — локальный
- `totalIncomes` (стр. 1482) — локальный
- `profit` (стр. 1483) — локальный
- `averageCheck` (стр. 1484) — локальный
- `activeBookings` (стр. 1485) — локальный
- `pipelineCounts` (стр. 1486) — локальный
- `statusListItems` (стр. 1493) — локальный
- `totalStockValue` (стр. 1498) — локальный
- `washRevenue` (стр. 1501) — локальный
- `detailingRevenue` (стр. 1504) — локальный
- `washExpenses` (стр. 1507) — локальный
- `detailingExpenses` (стр. 1510) — локальный
- `washIncomes` (стр. 1513) — локальный
- `detailingIncomes` (стр. 1516) — локальный
- `resourceGroupLabel` (стр. 1520) — локальный
- `payrollRows` (стр. 1525) — локальный
- `workerPenalties` (стр. 1526) — локальный
- `complaintState` (стр. 1527) — локальный
- `payrollTotal` (стр. 1535) — локальный
- `formatComplaintDate` (стр. 1536) — локальный
- `resetPreviewRows` (стр. 1537) — локальный
- `resetExecuteLocked` (стр. 1551) — локальный
- `glass` (стр. 1553) — локальный
- `bg` (стр. 1554) — локальный
- `text` (стр. 1555) — локальный
- `sub` (стр. 1556) — локальный
- `primary` (стр. 1557) — локальный
- `accent` (стр. 1558) — локальный
- `surface` (стр. 1559) — локальный
- `inputCls` (стр. 1560) — локальный
- `selectCls` (стр. 1561) — локальный
- `tooltipStyle` (стр. 1562) — локальный
- `createDraftId` (стр. 1563) — локальный
- `handleAddBoxDraft` (стр. 1565) — локальный
- `handleRemoveBoxDraft` (стр. 1579) — локальный
- `handleAddServiceDraft` (стр. 1583) — локальный
- `handleRemoveServiceDraft` (стр. 1612) — локальный
- `handleHireWorker` (стр. 1616) — локальный
- `name` (стр. 1617) — локальный
...
### frontend/src/app/context/AppContext.tsx (2013 строк)

- `EMPTY_CONTENT` (стр. 903)
- `timeToMinutes` (стр. 925) — локальный
- `minutesToTime` (стр. 932) — локальный
- `hours` (стр. 933) — локальный
- `minutes` (стр. 934) — локальный
- `buildTimeSlots` (стр. 938) — локальный
- `timeRangesOverlap` (стр. 946) — локальный
- `AppContext` (стр. 950) — локальный
- `normalizeWorker` (стр. 952) — локальный
- `normalizeBootstrap` (стр. 966) — локальный
- `AppProvider` (стр. 990)
- `upcomingDates` (стр. 1014) — локальный
- `todayLabel` (стр. 1015) — локальный
- `tomorrowLabel` (стр. 1016) — локальный
- `applyBootstrap` (стр. 1018) — локальный
- `normalized` (стр. 1019) — локальный
- `refreshBootstrap` (стр. 1047) — локальный
- `bootstrap` (стр. 1048) — локальный
- `handleError` (стр. 1052) — локальный
- `message` (стр. 1053) — локальный
- `restoreSession` (стр. 1058) — локальный
- `bootstrap` (стр. 1060) — локальный
- `refreshActiveSessions` (стр. 1069) — локальный
- `applyTelegramTheme` (стр. 1073) — локальный
- `root` (стр. 1075) — локальный
- `theme` (стр. 1076) — локальный
- `cssVar` (стр. 1079) — локальный
- `tg` (стр. 1086) — локальный
- `logout` (стр. 1103) — локальный
- `loginClient` (стр. 1127) — локальный
- `bootstrap` (стр. 1131) — локальный
- `linkStaff` (стр. 1145) — локальный
- `bootstrap` (стр. 1149) — локальный
- `switchRole` (стр. 1163) — локальный
- `bootstrap` (стр. 1167) — локальный
- `updateClientProfile` (стр. 1181) — локальный
- `payload` (стр. 1182) — локальный
- `saved` (стр. 1183) — локальный
- `remindAdminAboutInactiveClients` (стр. 1187) — локальный
- `response` (стр. 1188) — локальный
- `addClient` (стр. 1192) — локальный
- `created` (стр. 1193) — локальный
- `normalized` (стр. 1194) — локальный
- `updateClientCard` (стр. 1199) — локальный
- `saved` (стр. 1200) — локальный
- `normalized` (стр. 1201) — локальный
- `deleteClient` (стр. 1205) — локальный
- `listDepositClients` (стр. 1210) — локальный
- `items` (стр. 1211) — локальный
- `getDepositOverview` (стр. 1215) — локальный
- `overview` (стр. 1216) — локальный
- `updateDepositSubscription` (стр. 1224) — локальный
- `overview` (стр. 1225) — локальный
- `depositTopUp` (стр. 1233) — локальный
- `txn` (стр. 1234) — локальный
- `depositAdjust` (стр. 1238) — локальный
- `overview` (стр. 1239) — локальный
- `depositRecordWash` (стр. 1247) — локальный
- `overview` (стр. 1248) — локальный
- `depositSettleMonth` (стр. 1256) — локальный
- `overview` (стр. 1257) — локальный
- `downloadDepositExport` (стр. 1265) — локальный
- `downloadDepositExportAll` (стр. 1269) — локальный
- `sendDepositExport` (стр. 1273) — локальный
- `sendDepositExportAll` (стр. 1277) — локальный
- `addBooking` (стр. 1281) — локальный
- `created` (стр. 1282) — локальный
- `existingClient` (стр. 1302) — локальный
- `nextClient` (стр. 1303) — локальный
- `updateBooking` (стр. 1327) — локальный
- `updated` (стр. 1328) — локальный
- `deleteBooking` (стр. 1354) — локальный
- `addBookingService` (стр. 1359) — локальный
- `updated` (стр. 1360) — локальный
- `addBookingAdditionalService` (стр. 1380) — локальный
- `updated` (стр. 1381) — локальный
- `updateBookingAdditionalService` (стр. 1401) — локальный
- `updated` (стр. 1402) — локальный
- `removeBookingAdditionalService` (стр. 1422) — локальный
- `updated` (стр. 1423) — локальный
- `addNotification` (стр. 1443) — локальный
- `created` (стр. 1444) — локальный
- `markNotificationRead` (стр. 1463) — локальный
- `markAllNotificationsRead` (стр. 1468) — локальный
- `addStockItem` (стр. 1482) — локальный
- `created` (стр. 1483) — локальный
- `updateStockItem` (стр. 1487) — локальный
- `updated` (стр. 1488) — локальный
- `writeOffStock` (стр. 1492) — локальный
- `updated` (стр. 1493) — локальный
- `getWriteOffHistory` (стр. 1497) — локальный
- `deleteStockItem` (стр. 1501) — локальный
- `addStockCategory` (стр. 1506) — локальный
- `created` (стр. 1507) — локальный
- `updateStockCategory` (стр. 1511) — локальный
- `updated` (стр. 1512) — локальный
- `deleteStockCategory` (стр. 1516) — локальный
- `addExpense` (стр. 1525) — локальный
- `created` (стр. 1526) — локальный
- `addIncome` (стр. 1530) — локальный
- `created` (стр. 1531) — локальный
- `updateExpense` (стр. 1535) — локальный
- `updated` (стр. 1536) — локальный
- `updateIncome` (стр. 1540) — локальный
- `updated` (стр. 1541) — локальный
- `addPenalty` (стр. 1545) — локальный
- `revokePenalty` (стр. 1550) — локальный
- `revokeAllPenalties` (стр. 1555) — локальный
- `createTelegramLinkCode` (стр. 1560) — локальный
- `created` (стр. 1561) — локальный
- `downloadOwnerExport` (стр. 1565) — локальный
- `fallback` (стр. 1566) — локальный
- `qs` (стр. 1569) — локальный
- `qstr` (стр. 1573) — локальный
- `sendOwnerExportToTelegram` (стр. 1579) — локальный
- `qs` (стр. 1582) — локальный
- `qstr` (стр. 1586) — локальный
- `sendOwnerSummaryReport` (стр. 1592) — локальный
- `response` (стр. 1593) — локальный
- `dispatchOwnerReminders` (стр. 1597) — локальный
- `saveServices` (стр. 1607) — локальный
- `saveBoxes` (стр. 1612) — локальный
- `saveSchedule` (стр. 1616) — локальный
- `saveAdminProfile` (стр. 1620) — локальный
- `saved` (стр. 1621) — локальный
- `saveAdminNotificationSettings` (стр. 1625) — локальный
- `saved` (стр. 1626) — локальный
- `saveWorkerProfile` (стр. 1630) — локальный
- `saved` (стр. 1631) — локальный
- `normalized` (стр. 1632) — локальный
- `saveWorkerNotificationSettings` (стр. 1639) — локальный
- `saved` (стр. 1640) — локальный
- `saveOwnerCompany` (стр. 1647) — локальный
- `saved` (стр. 1648) — локальный
- `saveOwnerNotificationSettings` (стр. 1652) — локальный
- `saved` (стр. 1653) — локальный
- `saveOwnerIntegrations` (стр. 1657) — локальный
- `saved` (стр. 1658) — локальный
- `saveOwnerSecurity` (стр. 1662) — локальный
- `saved` (стр. 1663) — локальный
- `saveWorkerSettings` (стр. 1667) — локальный
- `saved` (стр. 1668) — локальный
- `saveAdminWorkerPayroll` (стр. 1672) — локальный
- `saved` (стр. 1673) — локальный
- `normalized` (стр. 1674) — локальный
- `nextWorker` (стр. 1676) — локальный
- `saveContent` (стр. 1681) — локальный
- `saved` (стр. 1682) — локальный
- `createPayrollEntry` (стр. 1686) — локальный
- `checkConsent` (стр. 1691) — локальный
- `response` (стр. 1692) — локальный
- `submitConsent` (стр. 1696) — локальный
- `listShiftChecklists` (стр. 1700) — локальный
- `entries` (стр. 1701) — локальный
- `submitShiftChecklist` (стр. 1705) — локальный
- `entry` (стр. 1706) — локальный
- `listAdminShiftInspections` (стр. 1713) — локальный
- `entries` (стр. 1714) — локальный
- `submitAdminShiftInspection` (стр. 1722) — локальный
- `entry` (стр. 1729) — локальный
- `openShiftForMasters` (стр. 1740) — локальный
- `entry` (стр. 1741) — локальный
- `hireWorker` (стр. 1752) — локальный
- `created` (стр. 1753) — локальный
- `normalized` (стр. 1754) — локальный
- `fireWorker` (стр. 1764) — локальный
- `resetWorkerPassword` (стр. 1769) — локальный
- `changePassword` (стр. 1776) — локальный
- `requestOwnerDatabaseReset` (стр. 1783) — локальный
- `response` (стр. 1784) — локальный
- `approveOwnerDatabaseReset` (стр. 1801) — локальный
- `response` (стр. 1802) — локальный
- `executeOwnerDatabaseReset` (стр. 1818) — локальный
- `response` (стр. 1819) — локальный
- `getTimeSlotsForDate` (стр. 1827) — локальный
- `parsedDate` (стр. 1828) — локальный
- `day` (стр. 1830) — локальный
- `openMinutes` (стр. 1833) — локальный
- `closeMinutes` (стр. 1834) — локальный
- `durationMinutes` (стр. 1837) — локальный
- `scheduleSlots` (стр. 1838) — локальный
- `candidateBoxes` (стр. 1839) — локальный
- `boxNames` (стр. 1844) — локальный
- `slotStart` (стр. 1846) — локальный
- `slotEnd` (стр. 1848) — локальный
- `bookingStart` (стр. 1855) — локальный
- `getBookingAvailabilityForDate` (стр. 1862) — локальный
- `durationMinutes` (стр. 1863) — локальный
- `params` (стр. 1865) — локальный
- `response` (стр. 1875) — локальный
- `useApp` (стр. 2005)
- `ctx` (стр. 2006) — локальный
- `getWorkerNotificationSettings` (стр. 2011)
...
- `frontend/src/app/components/owner/OwnerApp.tsx` (2026-08-19 10:39)
- `scripts/.project-map-watch.lock` (2026-08-19 09:10)
- `backend/app/main.py` (2026-08-18 16:59)
...
- `backend/app/exports.py` (2026-08-18 14:41)
- `frontend/src/app/context/AppContext.tsx` (2026-08-18 13:25)
- `frontend/src/app/components/admin/AdminApp.tsx` (2026-08-18 13:25)
- `backend/tests/test_booking_logic.py` (2026-08-18 13:25)
- `backend/app/schemas.py` (2026-08-18 13:25)
- `backend/app/models.py` (2026-08-18 13:25)
