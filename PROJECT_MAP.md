...
> Автосгенерировано 2026-08-18 11:48 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
- Файлов кода: **275**
- Строк кода: **100 370**
- По расширениям: `.js`: 3, `.mjs`: 3, `.py`: 61, `.ts`: 19, `.tsx`: 189
...
### backend/app/main.py (21030 строк)
...
```
  ...
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 12391)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 12463)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 12540)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 12620)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 12698)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 12764)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 12800)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 12848)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 12897)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 12928)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 12964)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 13006)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 13128)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 13174)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 13256)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 13412)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 13451)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 13577)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 13623)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 13681)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 13729)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 13793)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 13873)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 14374)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 14516)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 14962)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 15006)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 15041)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 15068)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 15094)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 15112)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 15124)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 15142)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 15155)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 15234)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 15319)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 15379)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 15591)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 15687)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 15767)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 15835)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 15970)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 16050)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 16200)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 16342)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 16488)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 16542)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 16618)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 16676)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 16724)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 16798)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 16822)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 16882)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 16924)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 16948)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 16972)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 17007)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 17062)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 17130)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 17148)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 17181)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 17214)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 17226)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 17250)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 17275)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 17311)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 17414)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 17514)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 17584)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 17814)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 17910)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 18135)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 18214)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 18387)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 18689)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 18703)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 19041)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 19458)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 19860)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 20051)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 20303)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 20521)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 20659)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 20719)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 20913)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 20921)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 20933)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 20957)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 20965)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 20973)
```
...
- `_PartialBroadcastError._parse_date_strdef _parse_date_str(s: str) -> date | None: try: if "." in s: parts = s.split(".") return date(int(parts[2]), int(parts[1]), int(parts[0])) return date.fromisoformat(s) except (Val` (стр. 13895)
- `_PartialBroadcastError._in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date_str(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_t` (стр. 13919)
- `_deposit_balancedef _deposit_balance(db: Session, client_id: str) -> Decimal: return sum( ( t.amount for t in db.scalars( select(DepositTransaction).where(DepositTransaction.client_id == client_id` (стр. 14634)
- `_deposit_add_transactiondef _deposit_add_transaction( db: Session, client_id: str, txn_type: str, amount: float, description: str, *, date: str, booking_id: str | None = None, created_by_id: str | None = ` (стр. 14646)
- `_deposit_txn_payloaddef _deposit_txn_payload(db: Session, txn: DepositTransaction) -> DepositTransactionPayload: car, plate = "", "" if txn.booking_id: booking = db.get(Booking, txn.booking_id) if boo` (стр. 14675)
- `_deposit_month_labeldef _deposit_month_label() -> str: return datetime.now().strftime("%m.%Y")` (стр. 14698)
- `_deposit_month_ofdef _deposit_month_of(date_str: str) -> str: parts = (date_str or "").split(".") if len(parts) == 3: return f"{parts[1]}.{parts[2]}" return ""` (стр. 14700)
- `_deposit_month_wash_total_fordef _deposit_month_wash_total_for(db: Session, client_id: str, month: str) -> float: total = 0.0 rows = db.scalars( select(Booking).where( Booking.client_id == client_id, Booking.p` (стр. 14707)
- `_deposit_plan_keydef _deposit_plan_key(value: str | None) -> str: plan = (value or "").strip() or "fee" return plan if plan in {"fee", "washes", "per_wash", "unlimited"} else "fee"` (стр. 14723)
- `_deposit_prev_monthdef _deposit_prev_month(month: str) -> str: try: m, y = month.split(".") yi = int(y) - (1 if int(m) == 1 else 0) mi = 12 if int(m) == 1 else int(m) - 1 return f"{mi:02d}.{yi}" exce` (стр. 14728)
- `_deposit_month_wash_count_fordef _deposit_month_wash_count_for(db: Session, client_id: str, month: str) -> int: rows = db.scalars( select(Booking).where( Booking.client_id == client_id, Booking.payment_type ==` (стр. 14738)
- ...ещё 28
...
### frontend/src/app/components/owner/OwnerApp.tsx (12284 строк)

- `EXPENSE_CATEGORIES` (стр. 226) — локальный
- `STOCK_UNITS` (стр. 227) — локальный
- `SERVICE_TYPE_OPTIONS` (стр. 228) — локальный
- `ownerBookingStatusRequiresScheduledSlot` (стр. 239) — локальный
- `employeeRoleLabel` (стр. 242) — локальный
- `ownerServiceResourceGroup` (стр. 248) — локальный
- `ownerDefaultBoxForService` (стр. 252) — локальный
- `rg` (стр. 253) — локальный
- `match` (стр. 254) — локальный
- `ownerBookingBoxes` (стр. 258) — локальный
- `ownerLocationLabel` (стр. 266) — локальный
- `parseOwnerBookingMinutes` (стр. 270) — локальный
- `match` (стр. 271) — локальный
- `hours` (стр. 273) — локальный
- `minutes` (стр. 274) — локальный
- `OWNER_CALENDAR_WEEKDAYS` (стр. 279) — локальный
- `OWNER_CALENDAR_MONTHS` (стр. 280) — локальный
- `OWNER_CALENDAR_DEFAULT_OPEN` (стр. 284) — локальный
- `OWNER_CALENDAR_DEFAULT_CLOSE` (стр. 285) — локальный
- `ownerScheduleTimeToMinutes` (стр. 287) — локальный
- `ownerMonthTitle` (стр. 291) — локальный
- `ownerBuildMonthCells` (стр. 295) — локальный
- `year` (стр. 296) — локальный
- `month` (стр. 297) — локальный
- `first` (стр. 298) — локальный
- `offset` (стр. 299) — локальный
- `daysInMonth` (стр. 300) — локальный
- `date` (стр. 306) — локальный
- `ownerCalendarDayHours` (стр. 315) — локальный
- `parsedDate` (стр. 316) — локальный
- `daySchedule` (стр. 320) — локальный
- `open` (стр. 324) — локальный
- `close` (стр. 325) — локальный
- `OWNER_CALENDAR_LOAD_COLORS` (стр. 329) — локальный
- `ownerCalendarLoadTone` (стр. 335) — локальный
- `ratio` (стр. 337) — локальный
- `ownerGroupBookingsByHour` (стр. 347) — локальный
- `timed` (стр. 352) — локальный
- `hourLabel` (стр. 355) — локальный
- `slotEnd` (стр. 356) — локальный
- `slotBookings` (стр. 357) — локальный
- `start` (стр. 359) — локальный
- `ownerOpenBookingDetail` (стр. 371) — локальный
- `ownerBookingBlocksBox` (стр. 380) — локальный
- `nextStart` (стр. 383) — локальный
- `existingStart` (стр. 384) — локальный
- `nextEnd` (стр. 386) — локальный
- `existingEnd` (стр. 387) — локальный
- `ownerPickDefaultBookingBox` (стр. 391) — локальный
- `resourceGroup` (стр. 400) — локальный
- `preferred` (стр. 401) — локальный
- `fallback` (стр. 402) — локальный
- `candidates` (стр. 403) — локальный
- `serviceResourceGroupForCategory` (стр. 408) — локальный
- `numberInputValue` (стр. 412) — локальный
- `ORDER_STEPS` (стр. 426) — локальный
- `serviceMoneySummary` (стр. 433) — локальный
- `piggyTargetLabel` (стр. 434) — локальный
- `master` (стр. 438) — локальный
- `piggy` (стр. 443) — локальный
- `owners` (стр. 450) — локальный
- `previewServiceSplit` (стр. 458) — локальный
- `materials` (стр. 463) — локальный
- `net` (стр. 464) — локальный
- `order` (стр. 465) — локальный
- `pipeline` (стр. 466) — локальный
- `piggyType` (стр. 467) — локальный
- `computeMaster` (стр. 474) — локальный
- `computePiggy` (стр. 483) — локальный
- `m` (стр. 490) — локальный
- `p` (стр. 492) — локальный
- `afterMasterPiggy` (стр. 494) — локальный
- `m` (стр. 513) — локальный
- `p` (стр. 517) — локальный
- `isLast` (стр. 521) — локальный
- `claimed` (стр. 522) — локальный
- `ownerPaymentLabel` (стр. 541) — локальный
- `normalizeOwnerPhoneSearchValue` (стр. 548) — локальный
- `numberFromInput` (стр. 554) — локальный
- `toISODate` (стр. 558) — локальный
- `parsed` (стр. 559) — локальный
- `y` (стр. 561) — локальный
- `m` (стр. 562) — локальный
- `d` (стр. 563) — локальный
- `TIME_SLOTS` (стр. 567) — локальный
- `h` (стр. 568) — локальный
- `m` (стр. 569) — локальный
- `OwnerApp` (стр. 576)
- `isAccountant` (стр. 654) — локальный
- `modalMaxHeight` (стр. 655) — локальный
- `financeRoleTitle` (стр. 656) — локальный
- `financeNotificationRole` (стр. 657) — локальный
- `__nowRpt` (стр. 730) — локальный
- `__dowRpt` (стр. 731) — локальный
- `__monRpt` (стр. 732) — локальный
- `__sunRpt` (стр. 733) — локальный
- `parentCategories` (стр. 754) — локальный
- `today` (стр. 902) — локальный
- `adminShiftPhotoUrlsRef` (стр. 929) — локальный
- `clearOwnerResetFlow` (стр. 1021) — локальный
- `nextBoxes` (стр. 1044) — локальный
- `params` (стр. 1078) — локальный
- `params` (стр. 1093) — локальный
- `handlePayOwnerSalary` (стр. 1104) — локальный
- `amount` (стр. 1105) — локальный
- `res` (стр. 1109) — локальный
- `updated` (стр. 1118) — локальный
- `loadPiggyBank` (стр. 1126) — локальный
- `params` (стр. 1130) — локальный
- `qs` (стр. 1133) — локальный
- `data` (стр. 1135) — локальный
- `loadWallet` (стр. 1143) — локальный
- `params` (стр. 1147) — локальный
- `qs` (стр. 1150) — локальный
- `data` (стр. 1152) — локальный
- `handlePiggyWithdraw` (стр. 1158) — локальный
- `f` (стр. 1159) — локальный
- `openPiggyAdjust` (стр. 1181) — локальный
- `current` (стр. 1182) — локальный
- `handlePiggyAdjust` (стр. 1191) — локальный
- `newBalance` (стр. 1192) — локальный
- `delta` (стр. 1194) — локальный
- `syncCountdown` (стр. 1267) — локальный
- `diffMs` (стр. 1268) — локальный
- `intervalId` (стр. 1273) — локальный
- `handleOpenShiftForMasters` (стр. 1303) — локальный
- `saved` (стр. 1312) — локальный
- `ownerNotifications` (стр. 1328) — локальный
- `unreadCount` (стр. 1329) — локальный
- `completedBookings` (стр. 1330) — локальный
- `todayBookings` (стр. 1331) — локальный
- `activeMasters` (стр. 1333) — локальный
- `masterCameOutTodayAt` (стр. 1338) — локальный
- `times` (стр. 1339) — локальный
- `mastersCameOutToday` (стр. 1348) — локальный
- `latestShiftChecklists` (стр. 1349) — локальный
- `latestAdminShiftInspections` (стр. 1350) — локальный
- `latestAdminShiftInspectionKey` (стр. 1351) — локальный
- `activeIds` (стр. 1367) — локальный
- `currentPhotoUrls` (стр. 1380) — локальный
- `missing` (стр. 1381) — локальный
- `next` (стр. 1393) — локальный
- `vv` (стр. 1415) — локальный
- `handler` (стр. 1417) — локальный
- `el` (стр. 1418) — локальный
- `bookingFormBoxes` (стр. 1430) — локальный
- `bookingFormLocationLabel` (стр. 1431) — локальный
- `editBookingLocationLabel` (стр. 1432) — локальный
- `todayRevenue` (стр. 1433) — локальный
- `now` (стр. 1436) — локальный
- `dayOfWeek` (стр. 1437) — локальный
- `diffToSaturday` (стр. 1438) — локальный
- `weekSaturday` (стр. 1439) — локальный
- `weekFriday` (стр. 1442) — локальный
- `isDateInWeek` (стр. 1445) — локальный
- `d` (стр. 1446) — локальный
- `weeklyCompletedBookings` (стр. 1449) — локальный
- `weeklyBookings` (стр. 1450) — локальный
- `weeklyExpenses` (стр. 1451) — локальный
- `weeklyIncomes` (стр. 1452) — локальный
- `totalRevenue` (стр. 1453) — локальный
- `totalExpenses` (стр. 1454) — локальный
- `totalIncomes` (стр. 1455) — локальный
- `profit` (стр. 1456) — локальный
- `averageCheck` (стр. 1457) — локальный
- `activeBookings` (стр. 1458) — локальный
- `pipelineCounts` (стр. 1459) — локальный
- `statusListItems` (стр. 1466) — локальный
- `totalStockValue` (стр. 1471) — локальный
- `washRevenue` (стр. 1474) — локальный
- `detailingRevenue` (стр. 1477) — локальный
- `washExpenses` (стр. 1480) — локальный
- `detailingExpenses` (стр. 1483) — локальный
- `washIncomes` (стр. 1486) — локальный
- `detailingIncomes` (стр. 1489) — локальный
- `resourceGroupLabel` (стр. 1493) — локальный
- `payrollRows` (стр. 1498) — локальный
- `workerPenalties` (стр. 1499) — локальный
- `complaintState` (стр. 1500) — локальный
- `payrollTotal` (стр. 1508) — локальный
- `formatComplaintDate` (стр. 1509) — локальный
- `resetPreviewRows` (стр. 1510) — локальный
- `resetExecuteLocked` (стр. 1524) — локальный
- `glass` (стр. 1526) — локальный
- `bg` (стр. 1527) — локальный
- `text` (стр. 1528) — локальный
- `sub` (стр. 1529) — локальный
- `primary` (стр. 1530) — локальный
- `accent` (стр. 1531) — локальный
- `surface` (стр. 1532) — локальный
- `inputCls` (стр. 1533) — локальный
- `selectCls` (стр. 1534) — локальный
- `tooltipStyle` (стр. 1535) — локальный
- `createDraftId` (стр. 1536) — локальный
- `handleAddBoxDraft` (стр. 1538) — локальный
- `handleRemoveBoxDraft` (стр. 1552) — локальный
- `handleAddServiceDraft` (стр. 1556) — локальный
- `handleRemoveServiceDraft` (стр. 1585) — локальный
- `handleHireWorker` (стр. 1589) — локальный
- `name` (стр. 1590) — локальный
...
- `backend/app/main.py` (2026-08-18 14:41)
- `frontend/src/app/context/AppContext.tsx` (2026-08-18 13:25)
- `frontend/src/app/components/worker/WorkerApp.tsx` (2026-08-18 13:25)
- `frontend/src/app/components/owner/OwnerApp.tsx` (2026-08-18 13:25)
