...
> Автосгенерировано 2026-08-28 11:44 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
### backend/app/main.py (23324 строк)

Роуты (133):

```
  `POST /api/auth/client` -> `register_or_login_client` (декоратор: стр. 5736)
  `POST /api/auth/staff/login` -> `staff_login` (декоратор: стр. 5849)
  `POST /api/auth/telegram` -> `authenticate_via_telegram` (декоратор: стр. 5899)
  `POST /api/auth/staff/link` -> `link_staff_account` (декоратор: стр. 5921)
  `POST /api/auth/telegram-owner` -> `authenticate_primary_owner_via_telegram` (декоратор: стр. 5971)
  `POST /api/auth/switch-role` -> `switch_role` (декоратор: стр. 6008)
  `POST /api/owner/database-reset/start` -> `start_owner_database_reset` (декоратор: стр. 7854)
  `POST /api/owner/database-reset/approve` -> `approve_owner_database_reset` (декоратор: стр. 7910)
  `POST /api/owner/database-reset/execute` -> `execute_owner_database_reset` (декоратор: стр. 7964)
  `GET /api/owner/exports/{kind}` -> `download_owner_export` (декоратор: стр. 8835)
  `POST /api/owner/exports/{kind}/telegram` -> `send_owner_export_to_telegram` (декоратор: стр. 8865)
  `POST /api/owner/reports/{period}/{segment}/telegram` -> `send_owner_summary_report_to_telegram` (декоратор: стр. 8895)
  `PATCH /api/clients/me` -> `update_client_me` (декоратор: стр. 10091)
  `DELETE /api/clients/{client_id}` -> `delete_client` (декоратор: стр. 10171)
  `PATCH /api/clients/{client_id}/card` -> `update_client_card` (декоратор: стр. 10196)
  `POST /api/clients` -> `create_client` (декоратор: стр. 10296)
  `GET /api/health` -> `health` (декоратор: стр. 10353)
  `GET /api/content` -> `get_public_content` (декоратор: стр. 10510)
  `PUT /api/content` -> `save_content` (декоратор: стр. 10524)
  `POST /api/upload` -> `upload_file` (декоратор: стр. 10588)
  `GET /api/uploads/{filename}` -> `serve_upload` (декоратор: стр. 10640)
  `POST /api/contact` -> `submit_contact` (декоратор: стр. 10661)
  `POST settings.telegram_webhook_path` -> `handle_telegram_webhook` (декоратор: стр. 10711)
  `POST /api/telegram/webhook/sync` -> `resync_telegram_webhook` (декоратор: стр. 10761)
  `GET /api/stock-categories` -> `list_stock_categories` (декоратор: стр. 10843)
  `POST /api/stock-categories` -> `create_stock_category` (декоратор: стр. 10856)
  `PATCH /api/stock-categories/{category_id}` -> `update_stock_category` (декоратор: стр. 10881)
  `DELETE /api/stock-categories/{category_id}` -> `delete_stock_category` (декоратор: стр. 10914)
  `GET /api/bookings/availability` -> `get_booking_availability` (декоратор: стр. 10948)
  `POST /api/bookings` -> `create_booking` (декоратор: стр. 11019)
  `PATCH /api/bookings/{booking_id}` -> `update_booking` (декоратор: стр. 12284)
  `DELETE /api/bookings/{booking_id}` -> `delete_booking` (декоратор: стр. 12964)
  `POST /api/bookings/{booking_id}/services` -> `add_booking_service` (декоратор: стр. 13064)
  `POST /api/bookings/{booking_id}/additional-services` -> `add_booking_additional_service` (декоратор: стр. 13130)
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 13252)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 13324)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 13408)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 13488)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 13566)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 13632)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 13668)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 13716)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 13765)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 13796)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 13832)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 13874)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 13996)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 14042)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 14124)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 14280)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 14319)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 14445)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 14491)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 14557)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 14605)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 14669)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 14762)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 15294)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 15557)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 16003)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 16047)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 16082)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 16109)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 16135)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 16153)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 16165)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 16183)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 16196)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 16275)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 16360)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 16420)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 16645)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 16741)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 16821)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 16889)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 17028)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 17110)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 17260)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 17402)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 17548)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 17602)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 17678)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 17736)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 17784)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 17858)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 17882)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 17942)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 17984)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 18008)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 18032)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 18067)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 18138)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 18236)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 18255)
  `POST /api/owner/integrations/google/invites` -> `create_google_calendar_invite` (декоратор: стр. 18292)
  `DELETE /api/owner/integrations/google/connections/{connection_id}` -> `delete_google_calendar_connection` (декоратор: стр. 18327)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 18353)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 18386)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 18398)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 18422)
  `GET /api/cron/reminders` -> `run_reminders_cron` (декоратор: стр. 18447)
  `POST /api/owner/inactive-clients/remind-admin` -> `remind_admin_about_inactive_clients` (декоратор: стр. 18479)
  `POST /api/owner/reminders/dispatch` -> `dispatch_owner_booking_reminders` (декоратор: стр. 18537)
  `GET /api/cron/reports` -> `run_reports_cron` (декоратор: стр. 18556)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 18602)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 18638)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 18741)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 18841)
  `GET /api/owner/outsource/payroll` -> `get_owner_outsource_payroll` (декоратор: стр. 18910)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 18980)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 19254)
  `DELETE /api/payroll/entries/{entry_id}` -> `delete_payroll_entry` (декоратор: стр. 19412)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 19502)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 19727)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 19814)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 19987)
  `GET /api/owner/money-flow` -> `get_owner_money_flow` (декоратор: стр. 20301)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 20804)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 20818)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 21207)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 21656)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 22064)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 22318)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 22566)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 22815)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 22953)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 23013)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 23207)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 23215)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 23227)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 23251)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 23259)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 23267)
```
...
- `_repair_text_valuedef _repair_text_value(value: str) -> str: if not value: return value # Ремонтируем только явный mojibake (UTF-8 -> cp1251), не трогаем корректную кириллицу # Маркеры mojibake: "Ð"` (стр. 2739)
- `_repair_nested_textdef _repair_nested_text(value): if isinstance(value, str):` (стр. 2756)
- `_repair_model_text_fieldsdef _repair_model_text_fields(db: Session, model, fields: tuple[str, ...]) -> bool: changed = False for item in db.scalars(select(model)).all():` (стр. 2776)
- `_sanitize_notification_messagedef _sanitize_notification_message(message: str) -> str: fixed = _repair_text_value(message).strip() for source, target in { "вЂў": "•", "в€¢": "•", "вВў": "•", "â€¢": "•", "вЂ”": ` (стр. 2804)
- `_repair_text_datadef _repair_text_data(db: Session) -> None: changed = False changed |= _repair_model_text_fields( db, StaffUser, ("name", "city", "experience", "specialty", "about"), ) changed |= ` (стр. 2838)
- `_settingdef _setting(db: Session, key: str, default: dict) -> dict: row = db.get(AppSetting, key) if row: return row.value row = AppSetting(key=key, value=default) db.add(row) db.flush() r` (стр. 2984)
- `_merge_setting_dictdef _merge_setting_dict(value: Any, default: dict[str, Any]) -> dict[str, Any]: if not isinstance(value, dict):` (стр. 3004)
- `_normalize_client_vehiclesdef _normalize_client_vehicles( vehicles: list[ClientVehiclePayload] | list[dict[str, Any]] | None, *, fallback_car: str = "", fallback_plate: str = "",` (стр. 3028)
- `_client_vehicles_mapdef _client_vehicles_map(db: Session) -> dict[str, Any]: return _setting(db, "client_vehicles", {})` (стр. 3140)
- `_client_vehicles_payloaddef _client_vehicles_payload(db: Session, client: Client) -> list[ClientVehiclePayload]: raw = _client_vehicles_map(db).get(client.id, []) return _normalize_client_vehicles( raw, f` (стр. 3148)
- `_save_client_vehiclesdef _save_client_vehicles( db: Session, client_id: str, vehicles: list[ClientVehiclePayload]` (стр. 3162)
- `_client_phone_verifications_mapdef _client_phone_verifications_map(db: Session) -> dict[str, Any]: value = _setting(db, CLIENT_PHONE_VERIFICATIONS_KEY, {}) return value if isinstance(value, dict) else {}` (стр. 3182)
- `_client_verified_phone_digitsdef _client_verified_phone_digits(db: Session, telegram_id: str | None) -> str | None: if not telegram_id: return None entry = _client_phone_verifications_map(db).get(str(telegram_` (стр. 3192)
- `_client_phone_is_verifieddef _client_phone_is_verified(db: Session, telegram_id: str | None, phone: str) -> bool: if not phone.strip():` (стр. 3212)
- `_require_client_phone_verificationdef _require_client_phone_verification( db: Session, telegram_id: str | None, phone: str` (стр. 3238)
- `_client_payloaddef _client_payload(client: Client | None) -> ClientProfilePayload | None: if client is None: return None with Session(engine) as vehicles_db: vehicles = _client_vehicles_payload(v` (стр. 3260)
- `_client_summary_payloaddef _client_summary_payload( client: Client, db: Session | None = None` (стр. 3300)
- `_booking_status_labeldef _booking_status_label(status_value: str) -> str: return { "new": "Новая заявка", "confirmed": "Подтверждена", "scheduled": "Запланирована", "in_progress": "В работе", "complete` (стр. 3358)
- `_booking_status_short_labeldef _booking_status_short_label(status_value: str) -> str: return { "new": "Новая", "confirmed": "Подтв.", "scheduled": "Запл.", "in_progress": "В работе", "completed": "Завершена"` (стр. 3384)
- `_format_local_datetimedef _format_local_datetime(value: datetime) -> str: return _as_utc(value).astimezone().strftime("%d.%m.%Y %H:%M")` (стр. 3410)
- `_parse_booking_datetimedef _parse_booking_datetime(date_value: str, time_value: str) -> datetime | None: raw = f"{date_value.strip()} {time_value.strip()}" for fmt in ("%d.%m.%Y %H:%M", "%Y-%m-%d %H:%M")` (стр. 3418)
- `_py_weekday_to_schedule_indexdef _py_weekday_to_schedule_index(py_weekday: int) -> int: # Конвенция day_index (сид и фронт getScheduleDayIndex=(getDay()+1)%7): Сб=0, Вс=1, Пн=2..Пт=6. # Python weekday(): Пн=0.` (стр. 3438)
- `_parse_time_to_minutesdef _parse_time_to_minutes(time_value: str) -> int | None: raw = time_value.strip() if len(raw) != 5 or raw[2] != ":": return None try: hours = int(raw[:2]) minutes = int(raw[3:]) ` (стр. 3449)
- `_today_labeldef _today_label() -> str: return datetime.now().strftime("%d.%m.%Y")` (стр. 3477)
- `_build_schedule_slotsdef _build_schedule_slots( open_minutes: int, close_minutes: int, step_minutes: int = 30` (стр. 3485)
- `_booking_requires_scheduled_slotdef _booking_requires_scheduled_slot(status_value: str) -> bool: return status_value in BOOKING_ACTIVE_STATUSES` (стр. 3509)
- `_booking_slot_fields_changeddef _booking_slot_fields_changed(booking: Booking, updates: dict) -> bool: if "date" in updates and (updates.get("date") or "").strip() != (booking.date or "").strip():` (стр. 3517)
- `_booking_time_rangedef _booking_time_range( date_value: str, time_value: str, duration: int` (стр. 3537)
- `_time_ranges_overlapdef _time_ranges_overlap( start_at: datetime, end_at: datetime, other_start_at: datetime, other_end_at: datetime,` (стр. 3555)
- `_ensure_booking_datetime_not_in_pastdef _ensure_booking_datetime_not_in_past(date_value: str, time_value: str, role: str) -> None: if role in {"admin", "owner"}: return scheduled_at = _parse_booking_datetime(date_val` (стр. 3573)
- `_ensure_booking_within_scheduledef _ensure_booking_within_schedule( db: Session, date_value: str, time_value: str, duration: int` (стр. 3607)
- `_box_is_availabledef _box_is_available( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str,` (стр. 3681)
- `_pick_available_boxdef _pick_available_box( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, resource_group: str | None = None, preferred_box: str | None = Non` (стр. 3705)
- `_booking_slot_availabilitydef _booking_slot_availability( db: Session, *, date_value: str, duration: int, service_id: str | None = None, resource_group: str | None = None,` (стр. 3767)
- `_ensure_booking_has_no_conflictsdef _ensure_booking_has_no_conflicts( db: Session, *, booking_id: str | None, date_value: str, time_value: str, duration: int, box: str, worker_ids: set[str], …` (стр. 3919)
- `_load_penaltiesdef _load_penalties( db: Session, *, worker_ids: set[str] | None = None` (стр. 3963)
- `_complaints_by_workerdef _complaints_by_worker(penalties: list[Penalty]) -> dict[str, list[Penalty]]: grouped: dict[str, list[Penalty]] = {} for penalty in penalties: grouped.setdefault(penalty.worker_` (стр. 3989)
- `_normalize_worker_rulesdef _normalize_worker_rules(db: Session) -> None: changed = False workers = db.scalars(select(StaffUser).where(StaffUser.role == "worker")).all() for worker in workers: capped_perc` (стр. 4003)
- `_worker_payloaddef _worker_payload(worker: StaffUser) -> WorkerPayload: return WorkerPayload( id=worker.id, role=worker.role, # type: ignore[arg-type] name=worker.name, experience=worker.experien` (стр. 4057)
- `_payroll_entry_payloaddef _payroll_entry_payload(entry: PayrollEntry, actor_name: str) -> PayrollEntryPayload: return PayrollEntryPayload( id=entry.id, workerId=entry.worker_id, kind=entry.kind, # type:` (стр. 4097)
- `_worker_payroll_summariesdef _worker_payroll_summaries( db: Session, workers: list[StaffUser], complaints_by_worker: dict[str, list[Penalty]],` (стр. 4125)
- `_worker_payroll_summaries_from_datadef _worker_payroll_summaries_from_data( db: Session, workers: list[StaffUser], completed_bookings: list[Booking], entries: list[PayrollEntry], complaints_by_worker: dict[str, list` (стр. 4173)
- `_worker_payload_with_payrolldef _worker_payload_with_payroll( worker: StaffUser, payroll_summaries: dict[str, WorkerPayrollSummaryPayload] | None = None,` (стр. 4353)
- `_booking_payloaddef _booking_payload( booking: Booking, complaints_by_worker: dict[str, list[Penalty]] | None = None` (стр. 4377)
- `_notification_payloaddef _notification_payload(notification: Notification) -> NotificationPayload: return NotificationPayload( id=notification.id, recipientRole=notification.recipient_role, # type: ign` (стр. 4547)
- `_stock_payloaddef _stock_payload(item: StockItem) -> StockItemPayload: return StockItemPayload( id=item.id, name=item.name, qty=item.qty, unit=item.unit, unitPrice=item.unit_price, category=item` (стр. 4569)
- `_expense_payloaddef _expense_payload(expense: Expense) -> ExpensePayload: return ExpensePayload( id=expense.id, title=expense.title, amount=expense.amount, category=expense.category, date=expense.` (стр. 4585)
- `_penalty_payloaddef _penalty_payload(penalty: Penalty) -> PenaltyPayload: worker_name = penalty.worker.name if penalty.worker else "" return PenaltyPayload( id=penalty.id, workerId=penalty.worker_` (стр. 4609)
- `_service_payloaddef _service_payload(service: Service) -> ServicePayload: return ServicePayload( id=service.id, name=service.name, category=service.category, price=service.price, duration=service.` (стр. 4639)
- `_box_payloaddef _box_payload(box: Box) -> BoxPayload: return BoxPayload( id=box.id, name=box.name, resourceGroup=(box.resource_group or DEFAULT_RESOURCE_GROUP).strip() or DEFAULT_RESOURCE_GROU` (стр. 4683)
- `_visible_boxesdef _visible_boxes(db: Session) -> list[Box]: boxes = db.scalars(select(Box).order_by(Box.name.asc())).all() wash_order_map = {name: index for index, name in enumerate(WASH_BOX_NAM` (стр. 4707)
- `box_orderdef box_order(box: Box) -> tuple[int, int, str, str]: resource_group = _resource_group_key( box.resource_group or _default_box_resource_group(box) ) if resource_group == DETAILING_` (стр. 4717)
- `_schedule_payloaddef _schedule_payload(entry: ScheduleEntry) -> SchedulePayload: return SchedulePayload( dayIndex=entry.day_index, day=entry.day_label, open=entry.open_time, close=entry.close_time,` (стр. 4753)
- `_settings_payloaddef _settings_payload(db: Session) -> SettingsBundlePayload: admin_profile_default = { "name": "Администратор", "email": "", "phone": "", "telegramChatId": "", } admin_notification` (стр. 4773)
- `_empty_settings_payloaddef _empty_settings_payload() -> SettingsBundlePayload: return SettingsBundlePayload( adminProfile=AdminProfilePayload( name="", email="", phone="", telegramChatId="" ), adminNotif` (стр. 4993)
- `_scoped_settings_payloaddef _scoped_settings_payload( db: Session, role: str, actor_id: str` (стр. 5073)
- `_session_payloaddef _session_payload(session_data: dict) -> SessionPayload: return SessionPayload( role=session_data["role"], actorId=session_data["actorId"], sessionId=session_data.get("sessionId` (стр. 5159)
- `_mark_overdue_bookings_for_admin_reviewdef _mark_overdue_bookings_for_admin_review(db: Session) -> None: now_local = datetime.now().replace(second=0, microsecond=0) changed = False for booking in db.scalars( select(Book` (стр. 5179)
- `_build_bootstrapdef _build_bootstrap(db: Session, session_data: dict) -> BootstrapPayload: role = session_data["role"] actor_id = session_data["actorId"] _mark_overdue_bookings_for_admin_review(db` (стр. 5223)
- `_resolve_user_from_init_datadef _resolve_user_from_init_data(authorization: str, db: Session) -> dict | None: try: validated = validate_telegram_init_data( authorization, settings.telegram_bot_token, max_age_` (стр. 5545)
- `_require_sessiondef _require_session( authorization: str | None = Header(default=None), db: Session = Depends(get_db),` (стр. 5666)
- `_extract_telegram_id_from_init_datadef _extract_telegram_id_from_init_data(authorization: str) -> str: if not authorization: raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing initData") t` (стр. 5697)
- `_ensure_staff_roledef _ensure_staff_role(session_data: dict, allowed: set[str]) -> None: if session_data["role"] not in allowed: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Fo` (стр. 6041)
- `_validated_booking_workersdef _validated_booking_workers( db: Session, workers: list[BookingWorkerPayload]` (стр. 6051)
- `_booking_payload_for_responsedef _booking_payload_for_response(db: Session, booking: Booking) -> BookingPayload: worker_ids = {link.worker_id for link in booking.worker_links} penalties = _load_penalties(db, w` (стр. 6147)
- `_sync_booking_workersdef _sync_booking_workers( db: Session, booking: Booking, workers: list[BookingWorkerPayload]` (стр. 6159)
- `_sync_booking_materialsdef _sync_booking_materials( db: Session, booking: Booking, materials: list[BookingMaterialPayload]` (стр. 6189)
- `_send_telegram_safedef _send_telegram_safe(chat_id: str | None, text: str) -> None: if not chat_id: logger.warning("Пропущена отправка Telegram-уведомления: у получателя нет chat_id") return try: sen` (стр. 6208)
- `_telegram_display_namedef _telegram_display_name(telegram_user: dict, fallback: str) -> str: first_name = str(telegram_user.get("first_name") or "").strip() last_name = str(telegram_user.get("last_name"` (стр. 6228)
- `_owner_two_factor_recipientdef _owner_two_factor_recipient(db: Session) -> StaffUser: owner = _primary_owner(db) if owner is None: raise HTTPException( status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail` (стр. 6244)
- `_all_active_ownersdef _all_active_owners(db: Session) -> list[StaffUser]: """Возвращает всех активных владельцев, отсортированных по created_at asc.""" return list( db.scalars( select(StaffUser) .wh` (стр. 6274)
- `_all_owner_telegram_recipientsdef _all_owner_telegram_recipients(db: Session) -> list[StaffUser]: """Возвращает всех владельцев с непустым telegram_chat_id, отсортированных по created_at asc.""" return list( db` (стр. 6296)
- `_booking_reminder_target_datedef _booking_reminder_target_date(days_ahead: int = 1) -> str: return (datetime.now() + timedelta(days=days_ahead)).strftime("%d.%m.%Y")` (стр. 6324)
- `_worker_notification_settings_mapdef _worker_notification_settings_map(db: Session) -> dict[str, dict[str, Any]]: return _setting(db, "worker_notification_settings", {})` (стр. 6332)
- `_booking_reminder_statedef _booking_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, BOOKING_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6340)
- `_return_reminder_statedef _return_reminder_state(db: Session) -> dict[str, Any]: return _setting(db, RETURN_REMINDER_STATE_KEY, {"deliveries": {}})` (стр. 6348)
- `_shift_checklists_statedef _shift_checklists_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, SHIFT_CHECKLISTS_KEY, []) return value if isinstance(value, list) else []` (стр. 6356)
- `_admin_shift_inspections_statedef _admin_shift_inspections_state(db: Session) -> list[dict[str, Any]]: value = _setting(db, ADMIN_SHIFT_INSPECTIONS_KEY, []) return value if isinstance(value, list) else []` (стр. 6366)
- `_compute_shift_attendancedef _compute_shift_attendance( inspections: list[dict], worker_id: str, date_from: date, date_to: date,` (стр. 6376)
- `_period_to_date_rangedef _period_to_date_range(period: str) -> tuple[date, date]: """ Преобразует строковый период в диапазон дат (date_from, date_to). - ``week`` → последние 7 дней - ``month`` → после` (стр. 6504)
- `_admin_shift_owner_bot_statedef _admin_shift_owner_bot_state(db: Session) -> dict[str, Any]: value = _setting(db, ADMIN_SHIFT_OWNER_BOT_STATE_KEY, {"pendingIssueByChat": {}}) return value if isinstance(value,` (стр. 6550)
- `_cleanup_booking_reminder_deliveriesdef _cleanup_booking_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=14) cleaned: dict[str, str] = {} for key, value in deliv` (стр. 6560)
- `_cleanup_return_reminder_deliveriesdef _cleanup_return_reminder_deliveries(deliveries: dict[str, Any]) -> dict[str, str]: threshold = _now() - timedelta(days=30) cleaned: dict[str, str] = {} for key, value in delive` (стр. 6580)
- `_booking_client_reminder_messagedef _booking_client_reminder_message(booking: Booking) -> str: add_block = _additional_services_block(booking) return ( "Напоминание о записи\n" f"Услуга: {booking.service}{add_blo` (стр. 6600)
...
- `_dispatch_booking_remindersdef _dispatch_booking_reminders( db: Session, *, target_date: str | None = None, force: bool = False,` (стр. 6644)
- `_dispatch_return_visit_remindersdef _dispatch_return_visit_reminders(db: Session) -> int: reminder_state = _return_reminder_state(db) deliveries = reminder_state.get("deliveries") if not isinstance(deliveries, di` (стр. 6910)
- `_shift_checklist_payloaddef _shift_checklist_payload(entry: dict[str, Any]) -> ShiftChecklistPayload: return ShiftChecklistPayload( id=str(entry.get("id") or ""), workerId=str(entry.get("workerId") or "")` (стр. 7020)
- `_chemistry_stock_itemsdef _chemistry_stock_items(db: Session) -> list[StockItem]: return db.scalars( select(StockItem) .where(StockItem.category == "Химия") .order_by(StockItem.name.asc()) ).all()` (стр. 7074)
- `_latest_shift_checklist_entrydef _latest_shift_checklist_entry( entries: list[dict[str, Any]], worker_id: str, phase: str` (стр. 7090)
- `_clean_data_url_prefixdef _clean_data_url_prefix(data_url: str) -> str: return data_url.split(",", 1)[1] if "," in data_url else data_url` (стр. 7112)
- `_decode_data_url_imagedef _decode_data_url_image(data_url: str) -> tuple[str, bytes]: raw = data_url.strip() if not raw.startswith("data:image/"):` (стр. 7120)
- `_admin_shift_inspection_suppliesdef _admin_shift_inspection_supplies(db: Session) -> list[dict[str, Any]]: items = db.scalars( select(StockItem) .where(StockItem.category.in_(("Химия", "Расходники"))) .order_by(S` (стр. 7186)
- `_admin_shift_inspection_payloaddef _admin_shift_inspection_payload( entry: dict[str, Any],` (стр. 7244)
- `_admin_shift_captiondef _admin_shift_caption(entry: dict[str, Any]) -> str: checked_supplies = [ item.get("name") for item in entry.get("supplies", []) if isinstance(item, dict) and item.get("checked"` (стр. 7333)
- `_admin_shift_owner_inline_keyboarddef _admin_shift_owner_inline_keyboard(inspection_id: str) -> dict[str, Any]: return { "inline_keyboard": [ [ { "text": "Подтвердить", "callback_data": f"shiftapprove:{inspection_i` (стр. 7385)
- `_notify_owner_about_admin_shiftdef _notify_owner_about_admin_shift(db: Session, entry: dict[str, Any]) -> None: caption = _admin_shift_caption(entry) mime_type, photo_bytes = _decode_data_url_image( str(entry.ge` (стр. 7413)
- `_apply_admin_shift_reviewdef _apply_admin_shift_review( db: Session, inspection_id: str, *, action: str, issue_note: str, owner_actor_id: str,` (стр. 7483)
- `_serialize_state_datetimedef _serialize_state_datetime(value: datetime | None) -> str | None: if value is None: return None return _as_utc(value).isoformat()` (стр. 7603)
- `_parse_state_datetimedef _parse_state_datetime(value: Any) -> datetime | None: if not value: return None if not isinstance(value, str):` (стр. 7615)
- `_owner_database_reset_statedef _owner_database_reset_state(db: Session) -> dict[str, Any] | None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is None or not isinstance(row.value, dict):` (стр. 7637)
- `_save_owner_database_reset_statedef _save_owner_database_reset_state( db: Session, value: dict[str, Any]` (стр. 7651)
- `_clear_owner_database_reset_statedef _clear_owner_database_reset_state(db: Session) -> None: row = db.get(AppSetting, OWNER_DATABASE_RESET_SETTING_KEY) if row is not None: db.delete(row) db.flush()` (стр. 7663)
- `_normalize_database_reset_phrasedef _normalize_database_reset_phrase(value: str) -> str: normalized = " ".join(value.replace("\n", " ").split()).strip().upper() return normalized.replace("Ё", "Е")` (стр. 7677)
- `_owner_database_reset_previewdef _owner_database_reset_preview( db: Session,` (стр. 7687)
- `_owner_database_reset_warningsdef _owner_database_reset_warnings( preview: OwnerDatabaseResetPreviewPayload,` (стр. 7741)
- `_perform_owner_database_resetdef _perform_owner_database_reset(db: Session) -> None: db.execute(sa_delete(TelegramLinkCode)) db.execute(sa_delete(Notification)) db.execute(sa_delete(BookingWorker)) db.execute(` (стр. 7781)
- `_parse_datedef _parse_date(s: str) -> date | None: if "." in s: parts = s.split(".") try: return date(int(parts[2]), int(parts[1]), int(parts[0])) except (ValueError, IndexError):` (стр. 8000)
- `_owner_export_filedef _owner_export_file( db: Session, actor_id: str, kind: str, segment: str = "all", date_from: str | None = None, date_to: str | None = None,` (стр. 8026)
- `_in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_to an` (стр. 8176)
- `_piggy_bank_export_filedef _piggy_bank_export_file( db: Session, actor_id: str, date_from: str | None = None, date_to: str | None = None,` (стр. 8256)
- `_download_responsedef _download_response(export_file: GeneratedExport) -> Response: return Response( content=export_file.content, media_type=export_file.media_type, headers={ "Content-Disposition": ` (стр. 8310)
- `class _PartialBroadcastError(Exception):` (стр. 8330)
- `_PartialBroadcastError.__init__def __init__(self, payload: TelegramBroadcastPayload) -> None: super().__init__("partial broadcast failure") self.payload = payload` (стр. 8336)
- `_send_export_to_telegramdef _send_export_to_telegram( db: Session, actor_id: str, export_file: GeneratedExport` (стр. 8346)
- `_owner_summary_reportdef _owner_summary_report( db: Session, actor_id: str, period: str, segment: str` (стр. 8466)
- `_owner_summary_export_filedef _owner_summary_export_file( db: Session, actor_id: str, period: str, segment: str` (стр. 8584)
- `_send_owner_summary_reportdef _send_owner_summary_report( db: Session, actor_id: str, report: OwnerSummaryReport, export_file: GeneratedExport,` (стр. 8702)
- `_booking_car_labeldef _booking_car_label(car: str | None, plate: str | None) -> str: car_value = (car or "").strip() or "Авто не указано" plate_value = (plate or "").strip() return f"{car_value}, {p` (стр. 8911)
- `_admin_booking_notification_titledef _admin_booking_notification_title( client_name: str, car: str | None, plate: str | None` (стр. 8923)
- `_booking_datetime_labeldef _booking_datetime_label(date: str | None, time: str | None) -> str: if not (date or "").strip():` (стр. 8935)
- `_admin_booking_notification_textdef _admin_booking_notification_text( client_name: str, car: str | None, plate: str | None, date: str | None, time: str | None,` (стр. 8951)
- `_additional_services_blockdef _additional_services_block(booking: Booking) -> str: """Форматирует блок доп. услуг для вставки в Telegram/внутренние сообщения.""" services = getattr(booking, "additional_serv` (стр. 8971)
- `_booking_all_worker_idsdef _booking_all_worker_ids(booking: Booking) -> set[str]: """Все мастера записи: основная услуга + доп. услуги.""" ids: set[str] = {link.worker_id for link in (booking.worker_link` (стр. 8994)
- `_notify_admins_about_bookingdef _notify_admins_about_booking(db: Session, booking: Booking) -> None: admins = db.scalars( select(StaffUser).where(StaffUser.role == "admin", StaffUser.active.is_(True)) ).all()` (стр. 9003)
- `_notify_owners_about_bookingdef _notify_owners_about_booking(db: Session, booking: Booking) -> None: owners = _all_owner_telegram_recipients(db) add_block = _additional_services_block(booking) text = ( "Новая` (стр. 9037)
- `_service_category_keydef _service_category_key(value: str | None) -> str: return (value or "").strip().lower()` (стр. 9067)
- `_resource_group_keydef _resource_group_key(value: str | None) -> str: return (value or "").strip().lower() or DEFAULT_RESOURCE_GROUP` (стр. 9075)
- `_normalized_textdef _normalized_text(value: str | None) -> str: return (value or "").strip()` (стр. 9083)
- `_default_service_resource_groupdef _default_service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_for_service_category(service.category)` (стр. 9091)
- `_default_box_resource_groupdef _default_box_resource_group(box: Box | None) -> str: if box is None: return DEFAULT_RESOURCE_GROUP name_key = (box.name or "").strip().lower() description_key = (box.descriptio` (стр. 9103)
- `_service_resource_groupdef _service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_key( service.resource_group or _default_servic` (стр. 9123)
- `_compatible_box_namesdef _compatible_box_names(db: Session, resource_group: str | None) -> list[str]: target_group = _resource_group_key(resource_group) return [ box.name for box in db.scalars( select(` (стр. 9139)
- `_is_box_rental_servicedef _is_box_rental_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "аренда бокса" )` (стр. 9165)
- `_is_detailing_servicedef _is_detailing_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "детейлинг" )` (стр. 9179)
- `_resource_group_for_service_categorydef _resource_group_for_service_category(category: str | None) -> str: category_key = _service_category_key(category) if category_key == "детейлинг": return DETAILING_RESOURCE_GROU` (стр. 9191)
- `_box_by_namedef _box_by_name(db: Session, box_name: str) -> Box | None: return db.scalar(select(Box).where(Box.name == box_name))` (стр. 9205)
- `_normalize_service_and_box_resourcesdef _normalize_service_and_box_resources(db: Session) -> None: changed = False # Группа ресурсов услуг больше не привязывается к категории принудительно. boxes = db.scalars(select(` (стр. 9213)
- `_box_hourly_pricedef _box_hourly_price(db: Session, box_name: str, fallback_price: int) -> int: box = _box_by_name(db, box_name) if box is not None and box.price_per_hour > 0: return box.price_per_` (стр. 9421)
- `_payment_type_labeldef _payment_type_label(payment_type: str) -> str: return { "cash": "Наличные", "transfer": "Перевод", "invoice": "По счёту", "credit": "В долг (депозит)", }.get(payment_type, paym` (стр. 9435)
- `_booking_payment_labeldef _booking_payment_label(booking: Booking) -> str: if not booking.payment_settled: return "Не оплачено" return _payment_type_label(booking.payment_type)` (стр. 9453)
- `_notify_ownersdef _notify_owners(db: Session, text: str) -> None: db.add( Notification( id=f"n-{uuid4()}", recipient_role="owner", recipient_id=None, message=text, read=False, created_at=_now(),` (стр. 9465)
- `_booking_receipt_textdef _booking_receipt_text(booking: Booking, *, worker_name: str | None = None) -> str: worker_line = f"\nМастер: {worker_name}" if worker_name else "" add_block = _additional_servi` (стр. 9501)
- `_notify_booking_completion_receiptdef _notify_booking_completion_receipt( db: Session, booking: Booking, *, worker_name: str | None = None` (стр. 9533)
- `_notify_owner_about_worker_booking_eventdef _notify_owner_about_worker_booking_event( db: Session, booking: Booking, *, worker_name: str, event_label: str` (стр. 9605)
- `_notify_workers_about_assignmentdef _notify_workers_about_assignment( db: Session, booking: Booking, worker_ids: set[str]` (стр. 9648)
- `_notify_workers_about_additional_servicedef _notify_workers_about_additional_service( db: Session, booking: Booking, asvc: BookingAdditionalService` (стр. 9736)
- `_notify_workers_about_notedef _notify_workers_about_note( db: Session, booking: Booking, worker_ids: set[str]` (стр. 9838)
- `_notify_workers_about_rescheduledef _notify_workers_about_reschedule( db: Session, booking: Booking, worker_ids: set[str], previous_date: str, previous_time: str, previous_box: str,` (стр. 9910)
- `_payroll_entry_labeldef _payroll_entry_label(kind: str) -> str: return { "bonus": "премия", "advance": "аванс", "deduction": "удержание", "payout": "выплата", "adjustment": "корректировка", }.get(kind` (стр. 10000)
- `_notify_worker_about_payroll_entrydef _notify_worker_about_payroll_entry( db: Session, worker: StaffUser, *, actor_role: str, actor_id: str, kind: str, amount: int, note: str, …` (стр. 10020)
- `_PartialBroadcastError._safe_digitsdef _safe_digits(value: str) -> str: try: return normalize_phone_digits(value) except ValueError: return ""` (стр. 10108)
- `_default_contentdef _default_content() -> ContentPayload: return ContentPayload( hero=ContentHeroPayload(), about=ContentAboutPayload( text=( "<b>\u2728 \u041e \u0441\u0442\u0443\u0434\u0438\u0438` (стр. 10369)
- `_normalize_legacy_contentdef _normalize_legacy_content(value: dict) -> dict: """Мигрирует старый формат контента (hero.title строкой + hero.titleHighlight) в новый (hero.title => {before, highlight, after}` (стр. 10449)
- `_get_or_create_contentdef _get_or_create_content(db: Session) -> ContentPayload: row = db.get(AppSetting, "content") if row is None or not isinstance(row.value, dict):` (стр. 10470)
- `_detected_image_formatdef _detected_image_format(header: bytes) -> str | None: if header.startswith(b"\xff\xd8\xff"):` (стр. 10568)
- `_upload_headersdef _upload_headers(filename: str) -> dict[str, str]: return { "Cache-Control": "public, max-age=31536000, immutable", "Content-Disposition": f'inline; filename="{filename}"', "X-C` (стр. 10580)
- `_stock_category_descendant_idsdef _stock_category_descendant_ids(db: Session, root_id: str) -> set[str]: """Собрать все id потомков категории root_id (BFS, безлимитная глубина).""" descendants: set[str] = set()` (стр. 10800)
- `_normalize_parent_iddef _normalize_parent_id(value: str | None) -> str | None: if value is None or (isinstance(value, str) and value.strip() == ""):` (стр. 10818)
- `_validate_stock_category_parentdef _validate_stock_category_parent(db: Session, category_id: str | None, new_parent_id: str | None) -> None: new_parent_id = _normalize_parent_id(new_parent_id) if new_parent_id i` (стр. 10824)
- `_google_sync_bookingdef _google_sync_booking(db, booking, *, action="upsert") -> None: """Best-effort синхронизация записи с Google Calendar. No-op, если интеграция не настроена или токены не привязан` (стр. 10988)
- `_google_sync_loopdef _google_sync_loop() -> None: """Фоновый цикл обратной синхронизации «Google Calendar -> CRM». Запускается daemon-потоком при старте приложения. No-op, если интеграция не настро` (стр. 11000)
- `_write_off_booking_materialsdef _write_off_booking_materials(db: Session, booking: Booking) -> None: if booking.materials_written_off: print(f"[WRITE_OFF] skip booking {booking.id[:8]} — already written off")` (стр. 11538)
- `_booking_materials_costdef _booking_materials_cost(db: Session, booking: Booking) -> int: """Фактическая стоимость материалов по записи; fallback — материалы услуги со склада.""" override = (booking.mone` (стр. 11626)
- `_booking_materials_cost_actualdef _booking_materials_cost_actual(db: Session, booking: Booking) -> int: """Фактическая стоимость материалов по записи; fallback — материалы услуги со склада.""" materials_cost = ` (стр. 11634)
- `_asvc_paid_amountdef _asvc_paid_amount(asvc: BookingAdditionalService) -> int: """Сколько уходит с доп. услуги: аутсорсеру или мастерам (фикс/процент).""" if asvc.is_outsource: return int(asvc.outs` (стр. 11653)
- `_booking_money_splitdef _booking_money_split( db: Session, booking: Booking, complaints_by_worker: dict[str, list] | None = None,` (стр. 11666)
- `_PartialBroadcastError._asvc_servicedef _asvc_service(asvc): if asvc.service_id is None: return None if asvc.service_id not in _asvc_svc_cache: _asvc_svc_cache[asvc.service_id] = db.get(Service, asvc.service_id) retu` (стр. 11683)
- `_PartialBroadcastError._compute_masterdef _compute_master(base: int) -> tuple[dict[str, int], int, int]: """Доля мастеров: явные суммы (override/fixed) + сервисный режим/проценты профиля от base. Возвращает (master_by_` (стр. 11718)
- `_PartialBroadcastError._compute_piggydef _compute_piggy(base: int) -> int: if piggy_pay_type == "fixed": return piggy_pay_value if piggy_pay_type == "percent": return money_int(base * piggy_pay_value / 100) if piggy_p` (стр. 11852)
- `_PartialBroadcastError._allocate_ownersdef _allocate_owners(claimed: int, limit: int) -> tuple[int, dict[str, int]]: owner_by_owner: dict[str, int] = {} if claimed <= 0 or not owner_split_enabled: return 0, owner_by_own` (стр. 11865)
- `_process_piggy_bank_for_bookingdef _process_piggy_bank_for_booking(db: Session, booking: Booking) -> None: """Auto-deposit 24% into piggy bank for detailing bookings and repay material withdrawals for any servic` (стр. 11971)
- `_process_owner_profit_sharedef _process_owner_profit_share(db: Session, booking: Booking) -> None: """Расчёт доли владельцев: цена → материалы → мастера → копилка → остаток владельцам (50/50).""" if booking.` (стр. 12208)
- `_PartialBroadcastError._parse_date_strdef _parse_date_str(s: str) -> date | None: try: if "." in s: parts = s.split(".") return date(int(parts[2]), int(parts[1]), int(parts[0])) return date.fromisoformat(s) except (Val` (стр. 14784)
- ...ещё 48
...
### backend/app/schemas.py (2285 строк)
...
- `PayrollEntryCreateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 877)
- `class PayrollEntryUpdateRequest(BaseModel):` (стр. 881)
- `PayrollEntryUpdateRequest.validate_notedef validate_note(cls, value: str) -> str: return value.strip()` (стр. 887)
- `class SettingsBundlePayload(BaseModel):` (стр. 891)
- `class SessionPayload(BaseModel):` (стр. 901)
- `class BootstrapPayload(BaseModel):` (стр. 909)
- `class ClientRegisterRequest(BaseModel):` (стр. 927)
- `ClientRegisterRequest.lift_profiledef lift_profile(cls, raw: Any) -> Any: if isinstance(raw, dict) and isinstance(raw.get("profile"), dict):` (стр. 939)
- `ClientRegisterRequest.validate_namedef validate_name(cls, value: str) -> str: if not value.strip():` (стр. 948)
- `ClientRegisterRequest.validate_phonedef validate_phone(cls, value: str) -> str: if not value.strip():` (стр. 955)
- `ClientRegisterRequest.validate_vehicledef validate_vehicle(self) -> "ClientRegisterRequest": if self.plate.strip():` (стр. 961)
- `class ConsentRecordPayload(BaseModel):` (стр. 967)
- `class ConsentCheckResponse(BaseModel):` (стр. 972)
- `class StaffLinkRequest(BaseModel):` (стр. 976)
- `class SwitchRoleRequest(BaseModel):` (стр. 981)
- `_normalize_booking_datedef _normalize_booking_date(value: str) -> str: """Нормализует дату записи к канону ДД.ММ.ГГГГ. Принимает и ДД.ММ.ГГГГ, и ГГГГ-ММ-ДД (форматы, которые могут слать клиенты). Остальн` (стр. 985)
- `class BookingCreateRequest(BaseModel):` (стр. 1002)
- `BookingCreateRequest.validate_datedef validate_date(cls, value: str) -> str: return _normalize_booking_date(value)` (стр. 1028)
- `BookingCreateRequest.validate_client_namedef validate_client_name(cls, value: str) -> str: if not value.strip():` (стр. 1033)
- `BookingCreateRequest.validate_client_phonedef validate_client_phone(cls, value: str) -> str: if not value.strip():` (стр. 1040)
- `BookingCreateRequest.validate_vehicledef validate_vehicle(self) -> "BookingCreateRequest": if self.car is not None and self.car.strip():` (стр. 1046)
- `class AddBookingServiceRequest(BaseModel):` (стр. 1054)
- `class BookingUpdateRequest(BaseModel):` (стр. 1061)
- `BookingUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None return _normalize_booking_date(value)` (стр. 1087)
- `BookingUpdateRequest.validate_client_namedef validate_client_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1094)
- `BookingUpdateRequest.validate_client_phonedef validate_client_phone(cls, value: str | None) -> str | None: if value is None: return None return normalize_phone(value)` (стр. 1101)
- `BookingUpdateRequest.validate_vehicledef validate_vehicle(self) -> "BookingUpdateRequest": if self.car is not None and not self.car.strip():` (стр. 1107)
- `class ClientCardUpdateRequest(BaseModel):` (стр. 1120)
- `ClientCardUpdateRequest.validate_namedef validate_name(cls, value: str | None) -> str | None: if value is None: return None return normalize_person_name(value)` (стр. 1138)
- `ClientCardUpdateRequest.validate_phonedef validate_phone(cls, value: str | None) -> str | None: if value is None or not value.strip():` (стр. 1145)
- `ClientCardUpdateRequest.validate_vehicledef validate_vehicle(self) -> "ClientCardUpdateRequest": if self.car is not None and not self.car.strip():` (стр. 1151)
- `class NotificationCreateRequest(BaseModel):` (стр. 1164)
- `class ReadAllNotificationsRequest(BaseModel):` (стр. 1171)
- `class StockItemCreateRequest(BaseModel):` (стр. 1175)
- `class StockItemUpdateRequest(BaseModel):` (стр. 1184)
- `class StockCategoryCreateRequest(BaseModel):` (стр. 1193)
- `class StockCategoryUpdateRequest(BaseModel):` (стр. 1198)
- `class StockWriteOffRequest(BaseModel):` (стр. 1203)
- `class StockWriteOffPayload(BaseModel):` (стр. 1207)
- `class IncomeCreateRequest(BaseModel):` (стр. 1225)
- `IncomeCreateRequest.validate_sourcedef validate_source(cls, value: str) -> str: stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или состоять только из пробелов") return stripp` (стр. 1234)
- `IncomeCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1242)
- `class IncomePayload(BaseModel):` (стр. 1248)
- `class ExpenseCreateRequest(BaseModel):` (стр. 1259)
- `ExpenseCreateRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1269)
- `class PenaltyCreateRequest(BaseModel):` (стр. 1275)
- `class OwnerReminderDispatchRequest(BaseModel):` (стр. 1281)
- `class OwnerReminderDispatchPayload(BaseModel):` (стр. 1286)
- `class StaffLoginRequest(BaseModel):` (стр. 1294)
- `class ChangePasswordRequest(BaseModel):` (стр. 1299)
- `class OwnerDatabaseResetPreviewPayload(BaseModel):` (стр. 1304)
- `class OwnerDatabaseResetStartRequest(BaseModel):` (стр. 1319)
- `class OwnerDatabaseResetApproveRequest(BaseModel):` (стр. 1323)
- `class OwnerDatabaseResetExecuteRequest(BaseModel):` (стр. 1329)
- `class OwnerDatabaseResetStartPayload(BaseModel):` (стр. 1333)
- `class OwnerDatabaseResetApprovePayload(BaseModel):` (стр. 1342)
- `class OwnerDatabaseResetExecutePayload(BaseModel):` (стр. 1350)
- `class ContentAboutPayload(BaseModel):` (стр. 1355)
- `class ContentServicePayload(BaseModel):` (стр. 1361)
- `class ContentWorksPayload(BaseModel):` (стр. 1372)
- `class ContentStatsPayload(BaseModel):` (стр. 1378)
- `class ContentTitlePayload(BaseModel):` (стр. 1383)
- `ContentTitlePayload.to_full_titledef to_full_title(self) -> str: return f"{self.before}{self.highlight}{self.after}"` (стр. 1388)
- `class ContentHeroPayload(BaseModel):` (стр. 1392)
- `class ContentPayload(BaseModel):` (стр. 1408)
- `class ContactPayload(BaseModel):` (стр. 1415)
- `class ResetPasswordRequest(BaseModel):` (стр. 1422)
- `class GenericMessage(BaseModel):` (стр. 1426)
- `class TelegramDeliveryResult(BaseModel):` (стр. 1430)
- `class TelegramBroadcastPayload(BaseModel):` (стр. 1436)
- `class OwnerExportDeliveryPayload(BaseModel):` (стр. 1442)
- `class ShiftAttendancePayload(BaseModel):` (стр. 1449)
- `class ExpenseUpdateRequest(BaseModel):` (стр. 1460)
- `ExpenseUpdateRequest.validate_titledef validate_title(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("title не может быть пустым или с` (стр. 1470)
- `ExpenseUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1480)
- `ExpenseUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> "ExpenseUpdateRequest": if all(v is None for v in [self.title, self.amount, self.category, self.date, self.note]):` (стр. 1488)
- `class IncomeUpdateRequest(BaseModel):` (стр. 1494)
- `IncomeUpdateRequest.validate_sourcedef validate_source(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() if not stripped: raise ValueError("source не может быть пустым или` (стр. 1503)
- `IncomeUpdateRequest.validate_datedef validate_date(cls, value: str | None) -> str | None: if value is None: return None if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value):` (стр. 1513)
- `IncomeUpdateRequest.require_at_least_one_fielddef require_at_least_one_field(self) -> "IncomeUpdateRequest": # Use model_fields_set to detect explicitly provided fields (including null). # This allows {"note": null} to pass as` (стр. 1521)
- `class PiggyBankTransactionPayload(BaseModel):` (стр. 1529)
- `class PiggyBankWithdrawRequest(BaseModel):` (стр. 1553)
- `PiggyBankWithdrawRequest.validate_datedef validate_date(cls, value: str) -> str: if not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", value.strip()):` (стр. 1570)
- `PiggyBankWithdrawRequest.validate_spent_by_namedef validate_spent_by_name(cls, value: str | None) -> str | None: if value is None: return None stripped = value.strip() return stripped or None` (стр. 1577)
- `class PiggyBankAdjustRequest(BaseModel):` (стр. 1584)
- `PiggyBankAdjustRequest.validate_datedef validate_date(cls, value: str) -> str: stripped = value.strip() if stripped and not re.fullmatch(r"\d{2}\.\d{2}\.\d{4}", stripped):` (стр. 1592)
- `class PiggyBankWashBreakdown(BaseModel):` (стр. 1599)
- `class PiggyBankDetailingBreakdown(BaseModel):` (стр. 1612)
- `class PiggyBankSpenderDebt(BaseModel):` (стр. 1623)
- `class PiggyBankResponse(BaseModel):` (стр. 1630)
- `class WeeklyArchivePayload(BaseModel):` (стр. 1650)
- `class WalletResponse(BaseModel):` (стр. 1664)
- `class OwnerProfitShareItem(BaseModel):` (стр. 1681)
- `class OwnerProfitShareSummary(BaseModel):` (стр. 1698)
- `class OwnerSalaryDetailResponse(BaseModel):` (стр. 1707)
- `class PayOwnerSalaryRequest(BaseModel):` (стр. 1714)
- `class PayOwnerSalaryResponse(BaseModel):` (стр. 1723)
- `class OverrideEarnedRequest(BaseModel):` (стр. 1730)
- `class BookingHistoryItem(BaseModel):` (стр. 1734)
- `class BookingTotalsWorkerItem(BaseModel):` (стр. 1751)
- `class BookingTotalsOwnerItem(BaseModel):` (стр. 1769)
- `class BookingTotalsPiggyItem(BaseModel):` (стр. 1776)
- `class BookingHistoryTotals(BaseModel):` (стр. 1782)
- `class BookingMoneySplitWorkerItem(BaseModel):` (стр. 1788)
- `class BookingMoneySplitOwnerItem(BaseModel):` (стр. 1799)
- `class BookingPiggyTxItem(BaseModel):` (стр. 1807)
- `class BookingAdditionalServiceItem(BaseModel):` (стр. 1816)
- `class BookingAsvcPiggyItem(BaseModel):` (стр. 1825)
- `class BookingAsvcWorkerItem(BaseModel):` (стр. 1831)
- `class BookingMoneySplitDetail(BaseModel):` (стр. 1842)
- `class BookingWorkerEarnedUpdate(BaseModel):` (стр. 1890)
- `class BookingMoneySplitOwnerUpdate(BaseModel):` (стр. 1895)
- `class BookingMoneySplitUpdateRequest(BaseModel):` (стр. 1900)
- `class ArchiveBookingWorkerItem(BaseModel):` (стр. 1910)
- `class ArchiveAdditionalServiceItem(BaseModel):` (стр. 1920)
- `class ArchiveBookingItem(BaseModel):` (стр. 1926)
- `class ArchivePayrollItem(BaseModel):` (стр. 1953)
- `class ArchiveOwnerItem(BaseModel):` (стр. 1971)
- `class ArchiveSummary(BaseModel):` (стр. 1979)
- `class ArchiveResponse(BaseModel):` (стр. 1995)
- `class MoneyFlowDistributionWorkerItem(BaseModel):` (стр. 2012)
- `class MoneyFlowDistributionOwnerItem(BaseModel):` (стр. 2018)
- `class MoneyFlowDistribution(BaseModel):` (стр. 2025)
- `class MoneyFlowEntry(BaseModel):` (стр. 2036)
- ...ещё 19
...
### frontend/src/app/context/AppContext.tsx (2056 строк)

- `EMPTY_CONTENT` (стр. 911)
- `timeToMinutes` (стр. 933) — локальный
- `minutesToTime` (стр. 940) — локальный
- `hours` (стр. 941) — локальный
- `minutes` (стр. 942) — локальный
- `buildTimeSlots` (стр. 946) — локальный
- `timeRangesOverlap` (стр. 954) — локальный
- `AppContext` (стр. 958) — локальный
- `normalizeWorker` (стр. 960) — локальный
- `normalizeBootstrap` (стр. 974) — локальный
- `AppProvider` (стр. 998)
- `THEME_OVERRIDE_KEY` (стр. 1006) — локальный
- `readThemeOverride` (стр. 1007) — локальный
- `raw` (стр. 1009) — локальный
- `writeThemeOverride` (стр. 1015) — локальный
- `override` (стр. 1024) — локальный
- `upcomingDates` (стр. 1045) — локальный
- `todayLabel` (стр. 1046) — локальный
- `tomorrowLabel` (стр. 1047) — локальный
- `applyBootstrap` (стр. 1049) — локальный
- `normalized` (стр. 1050) — локальный
- `refreshBootstrap` (стр. 1078) — локальный
- `bootstrap` (стр. 1079) — локальный
- `handleError` (стр. 1083) — локальный
- `message` (стр. 1084) — локальный
- `restoreSession` (стр. 1089) — локальный
- `bootstrap` (стр. 1091) — локальный
- `refreshActiveSessions` (стр. 1100) — локальный
- `applyTelegramTheme` (стр. 1104) — локальный
- `root` (стр. 1108) — локальный
- `theme` (стр. 1109) — локальный
- `cssVar` (стр. 1112) — локальный
- `tg` (стр. 1119) — локальный
- `logout` (стр. 1136) — локальный
- `loginClient` (стр. 1160) — локальный
- `bootstrap` (стр. 1164) — локальный
- `linkStaff` (стр. 1178) — локальный
- `bootstrap` (стр. 1182) — локальный
- `switchRole` (стр. 1196) — локальный
- `bootstrap` (стр. 1200) — локальный
- `updateClientProfile` (стр. 1214) — локальный
- `payload` (стр. 1215) — локальный
- `saved` (стр. 1216) — локальный
- `remindAdminAboutInactiveClients` (стр. 1220) — локальный
- `response` (стр. 1221) — локальный
- `addClient` (стр. 1225) — локальный
- `created` (стр. 1226) — локальный
- `normalized` (стр. 1227) — локальный
- `updateClientCard` (стр. 1232) — локальный
- `saved` (стр. 1233) — локальный
- `normalized` (стр. 1234) — локальный
- `deleteClient` (стр. 1238) — локальный
- `listDepositClients` (стр. 1243) — локальный
- `items` (стр. 1244) — локальный
- `getDepositOverview` (стр. 1248) — локальный
- `overview` (стр. 1249) — локальный
- `updateDepositSubscription` (стр. 1257) — локальный
- `overview` (стр. 1258) — локальный
- `depositTopUp` (стр. 1266) — локальный
- `txn` (стр. 1267) — локальный
- `depositAdjust` (стр. 1271) — локальный
- `overview` (стр. 1272) — локальный
- `depositRecordWash` (стр. 1280) — локальный
- `overview` (стр. 1281) — локальный
- `depositSettleMonth` (стр. 1289) — локальный
- `overview` (стр. 1290) — локальный
- `downloadDepositExport` (стр. 1298) — локальный
- `downloadDepositExportAll` (стр. 1302) — локальный
- `sendDepositExport` (стр. 1306) — локальный
- `sendDepositExportAll` (стр. 1310) — локальный
- `addBooking` (стр. 1314) — локальный
- `created` (стр. 1315) — локальный
- `existingClient` (стр. 1335) — локальный
- `nextClient` (стр. 1336) — локальный
- `updateBooking` (стр. 1360) — локальный
- `updated` (стр. 1361) — локальный
- `deleteBooking` (стр. 1387) — локальный
- `addBookingService` (стр. 1392) — локальный
- `updated` (стр. 1393) — локальный
- `addBookingAdditionalService` (стр. 1413) — локальный
- `updated` (стр. 1414) — локальный
- `updateBookingAdditionalService` (стр. 1434) — локальный
- `updated` (стр. 1435) — локальный
- `removeBookingAdditionalService` (стр. 1455) — локальный
- `updated` (стр. 1456) — локальный
- `addNotification` (стр. 1476) — локальный
- `created` (стр. 1477) — локальный
- `markNotificationRead` (стр. 1496) — локальный
- `markAllNotificationsRead` (стр. 1501) — локальный
- `addStockItem` (стр. 1515) — локальный
- `created` (стр. 1516) — локальный
- `updateStockItem` (стр. 1520) — локальный
- `updated` (стр. 1521) — локальный
- `writeOffStock` (стр. 1525) — локальный
- `updated` (стр. 1526) — локальный
- `getWriteOffHistory` (стр. 1530) — локальный
- `deleteStockItem` (стр. 1534) — локальный
- `addStockCategory` (стр. 1539) — локальный
- `created` (стр. 1540) — локальный
- `updateStockCategory` (стр. 1544) — локальный
- `updated` (стр. 1545) — локальный
- `deleteStockCategory` (стр. 1549) — локальный
- `addExpense` (стр. 1558) — локальный
- `created` (стр. 1559) — локальный
- `addIncome` (стр. 1563) — локальный
- `created` (стр. 1564) — локальный
- `updateExpense` (стр. 1568) — локальный
- `updated` (стр. 1569) — локальный
- `updateIncome` (стр. 1573) — локальный
- `updated` (стр. 1574) — локальный
- `addPenalty` (стр. 1578) — локальный
- `revokePenalty` (стр. 1583) — локальный
- `revokeAllPenalties` (стр. 1588) — локальный
- `createTelegramLinkCode` (стр. 1593) — локальный
- `created` (стр. 1594) — локальный
- `downloadOwnerExport` (стр. 1598) — локальный
- `fallback` (стр. 1599) — локальный
- `qs` (стр. 1602) — локальный
- `qstr` (стр. 1606) — локальный
- `sendOwnerExportToTelegram` (стр. 1612) — локальный
- `qs` (стр. 1615) — локальный
- `qstr` (стр. 1619) — локальный
- `sendOwnerSummaryReport` (стр. 1625) — локальный
- `response` (стр. 1626) — локальный
- `dispatchOwnerReminders` (стр. 1630) — локальный
- `saveServices` (стр. 1640) — локальный
- `saveBoxes` (стр. 1645) — локальный
- `saveSchedule` (стр. 1649) — локальный
- `saveAdminProfile` (стр. 1653) — локальный
- `saved` (стр. 1654) — локальный
- `saveAdminNotificationSettings` (стр. 1658) — локальный
- `saved` (стр. 1659) — локальный
- `saveWorkerProfile` (стр. 1663) — локальный
- `saved` (стр. 1664) — локальный
- `normalized` (стр. 1665) — локальный
- `saveWorkerNotificationSettings` (стр. 1672) — локальный
- `saved` (стр. 1673) — локальный
- `saveOwnerCompany` (стр. 1680) — локальный
- `saved` (стр. 1681) — локальный
- `saveOwnerNotificationSettings` (стр. 1685) — локальный
- `saved` (стр. 1686) — локальный
- `saveOwnerIntegrations` (стр. 1690) — локальный
- `saved` (стр. 1691) — локальный
- `saveOwnerSecurity` (стр. 1695) — локальный
- `saved` (стр. 1696) — локальный
- `saveWorkerSettings` (стр. 1700) — локальный
- `saved` (стр. 1701) — локальный
- `saveAdminWorkerPayroll` (стр. 1705) — локальный
- `saved` (стр. 1706) — локальный
- `normalized` (стр. 1707) — локальный
- `nextWorker` (стр. 1709) — локальный
- `saveContent` (стр. 1714) — локальный
- `saved` (стр. 1715) — локальный
- `createPayrollEntry` (стр. 1719) — локальный
- `checkConsent` (стр. 1724) — локальный
- `response` (стр. 1725) — локальный
- `submitConsent` (стр. 1729) — локальный
- `listShiftChecklists` (стр. 1733) — локальный
- `entries` (стр. 1734) — локальный
- `submitShiftChecklist` (стр. 1738) — локальный
- `entry` (стр. 1739) — локальный
- `listAdminShiftInspections` (стр. 1746) — локальный
- `entries` (стр. 1747) — локальный
- `submitAdminShiftInspection` (стр. 1755) — локальный
- `entry` (стр. 1762) — локальный
- `openShiftForMasters` (стр. 1773) — локальный
- `entry` (стр. 1774) — локальный
- `hireWorker` (стр. 1785) — локальный
- `created` (стр. 1786) — локальный
- `normalized` (стр. 1787) — локальный
- `fireWorker` (стр. 1797) — локальный
- `resetWorkerPassword` (стр. 1802) — локальный
- `changePassword` (стр. 1809) — локальный
- `requestOwnerDatabaseReset` (стр. 1816) — локальный
- `response` (стр. 1817) — локальный
- `approveOwnerDatabaseReset` (стр. 1834) — локальный
- `response` (стр. 1835) — локальный
- `executeOwnerDatabaseReset` (стр. 1851) — локальный
- `response` (стр. 1852) — локальный
- `getTimeSlotsForDate` (стр. 1860) — локальный
- `parsedDate` (стр. 1861) — локальный
- `day` (стр. 1863) — локальный
- `openMinutes` (стр. 1866) — локальный
- `closeMinutes` (стр. 1867) — локальный
- `durationMinutes` (стр. 1870) — локальный
- `scheduleSlots` (стр. 1871) — локальный
- `candidateBoxes` (стр. 1872) — локальный
- `boxNames` (стр. 1877) — локальный
- `slotStart` (стр. 1879) — локальный
- `slotEnd` (стр. 1881) — локальный
- `bookingStart` (стр. 1888) — локальный
- `getBookingAvailabilityForDate` (стр. 1895) — локальный
- `durationMinutes` (стр. 1896) — локальный
- `params` (стр. 1898) — локальный
- `response` (стр. 1908) — локальный
- `next` (стр. 1931) — локальный
- `tg` (стр. 1937) — локальный
- `useApp` (стр. 2048)
- `ctx` (стр. 2049) — локальный
- `getWorkerNotificationSettings` (стр. 2054)
...
- `backend/app/schemas.py` (2026-08-28 14:44)
- `training/backend/app/main.py` (2026-08-28 14:36)
- `backend/app/main.py` (2026-08-28 14:34)
