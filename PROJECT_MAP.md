...
> Автосгенерировано 2026-09-24 05:45 UTC. **НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ.**
...
### backend/app/audit_log.py (44 строк)

Классы и функции (1):

- `log_actiondef log_action( db: Session, *, action: str, session_data: dict | None = None, actor_id: str = "", actor_role: str = "", object_type: str | None = None, object_id: str | None = Non` (стр. 17)

### backend/app/authz.py (42 строк)

Классы и функции (1):

- `authorize_roledef authorize_role(session_data: dict | None, allowed: set[str], *, action: str) -> None: """Проверить роль; в permissive-режиме — только залогировать и разрешить.""" from .config ` (стр. 21)
...
### backend/app/main.py (26693 строк)
...
```
  ...
  `POST /api/owner/data-cleanup/preview` -> `preview_data_cleanup` (декоратор: стр. 8989)
  `POST /api/owner/data-cleanup/execute` -> `execute_data_cleanup` (декоратор: стр. 8999)
  `GET /api/owner/data-cleanup/batches` -> `list_data_cleanup_batches` (декоратор: стр. 9066)
  `GET /api/owner/trash` -> `list_trash` (декоратор: стр. 9116)
  `POST /api/owner/trash/restore` -> `restore_trash` (декоратор: стр. 9155)
  `POST /api/owner/trash/purge` -> `purge_trash` (декоратор: стр. 9213)
  `GET /api/owner/exports/{kind}` -> `download_owner_export` (декоратор: стр. 10180)
  `POST /api/owner/exports/{kind}/telegram` -> `send_owner_export_to_telegram` (декоратор: стр. 10212)
  `POST /api/owner/reports/{period}/{segment}/telegram` -> `send_owner_summary_report_to_telegram` (декоратор: стр. 10244)
  `PATCH /api/clients/me` -> `update_client_me` (декоратор: стр. 11442)
  `DELETE /api/clients/{client_id}` -> `delete_client` (декоратор: стр. 11522)
  `PATCH /api/clients/{client_id}/card` -> `update_client_card` (декоратор: стр. 11547)
  `POST /api/clients` -> `create_client` (декоратор: стр. 11647)
  `GET /api/health` -> `health` (декоратор: стр. 11703)
  `GET /api/debug/db` -> `debug_db` (декоратор: стр. 11728)
  `GET /api/debug/mojibake-scan` -> `debug_mojibake_scan` (декоратор: стр. 11809)
  `POST /api/debug/mojibake-repair` -> `debug_mojibake_repair` (декоратор: стр. 11819)
  `GET /api/content` -> `get_public_content` (декоратор: стр. 12009)
  `PUT /api/content` -> `save_content` (декоратор: стр. 12023)
  `POST /api/upload` -> `upload_file` (декоратор: стр. 12087)
  `GET /api/uploads/{filename}` -> `serve_upload` (декоратор: стр. 12139)
  `POST /api/contact` -> `submit_contact` (декоратор: стр. 12160)
  `POST settings.telegram_webhook_path` -> `handle_telegram_webhook` (декоратор: стр. 12222)
  `POST /api/telegram/webhook/sync` -> `resync_telegram_webhook` (декоратор: стр. 12272)
  `GET /api/stock-categories` -> `list_stock_categories` (декоратор: стр. 12354)
  `POST /api/stock-categories` -> `create_stock_category` (декоратор: стр. 12367)
  `PATCH /api/stock-categories/{category_id}` -> `update_stock_category` (декоратор: стр. 12392)
  `DELETE /api/stock-categories/{category_id}` -> `delete_stock_category` (декоратор: стр. 12425)
  `GET /api/bookings/availability` -> `get_booking_availability` (декоратор: стр. 12459)
  `POST /api/bookings` -> `create_booking` (декоратор: стр. 12530)
  `PATCH /api/bookings/{booking_id}` -> `update_booking` (декоратор: стр. 13813)
  `DELETE /api/bookings/{booking_id}` -> `delete_booking` (декоратор: стр. 14497)
  `POST /api/bookings/{booking_id}/services` -> `add_booking_service` (декоратор: стр. 14597)
  `POST /api/bookings/{booking_id}/additional-services` -> `add_booking_additional_service` (декоратор: стр. 14663)
  `DELETE /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `remove_booking_additional_service` (декоратор: стр. 14800)
  `PATCH /api/bookings/{booking_id}/additional-services/{additional_service_id}` -> `update_booking_additional_service` (декоратор: стр. 14872)
  `POST /api/notifications` -> `create_notification` (декоратор: стр. 14965)
  `PATCH /api/notifications/{notification_id}/read` -> `mark_notification_read` (декоратор: стр. 15047)
  `POST /api/notifications/read-all` -> `mark_all_notifications_read` (декоратор: стр. 15125)
  `GET /api/notifications` -> `list_my_notifications` (декоратор: стр. 15191)
  `POST /api/broadcasts/workers` -> `broadcast_to_workers` (декоратор: стр. 15213)
  `POST /api/notifications/{notification_id}/take-to-work` -> `take_notification_to_work` (декоратор: стр. 15273)
  `POST /api/notifications/{notification_id}/complete` -> `complete_notification_task` (декоратор: стр. 15305)
  `POST /api/stock-items` -> `create_stock_item` (декоратор: стр. 15337)
  `PATCH /api/stock-items/{item_id}` -> `update_stock_item` (декоратор: стр. 15373)
  `POST /api/stock-items/{item_id}/write-off` -> `write_off_stock` (декоратор: стр. 15421)
  `GET /api/stock/write-off-history` -> `get_write_off_history` (декоратор: стр. 15470)
  `DELETE /api/stock-items/{item_id}` -> `delete_stock_item` (декоратор: стр. 15501)
  `GET /api/shift-checklists` -> `list_shift_checklists` (декоратор: стр. 15537)
  `POST /api/shift-checklists` -> `submit_shift_checklist` (декоратор: стр. 15579)
  `GET /api/admin/shift-inspections` -> `list_admin_shift_inspections` (декоратор: стр. 15701)
  `GET /api/admin/shift-inspections/{inspection_id}/photo` -> `get_admin_shift_inspection_photo` (декоратор: стр. 15747)
  `POST /api/admin/shift-inspections` -> `submit_admin_shift_inspection` (декоратор: стр. 15829)
  `POST /api/admin/shift-inspections/{inspection_id}/review` -> `review_admin_shift_inspection` (декоратор: стр. 15985)
  `POST /api/owner/shift-openings` -> `open_shift_for_masters` (декоратор: стр. 16024)
  `POST /api/expenses` -> `create_expense` (декоратор: стр. 16150)
  `PATCH /api/expenses/{expense_id}` -> `update_expense` (декоратор: стр. 16196)
  `GET /api/owner/incomes` -> `list_incomes` (декоратор: стр. 16262)
  `POST /api/owner/incomes` -> `create_income` (декоратор: стр. 16310)
  `PATCH /api/owner/incomes/{income_id}` -> `update_income` (декоратор: стр. 16374)
  `GET /api/owner/piggy-bank` -> `get_piggy_bank` (декоратор: стр. 16467)
  `POST /api/owner/piggy-bank/withdraw` -> `piggy_bank_withdraw` (декоратор: стр. 17678)
  `POST /api/owner/piggy-bank/repay` -> `piggy_bank_repay` (декоратор: стр. 18123)
  `POST /api/owner/piggy-bank/adjust` -> `piggy_bank_adjust` (декоратор: стр. 18443)
  `DELETE /api/owner/piggy-bank/transactions/{tx_id}` -> `delete_piggy_bank_transaction` (декоратор: стр. 18626)
  `GET /api/owner/deposits` -> `list_deposit_clients` (декоратор: стр. 18994)
  `PATCH /api/owner/deposits/{client_id}` -> `update_deposit_subscription` (декоратор: стр. 19038)
  `POST /api/owner/deposits/{client_id}/topup` -> `deposit_topup` (декоратор: стр. 19073)
  `POST /api/owner/deposits/{client_id}/adjust` -> `deposit_adjust` (декоратор: стр. 19100)
  `GET /api/owner/deposits/export-all.xlsx` -> `deposit_export_all_excel` (декоратор: стр. 19126)
  `POST /api/owner/deposits/export-all.xlsx/telegram` -> `deposit_export_all_excel_telegram` (декоратор: стр. 19144)
  `POST /api/owner/deposits/{client_id}/export.xlsx/telegram` -> `deposit_export_excel_telegram` (декоратор: стр. 19156)
  `GET /api/owner/deposits/{client_id}` -> `get_deposit_overview` (декоратор: стр. 19174)
  `POST /api/owner/deposits/{client_id}/washes` -> `deposit_record_wash` (декоратор: стр. 19187)
  `POST /api/owner/deposits/{client_id}/settle-month` -> `deposit_settle_month` (декоратор: стр. 19266)
  `GET /api/owner/deposits/{client_id}/export.xlsx` -> `deposit_export_excel` (декоратор: стр. 19351)
  `GET /api/owner/wallet` -> `get_wallet` (декоратор: стр. 19411)
  `GET /api/owner/workers/{worker_id}/shift-attendance` -> `get_worker_shift_attendance` (декоратор: стр. 19638)
  `GET /api/owner/shift-attendance` -> `get_all_workers_shift_attendance` (декоратор: стр. 19734)
  `GET /api/worker/shift-attendance` -> `get_own_shift_attendance` (декоратор: стр. 19814)
  `GET /api/worker/calendar` -> `get_worker_calendar_bookings` (декоратор: стр. 19882)
  `GET /api/worker/cars/search` -> `search_worker_cars` (декоратор: стр. 20061)
  `POST /api/penalties` -> `create_penalty` (декоратор: стр. 20194)
  `POST /api/penalties/{penalty_id}/revoke` -> `revoke_penalty` (декоратор: стр. 20344)
  `POST /api/workers/{worker_id}/penalties/revoke-all` -> `revoke_all_worker_penalties` (декоратор: стр. 20486)
  `POST /api/telegram/link-code` -> `generate_telegram_link_code` (декоратор: стр. 20632)
  `PUT /api/settings/services` -> `save_services` (декоратор: стр. 20686)
  `PUT /api/settings/boxes` -> `save_boxes` (декоратор: стр. 20770)
  `PUT /api/settings/schedule` -> `save_schedule` (декоратор: стр. 20828)
  `PUT /api/settings/admin/profile` -> `save_admin_profile` (декоратор: стр. 20876)
  `PUT /api/settings/admin/notifications` -> `save_admin_notifications` (декоратор: стр. 20950)
  `PUT /api/settings/workers/{worker_id}/profile` -> `save_worker_profile` (декоратор: стр. 20974)
  `PUT /api/settings/workers/{worker_id}/notifications` -> `save_worker_notifications` (декоратор: стр. 21034)
  `PUT /api/settings/owner/company` -> `save_owner_company` (декоратор: стр. 21076)
  `PUT /api/settings/owner/notifications` -> `save_owner_notifications` (декоратор: стр. 21100)
  `PUT /api/settings/owner/integrations` -> `save_owner_integrations` (декоратор: стр. 21124)
  `GET /api/owner/integrations/google/auth-url` -> `get_google_calendar_auth_url` (декоратор: стр. 21159)
  `GET /api/owner/integrations/google/callback` -> `google_calendar_callback` (декоратор: стр. 21230)
  `POST /api/owner/integrations/google/disconnect` -> `disconnect_google_calendar` (декоратор: стр. 21328)
  `GET /api/owner/integrations/google/status` -> `get_google_calendar_status` (декоратор: стр. 21347)
  `POST /api/owner/integrations/google/invites` -> `create_google_calendar_invite` (декоратор: стр. 21384)
  `DELETE /api/owner/integrations/google/connections/{connection_id}` -> `delete_google_calendar_connection` (декоратор: стр. 21419)
  `PUT /api/owner/integrations/google/credentials` -> `save_google_calendar_credentials` (декоратор: стр. 21445)
  `DELETE /api/owner/integrations/google/credentials` -> `delete_google_calendar_credentials` (декоратор: стр. 21478)
  `POST /api/owner/integrations/google/sync` -> `sync_google_calendar_now` (декоратор: стр. 21490)
  `GET /api/cron/google-sync` -> `run_google_calendar_sync_cron` (декоратор: стр. 21514)
  `GET /api/cron/reminders` -> `run_reminders_cron` (декоратор: стр. 21539)
  `POST /api/owner/inactive-clients/remind-admin` -> `remind_admin_about_inactive_clients` (декоратор: стр. 21571)
  `POST /api/owner/reminders/dispatch` -> `dispatch_owner_booking_reminders` (декоратор: стр. 21629)
  `GET /api/cron/reports` -> `run_reports_cron` (декоратор: стр. 21648)
  `PUT /api/settings/owner/security` -> `save_owner_security` (декоратор: стр. 21694)
  `PUT /api/workers/settings` -> `save_worker_settings` (декоратор: стр. 21730)
  `GET /api/admin/workers/payroll` -> `get_admin_workers_payroll` (декоратор: стр. 21833)
  `PUT /api/admin/workers/payroll` -> `save_admin_worker_payroll` (декоратор: стр. 21935)
  `GET /api/owner/outsource/payroll` -> `get_owner_outsource_payroll` (декоратор: стр. 22004)
  `POST /api/payroll/entries` -> `create_payroll_entry` (декоратор: стр. 22075)
  `PUT /api/payroll/entries/{entry_id}` -> `update_payroll_entry` (декоратор: стр. 22356)
  `DELETE /api/payroll/entries/{entry_id}` -> `delete_payroll_entry` (декоратор: стр. 22517)
  `PUT /api/payroll/booking-workers/{link_id}/override-earned` -> `update_booking_worker_override_earned` (декоратор: стр. 22607)
  `GET /api/owner/bookings-history` -> `get_owner_bookings_history` (декоратор: стр. 22838)
  `GET /api/owner/bookings-history/totals` -> `get_owner_bookings_history_totals` (декоратор: стр. 22926)
  `GET /api/owner/archive` -> `get_owner_archive` (декоратор: стр. 23134)
  `GET /api/owner/money-flow` -> `get_owner_money_flow` (декоратор: стр. 23459)
  `GET /api/owner/bookings/{booking_id}/money-split` -> `get_owner_booking_money_split` (декоратор: стр. 24007)
  `PUT /api/owner/bookings/{booking_id}/money-split` -> `update_owner_booking_money_split` (декоратор: стр. 24021)
  `GET /api/owner/workers/{worker_id}/salary-detail` -> `owner_worker_salary_detail` (декоратор: стр. 24412)
  `GET /api/worker/salary-detail` -> `worker_my_salary_detail` (декоратор: стр. 24863)
  `POST /api/owner/workers/{worker_id}/pay-salary` -> `owner_worker_pay_salary` (декоратор: стр. 25273)
  `GET /api/owner/owners/salary-detail` -> `owner_salary_detail` (декоратор: стр. 25534)
  `PATCH /api/owner/owners/{owner_id}/master-role` -> `set_owner_master_role` (декоратор: стр. 25800)
  `POST /api/owner/owners/pay-salary` -> `owner_pay_salary` (декоратор: стр. 25827)
  `POST /api/workers` -> `create_worker` (декоратор: стр. 26104)
  `POST /api/workers/{worker_id}/reset-password` -> `reset_worker_password` (декоратор: стр. 26242)
  `DELETE /api/workers/{worker_id}` -> `fire_worker` (декоратор: стр. 26302)
  `GET /api/auth/session` -> `get_session_bootstrap` (декоратор: стр. 26498)
  `GET /api/auth/role-preview` -> `get_role_preview` (декоратор: стр. 26512)
  `POST /api/auth/role-preview` -> `set_role_preview` (декоратор: стр. 26527)
  `GET /api/auth/consent/check` -> `check_consent` (декоратор: стр. 26579)
  `POST /api/auth/consent` -> `record_consent` (декоратор: стр. 26591)
  `GET /api/auth/sessions` -> `get_active_sessions` (декоратор: стр. 26615)
  `POST /api/auth/logout` -> `logout` (декоратор: стр. 26623)
  `POST /api/auth/change-password` -> `change_password` (декоратор: стр. 26636)
```
...
- `AsciiJSONResponse._created_at_in_scopedef _created_at_in_scope(created_at: datetime | None, fallback_dmy: str | None = None) -> bool: # Для payroll пробуем entry_date, иначе created_at. if fallback_dmy: try: d = parse_` (стр. 8828)
- `_data_cleanup_labeldef _data_cleanup_label(entity: str, obj: Any) -> tuple[str, str]: try: if entity == "bookings": return (f"{getattr(obj, 'date', '')} {getattr(obj, 'time', '')} · {getattr(obj, 'se` (стр. 8871)
- `_data_cleanup_preview_payloaddef _data_cleanup_preview_payload(db: Session, payload: DataCleanupRequest) -> DataCleanupPreviewPayload: period = _data_cleanup_resolve_period(payload) mode, dmy_from, dmy_to, _pa` (стр. 8898)
- `_trash_get_entitydef _trash_get_entity(obj_type: str, entity_id: str, db: Session) -> Any | None: mapping = { "bookings": Booking, "clients": Client, "incomes": Income, "expenses": Expense, "piggy"` (стр. 8946)
- `_trash_purge_expireddef _trash_purge_expired(db: Session) -> int: now = _now() expired = db.scalars( select(TrashItem).where( TrashItem.purged_at.is_(None), TrashItem.restored_at.is_(None), TrashItem.` (стр. 8963)
- `_parse_datedef _parse_date(s: str) -> date | None: if "." in s: parts = s.split(".") try: return date(int(parts[2]), int(parts[1]), int(parts[0])) except (ValueError, IndexError):` (стр. 9280)
- `_owner_export_filedef _owner_export_file( db: Session, actor_id: str, kind: str, segment: str = "all", date_from: str | None = None, date_to: str | None = None,` (стр. 9306)
- `AsciiJSONResponse._in_rangedef _in_range(d: str | None) -> bool: if not d: return True parsed = _parse_date(d) if not parsed: return True if parsed_from and parsed < parsed_from: return False if parsed_to an` (стр. 9458)
- `_piggy_bank_export_filedef _piggy_bank_export_file( db: Session, actor_id: str, date_from: str | None = None, date_to: str | None = None, resource_group: str | None = None,` (стр. 9538)
- `_download_responsedef _download_response(export_file: GeneratedExport) -> Response: return Response( content=export_file.content, media_type=export_file.media_type, headers={ "Content-Disposition": ` (стр. 9608)
- `class _PartialBroadcastError(Exception):` (стр. 9628)
- `_PartialBroadcastError.__init__def __init__(self, payload: TelegramBroadcastPayload) -> None: super().__init__("partial broadcast failure") self.payload = payload` (стр. 9634)
- `_partial_broadcast_error_handlerasync def _partial_broadcast_error_handler( request: Request, exc: _PartialBroadcastError` (стр. 9643)
- `_send_export_to_telegramdef _send_export_to_telegram( db: Session, actor_id: str, export_file: GeneratedExport` (стр. 9682)
- `_owner_summary_reportdef _owner_summary_report( db: Session, actor_id: str, period: str, segment: str` (стр. 9807)
- `_owner_summary_export_filedef _owner_summary_export_file( db: Session, actor_id: str, period: str, segment: str` (стр. 9927)
- `_send_owner_summary_reportdef _send_owner_summary_report( db: Session, actor_id: str, report: OwnerSummaryReport, export_file: GeneratedExport,` (стр. 10047)
- `_booking_car_labeldef _booking_car_label(car: str | None, plate: str | None) -> str: car_value = (car or "").strip() or "Авто не указано" plate_value = (plate or "").strip() return f"{car_value}, {p` (стр. 10260)
- `_admin_booking_notification_titledef _admin_booking_notification_title( client_name: str, car: str | None, plate: str | None` (стр. 10272)
- `_booking_datetime_labeldef _booking_datetime_label(date: str | None, time: str | None) -> str: if not (date or "").strip():` (стр. 10284)
- `_admin_booking_notification_textdef _admin_booking_notification_text( client_name: str, car: str | None, plate: str | None, date: str | None, time: str | None,` (стр. 10300)
- `_additional_services_blockdef _additional_services_block(booking: Booking) -> str: """Форматирует блок доп. услуг для вставки в Telegram/внутренние сообщения.""" services = getattr(booking, "additional_serv` (стр. 10320)
- `_booking_all_worker_idsdef _booking_all_worker_ids(booking: Booking) -> set[str]: """Все мастера записи: основная услуга + доп. услуги.""" ids: set[str] = {link.worker_id for link in (booking.worker_link` (стр. 10343)
- `_notify_admins_about_bookingdef _notify_admins_about_booking(db: Session, booking: Booking) -> None: admins = db.scalars( select(StaffUser).where(StaffUser.role == "admin", StaffUser.active.is_(True)) ).all()` (стр. 10352)
- `_notify_owners_about_bookingdef _notify_owners_about_booking(db: Session, booking: Booking) -> None: owners = _all_owner_telegram_recipients(db) add_block = _additional_services_block(booking) text = ( "Новая` (стр. 10386)
- `_service_category_keydef _service_category_key(value: str | None) -> str: return (value or "").strip().lower()` (стр. 10416)
- `_resource_group_keydef _resource_group_key(value: str | None) -> str: return (value or "").strip().lower() or DEFAULT_RESOURCE_GROUP` (стр. 10424)
- `_normalized_textdef _normalized_text(value: str | None) -> str: return (value or "").strip()` (стр. 10432)
- `_default_service_resource_groupdef _default_service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_for_service_category(service.category)` (стр. 10440)
- `_default_box_resource_groupdef _default_box_resource_group(box: Box | None) -> str: if box is None: return DEFAULT_RESOURCE_GROUP name_key = (box.name or "").strip().lower() description_key = (box.descriptio` (стр. 10452)
- `_service_resource_groupdef _service_resource_group(service: Service | None) -> str: if service is None: return DEFAULT_RESOURCE_GROUP return _resource_group_key( service.resource_group or _default_servic` (стр. 10472)
- `_compatible_box_namesdef _compatible_box_names(db: Session, resource_group: str | None) -> list[str]: target_group = _resource_group_key(resource_group) return [ box.name for box in db.scalars( select(` (стр. 10488)
- `_is_box_rental_servicedef _is_box_rental_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "аренда бокса" )` (стр. 10514)
- `_is_detailing_servicedef _is_detailing_service(service: Service | None) -> bool: return ( service is not None and _service_category_key(service.category) == "детейлинг" )` (стр. 10528)
- `_resource_group_for_service_categorydef _resource_group_for_service_category(category: str | None) -> str: category_key = _service_category_key(category) if category_key == "детейлинг": return DETAILING_RESOURCE_GROU` (стр. 10540)
- `_box_by_namedef _box_by_name(db: Session, box_name: str) -> Box | None: return db.scalar(select(Box).where(Box.name == box_name))` (стр. 10554)
- `_normalize_service_and_box_resourcesdef _normalize_service_and_box_resources(db: Session) -> None: changed = False # Группа ресурсов услуг больше не привязывается к категории принудительно. boxes = db.scalars(select(` (стр. 10562)
- `_box_hourly_pricedef _box_hourly_price(db: Session, box_name: str, fallback_price: int) -> int: box = _box_by_name(db, box_name) if box is not None and box.price_per_hour > 0: return box.price_per_` (стр. 10770)
- `_payment_type_labeldef _payment_type_label(payment_type: str) -> str: return { "cash": "Наличные", "transfer": "Перевод", "invoice": "По счёту", "credit": "В долг (депозит)", }.get(payment_type, paym` (стр. 10784)
- `_booking_payment_labeldef _booking_payment_label(booking: Booking) -> str: if not booking.payment_settled: return "Не оплачено" return _payment_type_label(booking.payment_type)` (стр. 10802)
- `_notify_ownersdef _notify_owners(db: Session, text: str) -> None: db.add( Notification( id=f"n-{uuid4()}", recipient_role="owner", recipient_id=None, message=text, read=False, created_at=_now(),` (стр. 10814)
- `_booking_receipt_textdef _booking_receipt_text(booking: Booking, *, worker_name: str | None = None) -> str: worker_line = f"\nМастер: {worker_name}" if worker_name else "" add_block = _additional_servi` (стр. 10850)
- ...ещё 89
...
- `migration_lockdef migration_lock( lock_dir: Path, *, timeout_s: float = 900, stale_after_s: float = 600,` (стр. 42)
- `_ensure_versions_tabledef _ensure_versions_table(engine) -> None: from sqlalchemy.exc import OperationalError from .models import SchemaMigration try: SchemaMigration.__table__.create(bind=engine, check` (стр. 94)
- `get_applied_versionsdef get_applied_versions(engine, *, create_if_missing: bool = True) -> set[str]: """Множество записанных версий. ``create_if_missing=False`` — строго read-only: таблицы нет → пусто` (стр. 110)
- `plandef plan(engine, *, extra_ids: list[str] | None = None) -> dict: """Строго read-only preflight: диалект, таблицы, применённые/pending версии.""" from sqlalchemy import inspect as s` (стр. 130)
- `_record_versiondef _record_version(engine, version: str) -> None: from .models import SchemaMigration, utc_now with engine.begin() as connection: connection.execute( SchemaMigration.__table__.ins` (стр. 153)
- `run_startup_migrationsdef run_startup_migrations( baseline_fn: Callable[[], None], *, engine=None, lock_dir: Path | None = None, lock_timeout_s: float = 900, extra_migrations: list[ExtraMigration] | Non` (стр. 171)
- `_apply_all_under_lockdef _apply_all_under_lock( eng, baseline_fn: Callable[[], None], extras: list[ExtraMigration]` (стр. 206)
...
- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 29)
...
- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 24)
- `make_enginedef make_engine(path: Path): from sqlalchemy import create_engine return create_engine( f"sqlite:///{path.as_posix()}", connect_args={"check_same_thread": False}, )` (стр. 36)
- `class RunnerUnitTests(unittest.TestCase):` (стр. 45)
- `RunnerUnitTests.setUpdef setUp(self) -> None: self.tmp = Path(tempfile.mkdtemp(prefix="mgr_")) self.db_path = self.tmp / f"m_{uuid4().hex}.sqlite3" self.lock_dir = self.tmp / "locks" from app.runtime_m` (стр. 46)
- `RunnerUnitTests.tearDowndef tearDown(self) -> None: reset_app_modules() for child in sorted(self.tmp.rglob("*"), reverse=True):` (стр. 54)
- `RunnerUnitTests.test_baseline_applies_once_then_skipsdef test_baseline_applies_once_then_skips(self) -> None: from app.runtime_migrations import get_applied_versions, run_startup_migrations engine = make_engine(self.db_path) calls: l` (стр. 69)
- `RunnerUnitTests.test_error_propagates_and_version_not_recordeddef test_error_propagates_and_version_not_recorded(self) -> None: from app.runtime_migrations import get_applied_versions, run_startup_migrations engine = make_engine(self.db_path)` (стр. 87)
- `RunnerUnitTests.boomdef boom() -> None: calls.append("run") raise ValueError("simulated DDL failure")` (стр. 93)
- `RunnerUnitTests.test_concurrent_startups_serializeddef test_concurrent_startups_serialized(self) -> None: from app.runtime_migrations import run_startup_migrations engine = make_engine(self.db_path) calls: list[str] = [] errors: li` (стр. 105)
- `RunnerUnitTests.slowdef slow() -> None: time.sleep(2) calls.append("run")` (стр. 112)
- `RunnerUnitTests.targetdef target() -> None: try: run_startup_migrations(slow, engine=engine, lock_dir=self.lock_dir) except BaseException as exc: # noqa: BLE001 errors.append(exc)` (стр. 116)
- `RunnerUnitTests.test_stale_lock_is_stolendef test_stale_lock_is_stolen(self) -> None: from app.runtime_migrations import LOCK_FILENAME, migration_lock self.lock_dir.mkdir(parents=True, exist_ok=True) stale = self.lock_dir` (стр. 132)
- `RunnerUnitTests.test_lock_timeout_is_louddef test_lock_timeout_is_loud(self) -> None: from app.runtime_migrations import migration_lock with migration_lock(self.lock_dir, timeout_s=10):` (стр. 144)
- `RunnerUnitTests.test_plan_is_read_only_preflightdef test_plan_is_read_only_preflight(self) -> None: from app.runtime_migrations import get_applied_versions, plan engine = make_engine(self.db_path) pre = plan(engine) self.assertE` (стр. 152)
- `RunnerUnitTests.test_plan_creates_no_tablesdef test_plan_creates_no_tables(self) -> None: # Оговорка T2.1 №1 закрыта: plan() строго read-only, DDL-побочек нет. from sqlalchemy import inspect as sa_inspect from sqlalchemy im` (стр. 163)
- `RunnerUnitTests.test_lock_dir_failure_is_fail_closeddef test_lock_dir_failure_is_fail_closed(self) -> None: # Оговорка T2.1 №4 закрыта кодом: перехватывается только TimeoutError, # недоступная lock-директория роняет старт громко (fa` (стр. 181)
- `RunnerUnitTests.test_downgrade_refuses_with_restore_pointerdef test_downgrade_refuses_with_restore_pointer(self) -> None: from app.runtime_migrations import downgrade with self.assertRaises(RuntimeError) as ctx: downgrade(self.baseline) se` (стр. 200)
- `class ConfigGateTests(unittest.TestCase):` (стр. 208)
- `ConfigGateTests.test_strong_env_rejects_demo_seed_flagdef test_strong_env_rejects_demo_seed_flag(self) -> None: from app.config import get_settings saved = { key: os.environ.get(key) for key in ("APP_ENV", "APP_SECRET", "ALLOW_DEMO_SE` (стр. 214)
- `class SeedGateTests(unittest.TestCase):` (стр. 236)
...
- `MoneyMatrixTests.piggy_bankdef piggy_bank(self) -> dict: response = self.client.get( "/api/owner/piggy-bank", headers=self.auth_headers(self.owner_token)) self.assertEqual(response.status_code, 200, response` (стр. 330)
- `MoneyMatrixTests.test_piggy_breakdown_follows_custom_splitdef test_piggy_breakdown_follows_custom_split(self) -> None: """R14: мини-карточки копилки считают реальным сплитом, а не 10/90 и 40/60.""" self.reset_services() self.cfg("s1", pig` (стр. 336)
- `MoneyMatrixTests.test_wallet_ignores_soft_deleteddef test_wallet_ignores_soft_deleted(self) -> None: """F1: удалённые (cleanup) доходы/расходы не входят в итоги кошелька.""" date = self.next_active_date() income = self.client.pos` (стр. 353)
...
### backend/tests/test_repro_piggy_cleanup.py (222 строк)

Классы и функции (10):

- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 18)
- `build_init_datadef build_init_data(telegram_id: str) -> str: return urllib.parse.urlencode({"user": json.dumps({"id": int(telegram_id)})})` (стр. 24)
- `class ReproPiggyCleanup(unittest.TestCase):` (стр. 28)
- `ReproPiggyCleanup.setUpdef setUp(self) -> None: from fastapi.testclient import TestClient data_dir = Path(__file__).resolve().parents[1] / "data" data_dir.mkdir(parents=True, exist_ok=True) self.db_path ` (стр. 32)
- `ReproPiggyCleanup.tearDowndef tearDown(self) -> None: if hasattr(self, "client_manager"):` (стр. 60)
- `ReproPiggyCleanup._set_staff_telegram_idsdef _set_staff_telegram_ids(self) -> None: from sqlalchemy import select from app.database import SessionLocal from app.models import StaffUser mapping = {"admin": self.ADMIN_TG_ID` (стр. 73)
- `ReproPiggyCleanup.auth_headersdef auth_headers(token: str) -> dict[str, str]: return {"Authorization": token}` (стр. 87)
- `ReproPiggyCleanup.next_active_datedef next_active_date() -> str: candidate = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0) for offset in range(1, 8):` (стр. 91)
- `ReproPiggyCleanup._seeddef _seed(self) -> str: """Completed booking (→ piggy deposits) + income + expense.""" from app.database import SessionLocal from app.models import Expense, Income booking_date = s` (стр. 99)
- `ReproPiggyCleanup.test_reprodef test_repro(self) -> None: owner = self.auth_headers(self.owner_token) booking_date = self._seed() before = self.client.get("/api/owner/piggy-bank", headers=owner) self.assertEq` (стр. 148)
...
- `reset_app_modulesdef reset_app_modules() -> None: for name in list(sys.modules):` (стр. 31)
...
### frontend/src/app/components/admin/AdminApp.tsx (3713 строк)
...
- `bw` (стр. 726) — локальный
- `addTotal` (стр. 729) — локальный
- `legacyTotal` (стр. 730) — локальный
- `base` (стр. 731) — локальный
- `avgCheck` (стр. 736) — локальный
- `conversionRate` (стр. 737) — локальный
- `scheduleSummary` (стр. 738) — локальный
- `revenueData` (стр. 739) — локальный
- `formatted` (стр. 740) — локальный
- `hourData` (стр. 746) — локальный
- `handleStatusChange` (стр. 750) — локальный
- `target` (стр. 751) — локальный
- `statusNeedsSlot` (стр. 752) — локальный
- `handleDeleteClient` (стр. 764) — локальный
- `confirmed` (стр. 765) — локальный
- `handleCreateClient` (стр. 770) — локальный
- `nameError` (стр. 772) — локальный
- `phoneError` (стр. 776) — локальный
- `carError` (стр. 780) — локальный
- `plateError` (стр. 784) — локальный
- `created` (стр. 792) — локальный
- `handleSaveClientCard` (стр. 814) — локальный
- `draft` (стр. 815) — локальный
- `handleShiftPhotoChange` (стр. 829) — локальный
- `file` (стр. 830) — локальный
- `dataUrl` (стр. 834) — локальный
- `handleSubmitShiftInspection` (стр. 846) — локальный
- `primaryPhoto` (стр. 850) — локальный
- `uploadedCategoriesLabel` (стр. 857) — локальный
- `composedNote` (стр. 858) — локальный
- `saved` (стр. 862) — локальный
- `validateClientName` (стр. 879) — локальный
- `validateClientPhone` (стр. 883) — локальный
- `validateBookingDate` (стр. 887) — локальный
- `parsedDate` (стр. 889) — локальный
- `scheduleDay` (стр. 894) — локальный
- `normalizedTime` (стр. 899) — локальный
- `slotStart` (стр. 900) — локальный
- `openMinutes` (стр. 909) — локальный
- `closeMinutes` (стр. 910) — локальный
- `slotEnd` (стр. 911) — локальный
- `validateBookingDateForEdit` (стр. 923) — локальный
- `parsedDate` (стр. 925) — локальный
- `scheduleDay` (стр. 930) — локальный
- `normalizedTime` (стр. 935) — локальный
- `slotStart` (стр. 936) — локальный
- `openMinutes` (стр. 942) — локальный
- `closeMinutes` (стр. 943) — локальный
- `slotEnd` (стр. 944) — локальный
- `validateBookingDateTimeFormat` (стр. 956) — локальный
- `parsedDate` (стр. 958) — локальный
- `validateNewBookingForm` (стр. 971) — локальный
- `selectedService` (стр. 973) — локальный
- `nameError` (стр. 975) — локальный
- `phoneError` (стр. 979) — локальный
- `carError` (стр. 983) — локальный
- `plateError` (стр. 987) — локальный
- `hasDate` (стр. 990) — локальный
- `hasTime` (стр. 991) — локальный
- `requiresScheduledSlot` (стр. 992) — локальный
- `validation` (стр. 1009) — локальный
- `resetNewBookingDraft` (стр. 1019) — локальный
- `openNewBookingModal` (стр. 1049) — локальный
- `openAdditionalServiceModal` (стр. 1054) — локальный
- `openNewBookingForClient` (стр. 1063) — локальный
- `historyDate` (стр. 1065) — локальный
- `clientVehicles` (стр. 1067) — локальный
- `mainVehicle` (стр. 1069) — локальный
- `hasPriorVisits` (стр. 1070) — локальный
- `closeNewBookingModal` (стр. 1088) — локальный
- `handleAddService` (стр. 1093) — локальный
- `svc` (стр. 1098) — локальный
- `workersList` (стр. 1099) — локальный
- `worker` (стр. 1100) — локальный
- `updatedBooking` (стр. 1103) — локальный
- `handleRemoveService` (стр. 1123) — локальный
- `handleCreateServiceFromQuery` (стр. 1127) — локальный
- `name` (стр. 1128) — локальный
...
### frontend/src/app/components/admin/screens/AdminStatsPage.tsx (401 строк)
...
- `bw` (стр. 153) — локальный
- `addTotal` (стр. 156) — локальный
- `legacyTotal` (стр. 157) — локальный
- `base` (стр. 158) — локальный
- `revenueData` (стр. 166) — локальный
- `days` (стр. 167) — локальный
- `formatted` (стр. 169) — локальный
- `hourData` (стр. 179) — локальный
- `source` (стр. 180) — локальный
- `referralStats` (стр. 189) — локальный
- `map` (стр. 190) — локальный
- `src` (стр. 192) — локальный
- `cur` (стр. 193) — локальный
- `glass` (стр. 201) — локальный
- `sub` (стр. 202) — локальный
- `tooltipStyle` (стр. 203) — локальный
- `maxRevenue` (стр. 292) — локальный
- `palette` (стр. 337) — локальный
- `palette` (стр. 346) — локальный
- `total` (стр. 347) — локальный
- `pct` (стр. 348) — локальный
...
### frontend/src/app/components/worker/screens/WorkerProfileScreen.tsx (633 строк)

- `bookingBasePrice` (стр. 18) — локальный
- `addTotal` (стр. 19) — локальный
- `legacyTotal` (стр. 23) — локальный
- `WorkerProfileScreen` (стр. 48)
- `isMyTask` (стр. 116) — локальный
- `allMyTasks` (стр. 120) — локальный
- `myEarnings` (стр. 121) — локальный
- `w` (стр. 124) — локальный
- `totalEarned` (стр. 133) — локальный
- `payrollSummary` (стр. 134) — локальный
- `earnedForDisplay` (стр. 135) — локальный
- `completedCount` (стр. 136) — локальный
- `myPenalties` (стр. 137) — локальный
- `complaintState` (стр. 138) — локальный
- `chemistryItems` (стр. 139) — локальный
- `handleSaveProfile` (стр. 142) — локальный
- `handleSubmitShiftChecklist` (стр. 148) — локальный
- `saved` (стр. 151) — локальный
- `handleSavePass` (стр. 166) — локальный
- `handleGenerateTelegramCode` (стр. 193) — локальный
- `handleSaveNotifications` (стр. 197) — локальный
- `glass` (стр. 204) — локальный
- `sub` (стр. 205) — локальный
- `primaryColor` (стр. 208) — локальный
- `accentColor` (стр. 209) — локальный
- `fmtDateTime` (стр. 211) — локальный
- `enabled` (стр. 462) — локальный
- `w` (стр. 508) — локальный
...
### frontend/src/app/components/worker/WorkerApp.tsx (798 строк)
...
- `bookingBaseWorkerEarned` (стр. 81) — локальный
- `link` (стр. 82) — локальный
- `formatBookingInstant` (стр. 89) — локальный
- `date` (стр. 91) — локальный
- `WorkerApp` (стр. 97)
- `workerId` (стр. 117) — локальный
- `myNotifications` (стр. 146) — локальный
- `unreadCount` (стр. 147) — локальный
- `isBroadcast` (стр. 153) — локальный
- `refreshRef` (стр. 157) — локальный
- `tick` (стр. 162) — локальный
- `timerId` (стр. 167) — локальный
- `onVisible` (стр. 168) — локальный
- `isMyTask` (стр. 173) — локальный
- `allTasks` (стр. 177) — локальный
- `todayTasks` (стр. 180) — локальный
- `formatTimer` (стр. 188) — локальный
- `glass` (стр. 190) — локальный
- `bg` (стр. 191) — локальный
- `text` (стр. 192) — локальный
- `sub` (стр. 193) — локальный
- `primary` (стр. 194) — локальный
- `accent` (стр. 195) — локальный
- `surface` (стр. 196) — локальный
- `handleStartTask` (стр. 198) — локальный
- `openFinishModal` (стр. 206) — локальный
- `handleFinish` (стр. 215) — локальный
- `nextNote` (стр. 221) — локальный
- `headerTitle` (стр. 261) — локальный
- `isMyService` (стр. 341) — локальный
- `isOutsource` (стр. 342) — локальный
- `myBaseLink` (стр. 390) — локальный
- `myAdditionalServices` (стр. 391) — локальный
- `baseEarned` (стр. 393) — локальный
- `additionalEarned` (стр. 394) — локальный
- `total` (стр. 395) — локальный
- `earned` (стр. 406) — локальный
- `created` (стр. 458) — локальный
- `started` (стр. 459) — локальный
- `completed` (стр. 460) — локальный
- `isActive` (стр. 538) — локальный
- `isOwnerMsg` (стр. 674) — локальный
...
### frontend/src/app/constants/referralSources.ts (10 строк)
...
- `backend/app/main.py` (2026-09-24 08:45)
...
- `backend/app/main.py.mine` (2026-09-22 18:23)
...
- `backend/app/schemas.py` (2026-09-22 10:03)
- `audit/scripts/prod_baseline.py` (2026-09-22 09:05)
- `backend/tests/test_audit_log.py` (2026-09-22 09:05)
- `backend/tests/test_idor_matrix.py` (2026-09-22 09:03)
- `backend/app/authz.py` (2026-09-22 08:57)
